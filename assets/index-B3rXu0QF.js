import{initializeApp as Ae}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";import{getAuth as je,onAuthStateChanged as Oe,signInWithEmailAndPassword as Ue,createUserWithEmailAndPassword as He,signOut as ze}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-auth.js";import{getFirestore as Ve,getDoc as q,doc as k,setDoc as se,query as ae,collection as J,where as ne,getDocs as ve,onSnapshot as ue,serverTimestamp as ee,updateDoc as D,arrayUnion as Le,deleteDoc as Ge}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";import{getFunctions as Ke,httpsCallable as qe}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-functions.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const l of a)if(l.type==="childList")for(const o of l.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function e(a){const l={};return a.integrity&&(l.integrity=a.integrity),a.referrerPolicy&&(l.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?l.credentials="include":a.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function s(a){if(a.ep)return;a.ep=!0;const l=e(a);fetch(a.href,l)}})();const Je={apiKey:"AIzaSyA11OPapSOfwLQzs-oEoF7papT9S3T5p7Q",authDomain:"ipsc-insight-coach.firebaseapp.com",projectId:"ipsc-insight-coach",storageBucket:"ipsc-insight-coach.firebasestorage.app",messagingSenderId:"864793320312",appId:"1:864793320312:web:c586e384bbe365444bc68d",measurementId:"G-3Y4NZTBBS2"},Ee=Ae(Je),re=je(Ee),f=Ve(Ee),We=Ke();let ge=null,F=null;function Ye(i){Oe(re,async t=>{if(t){ge=t;try{const e=await q(k(f,"users",t.uid));e.exists()?F={uid:t.uid,...e.data()}:F={uid:t.uid,email:t.email,name:t.email?t.email.split("@")[0]:"Bruker",role:"user"},i(F)}catch(e){console.error("Feil ved lasting av brukerprofil:",e),F={uid:t.uid,email:t.email,name:t.email?t.email.split("@")[0]:"Bruker",role:"user"},i(F)}}else ge=null,F=null,i(null)})}async function Qe(i,t){try{const e=(i||"").trim();return{success:!0,user:(await Ue(re,e,t||"")).user}}catch(e){console.error("Innlogging feilet:",e);let s="Innlogging feilet";return e.code==="auth/user-not-found"||e.code==="auth/wrong-password"||e.code==="auth/invalid-credential"?s="Feil e-post eller passord":e.code==="auth/invalid-email"?s="Ugyldig e-postadresse":e.code==="auth/user-disabled"&&(s="Denne kontoen er deaktivert"),{success:!1,error:s}}}async function Ze(i,t,e,s,a,l,o,p,g,y){try{const u=(i||"").trim(),h=t||"",I=(e||"").trim(),P=(s||"").trim(),W=(a||"").trim(),Y=(l||"").trim(),de=(o||"").trim(),H=(p||"minor").trim(),z=(g||"").trim(),Q=(y||"").trim(),_=(await He(re,u,h)).user,ce=qe(We,"validateInviteCode");try{await ce({code:I,userId:_.uid,userEmail:u})}catch(C){await _.delete();let v="Ugyldig invitasjonskode";return C.code==="functions/not-found"?v="Ugyldig invitasjonskode":C.code==="functions/permission-denied"?v="Denne koden er deaktivert":C.code==="functions/resource-exhausted"?v="Denne koden har nådd maksimalt antall bruk":C.code==="functions/already-exists"?v="Du har allerede brukt denne koden":C.message&&(v=C.message),{success:!1,error:v}}return await se(k(f,"users",_.uid),{email:u,firstName:P,lastName:W,division:Y,category:de,powerFactor:H,region:z,club:Q,role:"user",inviteCode:I,createdAt:new Date,draw:null,reloadTime:null,gdprConsent:!0,gdprConsentDate:new Date,gdprVersion:"1.0"}),{success:!0,user:_}}catch(u){console.error("Registrering feilet:",u);let h="Registrering feilet";return u.code==="auth/email-already-in-use"?h="E-postadressen er allerede i bruk":u.code==="auth/weak-password"?h="Passordet må være minst 6 tegn":u.code==="auth/invalid-email"?h="Ugyldig e-postadresse":u.message&&(h=u.message),{success:!1,error:h}}}async function Xe(){try{return await ze(re),{success:!0}}catch(i){return console.error("Utlogging feilet:",i),{success:!1,error:"Kunne ikke logge ut"}}}function S(){return ge}function me(){return F}const et=`
...
`;function tt(i,t){const e=document.getElementById("gdpr-modal");e&&e.remove();const s=document.createElement("div");s.id="gdpr-modal",s.className="gdpr-modal",s.innerHTML=`
  ...
  `,document.body.appendChild(s);const a=s.querySelector(".gdpr-close-btn"),l=s.querySelector(".gdpr-btn-accept"),o=s.querySelector(".gdpr-btn-decline"),p=s.querySelector(".gdpr-modal-overlay"),g=()=>{s.remove()};a.addEventListener("click",()=>{g(),t&&t()}),p.addEventListener("click",()=>{g(),t&&t()}),o.addEventListener("click",()=>{g(),t&&t()}),l.addEventListener("click",()=>{g(),i&&i()}),document.body.style.overflow="hidden";const y=g,u=()=>{document.body.style.overflow="",y()};a.onclick=()=>{u(),t&&t()},p.onclick=()=>{u(),t&&t()},o.onclick=()=>{u(),t&&t()},l.onclick=()=>{u(),i&&i()}}function it(){const i=document.createElement("div");return i.className="gdpr-checkbox-container",i.innerHTML=`
  ...
  `,setTimeout(()=>{const t=i.querySelector("#gdpr-open-modal");t&&t.addEventListener("click",e=>{e.preventDefault(),tt(()=>{const s=document.getElementById("gdpr-consent-checkbox");s&&(s.checked=!0)},()=>{const s=document.getElementById("gdpr-consent-checkbox");s&&(s.checked=!1)})})},0),i}function st(){const i=document.getElementById("gdpr-consent-checkbox");return!i||!i.checked?{valid:!1,error:"Du må godta personvernerklæringen for å registrere deg"}:{valid:!0}}function at(i,t){i.innerHTML=`
    <style>
      * {
...
<div class="modal-overlay" id="modal-confirm-delete" onclick="closeModalOutside(event,'modal-confirm-delete')">
  ...
  `,Et(),_e(),$(),U(),Ce(),le()}function Et(){window.switchTab=It,window.setFilter=Mt,window.openModal=N,window.closeModal=L,window.closeModalOutside=Tt,window.createMatch=_t,window.searchMatchByIdHandler=Ct,window.openEditProfile=Xt,window.saveProfileData=ti,window.selectPF=ei,window.updatePFOptions=$e,window.calcPrognose=le,window.renderMatchList=U,window.selectMatch=$t,window.addShooter=ii,window.addStageResult=si,window.handleLogout=ai,window.openEditMatch=Nt,window.saveEditMatch=Ft,window.openCreateStage=Rt,window.openEditStage=At,window.changeStageNumber=jt,window.changeStageField=Ot,window.saveStage=Ut,window.openInviteUser=Ht,window.openInvitationsModal=Yt,window.acceptInvitation=Qt,window.declineInvitation=Zt,window.searchUsers=zt,window.toggleUserSelection=Vt,window.sendMultipleInvitations=Gt,window.searchUsersNewMatch=Kt,window.toggleUserNewMatch=qt,window.searchUsersEditMatch=Jt,window.toggleUserEditMatch=Wt,window.confirmDeleteMatch=Bt,window.deleteMatchConfirmed=Dt}function It(i){document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active")),document.querySelectorAll(".tab-item").forEach(s=>s.classList.remove("active")),r(i).classList.add("active");const t=["screen-home","screen-matches","screen-prognose","screen-results","screen-profile"].indexOf(i),e=document.querySelectorAll(".tab-item");e[t]&&e[t].classList.add("active"),i==="screen-home"&&$(),i==="screen-matches"&&U(),i==="screen-results"&&ie()}function N(i){r(i).classList.add("open")}function L(i){r(i).classList.remove("open")}function Tt(i,t){i.target.id===t&&L(t)}function Mt(i,t){R=i,document.querySelectorAll(".filter-chip").forEach(e=>e.classList.remove("active")),t.classList.add("active"),U()}async function _t(){var e;const i={name:m("new-match-name")||"Ny match",type:m("new-match-type")||"Trening",date:m("new-match-date")||new Date().toISOString().split("T")[0],location:m("new-match-location")||"",plannedStages:x("new-match-stages",6),searchable:((e=r("new-match-searchable"))==null?void 0:e.checked)!==!1,stages:[],shooters:[],stageDefs:[]},t=await lt(i);if(t.success){let s=0;for(const a of j)(await he(a.email,{matchId:t.matchId,matchName:i.name})).success&&s++;L("modal-new-match"),r("new-match-name").value="",r("new-match-location").value="",r("new-match-stages").value="6",r("new-match-searchable")&&(r("new-match-searchable").checked=!0),r("new-match-user-search").value="",r("new-match-search-results").innerHTML="",j=[],s>0&&alert(`Match opprettet! Invitasjoner sendt til ${s} bruker(e).`)}else alert("Kunne ikke opprette match: "+t.error)}async function Ct(){const i=m("match-id-search").trim();if(!i){alert("Skriv inn et match-ID");return}const t=await ct(i);t.success?(alert(`Match funnet: ${t.match.name} (ID: ${t.match.id})`),r("match-id-search").value=""):alert(`Fant ingen match med ID ${i}${t.error?": "+t.error:""}`)}function $t(i){w=i;const t=b.find(e=>e.id==i);if(t){const e=t.id?"Match ID "+t.id+" "+t.name:t.name||"Match";["home-chip-name","prog-chip-name","results-chip-name"].forEach(a=>{const l=r(a);l&&(l.textContent=e)})}G&&G(),i&&(G=vt(i,e=>{const s=b.findIndex(a=>a.id==i);s!==-1&&e&&(b[s]=e,$(),ie())})),$(),ie(),le()}function Nt(){const i=b.find(s=>s.id==w);if(!i){alert("Ingen match valgt");return}r("edit-match-name").value=i.name||"",r("edit-match-type").value=i.type||"Trening",r("edit-match-date").value=i.date||"",r("edit-match-location").value=i.location||"",r("edit-match-stages").value=i.plannedStages||0,r("edit-match-searchable")&&(r("edit-match-searchable").checked=i.searchable!==!1),r("edit-match-user-search").value="",r("edit-match-search-results").innerHTML="",O=[];const t=me(),e=r("delete-match-btn");e&&t&&i.ownerId===t.uid?e.style.display="block":e&&(e.style.display="none"),N("modal-edit-match")}async function Ft(){var s;const i=b.find(a=>a.id==w);if(!i){alert("Ingen match valgt");return}const t={name:m("edit-match-name")||i.name,type:m("edit-match-type")||i.type,date:m("edit-match-date")||i.date,location:m("edit-match-location")||i.location,plannedStages:x("edit-match-stages",i.plannedStages),searchable:((s=r("edit-match-searchable"))==null?void 0:s.checked)!==!1},e=await oe(i.id,t);if(e.success){let a=0;for(const l of O)(await he(l.email,{matchId:i.id,matchName:t.name})).success&&a++;L("modal-edit-match"),a>0&&alert(`Match oppdatert! Invitasjoner sendt til ${a} bruker(e).`)}else alert("Kunne ikke oppdatere match: "+e.error)}function Bt(){const i=b.find(e=>e.id==w);if(!i){alert("Ingen match valgt");return}const t=i.id?"Match ID "+i.id+" "+i.name:i.name;r("delete-match-name").textContent=t,N("modal-confirm-delete")}async function Dt(){const i=b.find(e=>e.id==w);if(!i){alert("Ingen match valgt");return}const t=await dt(i.id);t.success?(L("modal-confirm-delete"),L("modal-edit-match"),w=null,$(),U(),alert("Match slettet")):alert("Kunne ikke slette match: "+t.error)}let te=null;function Rt(){var t;const i=b.find(e=>e.id==w);if(!i){alert("Ingen match valgt");return}te=null,r("stage-modal-title").textContent=n("create_stage"),r("stage-number").value=(((t=i.stages)==null?void 0:t.length)||0)+1,r("stage-name").value="",r("stage-paper-targets").value=0,r("stage-poppers").value=0,r("stage-plates").value=0,r("stage-no-shoots").value=0,r("stage-bonus-paper-targets").value=0,r("stage-bonus-included").checked=!1,N("modal-create-stage")}function At(i){const t=b.find(s=>s.id==w);if(!t||!t.stages||!t.stages[i]){alert("Stage ikke funnet");return}te=i;const e=t.stages[i];r("stage-modal-title").textContent=n("edit_stage"),r("stage-number").value=e.number||i+1,r("stage-name").value=e.name||"",r("stage-paper-targets").value=e.paperTargets||0,r("stage-poppers").value=e.poppers||0,r("stage-plates").value=e.plates||0,r("stage-no-shoots").value=e.noShoots||0,r("stage-bonus-paper-targets").value=e.bonusPaperTargets||0,r("stage-bonus-included").checked=e.bonusIncluded||!1,N("modal-create-stage")}function jt(i){const t=r("stage-number"),e=Math.max(1,parseInt(t.value)+i);t.value=e}function Ot(i,t){const e=r("stage-"+i),s=Math.max(0,parseInt(e.value)+t);e.value=s}async function Ut(){var a;const i=b.find(l=>l.id==w);if(!i){alert("Ingen match valgt");return}const t={number:x("stage-number",1),name:m("stage-name")||"",paperTargets:x("stage-paper-targets",0),poppers:x("stage-poppers",0),plates:x("stage-plates",0),noShoots:x("stage-no-shoots",0),bonusPaperTargets:x("stage-bonus-paper-targets",0),bonusIncluded:((a=r("stage-bonus-included"))==null?void 0:a.checked)||!1},e=i.stages||[];te!==null?e[te]=t:e.push(t);const s=await oe(i.id,{stages:e});s.success?L("modal-create-stage"):alert("Kunne ikke lagre stage: "+s.error)}let M=[],E=[],j=[],O=[];function Ht(){if(!w){alert("Ingen match valgt");return}E=[],r("user-search-input").value="",r("user-search-results").innerHTML="",r("send-invitations-btn").style.display="none",N("modal-invite-user")}async function zt(){const i=m("user-search-input").trim();if(i.length===0){r("user-search-results").innerHTML='<p style="color:#9ca3af;text-align:center;">Skriv inn et søk</p>';return}const t=await fe(i),e=r("user-search-results");if(!e)return;if(t.length===0){e.innerHTML='<p style="color:#9ca3af;text-align:center;">Ingen brukere funnet</p>';return}let s='<div style="margin-bottom:10px;font-weight:600;">RESULTATER ('+t.length+"):</div>";t.forEach(a=>{const l=a.displayName||a.email,o=E.some(g=>g.uid===a.uid),p=l.replace(/'/g,"\\'");s+=`
    ...
    `}),e.innerHTML=s,Me()}function Vt(i,t,e){const s=E.findIndex(a=>a.uid===i);s>-1?E.splice(s,1):E.push({uid:i,email:t,displayName:e}),Me()}function Me(){const i=r("send-invitations-btn");i&&(E.length>0?(i.style.display="block",i.textContent=`Send invitasjoner (${E.length} valgt)`):i.style.display="none")}async function Gt(){if(E.length===0){alert("Ingen brukere valgt");return}const i=b.find(s=>s.id==w);if(!i)return;let t=0,e=0;for(const s of E)(await he(s.email,{matchId:i.id,matchName:i.name})).success?t++:e++;L("modal-invite-user"),t>0&&alert(`Invitasjoner sendt til ${t} bruker(e)!`),e>0&&alert(`${e} invitasjon(er) feilet.`),E=[]}async function Kt(){const i=m("new-match-user-search").trim();if(i.length===0){r("new-match-search-results").innerHTML="";return}const t=await fe(i),e=r("new-match-search-results");if(!e)return;if(t.length===0){e.innerHTML='<p style="color:#9ca3af;text-align:center;font-size:14px;">Ingen brukere funnet</p>';return}let s='<div style="margin-bottom:8px;font-size:14px;font-weight:600;">RESULTATER ('+t.length+"):</div>";t.forEach(a=>{const l=a.displayName||a.email,o=j.some(g=>g.uid===a.uid),p=l.replace(/'/g,"\\'");s+=`
    ...
    `}),e.innerHTML=s}function qt(i,t,e){const s=j.findIndex(a=>a.uid===i);s>-1?j.splice(s,1):j.push({uid:i,email:t,displayName:e})}async function Jt(){console.log("🔍 searchUsersEditMatch called");const i=m("edit-match-user-search").trim();if(console.log("📝 Search term:",i),i.length===0){console.log("⚠️ Empty search term"),r("edit-match-search-results").innerHTML="";return}console.log("🌐 Calling searchUsersByNameOrEmail...");const t=await fe(i);console.log("✅ Results received:",t);const e=r("edit-match-search-results");if(!e){console.error("❌ Container not found: edit-match-search-results");return}if(t.length===0){console.log("⚠️ No users found"),e.innerHTML='<p style="color:#9ca3af;text-align:center;font-size:14px;">Ingen brukere funnet</p>';return}console.log("🎨 Rendering",t.length,"results");let s='<div style="margin-bottom:8px;font-size:14px;font-weight:600;">RESULTATER ('+t.length+"):</div>";t.forEach(a=>{const l=a.displayName||a.email,o=O.some(g=>g.uid===a.uid),p=l.replace(/'/g,"\\'");s+=`
      <label style="display:flex;align-items:center;gap:8px;padding:8px;background:#2d3748;border-radius:8px;margin-bottom:6px;cursor:pointer;font-size:14px;">
    ...
    `}),e.innerHTML=s,console.log("✅ HTML rendered to container")}function Wt(i,t,e){const s=O.findIndex(a=>a.uid===i);s>-1?O.splice(s,1):O.push({uid:i,email:t,displayName:e})}function Yt(){be(),N("modal-invitations")}function be(){const i=r("invitations-list");if(!i)return;if(!M||M.length===0){i.innerHTML='<div class="empty-state"><div class="empty-sub">'+n("no_invitations")+"</div></div>";return}let t="";M.forEach((e,s)=>{t+='<div class="card" style="margin-bottom:10px;">',t+='<div style="margin-bottom:10px;"><strong>'+n("invited_to_match")+"</strong></div>",t+='<div style="margin-bottom:10px;">Match ID '+e.matchId+" "+e.matchName+"</div>",t+='<div style="margin-bottom:10px;color:#94a3b8;">Fra: '+e.invitedBy+"</div>",t+='<div style="display:flex;gap:10px;">',t+='<button class="btn-primary" onclick="acceptInvitation('+s+')" style="flex:1;">'+n("accept")+"</button>",t+='<button onclick="declineInvitation('+s+')" style="flex:1;padding:12px;background:#374151;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">'+n("decline")+"</button>",t+="</div>",t+="</div>"}),i.innerHTML=t}async function Qt(i){const t=M[i];if(!t)return;const e=await ut(t.matchId);e.success?(M.splice(i,1),ye(),be()):alert("Kunne ikke akseptere invitasjon: "+e.error)}async function Zt(i){const t=M[i];if(!t)return;const e=await mt(t.matchId);e.success?(M.splice(i,1),ye(),be()):alert("Kunne ikke avvise invitasjon: "+e.error)}function ye(){const i=r("invitation-badge");if(!i)return;const t=M.length;t>0?(i.textContent=t,i.style.display="flex"):i.style.display="none"}function $(){var a,l;const i=r("home-content");if(!i)return;const t=b.find(o=>o.id==w);if(!t){i.innerHTML=`
    ...
    `;return}let e="";e+='<div class="card">',e+='<div class="mhc-name">'+t.name+"</div>",e+='<div class="mhc-meta">'+Te(t.date)+" · "+t.type+"</div>",e+='<div class="mhc-stats">',e+='<div><div class="mhc-val">'+(((a=t.stages)==null?void 0:a.length)||0)+'</div><div class="mhc-lbl">Stages</div></div>',e+='<div><div class="mhc-val">'+(((l=t.shooters)==null?void 0:l.length)||0)+'</div><div class="mhc-lbl">Skyttere</div></div>',e+="</div>",e+='<div style="display:flex;gap:10px;margin-top:15px;">',e+='<button class="btn-primary" onclick="openEditMatch()" style="flex:1;">Rediger match</button>',e+='<button class="btn-primary" onclick="openCreateStage()" style="flex:1;">Ny stage</button>',e+="</div>",e+='<div style="margin-top:10px;">',e+='<button class="btn-primary" onclick="openInviteUser()" style="width:100%;">Inviter bruker</button>',e+="</div>";const s=me();s&&t.ownerId===s.uid&&(e+='<div style="margin-top:10px;">',e+='<button onclick="confirmDeleteMatch()" style="width:100%;padding:12px;background:#ef4444;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">Slett match</button>',e+="</div>"),e+="</div>",t.stages&&t.stages.length>0&&(e+='<div class="section-title">Stages</div>',e+='<div class="card">',t.stages.forEach((o,p)=>{e+='<div class="stage-row" style="cursor:pointer;border-bottom:1px solid #2d3748;padding:12px 0;" onclick="openEditStage('+p+')">',e+='<div class="stage-num">S'+(o.number||p+1)+"</div>",e+='<div class="stage-info">',e+='<div class="stage-name">'+(o.name||"Stage "+(o.number||p+1))+"</div>",e+='<div class="stage-meta">',o.paperTargets&&(e+="Paper: "+o.paperTargets+" "),o.poppers&&(e+="Poppers: "+o.poppers+" "),o.plates&&(e+="Plates: "+o.plates+" "),o.noShoots&&(e+="NS: "+o.noShoots+" "),o.bonusPaperTargets&&(e+="Bonus: "+o.bonusPaperTargets+(o.bonusIncluded?" (included)":"")),e+="</div>",e+="</div>",e+="</div>"}),e+="</div>"),i.innerHTML=e}function U(){const i=r("match-list-container");if(!i)return;let t=b.filter(s=>{if(R==="all")return!0;if(R==="active")return s.id===w;if(R==="trening")return s.type==="Trening";if(R==="stevne")return s.type==="Stevne";const a=s.date?new Date(s.date).getFullYear().toString():"";return R===a});if(t.length===0){i.innerHTML='<div class="empty-state"><div class="empty-sub">Ingen matcher funnet</div></div>';return}let e="";t.forEach(s=>{var o;const a=s.id===w,l=s.id?"Match ID "+s.id+" "+s.name:s.name;e+='<div class="match-row">',e+='<div class="match-row-icon'+(a?" is-active":"")+`" onclick="selectMatch('`+s.id+`')">🏆</div>`,e+=`<div class="match-row-info" onclick="selectMatch('`+s.id+`')">`,e+='<div class="match-row-name">'+l+"</div>",e+='<div class="match-row-meta">'+Te(s.date)+" · "+(s.location||s.type)+"</div>",e+="</div>",e+='<div class="match-row-right">',e+=`<button onclick="event.stopPropagation(); selectMatch('`+s.id+`'); openEditMatch();" style="padding:8px 16px;background:#e8b84b;color:#1a202c;border:none;border-radius:8px;font-weight:600;cursor:pointer;margin-bottom:8px;">Rediger</button>`,e+='<div class="match-stg-count">'+(((o=s.stages)==null?void 0:o.length)||0)+"</div>",e+='<div class="match-stg-lbl">stages</div>',e+="</div>",e+="</div>"}),i.innerHTML=e}function ie(){var l;const i=r("results-content");if(!i)return;const t=b.find(o=>o.id===w);if(!t){i.innerHTML='<div class="empty-state"><div class="empty-sub">Velg en match først</div></div>';return}if(!t.shooters||t.shooters.length===0){i.innerHTML='<div class="empty-state"><div class="empty-icon">👥</div><div class="empty-title">Ingen skyttere</div><div class="empty-sub">Trykk + for å legge til skyttere</div></div>';return}let e='<div class="card">';e+='<div class="card-header"><div class="card-title">Standings</div></div>',e+='<div class="standings-table-wrap"><table class="standings-table">',e+='<thead><tr class="standings-header-row">',e+='<th class="standings-th standings-th-rank">#</th>',e+='<th class="standings-th standings-th-shooter">Skytter</th>',e+='<th class="standings-th standings-th-pts">Pts</th>',e+='<th class="standings-th standings-th-pct">%</th>',e+="</tr></thead>",e+="<tbody>";const s=t.shooters.map(o=>{var g;const p=((g=o.stages)==null?void 0:g.reduce((y,u)=>y+(u.pts||0),0))||0;return{...o,totalPts:p}}).sort((o,p)=>p.totalPts-o.totalPts),a=((l=s[0])==null?void 0:l.totalPts)||0;s.forEach((o,p)=>{const g=a>0?(o.totalPts/a*100).toFixed(2):"0.00";e+='<tr class="standings-row">',e+='<td class="standings-td standings-td-rank">'+(p+1)+"</td>",e+='<td class="standings-td standings-td-shooter">',e+='<div class="standings-shooter-name">'+o.firstName+" "+o.lastName+"</div>",e+='<div class="standings-shooter-meta">'+o.division+" · "+A(o.pf||"minor")+"</div>",e+="</td>",e+='<td class="standings-td standings-td-pts">'+o.totalPts.toFixed(2)+"</td>",e+='<td class="standings-td standings-td-pct">'+g+"%</td>",e+="</tr>"}),e+="</tbody></table></div>",e+="</div>",i.innerHTML=e}function _e(){const i=B();["prof-avatar","nav-av-home","nav-av-matches","nav-av-prog","nav-av-results"].forEach(p=>{const g=r(p);g&&(g.textContent=i)});const e=r("prof-name");e&&(e.textContent=(d.firstName||"")+" "+(d.lastName||""));const s=r("prof-div");s&&(s.textContent=(d.division||"—")+" · "+(d.club||"—"));const a=r("prof-badge-pf");a&&(a.textContent=d.powerFactor?A(d.powerFactor):"—");const l=r("prof-badge-region");l&&(l.textContent=d.region||"—");const o={"info-firstname":d.firstName||"—","info-lastname":d.lastName||"—","info-division":d.division||"—","info-category":d.category||"—","info-pf":d.powerFactor?A(d.powerFactor):"—","info-region":d.region||"—","info-club":d.club||"—"};Object.keys(o).forEach(p=>{const g=r(p);g&&(g.textContent=o[p])}),Ce()}function Ce(){const i=[];b.forEach(h=>{h.stages&&h.stages.forEach(I=>i.push(I))});let t=0,e=0,s=0;i.forEach(h=>{t+=h.a||0,e+=(h.a||0)+(h.c||0)+(h.d||0),s+=h.hf||0});const a=i.length?(s/i.length).toFixed(2):"—",l=e?Math.round(t/e*100)+"%":"—",o=r("stat-matches");o&&(o.textContent=b.length);const p=r("stat-stages");p&&(p.textContent=i.length);const g=r("stat-avg-hf");g&&(g.textContent=a);const y=r("stat-a-rate");y&&(y.textContent=l);const u=r("prog-a-rate");u&&(u.textContent=l)}function Xt(){r("edit-firstname").value=d.firstName||"",r("edit-lastname").value=d.lastName||"",r("edit-club").value=d.club||"",r("edit-draw").value=d.draw||"",r("edit-reload").value=d.reloadTime||"";let i="";yt.forEach(s=>{i+='<option value="'+s+'"'+(s===d.division?" selected":"")+">"+s+"</option>"}),r("edit-division").innerHTML=i;let t="";kt.forEach(s=>{t+='<option value="'+s+'"'+(s===d.category?" selected":"")+">"+s+"</option>"}),r("edit-category").innerHTML=t;let e="";xt.forEach(s=>{e+='<option value="'+s+'"'+(s===d.region?" selected":"")+">"+s+"</option>"}),r("edit-region").innerHTML=e,$e(),N("modal-edit-profile")}function $e(){const i=m("edit-division"),t=wt[i]||["minor","major"];let e="";t.forEach(s=>{const a=d.powerFactor===s;e+='<label class="pf-option'+(a?" active":"")+`" onclick="selectPF(this,'`+s+`')">`,e+='<input type="radio" name="pf" value="'+s+'">',e+='<div class="pf-label">'+s.toUpperCase()+"</div>",e+='<div class="pf-sub">'+(s==="major"?"≥170 PF":"<170 PF")+"</div>",e+="</label>"}),r("pf-options").innerHTML=e,t.indexOf(d.powerFactor)<0&&(d.powerFactor=t[0])}function ei(i,t){document.querySelectorAll(".pf-option").forEach(e=>e.classList.remove("active")),i.classList.add("active"),d.powerFactor=t}async function ti(){d.firstName=m("edit-firstname").trim()||"",d.lastName=m("edit-lastname").trim()||"",d.division=m("edit-division")||"",d.category=m("edit-category")||"",d.region=m("edit-region")||"",d.club=m("edit-club").trim()||"",d.draw=K("edit-draw")||null,d.reloadTime=K("edit-reload")||null;const i=await nt(d),t=r("save-profile-btn");i.success?(t.textContent="✓ Lagret!",t.style.background="var(--green)",setTimeout(()=>{t.textContent=n("save_profile"),t.style.background=""},1800)):(t.textContent="❌ Feil!",t.style.background="var(--red)",setTimeout(()=>{t.textContent=n("save_profile"),t.style.background=""},1800)),_e(),le(),$(),L("modal-edit-profile")}function le(){const i=x("prog-shots",12),t=x("prog-targets",6),e=x("prog-steel",2),s=K("prog-move",3),a=K("prog-draw",d.draw||1.42),l=d.division||"Classic",o=d.powerFactor||"minor",p=Pt(i,l,o);r("prog-reloads").value=p;const g=d.reloadTime||1.8,y=.18,u=a+i*y+p*g+s,h=t*10+e*10,I=u>0?h/u:0;r("prog-hf-out").textContent=I.toFixed(2);let P="";P+='<div class="prog-breakdown-detail">',P+="Trekk: "+a.toFixed(2)+"s · ",P+="Skudd: "+(i*y).toFixed(2)+"s · ",P+="Reload: "+(p*g).toFixed(2)+"s · ",P+="Beveg: "+s.toFixed(2)+"s",P+="</div>",r("prog-breakdown").innerHTML=P}async function ii(){const i=b.find(l=>l.id===w);if(!i)return;const t=m("new-shooter-firstname").trim(),e=m("new-shooter-lastname").trim(),s=m("new-shooter-division")||"Classic";if(!t||!e){alert("Fyll inn navn");return}const a={id:"s_"+Date.now(),isMe:!1,firstName:t,lastName:e,division:s,pf:"minor",club:"",stages:[]};i.shooters||(i.shooters=[]),i.shooters.push(a),await oe(i.id,i),L("modal-add-shooter"),ie()}async function si(){const i=b.find(o=>o.id===w);if(!i)return;const t=x("new-result-stage",1),e=K("new-result-time",0),s=x("new-result-points",0),a=e>0?s/e:0,l={num:t,name:"Stage "+t,hf:a,time:e,pts:s,pf:d.powerFactor||"minor"};i.stages||(i.stages=[]),i.stages.push(l),await oe(i.id,i),L("modal-add"),$()}async function ai(){V&&V(),G&&G(),await Xe(),window.location.reload()}const Ne=document.getElementById("app");function ni(){at(Ne,Fe)}function Fe(){Lt(Ne)}Ye(i=>{i?Fe():ni()});
