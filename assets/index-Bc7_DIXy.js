...
<div class="modal-overlay" id="modal-confirm-delete" onclick="closeModalOutside(event,'modal-confirm-delete')">
  ...
  `,It(),Ce(),$(),U(),$e(),le()}function It(){window.switchTab=Mt,window.setFilter=_t,window.openModal=N,window.closeModal=L,window.closeModalOutside=Tt,window.createMatch=Ct,window.searchMatchByIdHandler=$t,window.openEditProfile=ei,window.saveProfileData=ii,window.selectPF=ti,window.updatePFOptions=Ne,window.calcPrognose=le,window.renderMatchList=U,window.selectMatch=Nt,window.addShooter=si,window.addStageResult=ai,window.handleLogout=ni,window.openEditMatch=Ft,window.saveEditMatch=Bt,window.openCreateStage=Te,window.openCreateStageFromEdit=At,window.openEditStage=jt,window.changeStageNumber=Ot,window.changeStageField=Ut,window.saveStage=Ht,window.openInviteUser=zt,window.openInvitationsModal=Qt,window.acceptInvitation=Zt,window.declineInvitation=Xt,window.searchUsers=Vt,window.toggleUserSelection=Gt,window.sendMultipleInvitations=Kt,window.searchUsersNewMatch=qt,window.toggleUserNewMatch=Jt,window.searchUsersEditMatch=Wt,window.toggleUserEditMatch=Yt,window.confirmDeleteMatch=Dt,window.deleteMatchConfirmed=Rt}function Mt(t){document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active")),document.querySelectorAll(".tab-item").forEach(s=>s.classList.remove("active")),l(t).classList.add("active");const i=["screen-home","screen-matches","screen-prognose","screen-results","screen-profile"].indexOf(t),e=document.querySelectorAll(".tab-item");e[i]&&e[i].classList.add("active"),t==="screen-home"&&$(),t==="screen-matches"&&U(),t==="screen-results"&&ie()}function N(t){l(t).classList.add("open")}function L(t){l(t).classList.remove("open")}function Tt(t,i){t.target.id===i&&L(i)}function _t(t,i){R=t,document.querySelectorAll(".filter-chip").forEach(e=>e.classList.remove("active")),i.classList.add("active"),U()}async function Ct(){var e;const t={name:m("new-match-name")||"Ny match",type:m("new-match-type")||"Trening",date:m("new-match-date")||new Date().toISOString().split("T")[0],location:m("new-match-location")||"",plannedStages:x("new-match-stages",6),searchable:((e=l("new-match-searchable"))==null?void 0:e.checked)!==!1,status:"active",stages:[],shooters:[],stageDefs:[]},i=await dt(t);if(i.success){let s=0;for(const a of j)(await he(a.email,{matchId:i.matchId,matchName:t.name})).success&&s++;L("modal-new-match"),l("new-match-name").value="",l("new-match-location").value="",l("new-match-stages").value="6",l("new-match-searchable")&&(l("new-match-searchable").checked=!0),l("new-match-user-search").value="",l("new-match-search-results").innerHTML="",j=[],s>0&&alert(`Match opprettet! Invitasjoner sendt til ${s} bruker(e).`)}else alert("Kunne ikke opprette match: "+i.error)}async function $t(){const t=m("match-id-search").trim();if(!t){alert("Skriv inn et match-ID");return}const i=await pt(t);i.success?(alert(`Match funnet: ${i.match.name} (ID: ${i.match.id})`),l("match-id-search").value=""):alert(`Fant ingen match med ID ${t}${i.error?": "+i.error:""}`)}function Nt(t){w=t;const i=f.find(e=>e.id==t);if(i){const e=i.id?"Match ID "+i.id+" "+i.name:i.name||"Match";["home-chip-name","prog-chip-name","results-chip-name"].forEach(a=>{const r=l(a);r&&(r.textContent=e)})}G&&G(),t&&(G=ut(t,e=>{const s=f.findIndex(a=>a.id==t);s!==-1&&e&&(f[s]=e,$(),ie())})),$(),ie(),le()}function Ft(){const t=f.find(s=>s.id==w);if(!t){alert("Ingen match valgt");return}l("edit-match-name").value=t.name||"",l("edit-match-type").value=t.type||"Trening",l("edit-match-date").value=t.date||"",l("edit-match-location").value=t.location||"",l("edit-match-stages").value=t.plannedStages||0,l("edit-match-searchable")&&(l("edit-match-searchable").checked=t.searchable!==!1),l("edit-match-finished")&&(l("edit-match-finished").checked=t.status==="finished"),l("edit-match-user-search").value="",l("edit-match-search-results").innerHTML="",O=[];const i=me(),e=l("delete-match-btn");e&&i&&t.ownerId===i.uid?e.style.display="block":e&&(e.style.display="none"),N("modal-edit-match")}async function Bt(){var s,a;const t=f.find(r=>r.id==w);if(!t){alert("Ingen match valgt");return}const i={name:m("edit-match-name")||t.name,type:m("edit-match-type")||t.type,date:m("edit-match-date")||t.date,location:m("edit-match-location")||t.location,plannedStages:x("edit-match-stages",t.plannedStages),searchable:((s=l("edit-match-searchable"))==null?void 0:s.checked)!==!1,status:(a=l("edit-match-finished"))!=null&&a.checked?"finished":"active"},e=await oe(t.id,i);if(e.success){let r=0;for(const o of O)(await he(o.email,{matchId:t.id,matchName:i.name})).success&&r++;L("modal-edit-match"),r>0&&alert(`Match oppdatert! Invitasjoner sendt til ${r} bruker(e).`)}else alert("Kunne ikke oppdatere match: "+e.error)}function Dt(){const t=f.find(e=>e.id==w);if(!t){alert("Ingen match valgt");return}const i=t.id?"Match ID "+t.id+" "+t.name:t.name;l("delete-match-name").textContent=i,N("modal-confirm-delete")}async function Rt(){const t=f.find(e=>e.id==w);if(!t){alert("Ingen match valgt");return}const i=await ct(t.id);i.success?(L("modal-confirm-delete"),L("modal-edit-match"),w=null,$(),U(),alert("Match slettet")):alert("Kunne ikke slette match: "+i.error)}let te=null;function Te(){var i;const t=f.find(e=>e.id==w);if(!t){alert("Ingen match valgt");return}te=null,l("stage-modal-title").textContent=n("create_stage"),l("stage-number").value=(((i=t.stages)==null?void 0:i.length)||0)+1,l("stage-name").value="",l("stage-paper-targets").value=0,l("stage-poppers").value=0,l("stage-plates").value=0,l("stage-no-shoots").value=0,l("stage-bonus-paper-targets").value=0,l("stage-bonus-included").checked=!1,N("modal-create-stage")}function At(){L("modal-edit-match"),Te()}function jt(t){const i=f.find(s=>s.id==w);if(!i||!i.stages||!i.stages[t]){alert("Stage ikke funnet");return}te=t;const e=i.stages[t];l("stage-modal-title").textContent=n("edit_stage"),l("stage-number").value=e.number||t+1,l("stage-name").value=e.name||"",l("stage-paper-targets").value=e.paperTargets||0,l("stage-poppers").value=e.poppers||0,l("stage-plates").value=e.plates||0,l("stage-no-shoots").value=e.noShoots||0,l("stage-bonus-paper-targets").value=e.bonusPaperTargets||0,l("stage-bonus-included").checked=e.bonusIncluded||!1,N("modal-create-stage")}function Ot(t){const i=l("stage-number"),e=Math.max(1,parseInt(i.value)+t);i.value=e}function Ut(t,i){const e=l("stage-"+t),s=Math.max(0,parseInt(e.value)+i);e.value=s}async function Ht(){var a;const t=f.find(r=>r.id==w);if(!t){alert("Ingen match valgt");return}const i={number:x("stage-number",1),name:m("stage-name")||"",paperTargets:x("stage-paper-targets",0),poppers:x("stage-poppers",0),plates:x("stage-plates",0),noShoots:x("stage-no-shoots",0),bonusPaperTargets:x("stage-bonus-paper-targets",0),bonusIncluded:((a=l("stage-bonus-included"))==null?void 0:a.checked)||!1},e=t.stages||[];te!==null?e[te]=i:e.push(i);const s=await oe(t.id,{stages:e});s.success?L("modal-create-stage"):alert("Kunne ikke lagre stage: "+s.error)}let T=[],E=[],j=[],O=[];function zt(){if(!w){alert("Ingen match valgt");return}E=[],l("user-search-input").value="",l("user-search-results").innerHTML="",l("send-invitations-btn").style.display="none",N("modal-invite-user")}async function Vt(){const t=m("user-search-input").trim();if(t.length===0){l("user-search-results").innerHTML='<p style="color:#9ca3af;text-align:center;">Skriv inn et søk</p>';return}const i=await fe(t),e=l("user-search-results");if(!e)return;if(i.length===0){e.innerHTML='<p style="color:#9ca3af;text-align:center;">Ingen brukere funnet</p>';return}let s='<div style="margin-bottom:10px;font-weight:600;">RESULTATER ('+i.length+"):</div>";i.forEach(a=>{const r=a.displayName||a.email,o=E.some(g=>g.uid===a.uid),p=r.replace(/'/g,"\\'");s+=`
    ...
    `}),e.innerHTML=s,_e()}function Gt(t,i,e){const s=E.findIndex(a=>a.uid===t);s>-1?E.splice(s,1):E.push({uid:t,email:i,displayName:e}),_e()}function _e(){const t=l("send-invitations-btn");t&&(E.length>0?(t.style.display="block",t.textContent=`Send invitasjoner (${E.length} valgt)`):t.style.display="none")}async function Kt(){if(E.length===0){alert("Ingen brukere valgt");return}const t=f.find(s=>s.id==w);if(!t)return;let i=0,e=0;for(const s of E)(await he(s.email,{matchId:t.id,matchName:t.name})).success?i++:e++;L("modal-invite-user"),i>0&&alert(`Invitasjoner sendt til ${i} bruker(e)!`),e>0&&alert(`${e} invitasjon(er) feilet.`),E=[]}async function qt(){const t=m("new-match-user-search").trim();if(t.length===0){l("new-match-search-results").innerHTML="";return}const i=await fe(t),e=l("new-match-search-results");if(!e)return;if(i.length===0){e.innerHTML='<p style="color:#9ca3af;text-align:center;font-size:14px;">Ingen brukere funnet</p>';return}let s='<div style="margin-bottom:8px;font-size:14px;font-weight:600;">RESULTATER ('+i.length+"):</div>";i.forEach(a=>{const r=a.displayName||a.email,o=j.some(g=>g.uid===a.uid),p=r.replace(/'/g,"\\'");s+=`
    ...
    `}),e.innerHTML=s,console.log("✅ HTML rendered to container")}function Yt(t,i,e){const s=O.findIndex(a=>a.uid===t);s>-1?O.splice(s,1):O.push({uid:t,email:i,displayName:e})}function Qt(){be(),N("modal-invitations")}function be(){const t=l("invitations-list");if(!t)return;if(!T||T.length===0){t.innerHTML='<div class="empty-state"><div class="empty-sub">'+n("no_invitations")+"</div></div>";return}let i="";T.forEach((e,s)=>{i+='<div class="card" style="margin-bottom:10px;">',i+='<div style="margin-bottom:10px;"><strong>'+n("invited_to_match")+"</strong></div>",i+='<div style="margin-bottom:10px;">Match ID '+e.matchId+" "+e.matchName+"</div>",i+='<div style="margin-bottom:10px;color:#94a3b8;">Fra: '+e.invitedBy+"</div>",i+='<div style="display:flex;gap:10px;">',i+='<button class="btn-primary" onclick="acceptInvitation('+s+')" style="flex:1;">'+n("accept")+"</button>",i+='<button onclick="declineInvitation('+s+')" style="flex:1;padding:12px;background:#374151;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">'+n("decline")+"</button>",i+="</div>",i+="</div>"}),t.innerHTML=i}async function Zt(t){const i=T[t];if(!i)return;const e=await mt(i.matchId);e.success?(T.splice(t,1),ye(),be()):alert("Kunne ikke akseptere invitasjon: "+e.error)}async function Xt(t){const i=T[t];if(!i)return;const e=await ht(i.matchId);e.success?(T.splice(t,1),ye(),be()):alert("Kunne ikke avvise invitasjon: "+e.error)}function ye(){const t=l("invitation-badge");if(!t)return;const i=T.length;i>0?(t.textContent=i,t.style.display="flex"):t.style.display="none"}function $(){var a,r;const t=l("home-content");if(!t)return;const i=f.find(o=>o.id==w);if(!i){t.innerHTML=`
