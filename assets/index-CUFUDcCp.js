import{initializeApp as Me}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";import{getAuth as Fe,onAuthStateChanged as $e,signInWithEmailAndPassword as Ne,createUserWithEmailAndPassword as Be,signOut as De}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-auth.js";import{getFirestore as Ae,getDoc as Z,doc as w,setDoc as X,query as ee,collection as te,where as ie,getDocs as be,onSnapshot as ce,serverTimestamp as J,updateDoc as $,arrayUnion as ye}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";import{getFunctions as Re,httpsCallable as je}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-functions.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))s(r);new MutationObserver(r=>{for(const n of r)if(n.type==="childList")for(const d of n.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&s(d)}).observe(document,{childList:!0,subtree:!0});function e(r){const n={};return r.integrity&&(n.integrity=r.integrity),r.referrerPolicy&&(n.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?n.credentials="include":r.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(r){if(r.ep)return;r.ep=!0;const n=e(r);fetch(r.href,n)}})();const Oe={apiKey:"AIzaSyA11OPapSOfwLQzs-oEoF7papT9S3T5p7Q",authDomain:"ipsc-insight-coach.firebaseapp.com",projectId:"ipsc-insight-coach",storageBucket:"ipsc-insight-coach.firebasestorage.app",messagingSenderId:"864793320312",appId:"1:864793320312:web:c586e384bbe365444bc68d",measurementId:"G-3Y4NZTBBS2"},we=Me(Oe),se=Fe(we),b=Ae(we),He=Re();let de=null,M=null;function ze(i){$e(se,async t=>{if(t){de=t;try{const e=await Z(w(b,"users",t.uid));e.exists()?M={uid:t.uid,...e.data()}:M={uid:t.uid,email:t.email,name:t.email?t.email.split("@")[0]:"Bruker",role:"user"},i(M)}catch(e){console.error("Feil ved lasting av brukerprofil:",e),M={uid:t.uid,email:t.email,name:t.email?t.email.split("@")[0]:"Bruker",role:"user"},i(M)}}else de=null,M=null,i(null)})}async function Ue(i,t){try{const e=(i||"").trim();return{success:!0,user:(await Ne(se,e,t||"")).user}}catch(e){console.error("Innlogging feilet:",e);let s="Innlogging feilet";return e.code==="auth/user-not-found"||e.code==="auth/wrong-password"||e.code==="auth/invalid-credential"?s="Feil e-post eller passord":e.code==="auth/invalid-email"?s="Ugyldig e-postadresse":e.code==="auth/user-disabled"&&(s="Denne kontoen er deaktivert"),{success:!1,error:s}}}async function Ve(i,t,e,s,r,n,d,g,v,y){try{const u=(i||"").trim(),m=t||"",L=(e||"").trim(),P=(s||"").trim(),V=(r||"").trim(),G=(n||"").trim(),re=(d||"").trim(),R=(g||"minor").trim(),j=(v||"").trim(),K=(y||"").trim(),T=(await Be(se,u,m)).user,oe=je(He,"validateInviteCode");try{await oe({code:L,userId:T.uid,userEmail:u})}catch(C){await T.delete();let p="Ugyldig invitasjonskode";return C.code==="functions/not-found"?p="Ugyldig invitasjonskode":C.code==="functions/permission-denied"?p="Denne koden er deaktivert":C.code==="functions/resource-exhausted"?p="Denne koden har nådd maksimalt antall bruk":C.code==="functions/already-exists"?p="Du har allerede brukt denne koden":C.message&&(p=C.message),{success:!1,error:p}}return await X(w(b,"users",T.uid),{email:u,firstName:P,lastName:V,division:G,category:re,powerFactor:R,region:j,club:K,role:"user",inviteCode:L,createdAt:new Date,draw:null,reloadTime:null,gdprConsent:!0,gdprConsentDate:new Date,gdprVersion:"1.0"}),{success:!0,user:T}}catch(u){console.error("Registrering feilet:",u);let m="Registrering feilet";return u.code==="auth/email-already-in-use"?m="E-postadressen er allerede i bruk":u.code==="auth/weak-password"?m="Passordet må være minst 6 tegn":u.code==="auth/invalid-email"?m="Ugyldig e-postadresse":u.message&&(m=u.message),{success:!1,error:m}}}async function Ge(){try{return await De(se),{success:!0}}catch(i){return console.error("Utlogging feilet:",i),{success:!1,error:"Kunne ikke logge ut"}}}function S(){return de}function Ke(){return M}const qe=`
...
`;function We(i,t){const e=document.getElementById("gdpr-modal");e&&e.remove();const s=document.createElement("div");s.id="gdpr-modal",s.className="gdpr-modal",s.innerHTML=`
  ...
  `,document.body.appendChild(s);const r=s.querySelector(".gdpr-close-btn"),n=s.querySelector(".gdpr-btn-accept"),d=s.querySelector(".gdpr-btn-decline"),g=s.querySelector(".gdpr-modal-overlay"),v=()=>{s.remove()};r.addEventListener("click",()=>{v(),t&&t()}),g.addEventListener("click",()=>{v(),t&&t()}),d.addEventListener("click",()=>{v(),t&&t()}),n.addEventListener("click",()=>{v(),i&&i()}),document.body.style.overflow="hidden";const y=v,u=()=>{document.body.style.overflow="",y()};r.onclick=()=>{u(),t&&t()},g.onclick=()=>{u(),t&&t()},d.onclick=()=>{u(),t&&t()},n.onclick=()=>{u(),i&&i()}}function Je(){const i=document.createElement("div");return i.className="gdpr-checkbox-container",i.innerHTML=`
  ...
  `,setTimeout(()=>{const t=i.querySelector("#gdpr-open-modal");t&&t.addEventListener("click",e=>{e.preventDefault(),We(()=>{const s=document.getElementById("gdpr-consent-checkbox");s&&(s.checked=!0)},()=>{const s=document.getElementById("gdpr-consent-checkbox");s&&(s.checked=!1)})})},0),i}function Ye(){const i=document.getElementById("gdpr-consent-checkbox");return!i||!i.checked?{valid:!1,error:"Du må godta personvernerklæringen for å registrere deg"}:{valid:!0}}function Qe(i,t){i.innerHTML=`
    <style>
      * {
...
<div class="screen active" id="screen-home">
  <div class="navbar">
    ...
    <div class="nav-avatar" id="nav-av-home" onclick="switchTab('screen-profile')">${F()}</div>
  </div>
...
<div class="screen" id="screen-matches">
  <div class="navbar">
    ...
    <div style="display:flex;align-items:center;gap:15px;">
      ...
      <div class="nav-avatar" id="nav-av-matches" onclick="switchTab('screen-profile')">${F()}</div>
    </div>
  </div>
...
<div class="screen" id="screen-prognose">
  <div class="navbar">
    ...
    <div class="nav-avatar" id="nav-av-prog" onclick="switchTab('screen-profile')">${F()}</div>
  </div>
  <div class="scroll-content">
...
<div class="screen" id="screen-results">
  <div class="navbar">
    ...
    <div class="nav-avatar" id="nav-av-results" onclick="switchTab('screen-profile')">${F()}</div>
  </div>
...
<div class="screen" id="screen-profile">
  <div class="navbar">
    ...
    <div class="nav-avatar" id="prof-avatar" onclick="switchTab('screen-home')">${F()}</div>
  </div>
  <div class="scroll-content">
...
<div class="modal-overlay" id="modal-invitations" onclick="closeModalOutside(event,'modal-invitations')">
  ...
  `,yt(),Pe(),N(),U(),Se(),ne()}function yt(){window.switchTab=wt,window.setFilter=xt,window.openModal=B,window.closeModal=I,window.closeModalOutside=kt,window.createMatch=Pt,window.searchMatchByIdHandler=St,window.openEditProfile=Rt,window.saveProfileData=Ot,window.selectPF=jt,window.updatePFOptions=Le,window.calcPrognose=ne,window.renderMatchList=U,window.selectMatch=Lt,window.addShooter=Ht,window.addStageResult=zt,window.handleLogout=Ut,window.openEditMatch=Et,window.saveEditMatch=_t,window.openCreateStage=It,window.openEditStage=Tt,window.changeStageNumber=Ct,window.changeStageField=Mt,window.saveStage=Ft,window.openInviteUser=$t,window.sendInvitation=Nt,window.openInvitationsModal=Bt,window.acceptInvitation=Dt,window.declineInvitation=At}function wt(i){document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active")),document.querySelectorAll(".tab-item").forEach(s=>s.classList.remove("active")),l(i).classList.add("active");const t=["screen-home","screen-matches","screen-prognose","screen-results","screen-profile"].indexOf(i),e=document.querySelectorAll(".tab-item");e[t]&&e[t].classList.add("active"),i==="screen-home"&&N(),i==="screen-matches"&&U(),i==="screen-results"&&Q()}function B(i){l(i).classList.add("open")}function I(i){l(i).classList.remove("open")}function kt(i,t){i.target.id===t&&I(t)}function xt(i,t){D=i,document.querySelectorAll(".filter-chip").forEach(e=>e.classList.remove("active")),t.classList.add("active"),U()}async function Pt(){var e;const i={name:h("new-match-name")||"Ny match",type:h("new-match-type")||"Trening",date:h("new-match-date")||new Date().toISOString().split("T")[0],location:h("new-match-location")||"",plannedStages:x("new-match-stages",6),searchable:((e=l("new-match-searchable"))==null?void 0:e.checked)!==!1,stages:[],shooters:[],stageDefs:[]},t=await tt(i);t.success?(I("modal-new-match"),l("new-match-name").value="",l("new-match-location").value="",l("new-match-stages").value="6",l("new-match-searchable")&&(l("new-match-searchable").checked=!0)):alert("Kunne ikke opprette match: "+t.error)}async function St(){const i=h("match-id-search").trim();if(!i){alert("Skriv inn et match-ID");return}const t=await it(i);t.success?(alert(`Match funnet: ${t.match.name} (ID: ${t.match.id})`),l("match-id-search").value=""):alert(`Fant ingen match med ID ${i}${t.error?": "+t.error:""}`)}function Lt(i){k=i;const t=f.find(e=>e.id==i);if(t){const e=t.id?"Match ID "+t.id+" "+t.name:t.name||"Match";["home-chip-name","prog-chip-name","results-chip-name"].forEach(r=>{const n=l(r);n&&(n.textContent=e)})}H&&H(),i&&(H=nt(i,e=>{const s=f.findIndex(r=>r.id==i);s!==-1&&e&&(f[s]=e,N(),Q())})),N(),Q(),ne()}function Et(){const i=f.find(t=>t.id==k);if(!i){alert("Ingen match valgt");return}l("edit-match-name").value=i.name||"",l("edit-match-type").value=i.type||"Trening",l("edit-match-date").value=i.date||"",l("edit-match-location").value=i.location||"",l("edit-match-stages").value=i.plannedStages||0,l("edit-match-searchable")&&(l("edit-match-searchable").checked=i.searchable!==!1),B("modal-edit-match")}async function _t(){var s;const i=f.find(r=>r.id==k);if(!i){alert("Ingen match valgt");return}const t={name:h("edit-match-name")||i.name,type:h("edit-match-type")||i.type,date:h("edit-match-date")||i.date,location:h("edit-match-location")||i.location,plannedStages:x("edit-match-stages",i.plannedStages),searchable:((s=l("edit-match-searchable"))==null?void 0:s.checked)!==!1},e=await ae(i.id,t);e.success?I("modal-edit-match"):alert("Kunne ikke oppdatere match: "+e.error)}let Y=null;function It(){var t;const i=f.find(e=>e.id==k);if(!i){alert("Ingen match valgt");return}Y=null,l("stage-modal-title").textContent=a("create_stage"),l("stage-number").value=(((t=i.stages)==null?void 0:t.length)||0)+1,l("stage-name").value="",l("stage-paper-targets").value=0,l("stage-poppers").value=0,l("stage-plates").value=0,l("stage-no-shoots").value=0,l("stage-bonus-paper-targets").value=0,l("stage-bonus-included").checked=!1,B("modal-create-stage")}function Tt(i){const t=f.find(s=>s.id==k);if(!t||!t.stages||!t.stages[i]){alert("Stage ikke funnet");return}Y=i;const e=t.stages[i];l("stage-modal-title").textContent=a("edit_stage"),l("stage-number").value=e.number||i+1,l("stage-name").value=e.name||"",l("stage-paper-targets").value=e.paperTargets||0,l("stage-poppers").value=e.poppers||0,l("stage-plates").value=e.plates||0,l("stage-no-shoots").value=e.noShoots||0,l("stage-bonus-paper-targets").value=e.bonusPaperTargets||0,l("stage-bonus-included").checked=e.bonusIncluded||!1,B("modal-create-stage")}function Ct(i){const t=l("stage-number"),e=Math.max(1,parseInt(t.value)+i);t.value=e}function Mt(i,t){const e=l("stage-"+i),s=Math.max(0,parseInt(e.value)+t);e.value=s}async function Ft(){var r;const i=f.find(n=>n.id==k);if(!i){alert("Ingen match valgt");return}const t={number:x("stage-number",1),name:h("stage-name")||"",paperTargets:x("stage-paper-targets",0),poppers:x("stage-poppers",0),plates:x("stage-plates",0),noShoots:x("stage-no-shoots",0),bonusPaperTargets:x("stage-bonus-paper-targets",0),bonusIncluded:((r=l("stage-bonus-included"))==null?void 0:r.checked)||!1},e=i.stages||[];Y!==null?e[Y]=t:e.push(t);const s=await ae(i.id,{stages:e});s.success?I("modal-create-stage"):alert("Kunne ikke lagre stage: "+s.error)}let _=[];function $t(){if(!k){alert("Ingen match valgt");return}l("invite-user-email").value="",B("modal-invite-user")}async function Nt(){const i=h("invite-user-email").trim();if(!i){alert("Skriv inn en e-postadresse");return}const t=f.find(s=>s.id==k);if(!t)return;const e=await rt(i,{matchId:t.id,matchName:t.name});e.success?(I("modal-invite-user"),alert(a("invitation_sent"))):alert("Kunne ikke sende invitasjon: "+e.error)}function Bt(){pe(),B("modal-invitations")}function pe(){const i=l("invitations-list");if(!i)return;if(!_||_.length===0){i.innerHTML='<div class="empty-state"><div class="empty-sub">'+a("no_invitations")+"</div></div>";return}let t="";_.forEach((e,s)=>{t+='<div class="card" style="margin-bottom:10px;">',t+='<div style="margin-bottom:10px;"><strong>'+a("invited_to_match")+"</strong></div>",t+='<div style="margin-bottom:10px;">Match ID '+e.matchId+" "+e.matchName+"</div>",t+='<div style="margin-bottom:10px;color:#94a3b8;">Fra: '+e.invitedBy+"</div>",t+='<div style="display:flex;gap:10px;">',t+='<button class="btn-primary" onclick="acceptInvitation('+s+')" style="flex:1;">'+a("accept")+"</button>",t+='<button onclick="declineInvitation('+s+')" style="flex:1;padding:12px;background:#374151;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">'+a("decline")+"</button>",t+="</div>",t+="</div>"}),i.innerHTML=t}async function Dt(i){const t=_[i];if(!t)return;const e=await ot(t.matchId);e.success?(_.splice(i,1),ve(),pe()):alert("Kunne ikke akseptere invitasjon: "+e.error)}async function At(i){const t=_[i];if(!t)return;const e=await lt(t.matchId);e.success?(_.splice(i,1),ve(),pe()):alert("Kunne ikke avvise invitasjon: "+e.error)}function ve(){const i=l("invitation-badge");if(!i)return;const t=_.length;t>0?(i.textContent=t,i.style.display="flex"):i.style.display="none"}function N(){var s,r;const i=l("home-content");if(!i)return;const t=f.find(n=>n.id===k);if(!t){i.innerHTML=`
    ...
    `;return}let e="";e+='<div class="card">',e+='<div class="mhc-name">'+t.name+"</div>",e+='<div class="mhc-meta">'+xe(t.date)+" · "+t.type+"</div>",e+='<div class="mhc-stats">',e+='<div><div class="mhc-val">'+(((s=t.stages)==null?void 0:s.length)||0)+'</div><div class="mhc-lbl">Stages</div></div>',e+='<div><div class="mhc-val">'+(((r=t.shooters)==null?void 0:r.length)||0)+'</div><div class="mhc-lbl">Skyttere</div></div>',e+="</div>",e+='<div style="display:flex;gap:10px;margin-top:15px;">',e+='<button class="btn-primary" onclick="openEditMatch()" style="flex:1;">Rediger match</button>',e+='<button class="btn-primary" onclick="openCreateStage()" style="flex:1;">Ny stage</button>',e+="</div>",e+='<div style="margin-top:10px;">',e+='<button class="btn-primary" onclick="openInviteUser()" style="width:100%;">Inviter bruker</button>',e+="</div>",e+="</div>",t.stages&&t.stages.length>0&&(e+='<div class="section-title">Stages</div>',e+='<div class="card">',t.stages.forEach((n,d)=>{e+='<div class="stage-row" style="cursor:pointer;border-bottom:1px solid #2d3748;padding:12px 0;" onclick="openEditStage('+d+')">',e+='<div class="stage-num">S'+(n.number||d+1)+"</div>",e+='<div class="stage-info">',e+='<div class="stage-name">'+(n.name||"Stage "+(n.number||d+1))+"</div>",e+='<div class="stage-meta">',n.paperTargets&&(e+="Paper: "+n.paperTargets+" "),n.poppers&&(e+="Poppers: "+n.poppers+" "),n.plates&&(e+="Plates: "+n.plates+" "),n.noShoots&&(e+="NS: "+n.noShoots+" "),n.bonusPaperTargets&&(e+="Bonus: "+n.bonusPaperTargets+(n.bonusIncluded?" (included)":"")),e+="</div>",e+="</div>",e+="</div>"}),e+="</div>"),i.innerHTML=e}function U(){const i=l("match-list-container");if(!i)return;let t=f.filter(s=>{if(D==="all")return!0;if(D==="active")return s.id===k;if(D==="trening")return s.type==="Trening";if(D==="stevne")return s.type==="Stevne";const r=s.date?new Date(s.date).getFullYear().toString():"";return D===r});if(t.length===0){i.innerHTML='<div class="empty-state"><div class="empty-sub">Ingen matcher funnet</div></div>';return}let e="";t.forEach(s=>{var d;const r=s.id===k,n=s.id?"Match ID "+s.id+" "+s.name:s.name;e+='<div class="match-row">',e+='<div class="match-row-icon'+(r?" is-active":"")+`" onclick="selectMatch('`+s.id+`')">🏆</div>`,e+=`<div class="match-row-info" onclick="selectMatch('`+s.id+`')">`,e+='<div class="match-row-name">'+n+"</div>",e+='<div class="match-row-meta">'+xe(s.date)+" · "+(s.location||s.type)+"</div>",e+="</div>",e+='<div class="match-row-right">',e+=`<button onclick="event.stopPropagation(); selectMatch('`+s.id+`'); openEditMatch();" style="padding:8px 16px;background:#e8b84b;color:#1a202c;border:none;border-radius:8px;font-weight:600;cursor:pointer;margin-bottom:8px;">Rediger</button>`,e+='<div class="match-stg-count">'+(((d=s.stages)==null?void 0:d.length)||0)+"</div>",e+='<div class="match-stg-lbl">stages</div>',e+="</div>",e+="</div>"}),i.innerHTML=e}function Q(){var n;const i=l("results-content");if(!i)return;const t=f.find(d=>d.id===k);if(!t){i.innerHTML='<div class="empty-state"><div class="empty-sub">Velg en match først</div></div>';return}if(!t.shooters||t.shooters.length===0){i.innerHTML='<div class="empty-state"><div class="empty-icon">👥</div><div class="empty-title">Ingen skyttere</div><div class="empty-sub">Trykk + for å legge til skyttere</div></div>';return}let e='<div class="card">';e+='<div class="card-header"><div class="card-title">Standings</div></div>',e+='<div class="standings-table-wrap"><table class="standings-table">',e+='<thead><tr class="standings-header-row">',e+='<th class="standings-th standings-th-rank">#</th>',e+='<th class="standings-th standings-th-shooter">Skytter</th>',e+='<th class="standings-th standings-th-pts">Pts</th>',e+='<th class="standings-th standings-th-pct">%</th>',e+="</tr></thead>",e+="<tbody>";const s=t.shooters.map(d=>{var v;const g=((v=d.stages)==null?void 0:v.reduce((y,u)=>y+(u.pts||0),0))||0;return{...d,totalPts:g}}).sort((d,g)=>g.totalPts-d.totalPts),r=((n=s[0])==null?void 0:n.totalPts)||0;s.forEach((d,g)=>{const v=r>0?(d.totalPts/r*100).toFixed(2):"0.00";e+='<tr class="standings-row">',e+='<td class="standings-td standings-td-rank">'+(g+1)+"</td>",e+='<td class="standings-td standings-td-shooter">',e+='<div class="standings-shooter-name">'+d.firstName+" "+d.lastName+"</div>",e+='<div class="standings-shooter-meta">'+d.division+" · "+A(d.pf||"minor")+"</div>",e+="</td>",e+='<td class="standings-td standings-td-pts">'+d.totalPts.toFixed(2)+"</td>",e+='<td class="standings-td standings-td-pct">'+v+"%</td>",e+="</tr>"}),e+="</tbody></table></div>",e+="</div>",i.innerHTML=e}function Pe(){const i=F();["prof-avatar","nav-av-home","nav-av-matches","nav-av-prog","nav-av-results"].forEach(g=>{const v=l(g);v&&(v.textContent=i)});const e=l("prof-name");e&&(e.textContent=(o.firstName||"")+" "+(o.lastName||""));const s=l("prof-div");s&&(s.textContent=(o.division||"—")+" · "+(o.club||"—"));const r=l("prof-badge-pf");r&&(r.textContent=o.powerFactor?A(o.powerFactor):"—");const n=l("prof-badge-region");n&&(n.textContent=o.region||"—");const d={"info-firstname":o.firstName||"—","info-lastname":o.lastName||"—","info-division":o.division||"—","info-category":o.category||"—","info-pf":o.powerFactor?A(o.powerFactor):"—","info-region":o.region||"—","info-club":o.club||"—"};Object.keys(d).forEach(g=>{const v=l(g);v&&(v.textContent=d[g])}),Se()}function Se(){const i=[];f.forEach(m=>{m.stages&&m.stages.forEach(L=>i.push(L))});let t=0,e=0,s=0;i.forEach(m=>{t+=m.a||0,e+=(m.a||0)+(m.c||0)+(m.d||0),s+=m.hf||0});const r=i.length?(s/i.length).toFixed(2):"—",n=e?Math.round(t/e*100)+"%":"—",d=l("stat-matches");d&&(d.textContent=f.length);const g=l("stat-stages");g&&(g.textContent=i.length);const v=l("stat-avg-hf");v&&(v.textContent=r);const y=l("stat-a-rate");y&&(y.textContent=n);const u=l("prog-a-rate");u&&(u.textContent=n)}function Rt(){l("edit-firstname").value=o.firstName||"",l("edit-lastname").value=o.lastName||"",l("edit-club").value=o.club||"",l("edit-draw").value=o.draw||"",l("edit-reload").value=o.reloadTime||"";let i="";vt.forEach(s=>{i+='<option value="'+s+'"'+(s===o.division?" selected":"")+">"+s+"</option>"}),l("edit-division").innerHTML=i;let t="";ut.forEach(s=>{t+='<option value="'+s+'"'+(s===o.category?" selected":"")+">"+s+"</option>"}),l("edit-category").innerHTML=t;let e="";mt.forEach(s=>{e+='<option value="'+s+'"'+(s===o.region?" selected":"")+">"+s+"</option>"}),l("edit-region").innerHTML=e,Le(),B("modal-edit-profile")}function Le(){const i=h("edit-division"),t=gt[i]||["minor","major"];let e="";t.forEach(s=>{const r=o.powerFactor===s;e+='<label class="pf-option'+(r?" active":"")+`" onclick="selectPF(this,'`+s+`')">`,e+='<input type="radio" name="pf" value="'+s+'">',e+='<div class="pf-label">'+s.toUpperCase()+"</div>",e+='<div class="pf-sub">'+(s==="major"?"≥170 PF":"<170 PF")+"</div>",e+="</label>"}),l("pf-options").innerHTML=e,t.indexOf(o.powerFactor)<0&&(o.powerFactor=t[0])}function jt(i,t){document.querySelectorAll(".pf-option").forEach(e=>e.classList.remove("active")),i.classList.add("active"),o.powerFactor=t}async function Ot(){o.firstName=h("edit-firstname").trim()||"",o.lastName=h("edit-lastname").trim()||"",o.division=h("edit-division")||"",o.category=h("edit-category")||"",o.region=h("edit-region")||"",o.club=h("edit-club").trim()||"",o.draw=z("edit-draw")||null,o.reloadTime=z("edit-reload")||null;const i=await Ze(o),t=l("save-profile-btn");i.success?(t.textContent="✓ Lagret!",t.style.background="var(--green)",setTimeout(()=>{t.textContent=a("save_profile"),t.style.background=""},1800)):(t.textContent="❌ Feil!",t.style.background="var(--red)",setTimeout(()=>{t.textContent=a("save_profile"),t.style.background=""},1800)),Pe(),ne(),N(),I("modal-edit-profile")}function ne(){const i=x("prog-shots",12),t=x("prog-targets",6),e=x("prog-steel",2),s=z("prog-move",3),r=z("prog-draw",o.draw||1.42),n=o.division||"Classic",d=o.powerFactor||"minor",g=ft(i,n,d);l("prog-reloads").value=g;const v=o.reloadTime||1.8,y=.18,u=r+i*y+g*v+s,m=t*10+e*10,L=u>0?m/u:0;l("prog-hf-out").textContent=L.toFixed(2);let P="";P+='<div class="prog-breakdown-detail">',P+="Trekk: "+r.toFixed(2)+"s · ",P+="Skudd: "+(i*y).toFixed(2)+"s · ",P+="Reload: "+(g*v).toFixed(2)+"s · ",P+="Beveg: "+s.toFixed(2)+"s",P+="</div>",l("prog-breakdown").innerHTML=P}async function Ht(){const i=f.find(n=>n.id===k);if(!i)return;const t=h("new-shooter-firstname").trim(),e=h("new-shooter-lastname").trim(),s=h("new-shooter-division")||"Classic";if(!t||!e){alert("Fyll inn navn");return}const r={id:"s_"+Date.now(),isMe:!1,firstName:t,lastName:e,division:s,pf:"minor",club:"",stages:[]};i.shooters||(i.shooters=[]),i.shooters.push(r),await ae(i.id,i),I("modal-add-shooter"),Q()}async function zt(){const i=f.find(d=>d.id===k);if(!i)return;const t=x("new-result-stage",1),e=z("new-result-time",0),s=x("new-result-points",0),r=e>0?s/e:0,n={num:t,name:"Stage "+t,hf:r,time:e,pts:s,pf:o.powerFactor||"minor"};i.stages||(i.stages=[]),i.stages.push(n),await ae(i.id,i),I("modal-add"),N()}async function Ut(){O&&O(),H&&H(),await Ge(),window.location.reload()}const Ee=document.getElementById("app");function Vt(){Qe(Ee,_e)}function _e(){bt(Ee)}ze(i=>{i?_e():Vt()});
