...
`;function At(e,i){const t=document.getElementById("gdpr-modal");t&&t.remove();const s=document.createElement("div");s.id="gdpr-modal",s.className="gdpr-modal",s.innerHTML=`
 ...
 `,document.body.appendChild(s);const a=s.querySelector(".gdpr-close-btn"),n=s.querySelector(".gdpr-btn-accept"),r=s.querySelector(".gdpr-btn-decline"),u=s.querySelector(".gdpr-modal-overlay"),m=()=>{s.remove()};a.addEventListener("click",()=>{m(),i&&i()}),u.addEventListener("click",()=>{m(),i&&i()}),r.addEventListener("click",()=>{m(),i&&i()}),n.addEventListener("click",()=>{m(),e&&e()}),document.body.style.overflow="hidden";const b=m,f=()=>{document.body.style.overflow="",b()};a.onclick=()=>{f(),i&&i()},u.onclick=()=>{f(),i&&i()},r.onclick=()=>{f(),i&&i()},n.onclick=()=>{f(),e&&e()}}function Rt(){const e=document.createElement("div");e.className="gdpr-checkbox-container";e.style.cssText="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:14px 16px;margin-bottom:16px;"
e.innerHTML=`
 ...
 `,setTimeout(()=>{const i=e.querySelector("#gdpr-open-modal");i&&i.addEventListener("click",t=>{t.preventDefault(),At(()=>{const s=document.getElementById("gdpr-consent-checkbox");s&&(s.checked=!0)},()=>{const s=document.getElementById("gdpr-consent-checkbox");s&&(s.checked=!1)})})},0);return e}function Dt(){const e=document.getElementById("gdpr-consent-checkbox");return!e||!e.checked?{valid:!1,error:dt==="no"?"You must accept the Privacy Policy to create an account":"You must accept the Privacy Policy to create an account"}:{valid:!0}}function Bt(e,i){e.innerHTML=`
<style>
 *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
 :root {
  --accent: #f5c800; --accent2: #d9ae00;
  --bg: #0d0e10; --card: #181a1c;
  --border: rgba(255,255,255,0.08); --border-gold: rgba(245,200,0,0.28);
  --text: #eef0ec; --muted: rgba(255,255,255,0.38);
  --stripe: repeating-linear-gradient(-55deg,#f5c800 0px,#f5c800 7px,transparent 7px,transparent 14px);
 }
 ...
 .lang-btn.active { background: rgba(245,200,0,0.15); color: #f5c800; }
 ...
 .login-tagline { font-family: 'Share Tech Mono', monospace; font-size: 10px; letter-spacing: 0.18em; color: var(--muted); text-transform: uppercase; margin-top: 10px; }
 .login-status { display: flex; align-items: center; gap: 8px; background: rgba(245,200,0,0.05); border: 1px solid rgba(245,200,0,0.20); border-left: 2px solid #6aad3e; clip-path: polygon(6px 0%,100% 0%,calc(100% - 6px) 100%,0% 100%); padding: 9px 14px; margin-bottom: 18px; }
 .status-dot { width: 6px; height: 6px; border-radius: 50%; background: #22c55e; box-shadow: 0 0 6px #22c55e; flex-shrink: 0; }
 .status-text { font-family: 'Share Tech Mono', monospace; font-size: 11px; font-weight: 400; letter-spacing: 0.14em; color: rgba(255,255,255,0.55); text-transform: uppercase; }
 ...
 .field-lbl { font-family: 'Share Tech Mono', monospace; font-size: 10px; font-weight: 400; letter-spacing: 0.14em; color: var(--muted); text-transform: uppercase; margin-bottom: 7px; display: flex; align-items: center; gap: 6px; }
 ...
 .field:focus { border-color: rgba(245,200,0,0.55); background: rgba(255,255,255,0.05); box-shadow: 0 0 0 3px rgba(245,200,0,0.08); }
 ...
 .btn-secondary { width: 100%; padding: 11px; background: transparent; color: var(--accent); border: 1px solid rgba(245,200,0,0.30); border-radius: 6px; font-family: 'Outfit', sans-serif; font-size: 14px; font-weight: 600; letter-spacing: 0.04em; text-transform: uppercase; cursor: pointer; transition: background 0.15s; }
 .btn-secondary:hover { background: rgba(245,200,0,0.08); }
 ...
 .pf-btn.selected { background: rgba(245,200,0,0.10); border-color: var(--accent); color: var(--accent); }
 ...
 .gdpr-modal-inner h3 { font-family: 'Share Tech Mono', monospace; font-size: 10px; font-weight: 400; letter-spacing: 0.14em; color: var(--accent); text-transform: uppercase; margin: 14px 0 5px; }
...
/* === NAV AVATAR === */\
.nav-avatar{width:38px!important;height:38px!important;border-radius:50%!important;background:linear-gradient(135deg,var(--accent) 0%,var(--accent2) 100%)!important;color:#ffffff!important;font-weight:800!important;font-size:13px!important;display:flex!important;align-items:center!important;justify-content:center!important;cursor:pointer!important;box-shadow:0 0 0 2px rgba(245,200,0,0.25),0 0 12px rgba(245,200,0,0.15)!important;letter-spacing:0.03em;flex-shrink:0;}\
...
.nav-refresh-btn:hover{color:var(--accent)!important;background:rgba(245,200,0,0.07)!important;}\
...
/* === BUTTONS === */\
.btn-primary,.primary-btn{background:linear-gradient(135deg,var(--accent) 0%,var(--accent2) 100%)!important;color:#ffffff!important;border:none!important;border-radius:var(--radius-sm)!important;font-weight:800!important;font-size:12px!important;letter-spacing:0.12em!important;text-transform:uppercase!important;padding:12px 18px!important;cursor:pointer!important;transition:opacity 0.15s!important;}\
.btn-primary:hover,.primary-btn:hover{opacity:0.88!important;}\
.btn-secondary,.secondary-btn{background:transparent!important;border:1px solid rgba(245,200,0,0.28)!important;border-radius:var(--radius-sm)!important;color:var(--accent)!important;font-size:11px!important;font-weight:700!important;letter-spacing:0.08em!important;text-transform:uppercase!important;padding:8px 12px!important;cursor:pointer!important;transition:background 0.15s,border-color 0.15s!important;}\
.btn-secondary:hover{background:var(--accent-fade)!important;border-color:rgba(245,200,0,0.50)!important;}\
/* === FAB === */\
.fab{background:linear-gradient(135deg,var(--accent) 0%,var(--accent2) 100%)!important;color:#ffffff!important;border:none!important;border-radius:50%!important;width:54px!important;height:54px!important;font-size:22px!important;cursor:pointer!important;box-shadow:0 4px 20px rgba(245,200,0,0.30)!important;display:flex!important;align-items:center!important;justify-content:center!important;position:fixed!important;bottom:78px!important;right:18px!important;z-index:100!important;transition:transform 0.15s!important;}\
...
/* === PROFILE AVATAR === */\
#prof-avatar{width:78px!important;height:78px!important;border-radius:50%!important;background:linear-gradient(135deg,var(--accent) 0%,var(--accent2) 100%)!important;color:#ffffff!important;font-weight:900!important;font-size:26px!important;display:flex!important;align-items:center!important;justify-content:center!important;box-shadow:0 0 0 3px var(--bg2),0 0 0 5px rgba(245,200,0,0.30),0 0 20px rgba(245,200,0,0.15)!important;overflow:hidden!important;cursor:pointer!important;}\
...
::-webkit-scrollbar-thumb{background:rgba(245,200,0,0.25);border-radius:3px;}\
...
.ipad-sidebar{display:none;position:fixed;left:0;top:0;bottom:0;width:72px;background:linear-gradient(180deg,#0a0c0f 0%,#0d1017 60%,#0a0c0f 100%);border-right:1px solid rgba(245,200,0,0.10);flex-direction:column;align-items:center;padding:18px 0 16px;gap:2px;z-index:200;box-shadow:4px 0 40px rgba(0,0,0,0.6);}\
.ipad-logo{font-size:14px;font-weight:900;color:var(--accent);letter-spacing:0.1em;margin-bottom:20px;text-shadow:0 0 24px rgba(245,200,0,0.50);border-bottom:1px solid rgba(245,200,0,0.15);padding-bottom:16px;width:100%;text-align:center;}\
...
.ipad-nav-item:hover{color:rgba(245,200,0,0.8);background:rgba(245,200,0,0.05);}\
.ipad-nav-item.active{color:var(--accent);border-left-color:var(--accent);background:rgba(245,200,0,0.07);}\
.ipad-nav-item.active::after{content:'';position:absolute;right:0;top:50%;transform:translateY(-50%);width:3px;height:24px;background:var(--accent);border-radius:2px 0 0 2px;box-shadow:0 0 12px rgba(245,200,0,0.55);}\
...
.ipad-nav-item.active svg{filter:drop-shadow(0 0 6px rgba(245,200,0,0.50));}\
.ipad-nav-item:hover svg{transform:scale(1.12);}\
.ipad-panel-label{display:none;font-size:9px;font-weight:900;letter-spacing:0.12em;color:var(--accent);margin-bottom:14px;padding-bottom:10px;border-bottom:1px solid rgba(245,200,0,0.15);text-transform:uppercase;text-shadow:0 0 16px rgba(245,200,0,0.25);}\
...
.fab{right:24px!important;bottom:24px!important;box-shadow:0 4px 24px rgba(245,200,0,0.30)!important;}\
.ipad-panel-label{display:block!important;}\
#screen-home>.scroll-content{display:flex!important;flex-direction:row!important;padding:0 0 0 72px!important;overflow:hidden!important;height:calc(100vh - 56px)!important;}\
...
.ipad-live-left{width:36%!important;min-width:260px!important;padding:20px!important;overflow-y:auto!important;height:100%!important;box-sizing:border-box!important;border-right:1px solid rgba(245,200,0,0.07)!important;background:rgba(0,0,0,0.15)!important;}\
...
.ipad-left-panel{width:42%!important;min-width:260px!important;padding:20px!important;overflow-y:auto!important;height:100%!important;box-sizing:border-box!important;border-right:1px solid rgba(245,200,0,0.07)!important;background:rgba(0,0,0,0.2)!important;}\
...
#screen-profile>.scroll-content{display:flex!important;flex-direction:row!important;padding:0 0 0 72px!important;overflow:hidden!important;height:calc(100vh - 56px)!important;}\
.ipad-profile-left{width:32%!important;min-width:240px!important;padding:24px 20px!important;overflow-y:auto!important;height:100%!important;box-sizing:border-box!important;border-right:1px solid rgba(245,200,0,0.07)!important;background:rgba(0,0,0,0.25)!important;display:flex!important;flex-direction:column!important;align-items:center!important;}\
.ipad-profile-right{flex:1!important;min-width:0!important;padding:20px!important;overflow-y:auto!important;height:100%!important;box-sizing:border-box!important;}\
#screen-profile .profile-avatar{width:110px!important;height:110px!important;font-size:36px!important;}\
#screen-profile .profile-name{font-size:22px!important;text-align:center!important;}\
#screen-profile .profile-div{text-align:center!important;}\
...
.home-hero{background:linear-gradient(135deg,rgba(15,20,15,0.98),rgba(13,14,12,0.98));border:1px solid rgba(245,200,0,0.20);border-left:3px solid var(--accent);border-radius:10px;padding:22px 24px;margin-bottom:16px;position:relative;overflow:hidden;max-width:680px;}\
.hero-divider{height:1px;background:rgba(255,255,255,0.07);margin:16px 0;}\
.hero-goal-wrap{}\
#hero-goal-input:focus{border-color:rgba(245,200,0,0.55);box-shadow:0 0 0 2px rgba(245,200,0,0.10);}\
.home-hero::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 80% 40%,rgba(245,200,0,0.07),transparent 65%);pointer-events:none;}\
...
/* STATS GRID */\
.stats-g4{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-bottom:18px;max-width:600px;}\
.stat-c{background:var(--card);border:1px solid var(--border);border-radius:12px;padding:16px;}\
.stat-v{font-size:26px;font-weight:900;color:var(--accent);line-height:1;margin-bottom:5px;font-variant-numeric:tabular-nums;}\
.stat-l{font-size:9px;font-weight:800;letter-spacing:0.14em;color:var(--muted);text-transform:uppercase;}\
/* SECTION HEADERS */\
.sec-hdr{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;}\
.sec-lbl{font-size:9px;font-weight:800;letter-spacing:0.18em;color:var(--muted);text-transform:uppercase;}\
.sec-bdg{background:rgba(255,255,255,0.06);border:1px solid var(--border);border-radius:20px;padding:3px 10px;font-size:10px;font-weight:700;color:var(--muted);cursor:pointer;}\
.sec-bdg:hover,.sec-bdg.blue{background:rgba(245,200,0,0.10);border-color:rgba(245,200,0,0.25);color:var(--accent);}\
.sec-bdg.green{background:rgba(76,175,125,0.12);border-color:rgba(76,175,125,0.3);color:var(--green);}\
/* DATE BADGE */\
.dt-bdg{background:rgba(245,200,0,0.10);border:1px solid rgba(245,200,0,0.25);border-radius:10px;padding:7px 9px;text-align:center;flex-shrink:0;min-width:46px;}\
.dt-day{font-size:19px;font-weight:900;color:var(--accent);line-height:1;}\
.dt-mon{font-size:9px;font-weight:800;letter-spacing:0.06em;color:rgba(245,200,0,0.6);text-transform:uppercase;margin-top:2px;}\
/* DASHED EMPTY */\
.empty-dash{border:1.5px dashed rgba(255,255,255,0.1);border-radius:12px;padding:20px;text-align:center;color:var(--muted);font-size:13px;}\
/* ML ITEM */\
.ml-it{display:flex;align-items:center;gap:12px;padding:11px 13px;background:var(--card);border:1px solid var(--border);border-radius:11px;margin-bottom:7px;cursor:pointer;transition:border-color 0.15s;}\
.ml-it:hover{border-color:rgba(245,200,0,0.25);}\
.ml-nm{font-size:13px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:2px;}\
.ml-mt{font-size:11px;color:var(--muted);}\
.stg-bdg{background:rgba(255,255,255,0.06);border:1px solid var(--border);border-radius:50%;width:26px;height:26px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;}\
.reg-g{background:rgba(76,175,125,0.15);border:1px solid rgba(76,175,125,0.3);color:var(--green);border-radius:6px;padding:3px 8px;font-size:10px;font-weight:700;}\
.reg-b{background:rgba(245,200,0,0.15);border:1px solid rgba(245,200,0,0.25);color:var(--accent);border-radius:6px;padding:3px 8px;font-size:10px;font-weight:700;cursor:pointer;}\
/* ATTENTION */\
...
<div class="screen" id="screen-matches">
 ...
 <div class="tab-item active" onclick="switchTab('screen-matches')" style="position:relative;"><div class="tab-icon"><svg viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg></div><div class="tab-inv-badge" style="display:none;position:absolute;top:0;right:0;background:#ef4444;color:white;border-radius:50%;width:18px;height:18px;font-size:11px;font-weight:bold;align-items:center;justify-content:center;"></div><span class="lang-matches">${d("matches")}</span></div>
 <div class="tab-item" onclick="switchTab('screen-prognose')"><div class="tab-icon"><svg viewBox="0 0 24 24"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg></div><span class="lang-prognosis">${d("prognosis")}</span></div>
 <div class="tab-item" onclick="switchTab('screen-results')"><div class="tab-icon"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><circle cx="12" cy="12" r="7"/><line x1="12" y1="2" x2="12" y2="5"/><line x1="12" y1="19" x2="12" y2="22"/><line x1="2" y1="12" x2="5" y2="12"/><line x1="19" y1="12" x2="22" y2="12"/></svg></div><span class="lang-results">${d("results")}</span></div>
 <div class="tab-item" onclick="switchTab('screen-teams')"><div class="tab-icon"><svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg></div><span>Teams</span></div>
 <div class="tab-item" onclick="switchTab('screen-profile')"><div class="tab-icon"><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div><span class="lang-profile">${d("profile")}</span></div>
...
<div class="screen" id="screen-profile">
 ...
 <div class="scroll-content">
  <div class="ipad-profile-left">
  <div class="ipad-panel-label">Athlete</div>
  ...
  <div class="ipad-profile-right">
  <div class="ipad-panel-label">Statistics & Info</div>
...
<div class="screen" id="screen-teams">
 ...
 <div class="scroll-content">
 <div style="max-width:640px;margin:0 auto;width:100%;"><div id="teams-standings-container"></div>
...
<div class="modal-overlay" id="modal-new-match" onclick="closeModalOutside(event,'modal-new-match')">
 ...
 <input class="field-input" type="text" id="new-match-location" placeholder="Bergen" autocomplete="off" oninput="_geoDebounce(this.value,'new')">
 <div id="new-match-geo-results" style="position:relative;"></div>
 <input type="hidden" id="new-match-lat" value="">
 <input type="hidden" id="new-match-lng" value="">
 </div>
 ...
 <input class="field-input" type="text" id="new-match-user-search" oninput="searchUsersNewMatchLive()" placeholder="Search by name or email…" style="flex:1;">
 <button onclick="searchUsersNewMatch()" style="width:80px;padding:12px;background:#f5c800;color:#0a0c0e;border:none;border-radius:6px;font-family:'Outfit',sans-serif;font-weight:700;cursor:pointer;">Search</button>
 </div>
...
<div class="modal-overlay" id="modal-edit-match" onclick="closeModalOutside(event,'modal-edit-match')">
 ...
 <div class="field-group">
 <div class="field-label">${d("location")}</div>
 <input class="field-input" type="text" id="edit-match-location" placeholder="Bergen" autocomplete="off" oninput="_geoDebounce(this.value,'edit')">
 <div id="edit-match-geo-results" style="position:relative;"></div>
 <input type="hidden" id="edit-match-lat" value="">
 <input type="hidden" id="edit-match-lng" value="">
 </div>
 ...
 <div class="field-label">Invite users (optional)</div>
 <div style="display:flex;gap:8px;margin-bottom:10px;">
 <input class="field-input" type="text" id="edit-match-user-search" oninput="searchUsersEditMatchLive()" placeholder="Search by name or email…" style="flex:1;">
 <button onclick="searchUsersEditMatch()" style="width:80px;padding:12px;background:#f5c800;color:#0a0c0e;border:none;border-radius:6px;font-family:'Outfit',sans-serif;font-weight:700;cursor:pointer;">Search</button>
 </div>
 ...
 <button id="delete-match-btn" onclick="window.confirmDeleteMatch()" style="width:100%;margin-top:10px;padding:12px;background:#ef4444;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;display:none;">${dt==="no"?"Delete Match":"DELETE MATCH"}</button>
 </div>
 </div>
</div>
...
<div class="modal-overlay" id="modal-add" onclick="closeModalOutside(event,'modal-add')">
 ...
 <div style="display:flex;gap:8px;"><button class="btn-primary" style="flex:1;" onclick="addStageResult()">${d("save_result")}</button><button type="button" id="modal-add-dq-btn" onclick="window.icToggleDQFromModal()" style="padding:12px 16px;border-radius:8px;font-weight:700;font-size:13px;cursor:pointer;background:rgba(239,68,68,0.15);color:#ef4444;border:1px solid rgba(239,68,68,0.3);">DQ</button></div>
 </div>
 </div>
</div>
...
<div class="modal-overlay" id="modal-invite-user" onclick="closeModalOutside(event,'modal-invite-user')">
 ...
 <input class="field-input" type="text" id="user-search-input" placeholder="Search..." style="flex:1;" oninput="searchUsersLive()">
 <button onclick="searchUsers()" style="width:80px;padding:12px;background:#f5c800;color:#0a0c0e;border:none;border-radius:6px;font-family:'Outfit',sans-serif;font-weight:700;cursor:pointer;">Search</button>
 </div>
 </div>
 <div id="user-search-results" style="margin-top:15px;">
 <!-- Populated by JavaScript -->
 </div>
 <div id="invite-result-msg" style="display:none;padding:12px;border-radius:8px;text-align:center;font-weight:600;font-size:14px;margin-bottom:10px;"></div><button id="send-invitations-btn" class="btn-primary" onclick="sendMultipleInvitations()" style="margin-top:15px;display:none;">Send Invitations (0 selected)</button>
 </div>
 </div>
</div>
...
window.icPopulateTeamCountry=icPopulateTeamCountry,window.deleteCurrentStage=deleteCurrentStage,window.openInviteUser=ks,window.openInvitationsModal=Is,window.acceptInvitation=Cs,window.declineInvitation=_s,window.searchUsers=xs,window.toggleUserSelection=Ss,window.sendMultipleInvitations=Ls,window.searchUsersNewMatch=Ps,window.toggleUserNewMatch=Es,window.searchUsersEditMatch=Ts,window.toggleUserEditMatch=Ms,window.confirmDeleteMatch=gs,window.deleteMatchConfirmed=vs,window.uploadAndScanResult=Hs,window.saveOCRResult=Gs,window.icOpenManualResult=icOpenManualResult,window.importESSVerify=importESSVerify,window.essConfirmPaste=essConfirmPaste;window.essSelectAllStages=essSelectAllStages,window.openCreateStageFromUpload=Us,window.toggleMatchDropdown=ds,window.selectMatchFromDropdown=cs,window.toggleReferenceShootersEnabled=toggleReferenceShootersEnabled,window.toggleReferenceShooterChoice=toggleReferenceShooterChoice,window.updateReferenceShooterOverride=updateReferenceShooterOverride,window.toggleLiveFilter=toggleLiveFilter,window.toggleTips=toggleTips,window.deleteRun=deleteRun,window.openCreateTeam=openCreateTeam,window.editTeam=editTeam,window.deleteTeam=deleteTeam,window.saveTeam=saveTeam,window.renderTeamsScreen=renderTeamsScreen;window.toggleTeamDetail=toggleTeamDetail;window.icToggleDQFromModal=async function(){const match=$.find(m=>m.id!=null&&m.id.toString()===String(R));if(!match){alert("No active match selected");return;}const shooterId=icUploadShooterSel||icCurrentShooterId();if(!shooterId){alert("No shooter selected");return;}await toggleDQ(match.id,shooterId);G("modal-add");_e();};window.toggleDQ=toggleDQ;window.icToggleSecEl=function(btn){var id=btn.getAttribute('data-secid');var ac=btn.getAttribute('data-arrcls');if(!id)return;var el=document.getElementById(id);if(!el)return;var open=el.style.display!=='none';el.style.display=open?'none':'block';var arr=btn.querySelector('.'+ac);if(arr)arr.textContent=open?'▶':'▼';};window.icToggleRefSection=function(){var b=document.getElementById('ref-sec-body');var a=document.getElementById('ref-sec-arr');if(!b)return;var o=b.style.display!=='none';b.style.display=o?'none':'block';if(a)a.textContent=o?'▶':'▼';};window.liveToggleShooterRows=function(){var rows=document.querySelectorAll('tr.rs-sh');var btn=document.getElementById('ref-btn-sh');if(!rows.length)return;var hide=rows[0].style.display!=='none';rows.forEach(function(r){r.style.display=hide?'none':'';});if(btn)btn.textContent=hide?d('ref_show_shooters'):d('ref_hide_shooters');};window.liveToggleRefRows=function(){var rows=document.querySelectorAll('tr.rs-rf');var btn=document.getElementById('ref-btn-rf');if(!rows.length)return;var hide=rows[0].style.display!=='none';rows.forEach(function(r){r.style.display=hide?'none':'';});if(btn)btn.textContent=hide?d('ref_show_ref'):d('ref_hide_ref');};window.icToggleSecEl=function(btn){var id=btn.getAttribute('data-secid');var ac=btn.getAttribute('data-arrcls');if(!id)return;var el=document.getElementById(id);if(!el)return;var open=el.style.display!=='none';el.style.display=open?'none':'block';var arr=btn.querySelector('.'+ac);if(arr)arr.textContent=open?'▶':'▼';};window.icToggleRefSection=function(){var b=document.getElementById('ref-sec-body');var a=document.getElementById('ref-sec-arr');if(!b)return;var o=b.style.display!=='none';b.style.display=o?'none':'block';if(a)a.textContent=o?'▶':'▼';};window.liveToggleShooterRows=function(){var rows=document.querySelectorAll('tr.rs-sh');var btn=document.getElementById('ref-btn-sh');if(!rows.length)return;var hide=rows[0].style.display!=='none';rows.forEach(function(r){r.style.display=hide?'none':'';});if(btn)btn.textContent=hide?d('ref_show_shooters'):d('ref_hide_shooters');};window.liveToggleRefRows=function(){var rows=document.querySelectorAll('tr.rs-rf');var btn=document.getElementById('ref-btn-rf');if(!rows.length)return;var hide=rows[0].style.display!=='none';rows.forEach(function(r){r.style.display=hide?'none':'';});if(btn)btn.textContent=hide?d('ref_show_ref'):d('ref_hide_ref');};
/* ══════════════════════════════════════════════════════
   REWARD SYSTEM
   ══════════════════════════════════════════════════════ */
function icCalcXP(profile, matches) {
  let xp = 0;
  if (!matches || !matches.length) return xp;
  matches.forEach(function(m) {
    const shooter = icCurrentShooter(m);
    if (!shooter || !shooter.stages || !shooter.stages.length) return;
    // 100 XP per completed match
    xp += 100;
    shooter.stages.forEach(function(st) {
      // 10 XP per stage
      xp += 10;
      // 50 XP for stage win
      const metrics = icStageMetricsForMatch(m, {number: st.stageNumber});
      if (metrics && metrics.length && String(metrics[0].id) === String(shooter.id)) xp += 50;
      // 40 XP for >90% A
      if (st.a != null && st.shots > 0 && (st.a / st.shots) >= 0.9) xp += 40;
    });
    // 250 XP for match win
    const totals = icMatchTotals(m, false);
    if (totals && totals.length && String(totals[0].id) === String(shooter.id)) xp += 250;
    // 75 XP placeholder for PR (tracked per match separately)
  });
  // Add stored XP from profile (training sessions etc.)
  if (profile && profile.bonusXP) xp += profile.bonusXP;
  return xp;
}

function icGetXPLevel(xp) {
  if (xp >= 5000) return {level: 5, name: 'Elite', color: '#74BFFF', glow: 'rgba(116,191,255,0.4)'};
  if (xp >= 2500) return {level: 4, name: 'Diamond', color: '#74BFFF', glow: 'rgba(116,191,255,0.3)'};
  if (xp >= 1000) return {level: 3, name: 'Gold', color: '#E8B84B', glow: 'rgba(245,200,0,0.25)'};
  if (xp >= 500)  return {level: 2, name: 'Silver', color: '#C0C0C0', glow: 'rgba(192,192,192,0.3)'};
  return           {level: 1, name: 'Bronze', color: '#CD7F32', glow: 'rgba(205,127,50,0.3)'};
}

function icNextXPThreshold(xp) {
...
function icEvaluateRewards(profile, matches) {
  const rewards = [];
  if (!matches) return rewards;

  const totalMatches = matches.length;
  const allStages = [];
  let totalA = 0, totalShots = 0;
  let stageWins = 0;

  matches.forEach(function(m) {
    const shooter = icCurrentShooter(m);
    if (!shooter || !shooter.stages) return;
    shooter.stages.forEach(function(st) {
      allStages.push({match: m, stage: st, shooter: shooter});
      if (st.a != null) totalA += st.a;
      if (st.shots != null) totalShots += st.shots;
      const metrics = icStageMetricsForMatch(m, {number: st.stageNumber});
      if (metrics && metrics.length && String(metrics[0].id) === String(shooter.id)) stageWins++;
    });
  });

  const overallAPct = totalShots > 0 ? (totalA / totalShots * 100) : 0;
  const xp = icCalcXP(profile, matches);

  // ── CATEGORY 1: Placement ─────────────────────────────────────────────────
  const matchWins = matches.filter(function(m) {
    const t = icMatchTotals(m, false);
    const s = icCurrentShooter(m);
    return t && t.length && s && String(t[0].id) === String(s.id);
  }).length;

  if (matchWins >= 1) rewards.push({id:'placement_gold',   cat:1, tier:'gold',    name:'Match Winner',    desc:'Won a match',           icon:'🏆'});
  else {
    const top3 = matches.filter(function(m) {
      const t = icMatchTotals(m, false); const s = icCurrentShooter(m);
      if (!t || !s) return false;
      const idx = t.findIndex(function(r){return String(r.id)===String(s.id);});
      return idx >= 0 && idx < 3;
    }).length;
    if (top3 >= 1) rewards.push({id:'placement_silver', cat:1, tier:'silver', name:'Podium Finish',   desc:'Top 3 in a match',      icon:'🥈'});
    else {
      const top5 = matches.filter(function(m) {
        const t = icMatchTotals(m, false); const s = icCurrentShooter(m);
        if (!t || !s) return false;
        const idx = t.findIndex(function(r){return String(r.id)===String(s.id);});
        return idx >= 0 && idx < 5;
      }).length;
      if (top5 >= 1) rewards.push({id:'placement_bronze', cat:1, tier:'bronze', name:'Top 5',          desc:'Top 5 in a match',      icon:'🎯'});
    }
  }

  // Stage dominance
  if (stageWins >= 5)      rewards.push({id:'stage_dom_gold',   cat:1, tier:'gold',    name:'Stage Dominator', desc:'Won 5+ stages',         icon:'⚡'});
  else if (stageWins >= 3) rewards.push({id:'stage_dom_silver', cat:1, tier:'silver',  name:'Stage Fighter',   desc:'Won 3+ stages',         icon:'⚡'});
  else if (stageWins >= 1) rewards.push({id:'stage_dom_bronze', cat:1, tier:'bronze',  name:'Stage Winner',    desc:'Won a stage',           icon:'⚡'});

  // ── CATEGORY 2: Accuracy ──────────────────────────────────────────────────
  if (overallAPct >= 95)      rewards.push({id:'acc_diamond', cat:2, tier:'diamond', name:'Diamond Accuracy', desc:'95%+ A rate overall',   icon:'💎'});
  else if (overallAPct >= 90) rewards.push({id:'acc_gold',    cat:2, tier:'gold',    name:'Gold Accuracy',    desc:'90%+ A rate overall',   icon:'🎯'});
  else if (overallAPct >= 85) rewards.push({id:'acc_silver',  cat:2, tier:'silver',  name:'Silver Accuracy',  desc:'85%+ A rate overall',   icon:'🎯'});
  else if (overallAPct >= 75) rewards.push({id:'acc_bronze',  cat:2, tier:'bronze',  name:'Bronze Accuracy',  desc:'75%+ A rate overall',   icon:'🎯'});

  // ── CATEGORY 3: Hit Factor (XP-unlocked at 500 XP) ────────────────────────
  if (xp >= 500) {
    let matchHFs = [];
    matches.forEach(function(m) {
      const shooter = icCurrentShooter(m);
      if (!shooter || !shooter.stages) return;
      shooter.stages.forEach(function(st) {
        if (st.hf && st.hf > 0) matchHFs.push(st.hf);
      });
    });
    if (matchHFs.length) {
      const maxHF = Math.max.apply(null, matchHFs);
      if (maxHF >= 8)      rewards.push({id:'hf_diamond', cat:3, tier:'diamond', name:'HF Legend',    desc:'Achieved 8.0+ HF',      icon:'🔥'});
      else if (maxHF >= 6) rewards.push({id:'hf_gold',    cat:3, tier:'gold',    name:'Speed Demon',  desc:'Achieved 6.0+ HF',      icon:'🔥'});
      else if (maxHF >= 4) rewards.push({id:'hf_silver',  cat:3, tier:'silver',  name:'Fast Shooter', desc:'Achieved 4.0+ HF',      icon:'🔥'});
      else if (maxHF >= 2) rewards.push({id:'hf_bronze',  cat:3, tier:'bronze',  name:'Getting Fast', desc:'Achieved 2.0+ HF',      icon:'🔥'});
    }
  }

  // ── CATEGORY 5: Improvement (XP-unlocked at 2500 XP) ─────────────────────
  if (xp >= 2500 && matches.length >= 2) {
    // Compare last match total% vs first match total%
    const getPct = function(m) {
      const t = icMatchTotals(m, false); const s = icCurrentShooter(m);
      if (!t || !s || !t.length) return null;
      const top = t[0].totalStagePts; if (!top) return null;
      const mine = t.find(function(r){return String(r.id)===String(s.id);});
      return mine ? mine.totalStagePts / top * 100 : null;
    };
    const first = getPct(matches[0]);
    const last = getPct(matches[matches.length-1]);
    if (first != null && last != null) {
      const diff = last - first;
      if (diff >= 10)     rewards.push({id:'impr_gold',   cat:5, tier:'gold',   name:'Major Improvement', desc:'+10% total score growth',  icon:'📈'});
      else if (diff >= 5) rewards.push({id:'impr_silver', cat:5, tier:'silver', name:'Improving Fast',    desc:'+5% total score growth',   icon:'📈'});
      else if (diff >= 2) rewards.push({id:'impr_bronze', cat:5, tier:'bronze', name:'On The Up',         desc:'+2% total score growth',   icon:'📈'});
    }
  }

  // ── CATEGORY 6: Activity ──────────────────────────────────────────────────
  if (totalMatches >= 100)     rewards.push({id:'act_diamond', cat:6, tier:'diamond', name:'Century Club',   desc:'100+ matches played',   icon:'🏅'});
  else if (totalMatches >= 25) rewards.push({id:'act_gold',    cat:6, tier:'gold',    name:'Veteran',        desc:'25+ matches played',    icon:'🏅'});
  else if (totalMatches >= 10) rewards.push({id:'act_silver',  cat:6, tier:'silver',  name:'Regular',        desc:'10+ matches played',    icon:'🏅'});
  else if (totalMatches >= 5)  rewards.push({id:'act_bronze',  cat:6, tier:'bronze',  name:'Getting Started',desc:'5+ matches played',     icon:'🏅'});

  if (allStages.length >= 500)      rewards.push({id:'stages_diamond', cat:6, tier:'diamond', name:'Stage Machine',  desc:'500+ stages shot',      icon:'🎪'});
  else if (allStages.length >= 100) rewards.push({id:'stages_gold',    cat:6, tier:'gold',    name:'Stage Veteran',  desc:'100+ stages shot',      icon:'🎪'});
  else if (allStages.length >= 25)  rewards.push({id:'stages_silver',  cat:6, tier:'silver',  name:'Stage Regular',  desc:'25+ stages shot',       icon:'🎪'});

  // ── CATEGORY 7: Streaks (XP-unlocked at 2500 XP) ─────────────────────────
  if (xp >= 2500) {
    // Accuracy streak: consecutive stages >90% A
    let streak = 0, bestStreak = 0;
    allStages.forEach(function(s) {
      const pct = s.stage.shots > 0 ? (s.stage.a||0)/s.stage.shots*100 : 0;
      if (pct >= 90) { streak++; if (streak > bestStreak) bestStreak = streak; }
      else streak = 0;
    });
    if (bestStreak >= 10)    rewards.push({id:'streak_diamond', cat:7, tier:'diamond', name:'Accuracy Machine',  desc:'10 consecutive stages >90%A', icon:'🎯'});
    else if (bestStreak >= 5) rewards.push({id:'streak_gold',   cat:7, tier:'gold',    name:'On Fire',           desc:'5 consecutive stages >90%A',  icon:'🎯'});
    else if (bestStreak >= 3) rewards.push({id:'streak_silver', cat:7, tier:'silver',  name:'Hot Streak',        desc:'3 consecutive stages >90%A',  icon:'🎯'});
  }

  // ── CATEGORY 8: Reference Shooter (XP-unlocked at 1000 XP) ───────────────
  if (xp >= 1000) {
    let refBeats = 0;
    matches.forEach(function(m) {
      if (!m.referenceShootersEnabled) return;
      const refAct = getActiveReferenceShooters ? getActiveReferenceShooters(m) : [];
      if (!refAct.length) return;
      const shooter = icCurrentShooter(m);
      if (!shooter) return;
      icStageDefs(m).forEach(function(_sd) {
        const metrics = icStageMetricsForMatch(m, _sd);
        const myMetric = metrics.find(function(mm){return String(mm.id)===String(shooter.id);});
        if (!myMetric) return;
        refAct.forEach(function(_rf) {
          const proj = projectReferenceShooterForStage(_rf, _sd);
          if (proj && myMetric.hf > proj.benchmarkHF) refBeats++;
        });
      });
    });
    if (refBeats >= 10)    rewards.push({id:'ref_diamond', cat:8, tier:'diamond', name:'Giant Killer',    desc:'Beat reference 10+ times',  icon:'⭐'});
    else if (refBeats >= 5) rewards.push({id:'ref_gold',   cat:8, tier:'gold',    name:'Reference Beater',desc:'Beat reference 5+ times',   icon:'⭐'});
    else if (refBeats >= 3) rewards.push({id:'ref_silver', cat:8, tier:'silver',  name:'Challenger',      desc:'Beat reference 3+ times',   icon:'⭐'});
    else if (refBeats >= 1) rewards.push({id:'ref_bronze', cat:8, tier:'bronze',  name:'First Blood',     desc:'Beat a reference shooter',  icon:'⭐'});
  }
...
function icRenderRewards() {
  ...
  // XP unlocks
  const unlockLevels = [
    {xp:500,  label: d('rewards_level') + ' 2 — Hit Factor'},
    {xp:1000, label: d('rewards_level') + ' 3 — Reference Shooter'},
    {xp:2500, label: d('rewards_level') + ' 4 — Improvement + Streaks'},
    {xp:5000, label: d('rewards_level') + ' 5 — Elite'},
  ];
  ...
  function badgeSVG(tier, size) {
    size = size || 36;
    const colors = {
      bronze:  {fill:'#CD7F32', glow:'rgba(205,127,50,0.5)',  stroke:'#a0621f'},
      silver:  {fill:'#C0C0C0', glow:'rgba(192,192,192,0.5)', stroke:'#909090'},
      gold:    {fill:'#E8B84B', glow:'rgba(245,200,0,0.55)',  stroke:'#b8901f'},
      diamond: {fill:'#74BFFF', glow:'rgba(116,191,255,0.6)', stroke:'#4090cc'},
    };
    ...
    if (tier === 'diamond') {
      ...
      const pts = `${half},${half*0.18} ${half*1.82},${half} ${half},${half*1.82} ${half*0.18},${half}`;
      return `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" style="filter:drop-shadow(0 0 4px ${c.glow});flex-shrink:0;">
        <polygon points="${pts}" fill="${c.fill}" stroke="${c.stroke}" stroke-width="1.5"/>
        <polygon points="${pts}" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="0.8" transform="scale(0.75) translate(${size*0.125},${size*0.125})"/>
      </svg>`;
    }
    // Shield shape
    return `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" style="filter:drop-shadow(0 0 4px ${c.glow});flex-shrink:0;">
      <path d="M${half} 2 L${size-4} ${size*0.25} L${size-4} ${size*0.55} Q${size-4} ${size*0.82} ${half} ${size-2} Q4 ${size*0.82} 4 ${size*0.55} L4 ${size*0.25} Z"
            fill="${c.fill}" stroke="${c.stroke}" stroke-width="1.5"/>
      <path d="M${half} 5 L${size-7} ${size*0.27} L${size-7} ${size*0.54} Q${size-7} ${size*0.79} ${half} ${size-5} Q7 ${size*0.79} 7 ${size*0.54} L7 ${size*0.27} Z"
            fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="1"/>
    </svg>`;
  }

  // XP progress bar
  const xpForLevel = nextXP ? (xp / nextXP * 100) : 100;
  const prevThreshold = lvl.level === 1 ? 0 : [0,500,1000,2500][lvl.level-1];
  const xpInLevel = xp - prevThreshold;
  const xpNeeded = nextXP ? nextXP - prevThreshold : xpInLevel;
  const pct = nextXP ? Math.min(xpInLevel / xpNeeded * 100, 100) : 100;

  let html = `<div class="card" style="margin-bottom:12px;">
    <div class="card-header" style="cursor:pointer;display:flex;align-items:center;justify-content:space-between;" onclick="(function(){var b=document.getElementById('rewards-body');var a=document.getElementById('rewards-chev');if(b){var o=b.style.display!=='none';b.style.display=o?'none':'block';a.style.transform=o?'':'rotate(180deg)';}})()">
      <div style="display:flex;align-items:center;gap:8px;">
        <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="var(--accent)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
        <div class="card-title">${d('rewards')}</div>
      </div>
      <svg id="rewards-chev" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="var(--muted)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="transition:transform 0.2s;flex-shrink:0;"><polyline points="6 9 12 15 18 9"/></svg>
    </div>
    <div id="rewards-body" style="display:none;margin-top:8px;">`;

  // XP + level block
  html += `<div style="background:var(--bg);border-radius:10px;padding:14px;margin-bottom:12px;">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;">
      ${badgeSVG(lvl.name.toLowerCase() === 'elite' ? 'diamond' : lvl.name.toLowerCase(), 44)}
      <div style="flex:1;">
        <div style="font-size:16px;font-weight:800;color:${lvl.color};text-shadow:0 0 12px ${lvl.glow};">${lvl.name}</div>
        <div style="font-size:12px;color:var(--muted);margin-top:2px;">${xp.toLocaleString()} XP</div>
      </div>
      ${nextXP ? `<div style="font-size:11px;color:var(--muted);text-align:right;">${nextXP - xp} XP<br><span style="font-size:10px;">to next</span></div>` : '<div style="font-size:11px;color:var(--blue);">MAX</div>'}
    </div>
    <div style="background:rgba(255,255,255,0.08);border-radius:4px;height:6px;overflow:hidden;">
      <div style="height:100%;width:${pct}%;background:linear-gradient(90deg,${lvl.color},${lvl.color}cc);border-radius:4px;transition:width 0.5s;"></div>
    </div>
    ${nextXP ? `<div style="font-size:10px;color:var(--muted);margin-top:4px;text-align:right;">${xpInLevel}/${xpNeeded} XP</div>` : ''}
  </div>`;

  // Upcoming unlocks
  const upcoming = unlockLevels.filter(function(u){return xp < u.xp;}).slice(0,2);
  if (upcoming.length) {
    ...
    upcoming.forEach(function(u) {
      html += `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
        <div style="font-size:16px;">🔒</div>
        <div style="flex:1;font-size:12px;color:var(--muted);">${u.label}</div>
        <div style="font-size:11px;color:var(--accent);font-weight:700;">${u.xp} XP</div>
      </div>`;
    });
    html += `</div>`;
  }
  ...
  // Earned rewards
  if (earned.length) {
    html += `<div style="font-size:11px;font-weight:700;color:var(--accent);letter-spacing:0.08em;text-transform:uppercase;margin-bottom:8px;">${d('rewards_earned')}</div>`;
    html += `<div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px;">`;
    earned.forEach(function(r) {
      html += `<div style="background:var(--bg);border-radius:8px;padding:10px;display:flex;align-items:flex-start;gap:8px;border:1px solid rgba(255,255,255,0.06);">
        ${badgeSVG(r.tier, 32)}
        <div style="flex:1;min-width:0;">
          <div style="font-size:12px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${r.icon} ${r.name}</div>
          <div style="font-size:10px;color:var(--muted);margin-top:2px;line-height:1.3;">${r.desc}</div>
        </div>
      </div>`;
    });
    html += `</div>`;
  ...
  if (manualRewards.length) {
    ...
    manualRewards.forEach(function(r) {
      ...
      if(r.id==='classic_landslaget'){
        ...
        html += '<span style="font-size:9px;background:rgba(245,200,0,0.12);color:#d9ae00;padding:2px 8px;border-radius:4px;font-family:'Share Tech Mono',monospace;font-weight:400;letter-spacing:0.10em;border:1px solid rgba(245,200,0,0.25);">LANDSLAGET</span>';
        html += '</div></div></div>';
        return;
      }
  ...
  // Save XP to Firebase (async, fire and forget)
  if (g && Pe && Pe() && xp !== (g.xp||0)) {
    g.xp = xp;
    Ot(g).catch(function(){});
  }
}
window.icRenderRewards = icRenderRewards;
/* ══ END REWARD SYSTEM ══ */
window.refreshFromFirebase=refreshFromFirebase;window.uploadProfilePhoto=uploadProfilePhoto}
var _icSearchTimer=null;function _icDebounce(fn){clearTimeout(_icSearchTimer);_icSearchTimer=setTimeout(fn,380);}function searchUsersEditMatchLive(){_icDebounce(function(){if(window.searchUsersEditMatch)window.searchUsersEditMatch();});}function searchUsersNewMatchLive(){_icDebounce(function(){if(window.searchUsersNewMatch)window.searchUsersNewMatch();});}function searchUsersLive(){_icDebounce(function(){if(window.searchUsers)window.searchUsers();});}window.searchUsersEditMatchLive=searchUsersEditMatchLive;window.searchUsersNewMatchLive=searchUsersNewMatchLive;window.searchUsersLive=searchUsersLive;
...
function toggleDQ(matchId,shooterId){
  ...
  Ee(matchId,{shooters:match.shooters}).then(function(res){
    if(res.success){renderTeamsScreen();te();_e();De();}
...
function toggleLiveFilter(){liveShowAll=!liveShowAll,_e()}function toggleTips(id){const el=document.getElementById(id);if(el){const open=el.style.display!=="none";el.style.display=open?"none":"block";const ch=document.getElementById("chev-"+id);if(ch)ch.style.transform=open?"rotate(0deg)":"rotate(180deg)";}}async function deleteRun(stageNum,runIdx){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match)return;const shooter=icCurrentShooter(match);if(!shooter)return;const stageIdx=shooter.stages.findIndex(s=>(s.num||s.number)==stageNum);if(stageIdx<0)return;const stage=shooter.stages[stageIdx];if(!stage.runs||stage.runs.length<=1){alert("Cannot delete the last run. Delete the entire result instead.");return;}if(!confirm("Delete Run "+(runIdx+1)+"?"))return;stage.runs.splice(runIdx,1);stage.runs.forEach((r,i)=>{r.runNum=i+1});const best=stage.runs.reduce((a,b)=>(b.hf||0)>(a.hf||0)?b:a,stage.runs[0]);Object.assign(stage,best,{num:stage.num,name:stage.name,runs:stage.runs});const res=await Ee(match.id,{shooters:match.shooters});if(res.success){te(),_e(),De()}else alert("Unable to delete run: "+res.error);}function icTeamStandings(e){
  ...
  standings.forEach(function(t){
    ...
    lh+="<button onclick='editTeam("+origIdx+")' style='padding:6px 12px;background:var(--accent);color:#0a0c0e;border:none;border-radius:6px;font-family:'Outfit',sans-serif;font-size:12px;font-weight:700;cursor:pointer;'>"+d("edit_btn")+"</button>";
...
match.teams=match.teams||[];if(currentEditTeamIdx!==null){match.teams[currentEditTeamIdx]={...match.teams[currentEditTeamIdx],name,country,shooterIds}}else{match.teams.push({id:"team_"+Date.now(),name,country,shooterIds})}const res=await Ee(match.id,{teams:match.teams});if(res.success){G("modal-create-team");renderTeamsScreen();te();_e();De();}else alert("Unable to save team: "+res.error)}async function deleteTeam(idx){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match||!match.teams)return;const team=match.teams[idx];if(!confirm("Delete team "+team.name+"?"))return;match.teams.splice(idx,1);const res=await Ee(match.id,{teams:match.teams});if(res.success){renderTeamsScreen();te();_e();De();}else alert("Unable to delete team: "+res.error)}function ss(e){document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active")),document.querySelectorAll(".tab-item").forEach(s=>s.classList.remove("active")),o(e).classList.add("active");document.querySelectorAll(".ipad-nav-item").forEach(s=>s.classList.remove("active"));const _ipadNavMap={"screen-home":"ipad-nav-home","screen-matches":"ipad-nav-matches","screen-prognose":"ipad-nav-prog","screen-results":"ipad-nav-results","screen-teams":"ipad-nav-teams","screen-profile":"ipad-nav-profile"};if(_ipadNavMap[e]){const _n=document.getElementById(_ipadNavMap[e]);if(_n)_n.classList.add("active");}const i=["screen-home","screen-matches","screen-prognose","screen-results","screen-teams","screen-profile"].indexOf(e),t=document.querySelectorAll(".tab-item");t[i]&&t[i].classList.add("active");document.querySelectorAll(".tab-bar").forEach(function(bar){var items=bar.querySelectorAll(".tab-item");if(items[i])items[i].classList.add("active");}),e==="screen-home"&&te(),e==="screen-matches"&&fe(),e==="screen-results"&&_e(),e==="screen-prognose"&&De(),e==="screen-teams"&&(renderTeamsScreen(),R&&pt(R))}function ie(e){o(e).classList.add("open"),e==="modal-upload-result"&&as(),e==="modal-add-shooter"&&icUpdateNewShooterPFOptions(),e==="modal-add"&&icInitManualResult()}async function as(){const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e)return;const i=o("upload-match-name");i&&(i.textContent=e.name||"Unknown Match");const t=o("upload-stage-select"),s=icStageDefs(e);if(t.innerHTML="",s.length>0)s.forEach(n=>{const r=document.createElement("option");r.value=n.number,r.textContent=n.name&&n.name!=="Stage "+n.number?`Stage ${n.number} - ${n.name}`:`Stage ${n.number}`,t.appendChild(r)});else{const n=document.createElement("option");n.value="",n.textContent="No stages — create one",t.appendChild(n)}const a=o("upload-shooter-select");a.innerHTML="";const n=new Set,r=icCurrentShooterId(),u=document.createElement("option");u.value=r,u.textContent="Meg ("+(g.firstName||"")+" "+(g.lastName||"")+")",a.appendChild(u),n.add(r);if(e.participants&&e.participants.length>0)for(const m of e.participants)if(!n.has(m))try{const b=await Ut(m);if(b){const f=document.createElement("option");f.value=m,f.textContent=(b.firstName||"")+" "+(b.lastName||""),a.appendChild(f),n.add(m)}}catch(b){console.error("Error fetching user:",m,b)}e.shooters&&e.shooters.length>0&&e.shooters.forEach(m=>{if(!n.has(m.id)){const b=document.createElement("option");b.value=m.id,b.textContent=(m.firstName||"")+" "+(m.lastName||""),a.appendChild(b),n.add(m.id)}})}function G(e){o(e).classList.remove("open")}function ns(e,i){e.target.id===i&&G(i)}function rs(e,i){ue=e,document.querySelectorAll(".filter-chip").forEach(t=>t.classList.remove("active")),i.classList.add("active"),fe()}async function os(){var t;const e={name:F("new-match-name")||"New match",type:F("new-match-type")||"Trening",date:F("new-match-date")||new Date().toISOString().split("T")[0],location:F("new-match-location")||"",lat:(o("new-match-lat")&&o("new-match-lat").value?parseFloat(o("new-match-lat").value):null),lng:(o("new-match-lng")&&o("new-match-lng").value?parseFloat(o("new-match-lng").value):null),plannedStages:A("new-match-stages",6),searchable:((t=o("new-match-searchable"))==null?void 0:t.checked)!==!1,status:"active",stages:[],shooters:[],referenceShootersEnabled:!1,referenceShooterIds:[],referenceOverrides:{}};showSpinner("Creating match…");zt(e).then(function(i){hideSpinner();if(i.success){let s=0;Promise.all(ve.map(function(a){return Ge(a.uid,{matchId:i.matchId,matchName:e.name,firstName:a.firstName||"",lastName:a.lastName||(a.displayName||"")})})).then(function(results){results.forEach(function(r){if(r.success)s++});G("modal-new-match");o("new-match-name").value="";o("new-match-location").value="";o("new-match-stages").value="6";o("new-match-searchable")&&(o("new-match-searchable").checked=!0);o("new-match-user-search").value="";o("new-match-search-results").innerHTML="";ve=[];const _cu=W();if(_cu){const _nm=Object.assign({},e,{id:i.matchId.toString(),ownerId:_cu.uid,participants:[_cu.uid],shooters:[],stages:[],createdAt:new Date(),updatedAt:new Date()});$.unshift(_nm);pt(i.matchId);ss("screen-home");}if(s>0)alert("Match created! Invitations sent to "+s+" user(s).")})}else alert("Unable to create match: "+i.error)})}async function ls(){const e=F("match-id-search").trim();if(!e){alert("Please enter a match ID");return}const i=await Kt(e);if(i.success){const t=$.findIndex(s=>s.id&&s.id.toString()===i.match.id.toString());t!==-1?R=$[t].id:($.unshift(i.match),R=i.match.id),pt(R),ss("screen-home"),o("match-id-search").value=""}else alert(`No match found with ID ${e}${i.error?": "+i.error:""}`)}function pt(e){R=e!=null?e.toString():e;const i=$.find(t=>t.id!=null&&t.id.toString()===R);if(i){const t=i.id?"Match ID "+i.id+" "+i.name:i.name||"Match";["home-chip-name","prog-chip-name","results-chip-name","teams-chip-name"].forEach(a=>{const n=o(a);n&&(n.textContent=t)})}xe&&xe(),R&&(xe=Wt(R,t=>{const s=$.findIndex(a=>a.id!=null&&a.id.toString()===R);s!==-1&&t&&($[s]=t,te(),_e(),De())})),te(),_e(),De(),fe()}function ds(e){const i=e+"-match-dropdown",t=o(i);if(!t)return;if(t.classList.contains("open")){t.classList.remove("open");return}document.querySelectorAll(".match-dropdown").forEach(a=>a.classList.remove("open"));let s="";$.length===0?s+='<div class="match-dropdown-item" style="color:var(--muted);text-align:center;">No matches yet</div>':$.forEach(a=>{const n=a.id!=null&&a.id.toString()===String(R),r=a.id?"Match ID "+a.id+" "+a.name:a.name||"Match";s+='<div class="match-dropdown-item '+(n?"active":"")+`" onclick="selectMatchFromDropdown('`+a.id+"', '"+e+`')">`,s+='<div class="match-dropdown-dot"></div>',s+='<div class="match-dropdown-info">',s+='<div class="match-dropdown-name">'+r+"</div>",s+='<div class="match-dropdown-meta">'+We(a.date)+" · "+a.location+"</div>",s+="</div>",s+="</div>"}),t.innerHTML=s,t.classList.add("open")}function cs(e,i){pt(e);const t=i+"-match-dropdown",s=o(t);s&&s.classList.remove("open")}function ps(){const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e){alert("No match selected");return}o("edit-match-name").value=e.name||"",o("edit-match-type").value=e.type||"Trening",o("edit-match-date").value=e.date||"",o("edit-match-location").value=e.location||"",o("edit-match-lat")&&(o("edit-match-lat").value=e.lat||""),o("edit-match-lng")&&(o("edit-match-lng").value=e.lng||""),o("edit-match-stages").value=e.plannedStages||0,o("edit-match-searchable")&&(o("edit-match-searchable").checked=e.searchable!==!1),o("edit-match-finished")&&(o("edit-match-finished").checked=e.status==="finished");const i=o("edit-match-rival");i.innerHTML='<option value="">No rival selected</option>';const t=document.createElement("option");t.value="me",t.textContent="Meg ("+(g.firstName||"")+" "+(g.lastName||"")+")",i.appendChild(t),e.shooters&&e.shooters.length>0&&e.shooters.forEach(n=>{const r=document.createElement("option");r.value=n.id,r.textContent=n.firstName+" "+n.lastName,i.appendChild(r)}),e.rivalId&&(i.value=e.rivalId),o("edit-match-user-search").value="",o("edit-match-search-results").innerHTML="",me=[],initReferenceEditState(e),o("edit-match-reference-enabled")&&(o("edit-match-reference-enabled").checked=!!e.referenceShootersEnabled),renderReferenceShooterConfig();const s=Pe(),a=o("delete-match-btn");a&&s&&e.ownerId===s.uid?a.style.display="block":a&&(a.style.display="none"),icRenderEditMatchShootersList(e),ie("modal-edit-match")}async function us(){var s,a;const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e){alert("No match selected");return}refEditState.enabled=!!((o("edit-match-reference-enabled")||{}).checked);const i={name:F("edit-match-name")||e.name,type:F("edit-match-type")||e.type,date:F("edit-match-date")||e.date,location:F("edit-match-location")||e.location,lat:(o("edit-match-lat")&&o("edit-match-lat").value?parseFloat(o("edit-match-lat").value):(e.lat||null)),lng:(o("edit-match-lng")&&o("edit-match-lng").value?parseFloat(o("edit-match-lng").value):(e.lng||null)),plannedStages:A("edit-match-stages",e.plannedStages),searchable:((s=o("edit-match-searchable"))==null?void 0:s.checked)!==!1,status:(a=o("edit-match-finished"))!=null&&a.checked?"finished":"active",rivalId:F("edit-match-rival")||null,referenceShootersEnabled:!!refEditState.enabled,referenceShooterIds:(refEditState.ids||[]).slice(),referenceOverrides:JSON.parse(JSON.stringify(refEditState.overrides||{}))};showSpinner("Saving match…");Ee(e.id,i).then(function(t){hideSpinner();if(t.success){let n=0;Promise.all(me.map(function(r){return Ge(r.uid,{matchId:e.id,matchName:i.name,firstName:r.firstName||"",lastName:r.lastName||(r.displayName||"")})})).then(function(results){results.forEach(function(r){if(r.success)n++});G("modal-edit-match");_e();De();if(n>0)alert("Match updated! Invitations sent to "+n+" user(s).")})}else alert("Unable to update match: "+t.error)})}function gs(){const e=$.find(t=>t.id!=null&&t.id.toString()===String(R));if(!e){alert("No match selected");return}const i=e.id?"Match ID "+e.id+" "+e.name:e.name;o("delete-match-name").textContent=i,ie("modal-confirm-delete")}async function vs(){const e=$.find(t=>t.id!=null&&t.id.toString()===String(R));if(!e){alert("No match selected");return}showSpinner("Deleting match…");Gt(e.id).then(function(i){hideSpinner();if(i.success){G("modal-confirm-delete");G("modal-edit-match");R=null;te();fe();_e();De();alert("Match deleted")}else alert("Unable to delete match: "+i.error)})}let Ce=null,Me=null;function Re(){var i;const e=$.find(t=>t.id!=null&&t.id.toString()===String(R));if(!e){alert("No match selected");return}Ce=null,o("stage-modal-title").textContent=d("create_stage"),o("stage-number").value=(((i=e.stages)==null?void 0:i.length)||0)+1,o("stage-name").value="",o("stage-paper-targets").value=0,o("stage-poppers").value=0,o("stage-plates").value=0,o("stage-no-shoots").value=0,o("stage-bonus-paper-targets").value=0;const _ssh=document.getElementById("stage-single-shot");if(_ssh)_ssh.checked=false,ie("modal-create-stage")}function ms(){G("modal-edit-match"),Re()}function hs(){G("modal-new-match"),Re()}function fs(e){const i=$.find(s=>s.id!=null&&s.id.toString()===String(R));if(!i||!i.stages||!i.stages[e]){alert("Stage could not be found");return}Ce=e;const t=i.stages[e];o("stage-modal-title").textContent=d("edit_stage"),o("stage-number").value=t.number||e+1,o("stage-name").value=t.name||"",o("stage-paper-targets").value=t.paperTargets||0,o("stage-poppers").value=t.poppers||0,o("stage-plates").value=t.plates||0,o("stage-no-shoots").value=t.noShoots||0,o("stage-bonus-paper-targets").value=t.bonusPaperTargets||0;const _ssedit=document.getElementById("stage-single-shot");if(_ssedit)_ssedit.checked=!!t.singleShot,ie("modal-create-stage")}function bs(e){const i=o("stage-number"),t=Math.max(1,parseInt(i.value)+e);i.value=t}function ys(e,i){const t=o("stage-"+e),s=Math.max(0,parseInt(t.value)+i);t.value=s}async function ws(){var a;const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e){alert("No match selected");return}const i={number:A("stage-number",1),name:F("stage-name")||"",paperTargets:A("stage-paper-targets",0),poppers:A("stage-poppers",0),plates:A("stage-plates",0),noShoots:A("stage-no-shoots",0),bonusPaperTargets:A("stage-bonus-paper-targets",0),bonusIncluded:true,singleShot:document.getElementById("stage-single-shot")&&document.getElementById("stage-single-shot").checked},t=e.stages||[];Ce!==null?t[Ce]=i:t.push(i);showSpinner("Saving stage…");Ee(e.id,{stages:t}).then(function(s){hideSpinner();if(s.success){G("modal-create-stage");te();_e();De();}else alert("Unable to save stage: "+s.error)})}function icOpenEditStageResult(stageNum,shooterId){
  ...
  const shooter=icFindShooter(match,shooterId);
  ...
  if(shooterEl){
    // Ensure the target shooter has an option in the select (may be missing for non-self shooters)
    if(!Array.from(shooterEl.options).some(function(opt){return opt.value===shooterId;})){
      const _opt=document.createElement("option");
      _opt.value=shooterId;
      _opt.textContent=((shooter.firstName||"")+" "+(shooter.lastName||"")).trim()||"Shooter";
      shooterEl.appendChild(_opt);
    }
    shooterEl.value=shooterId;
  }
...
async function icResetStageResult(stageNum,shooterId){const _cu=Pe();const match=$.find(function(m){return m.id!=null&&m.id.toString()===String(R);});if(!match){alert("No active match selected");return;}if(!_cu||match.ownerId!==_cu.uid){alert("Only the match owner can reset stage results.");return;}const shooter=icFindShooter(match,shooterId);if(!shooter){alert("Shooter not found.");return;}const stageDef=icStageDefs(match).find(function(s){return Number(s.number)===Number(stageNum);});const stageName=(stageDef&&stageDef.name)||("Stage "+stageNum);const shooterName=((shooter.firstName||"")+" "+(shooter.lastName||"")).trim()||"Shooter";if(!confirm("Reset result for "+stageName+" for "+shooterName+"?\n\nThis will permanently delete this stage result and cannot be undone.")){return;}shooter.stages=(shooter.stages||[]).filter(function(s){return String(s.num||s.number)!==String(stageNum);});showSpinner("Resetting stage result…");const res=await Ee(match.id,{shooters:match.shooters});hideSpinner();if(res.success){te();_e();De();}else{alert("Unable to reset stage result: "+res.error);}}window.icResetStageResult=icResetStageResult;
function deleteCurrentStage(){const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e||Ce===null||Ce===undefined){alert("No stage selected");return;}const t=e.stages||[];const st=t[Ce];const stName=st?(st.name||"Stage "+(st.number||"")):"this stage";if(!confirm("Are you sure you want to delete "+stName+"? This action cannot be undone.")){return;}showSpinner("Deleting stage…");t.splice(Ce,1);Ee(e.id,{stages:t}).then(function(s){hideSpinner();if(s.success){G("modal-create-stage");te();_e();De();}else alert("Unable to delete stage: "+s.error);});}
let ee=[],Q=[],ve=[],me=[];function ks(){if(!R){alert("No match selected");return}Q=[],o("user-search-input").value="",o("user-search-results").innerHTML="",o("send-invitations-btn").style.display="none",ie("modal-invite-user")}async function xs(){const e=F("user-search-input").trim();if(e.length===0){o("user-search-results").innerHTML='<p style="color:#9ca3af;text-align:center;">Skriv inn et søk</p>';return}const i=await Ke(e),t=o("user-search-results");if(!t)return;if(i.length===0){t.innerHTML='<p style="color:#9ca3af;text-align:center;">No users found.</p>';return}let s='<div style="margin-bottom:10px;font-weight:600;">RESULTS ('+i.length+"):</div>";i.forEach(a=>{const n=a.displayName||a.email,r=Q.some(m=>m.uid===a.uid),u=n.replace(/'/g,"\\'");const _match=$.find(_m=>_m.id!=null&&_m.id.toString()===String(R));const _isParticipant=(_match&&_match.participants||[]).includes(a.uid);const _isInvited=!_isParticipant&&(_match&&_match.invitedUids||[]).includes(a.uid);const _disabled=_isParticipant||_isInvited;const _badge=_isParticipant?'<span style="font-size:11px;font-weight:700;color:#4caf7d;background:rgba(76,175,125,0.15);border:1px solid rgba(76,175,125,0.3);border-radius:4px;padding:2px 7px;margin-left:8px;">In match</span>':_isInvited?'<span style="font-size:11px;font-weight:700;color:#f5c800;background:rgba(245,200,0,0.10);border:1px solid rgba(245,200,0,0.25);border-radius:4px;padding:2px 7px;margin-left:8px;">Invited</span>':'';s+=`
 <label style="display:flex;align-items:center;gap:10px;padding:10px;background:#2d3748;border-radius:8px;margin-bottom:8px;cursor:${_disabled?'default':'pointer'};">
 <input type="checkbox" ${r&&!_disabled?"checked":""} ${_disabled?"disabled":""} onchange="${_disabled?'':("toggleUserSelection('"+a.uid+"', '"+a.email.replace(/'/g,"\\'")+  "', '"+u+"')")}" style="width:18px;height:18px;${_disabled?'opacity:0.4;':''}">
 <div>
 <div style="font-weight:600;">${n}${_badge}</div>
 
 </div>
 </label>
 `}),t.innerHTML=s,ut()}function Ss(e,i,t){const s=Q.findIndex(a=>a.uid===e);s>-1?Q.splice(s,1):Q.push({uid:e,email:i,displayName:t}),ut()}function ut(){const e=o("send-invitations-btn");e&&(Q.length>0?(e.style.display="block",e.textContent=`Send Invitations (${Q.length} selected)`):e.style.display="none")}async function Ls(){if(Q.length===0){alert("No users selected");return}const e=$.find(s=>s.id!=null&&s.id.toString()===String(R));if(!e)return;showSpinner("Sending…");try{const _toInvite=Q.filter(s=>!(e.participants||[]).includes(s.uid));const results=await Promise.all(_toInvite.map(s=>Ge(s.uid,{matchId:e.id,matchName:e.name,firstName:s.firstName||"",lastName:s.lastName||(s.displayName||"")})));let sent=0,failed=0;results.forEach(r=>{if(r.success)sent++;else failed++;});hideSpinner();const el=document.getElementById("invite-result-msg");if(el){el.textContent=sent>0?"✓ Invitasjon"+(sent>1?"er":"")+" sendt til "+sent+" bruker"+(sent>1?"e":"")+"!":"Kunne ikke sende invitasjon.";el.style.background=sent>0?"rgba(34,197,94,0.15)":"rgba(239,68,68,0.15)";el.style.color=sent>0?"#22c55e":"#ef4444";el.style.display="block";setTimeout(()=>{el.style.display="none";if(sent>0)G("modal-invite-user");},2500);}else{if(sent>0){G("modal-invite-user");alert("Invitasjon sendt!");}}Q=[];}catch(err){hideSpinner();alert("Feil: "+err.message);}}async function Ps(){const e=F("new-match-user-search").trim();if(e.length===0){o("new-match-search-results").innerHTML="";return}const i=await Ke(e),t=o("new-match-search-results");if(!t)return;if(i.length===0){t.innerHTML='<p style="color:#9ca3af;text-align:center;font-size:14px;">No users found.</p>';return}let s='<div style="margin-bottom:8px;font-size:14px;font-weight:600;">RESULTS ('+i.length+"):</div>";i.forEach(a=>{const n=a.displayName||a.email,r=ve.some(m=>m.uid===a.uid),u=n.replace(/'/g,"\\'");s+=`
 ...
 `}),t.innerHTML=s,console.log(" HTML rendered to container")}function Ms(e,i,t){const s=me.findIndex(a=>a.uid===e);s>-1?me.splice(s,1):me.push({uid:e,email:i,displayName:t})}function Is(){Je(),ie("modal-invitations")}function Je(){const e=o("invitations-list");if(!e)return;if(!ee||ee.length===0){e.innerHTML='<div class="empty-state"><div class="empty-sub">'+d("no_invitations")+"</div></div>";return}let i="";ee.forEach((t,s)=>{i+='<div class="card" style="margin-bottom:10px;">',i+='<div style="margin-bottom:10px;"><strong>'+d("invited_to_match")+"</strong></div>",i+='<div style="margin-bottom:10px;">Match ID '+t.matchId+" "+t.matchName+"</div>",i+='<div style="margin-bottom:10px;color:#94a3b8;">From: '+(t.invitedBy||t.invitedByUid||"Ukjent")+"</div>",i+='<div style="display:flex;gap:10px;">',i+='<button class="btn-primary" onclick="acceptInvitation('+s+')" style="flex:1;">'+d("accept")+"</button>",i+='<button onclick="declineInvitation('+s+')" style="flex:1;padding:12px;background:#374151;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">'+d("decline")+"</button>",i+="</div>",i+="</div>"}),e.innerHTML=i}async function Cs(e){const i=ee[e];if(!i)return;showSpinner("Accepting invitation…");Jt(i.matchId).then(function(t){hideSpinner();if(t.success){ee.splice(e,1);Ye();Je();fe();_e();De();// phase2 runs silently in background - no spinner needed
}else alert("Unable to accept invitation: "+t.error)})}async function _s(e){const i=ee[e];if(!i)return;const t=await Yt(i.matchId);t.success?(ee.splice(e,1),Ye(),Je(),_e(),De()):alert("Unable to decline invitation: "+t.error)}function Ye(){const e=o("invitation-badge");if(!e)return;const i=ee.length;i>0?(e.textContent=i,e.style.display="flex"):e.style.display="none";const tb=o("tab-invitation-badge");if(tb){if(i>0){tb.textContent=i;tb.style.display="flex";}else{tb.style.display="none";}}document.querySelectorAll(".tab-inv-badge").forEach(function(b){if(i>0){b.textContent=i;b.style.display="flex";}else{b.style.display="none";}});}function te(){var a,n;const e=o("home-content");if(!e)return;
const _cu=Pe();
const _fn=(_cu&&g&&g.firstName)||(_cu&&_cu.email&&_cu.email.split("@")[0])||"Shooter";
const _goal=(oe&&oe.seasonGoal)||"";
let t="";
t+='<div class="home-hero">';
t+='<div class="home-hero-label">Performance Intelligence Dashboard</div>';
t+='<div class="home-hero-title">Welcome back, '+_fn+'</div>';
t+='<div class="home-hero-sub">Analyze your performance, track your progress, and use data-driven forecasts to make smarter decisions. Build mental focus, identify what matters most, and prepare for your best possible performance.</div>';
t+='<div class="hero-divider"></div>';
t+='<div class="hero-goal-wrap">';
t+='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">';
t+='<span class="home-hero-label" style="margin-bottom:0;color:#e0b649;">My Season Goal</span>';
t+='<button onclick="teGoalEdit()" style="background:none;border:none;color:var(--muted);cursor:pointer;padding:4px 6px;border-radius:6px;font-size:16px;line-height:1;transition:color 0.15s;" title="Edit goal">✎</button>';
t+="</div>";
t+='<div id="hero-goal-display">';
if(_goal){
t+='<div style="font-size:14px;color:var(--text);line-height:1.5;">'+_goal.replace(/</g,"&lt;").replace(/>/g,"&gt;")+'</div>';
}else{
t+='<div style="font-size:13px;color:var(--muted);font-style:italic;">Tap ✎ to set your season goal...</div>';
}
t+="</div>";
t+='<div id="hero-goal-edit" style="display:none;">';
t+='<textarea id="hero-goal-input" style="width:100%;background:rgba(255,255,255,0.06);border:1px solid rgba(245,200,0,0.35);border-radius:8px;color:var(--text);font-size:14px;padding:10px 12px;resize:none;outline:none;box-sizing:border-box;font-family:inherit;line-height:1.5;" rows="3" placeholder="Write your season goal here...">'+_goal.replace(/</g,"&lt;").replace(/>/g,"&gt;")+'</textarea>';
t+='<div style="display:flex;gap:8px;margin-top:8px;">';
t+='<button onclick="teGoalSave()" style="background:var(--accent);color:#0a0c0e;border:none;border-radius:6px;padding:8px 16px;font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;cursor:pointer;">Save</button>';
...
e.innerHTML=t;
try{icRenderRewards();}catch(err){console.warn("icRenderRewards error:",err);}
}
window.teGoalEdit=function(){
  var d=document.getElementById('hero-goal-display');
  var ed=document.getElementById('hero-goal-edit');
  var inp=document.getElementById('hero-goal-input');
  if(d)d.style.display='none';
  if(ed)ed.style.display='block';
  if(inp){inp.focus();inp.selectionStart=inp.value.length;}
};
window.teGoalCancel=function(){
  var d=document.getElementById('hero-goal-display');
  var ed=document.getElementById('hero-goal-edit');
  if(d)d.style.display='block';
  if(ed)ed.style.display='none';
};
window.teGoalSave=async function(){
  var inp=document.getElementById('hero-goal-input');
  if(!inp)return;
  var val=(inp.value||'').trim();
  var _cu2=Pe();
  if(!_cu2||!_cu2.uid)return;
  try{
    await de(U(O,'users',_cu2.uid),{seasonGoal:val});
    if(oe)oe.seasonGoal=val;
    var d=document.getElementById('hero-goal-display');
    if(d){
      if(val){d.innerHTML='<div style="font-size:14px;color:var(--text);line-height:1.5;">'+val.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</div>';}
      else{d.innerHTML='<div style="font-size:13px;color:var(--muted);font-style:italic;">Tap \u270e to set your season goal...</div>';}
      d.style.display='block';
    }
    var ed=document.getElementById('hero-goal-edit');
    if(ed)ed.style.display='none';
  }catch(err){console.error('Goal save failed:',err);}
};function fe(){const e=o("match-list-container");if(!e)return;let i=$.filter(s=>{if(ue==="all")return!0;if(ue==="active")return s.status!=="finished";if(ue==="trening")return s.type==="Trening";if(ue==="stevne")return s.type==="Stevne";const a=s.date?new Date(s.date).getFullYear().toString():"";return ue===a});if(i.length===0){e.innerHTML='<div class="sec-hdr"><span class="sec-lbl">Matches</span></div><div class="empty-dash">'+d('no_matches_found')+'</div>';return}let t="";t+='<div class="sec-hdr"><span class="sec-lbl">'+i.length+' Match'+(i.length===1?'':'es')+'</span></div>';i.forEach((s,_mi)=>{var r;const a=s.id!=null&&s.id.toString()===String(R),n=s.id?"Match ID "+s.id+" "+s.name:s.name;const isOwner=(()=>{const _cu=$.find(_m=>_m.id!=null&&_m.id.toString()===s.id.toString());const _pu=Pe();return _cu&&_pu&&_cu.ownerId===_pu.uid;})();const _bg=_mi%2===0?"var(--card)":"var(--bg3)";t+=`<div class="match-row${a?" is-active":""}" style="background:${_bg};border-radius:14px;margin-bottom:2px;border:1px solid ${a?"rgba(245,200,0,.25)":"var(--border)"};">`;t+=`<div onclick="window.selectMatch('${s.id}')" style="display:flex;align-items:center;padding:12px 14px 8px;cursor:pointer;gap:10px;">`;const _mrd=s.date?new Date(s.date):null;const _mrday=_mrd?_mrd.getDate():null;const _mrmon=_mrd?_mrd.toLocaleDateString('en-GB',{month:'short'}).toUpperCase():null;if(_mrday){t+=`<div class="dt-bdg"><div class="dt-day">${_mrday}</div><div class="dt-mon">${_mrmon}</div></div>`;}t+=`<div style="flex:1;min-width:0;">`;t+=`<div class="match-row-name${a?" is-active":""}" style="font-weight:700;font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${n}</div>`;t+=`<div style="font-size:12px;color:var(--muted);margin-top:2px;">${We(s.date)}${s.location?" · "+s.location:""}</div><div style="font-size:11px;color:#e0b649;font-weight:700;letter-spacing:0.04em;margin-top:1px;">${s.type||""}</div>`;t+="</div>";t+=`<button onclick="event.stopPropagation();(function(){var el=document.getElementById('match-actions-${s.id}');var btn=document.getElementById('match-chevron-${s.id}');if(el){var open=el.style.display==='block';el.style.display=open?'none':'block';if(btn)btn.style.transform=open?'rotate(0deg)':'rotate(180deg)';}})();" style="background:none;border:none;cursor:pointer;padding:4px 6px;color:var(--muted);flex-shrink:0;transition:transform 0.2s;"><svg id="match-chevron-${s.id}" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="transition:transform 0.2s;"><polyline points="6 9 12 15 18 9"/></svg></button>`;t+='<div style="display:flex;flex-direction:column;align-items:center;min-width:36px;">';t+='<div style="font-size:18px;font-weight:800;color:var(--accent);line-height:1;">'+(((r=s.stages)==null?void 0:r.length)||0)+'</div>';t+='<div style="font-size:10px;color:var(--muted);">Stages</div>';t+='</div>';if(isOwner){t+=`<button onclick="event.stopPropagation();window.selectMatchSilent('${s.id}');window.confirmDeleteMatch();" style="background:none;border:none;cursor:pointer;padding:4px 6px;color:rgba(239,68,68,0.65);font-size:18px;line-height:1;border-radius:8px;" title="${d('delete_match')}">&#128465;</button>`;}else{t+='<div style="width:30px;"></div>';}t+="</div>";t+="</div>";t+=`<div id="match-actions-${s.id}" style="display:none;padding:0 12px 12px;"><div style="display:flex;gap:6px;">`;t+=`<button onclick="event.stopPropagation();window.selectMatchSilent('${s.id}');window.openEditMatch();" style="flex:1;padding:9px 0;background:rgba(245,200,0,0.10);border:1px solid rgba(245,200,0,0.25);color:#f5c800;border-radius:9px;font-size:11px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;cursor:pointer;">${d('edit')}</button>`;t+=`<button onclick="event.stopPropagation();window.selectMatchSilent('${s.id}');window.openCreateStage();" style="flex:1;padding:9px 0;background:rgba(245,200,0,0.10);border:1px solid rgba(245,200,0,0.25);color:#f5c800;border-radius:9px;font-size:11px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;cursor:pointer;">+ Stage</button>`;t+=`<button onclick="event.stopPropagation();window.selectMatchSilent('${s.id}');window.openInviteUser();" style="flex:1;padding:9px 0;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);color:var(--muted);border-radius:9px;font-size:11px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;cursor:pointer;">${d('invite')}</button>`;t+="</div></div>";t+="</div>";}),e.innerHTML=t}function _e(){var n;const e=o("results-content");if(!e)return;const i=$.find(r=>r.id!=null&&r.id.toString()===String(R));if(!i){e.innerHTML='<div class="empty-state"><div class="empty-sub">'+d('select_match_first')+'</div></div>';return}(function(){const _p=Pe();if(!_p)return;const _inPart=(i.participants||[]).includes(_p.uid);const _inShooters=(i.shooters||[]).some(function(sh){return sh.uid===_p.uid||sh.id===_p.uid||sh.id==="s_"+_p.uid;});if(_inPart&&!_inShooters){const _ns={id:_p.uid,isMe:_p.uid===i.ownerId,uid:_p.uid,firstName:_p.firstName||"",lastName:_p.lastName||"",division:_p.division||"Standard",pf:(_p.powerFactor||"minor").toLowerCase(),club:_p.club||"",stages:[]};i.shooters=(i.shooters||[]).concat([_ns]);Ee(i.id,{shooters:i.shooters}).catch(function(err){console.warn("Auto-add shooter failed:",err);});}})();if(!i.shooters||i.shooters.length===0){e.innerHTML='<div class="empty-state"><div class="empty-icon"></div><div class="empty-title">'+d('no_shooters')+'</div><div class="empty-sub">'+d('add_shooters_hint')+'</div></div>';return}const t=icMatchTotals(i,!liveShowAll);if(!t.length){e.innerHTML='<div class="empty-state"><div class="empty-title">'+d('no_results_yet')+'</div><div class="empty-sub">'+d('live_when_results')+'</div></div>';return}const commonNums=icCommonStageNumbers(i),toggleLabel=liveShowAll?d('common_stages_btn'):d('show_all_stages_btn');let s='<div class="ipad-live-left"><div class="card">';s+='<div class="card-header"><div class="card-title">Standings</div><button onclick="toggleLiveFilter()" style="padding:6px 12px;background:var(--accent);color:#0a0c0e;border:none;border-radius:6px;font-family:'Outfit',sans-serif;font-size:12px;font-weight:700;cursor:pointer;">'+toggleLabel+"</button></div>",s+=!liveShowAll&&commonNums.length>0?'<div style="font-size:12px;color:var(--muted);padding:8px 0;">'+d('based_on')+' '+commonNums.length+" common stage"+(commonNums.length===1?"":"s")+"</div>":"",s+='<div class="standings-table-wrap"><table class="standings-table">',s+='<thead><tr class="standings-header-row">',s+='<th class="standings-th standings-th-rank">#</th>',s+='<th class="standings-th standings-th-shooter">Shooter</th>',s+='<th class="standings-th standings-th-pts">STG PTS</th>',s+='<th class="standings-th standings-th-pct">%</th>',s+="</tr></thead>",s+="<tbody>";const dqSet=new Set((i.shooters||[]).filter(function(sh){return sh.dq;}).map(function(sh){return String(sh.id);}));const tNonDQ=t.filter(function(r){return!dqSet.has(String(r.id));});const tDQ=t.filter(function(r){return dqSet.has(String(r.id));});const a=((n=tNonDQ[0])==null?void 0:tNonDQ[0].totalStagePts)||0;let rankCounter=0;tNonDQ.forEach((r,u)=>{rankCounter++;const m=a>0?(r.totalStagePts/a*100).toFixed(2):"0.00";s+='<tr class="standings-row">',s+='<td class="standings-td standings-td-rank'+(rankCounter===1?' rank-1':rankCounter===2?' rank-2':rankCounter===3?' rank-3':'')+'">'+rankCounter+"</td>",s+='<td class="standings-td standings-td-shooter">',s+='<div class="standings-shooter-name">'+r.name+"</div>",s+='<div class="standings-shooter-meta">'+r.division+" · "+ge(r.pf||"minor")+"</div>",s+="</td>",s+='<td class="standings-td standings-td-pts">'+r.totalStagePts.toFixed(2)+"</td>",s+='<td class="standings-td standings-td-pct">'+m+"%</td>",s+="</tr>"});tDQ.forEach(function(r){s+='<tr class="standings-row" style="background:rgba(239,68,68,0.08);border-left:3px solid #ef4444;">',s+='<td class="standings-td standings-td-rank" style="color:#ef4444;">—</td>',s+='<td class="standings-td standings-td-shooter">',s+='<div class="standings-shooter-name" style="color:#ef4444;text-decoration:line-through;">'+r.name+' <span style="font-size:10px;background:rgba(239,68,68,0.2);color:#ef4444;padding:2px 6px;border-radius:4px;font-weight:700;text-decoration:none;display:inline-block;">DQ</span></div>',s+='<div class="standings-shooter-meta">'+r.division+" · "+ge(r.pf||"minor")+"</div>",s+="</td>",s+='<td class="standings-td standings-td-pts" style="color:#ef4444;">DQ</td>',s+='<td class="standings-td standings-td-pct" style="color:#ef4444;">—</td>',s+="</tr>";});const _cu=Pe();const _invUids=i.invitedUids||[];if(_cu&&i.ownerId===_cu.uid&&_invUids.length>0){const _invUsersArr=i.invitedUsers||[];const _invUsers={};_invUsersArr.forEach(function(u){if(u&&u.uid)_invUsers[u.uid]=((u.firstName||"")+" "+(u.lastName||"")).trim()||u.uid;});const _acceptedUids=new Set((i.participants||[]).concat((i.shooters||[]).map(function(sh){return sh.uid||sh.id||"";})));const _pendingInvUids=_invUids.filter(function(uid){return!_acceptedUids.has(uid);});_pendingInvUids.forEach(function(uid){s+='<tr class="standings-row" style="background:rgba(245,200,0,0.05);border-left:3px solid rgba(245,200,0,0.35);">';s+='<td class="standings-td standings-td-rank" style="color:rgba(245,200,0,0.50);">—</td>';s+='<td class="standings-td standings-td-shooter">';s+='<div class="standings-shooter-name" style="color:var(--muted);">'+(_invUsers[uid]||"—")+'<span style="font-size:10px;background:rgba(245,200,0,0.15);color:#f5c800;border:1px solid rgba(245,200,0,0.25);border-radius:4px;padding:2px 7px;font-weight:700;margin-left:8px;">Invited</span></div>';s+='<div class="standings-shooter-meta">Awaiting acceptance</div>';s+='</td>';s+='<td class="standings-td standings-td-pts" style="color:var(--muted);">—</td>';s+='<td class="standings-td standings-td-pct" style="color:var(--muted);">—</td>';s+='</tr>';});}s+="</tbody></table></div>",s+="</div>";// ── MATCHUTVIKLING ─────────────────────────────────────────────────────────
(function(){
  ...
  var shotNums=stageNums.filter(function(sn){
    return shooters.some(function(sh){
      return (sh.stages||[]).some(function(sr){return String(sr.num||sr.number)===sn&&sr.time>0;});
    });
  });
  ...
  var colors=['#f5c800','#4caf7d','#60a5fa','#f87171','#a78bfa','#fb923c','#34d399','#f472b6'];
  ...
  sorted.forEach(function(sd){
    var rowStyle=sd.isMe?'background:rgba(255,255,255,0.06);border-left:2px solid var(--accent);':'';
  ...
  sortedByLast.forEach(function(sd){
    ...
    sd.stagePoints.forEach(function(sp,si){
      ...
      if(sp.hasResult&&pct!==null){tblStg+='<div style="font-weight:'+(isWinner?'700':'400')+';color:'+(isWinner?'var(--accent)':'var(--text)')+';">'+pct.toFixed(1)+'%</div><div style="font-size:10px;color:var(--muted);">'+sp.hf.toFixed(2)+'</div>';}
  ...
  // Wrapper med toggle
  s+='</div><div class="ipad-live-right">';
...
s+='<tr style="border-top:2px solid var(--accent);font-weight:700;background:rgba(255,255,255,0.04);font-size:11px;">';
...
s+="</tr>";
s+="</table></div>";s+='<div style="margin-top:10px;">';s+='<button onclick="window.icToggleSecEl(this)" data-secid="stage-results-table" data-arrcls="sr-arrow" style="width:100%;padding:12px;background:var(--card);border:1px solid rgba(255,255,255,.08);border-radius:8px;color:var(--text);font-size:14px;font-weight:600;cursor:pointer;text-align:left;display:flex;justify-content:space-between;align-items:center;"><span>'+d('stage_results')+'</span><span class="sr-arrow">&#9654;</span></button>';s+='<div id="stage-results-table" style="display:none;margin-top:10px;">';var ctGroups={short:[],medium:[],long:[]};myResults.forEach(function(rr){var sd=icStageDefs(i).find(function(x){return Number(x.number)===Number(rr.num||rr.number);});if(!sd)return;var ct=getReferenceCourseType(sd);if(ctGroups[ct])ctGroups[ct].push({rr:rr,sd:sd});});var rAct=(i.referenceShootersEnabled&&refShooters.length)?getActiveReferenceShooters(i):[];["short","medium","long"].forEach(function(ct){var items=ctGroups[ct];if(!items.length)return;s+='<div style="margin-bottom:12px;">';s+='<div style="font-size:11px;font-weight:700;color:var(--accent);padding:6px 0;letter-spacing:0.06em;border-bottom:1px solid var(--border);margin-bottom:6px;">'+ct.toUpperCase()+' COURSE</div>';s+='<table style="width:100%;font-size:11px;border-collapse:collapse;">';s+='<tr style="border-bottom:1px solid var(--border);color:var(--muted);font-size:11px;">';s+='<th style="padding:4px 3px;text-align:left;">STG</th>';s+='<th style="padding:4px 3px;text-align:right;">TIME</th>';s+='<th style="padding:4px 3px;text-align:right;width:32px;">A</th>';s+='<th style="padding:4px 3px;text-align:right;width:32px;">C</th>';s+='<th style="padding:4px 3px;text-align:right;width:32px;">D</th>';s+='<th style="padding:4px 3px;text-align:right;width:32px;">M</th>';s+='<th style="padding:4px 3px;text-align:right;width:32px;">NS</th>';s+='<th style="padding:4px 3px;text-align:right;width:32px;">P</th>';s+='<th style="padding:4px 3px;text-align:right;">HF</th>';s+='</tr>';items.forEach(function(x){var rr=x.rr;var hf=rr.hf&&rr.hf>0?rr.hf:(rr.time>0?(rr.pts||0)/rr.time:0);s+='<tr style="border-bottom:1px solid var(--border);font-size:11px;">';s+='<td style="padding:4px 3px;">'+(rr.num||rr.number)+'</td>';s+='<td style="padding:4px 3px;text-align:right;">'+(rr.time||0).toFixed(2)+'s</td>';s+='<td style="padding:8px 6px;text-align:right;color:var(--green);">'+(rr.a||0)+'</td>';s+='<td style="padding:4px 3px;text-align:right;">'+(rr.c||0)+'</td>';s+='<td style="padding:4px 3px;text-align:right;">'+(rr.d||0)+'</td>';s+='<td style="padding:8px 6px;text-align:right;color:var(--red);">'+(rr.miss||0)+'</td>';s+='<td style="padding:8px 6px;text-align:right;color:var(--red);">'+(rr.ns||0)+'</td>';s+='<td style="padding:8px 6px;text-align:right;color:var(--red);">'+(rr.proc||0)+'</td>';s+='<td style="padding:8px 6px;text-align:right;color:var(--accent);">'+hf.toFixed(2)+'</td>';s+='</tr>';});var ctA=items.reduce(function(a,x){return a+(x.rr.a||0);},0);var ctC=items.reduce(function(a,x){return a+(x.rr.c||0);},0);var ctD=items.reduce(function(a,x){return a+(x.rr.d||0);},0);var ctMiss=items.reduce(function(a,x){return a+(x.rr.miss||0);},0);var ctNS=items.reduce(function(a,x){return a+(x.rr.ns||0);},0);var ctProc=items.reduce(function(a,x){return a+(x.rr.proc||0);},0);var ctTime=items.reduce(function(a,x){return a+(x.rr.time||0);},0);var ctTot=ctA+ctC+ctD+ctMiss;var ctAP=ctTot>0?(ctA/ctTot*100).toFixed(1):"0.0";var ctCP=ctTot>0?(ctC/ctTot*100).toFixed(1):"0.0";var ctDP=ctTot>0?(ctD/ctTot*100).toFixed(1):"0.0";var ctMissP=ctTot>0?(ctMiss/ctTot*100).toFixed(1):"0.0";var ctHFs=items.map(function(x){return x.rr.hf&&x.rr.hf>0?x.rr.hf:(x.rr.time>0?(x.rr.pts||0)/x.rr.time:0);});var ctAvgHF=ctHFs.length?ctHFs.reduce(function(a,b){return a+b;},0)/ctHFs.length:0;s+='<tr style="border-top:2px solid var(--accent);font-weight:700;background:rgba(255,255,255,0.04);font-size:11px;">';s+='<td style="padding:4px 3px;">'+d('col_sum')+'</td>';s+='<td style="padding:4px 3px;text-align:right;">'+ctTime.toFixed(2)+'s</td>';s+='<td style="padding:8px 6px;text-align:right;color:var(--green);">'+ctA+'<br><span style="font-size:10px;color:var(--muted);">'+ctAP+'%</span></td>';s+='<td style="padding:4px 3px;text-align:right;">'+ctC+'<br><span style="font-size:10px;color:var(--muted);">'+ctCP+'%</span></td>';s+='<td style="padding:4px 3px;text-align:right;">'+ctD+'<br><span style="font-size:10px;color:var(--muted);">'+ctDP+'%</span></td>';s+='<td style="padding:8px 6px;text-align:right;color:var(--red);">'+ctMiss+(ctMiss>0?'<br><span style=\"font-size:10px;color:var(--muted);\">'+ctMissP+'%</span>':'')+'</td>';s+='<td style="padding:8px 6px;text-align:right;color:var(--red);">'+ctNS+'</td>';s+='<td style="padding:8px 6px;text-align:right;color:var(--red);">'+ctProc+'</td>';s+='<td style="padding:8px 6px;text-align:right;color:var(--accent);">'+ctAvgHF.toFixed(2)+'</td>';s+='</tr>';s+='</table>';if(rAct.length){rAct.forEach(function(rf){var rfAP=Number(rf[ct+'AP']||0),rfCP=Number(rf[ct+'CP']||0),rfDP=Number(rf[ct+'DP']||0),rfHF=Number(rf[ct+'HF']||0);if(!rfAP&&!rfHF)return;var dA=ctTot>0?parseFloat(ctAP)-rfAP:null,dC=ctTot>0?parseFloat(ctCP)-rfCP:null,dD=ctTot>0?parseFloat(ctDP)-rfDP:null,dHF=rfHF>0?ctAvgHF-rfHF:null;s+='<div style="margin-top:6px;padding:8px;background:rgba(255,255,255,0.04);border-radius:6px;font-size:11px;">';s+='<div style="color:var(--muted);margin-bottom:6px;">vs '+rf.name+'</div>';s+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;text-align:center;margin-bottom:6px;">';s+='<div><div style="color:var(--muted);font-size:10px;">REF HF</div><div style="font-weight:700;color:var(--accent);">'+rfHF.toFixed(2)+'</div></div>';s+='<div><div style="color:var(--muted);font-size:10px;">YOUR HF</div><div style="font-weight:700;">'+ctAvgHF.toFixed(2)+'</div></div>';var dHFcol=dHF==null?'var(--text)':(dHF>=0?'var(--green)':'var(--red)');s+='<div><div style="color:var(--muted);font-size:10px;">DELTA</div><div style="font-weight:700;color:'+dHFcol+';">'+(dHF!=null?(dHF>=0?'+':'')+dHF.toFixed(2):'—')+'</div></div>';s+='</div>';s+='<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:4px;">';[['A%',rfAP,parseFloat(ctAP),dA,true],['C%',rfCP,parseFloat(ctCP),dC,false],['D%',rfDP,parseFloat(ctDP),dD,false]].forEach(function(z){var lbl=z[0],rfVal=z[1],myVal=z[2],delta=z[3],hg=z[4];var col=delta==null?'var(--text)':(hg?(delta>=0?'var(--green)':'var(--red)'):(delta<=0?'var(--green)':'var(--red)'));s+='<div style="text-align:center;padding:4px;background:rgba(255,255,255,0.03);border-radius:4px;">';s+='<div style="color:var(--muted);font-size:10px;">'+lbl+'</div>';s+='<div style="font-size:10px;">Ref: '+rfVal+'%</div>';s+='<div style="font-size:10px;">Yours: '+myVal.toFixed(1)+'%</div>';s+='<div style="font-weight:700;color:'+col+';">'+(delta!=null?(delta>=0?'+':'')+delta.toFixed(1)+'%':'—')+'</div>';s+='</div>';});s+='</div>';s+='</div>';});}s+='</div>';});s+='</div>';s+='</div>';s+='</div>';s+='<div style="margin-top:12px;">';s+='<button onclick="window.icToggleRefSection()" style="width:100%;padding:12px;background:var(--card);border:1px solid rgba(74,158,255,0.3);border-radius:10px;color:var(--text);font-size:14px;font-weight:600;cursor:pointer;text-align:left;display:flex;justify-content:space-between;align-items:center;">';s+='<span style=\'color:var(--blue);\'>'+d('ref_section')+'</span><span id="ref-sec-arr" style="font-size:11px;">&#9654;</span></button>';s+='<div id="ref-sec-body" style="display:none;margin-top:8px;">';var _refa=(i.referenceShootersEnabled&&refShooters.length)?getActiveReferenceShooters(i):[];if(_refa.length){s+='<div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap;">';s+='<button id="ref-btn-sh" onclick="window.liveToggleShooterRows()" style="padding:6px 14px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;border:1px solid rgba(255,255,255,0.2);background:rgba(245,200,0,0.15);color:var(--accent);">'+d('ref_hide_shooters')+'</button>';s+='<button id="ref-btn-rf" onclick="window.liveToggleRefRows()" style="padding:6px 14px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;border:1px solid rgba(74,158,255,0.3);background:rgba(74,158,255,0.15);color:var(--blue);">'+d('ref_hide_ref')+'</button>';s+='</div>';s+='<div style="font-size:11px;font-weight:700;color:var(--accent);letter-spacing:0.08em;text-transform:uppercase;margin-bottom:8px;border-bottom:1px solid var(--border);padding-bottom:6px;">'+d('ref_stage_comparison')+'</div>';icStageDefs(i).forEach(function(_sd){var _sm=icStageMetricsForMatch(i,_sd);var _sh=icStageShots(_sd);var _mp=icStageMaxPts(_sd);if(!_sm.length&&!_refa.length)return;s+='<div style="margin-bottom:10px;padding:10px;background:var(--bg);border-radius:8px;border:1px solid rgba(255,255,255,0.06)">';s+='<div style="font-size:12px;font-weight:700;margin-bottom:6px;">Stage '+_sd.number+(_sd.name&&_sd.name!=='Stage '+_sd.number?' · '+_sd.name:'')+' <span style="font-size:11px;color:var(--accent);">'+(_mp<=60?'Short':_mp<=120?'Medium':'Long')+'</span></div>';s+='<div style="overflow-x:auto"><table style="width:100%;font-size:10px;border-collapse:collapse">';s+='<tr style="border-bottom:1px solid var(--border);color:var(--muted);font-size:9px"><th style="padding:3px 2px;text-align:center;width:16px">#</th><th style="padding:3px 2px;text-align:left">Name</th><th style="padding:3px 2px;text-align:right">HF</th><th style="padding:3px 2px;text-align:right">PTS</th><th style="padding:3px 2px;text-align:right">Time</th><th style="padding:3px 2px;text-align:right;width:20px">A</th><th style="padding:3px 2px;text-align:right;width:20px">C</th><th style="padding:3px 2px;text-align:right;width:20px">D</th><th style="padding:3px 2px;text-align:right;width:20px">M</th></tr>';var _stRows=[];_sm.forEach(function(sm){var _hf=sm.hf||0;var _topHF2=0;_sm.forEach(function(s2){if((s2.hf||0)>_topHF2)_topHF2=s2.hf||0;});_refa.forEach(function(_rf2){var _p2=projectReferenceShooterForStage(_rf2,_sd);if(_p2&&_p2.benchmarkHF>_topHF2)_topHF2=_p2.benchmarkHF;});var _stagePts2=_topHF2>0?(_hf/_topHF2)*_mp:0;_stRows.push({isRef:false,name:sm.name,hf:_hf,pts:_stagePts2,time:sm.res&&sm.res.time?sm.res.time:0,a:sm.res&&sm.res.a!=null?sm.res.a:null,c:sm.res&&sm.res.c!=null?sm.res.c:null,dv:sm.res&&sm.res.d!=null?sm.res.d:null,miss:sm.res&&sm.res.miss!=null?sm.res.miss:null});});_refa.forEach(function(_rf){var _pr=projectReferenceShooterForStage(_rf,_sd);if(!_pr)return;var _topHF2=0;_sm.forEach(function(s2){if((s2.hf||0)>_topHF2)_topHF2=s2.hf||0;});_refa.forEach(function(_rf2){var _p2=projectReferenceShooterForStage(_rf2,_sd);if(_p2&&_p2.benchmarkHF>_topHF2)_topHF2=_p2.benchmarkHF;});var _refPts=_topHF2>0?(_pr.benchmarkHF/_topHF2)*_mp:0;var _estA=_sh>0&&_pr.aPct?Math.round(_sh*_pr.aPct/100):0;var _estC=_sh>0&&_pr.cPct?Math.round(_sh*_pr.cPct/100):0;var _estD=_sh>0&&_pr.dPct?Math.round(_sh*_pr.dPct/100):0;var _estM=_sh>0&&_pr.mPct?Math.round(_sh*_pr.mPct/100):0;_stRows.push({isRef:true,name:_rf.name,hf:_pr.benchmarkHF,pts:_refPts,time:_pr.projectedTotalTime,a:_estA,c:_estC,dv:_estD,miss:_estM});});_stRows.sort(function(a,b){return b.hf-a.hf;});var _stRank=0;_stRows.forEach(function(row){_stRank++;var _cls=row.isRef?'rs-rf':'rs-sh';var _bg=row.isRef?'background:rgba(74,158,255,0.05);':'';s+='<tr class="'+_cls+'" style="border-bottom:1px solid rgba(255,255,255,0.04);'+_bg+'">';s+='<td style="padding:3px 2px;text-align:center;font-weight:700;color:'+(row.isRef?'var(--blue)':'var(--muted)')+';font-size:9px;">'+_stRank+'</td>';s+='<td style="padding:3px 2px;font-weight:'+(row.isRef?'600':'700')+';color:'+(row.isRef?'var(--blue)':'inherit')+'">'+row.name+(row.isRef?' <span style="font-size:8px;background:rgba(74,158,255,0.2);color:var(--blue);padding:1px 3px;border-radius:2px">REF</span>':'')+'</td>';s+='<td style="padding:3px 2px;text-align:right;font-weight:700;color:'+(row.isRef?'var(--blue)':'var(--accent)')+'">'+(row.hf>0?row.hf.toFixed(2):'--')+'</td>';s+='<td style="padding:3px 2px;text-align:right;font-weight:'+(row.isRef?'600':'400')+'">'+row.pts.toFixed(2)+'</td>';s+='<td style="padding:3px 2px;text-align:right">'+(row.time>0?row.time.toFixed(2)+'s':'--')+'</td>';s+='<td style="padding:3px 2px;text-align:right;color:var(--green)">'+(row.a!=null?row.a:'--')+'</td>';s+='<td style="padding:3px 2px;text-align:right">'+(row.c!=null?row.c:'--')+'</td>';s+='<td style="padding:3px 2px;text-align:right">'+(row.dv!=null?row.dv:'--')+'</td>';s+='<td style="padding:3px 2px;text-align:right;color:var(--red)">'+(row.miss!=null?row.miss:'--')+'</td>';s+='</tr>';});s+='</table></div></div>';});s+='<div style="font-size:11px;font-weight:700;color:var(--accent);letter-spacing:0.08em;text-transform:uppercase;margin:8px 0;border-bottom:1px solid var(--border);padding-bottom:6px;">'+d('ref_total_standings')+'</div>';var _rt3=icMatchTotals(i,false);var _sd3=icStageDefs(i);var _rfhp3={};_sd3.forEach(function(_s3){var _k3=String(_s3.number);_rfhp3[_k3]={};_refa.forEach(function(_r3){var _p3=projectReferenceShooterForStage(_r3,_s3);_rfhp3[_k3][_r3.id]=_p3?_p3.benchmarkHF:0;});});var _thp3={};_sd3.forEach(function(_s3){var _k3=String(_s3.number);var _mx3=icStageMetricsForMatch(i,_s3);var _rh3=_mx3.length>0?(_mx3[0].hf||0):0;var _rfh3=0;_refa.forEach(function(_r3){var _h3=_rfhp3[_k3][_r3.id]||0;if(_h3>_rfh3)_rfh3=_h3;});_thp3[_k3]=Math.max(_rh3,_rfh3);});var _ra3=_rt3.map(function(_rt){var _ap3=0;_sd3.forEach(function(_s3){var _k3=String(_s3.number);var _tp3=_thp3[_k3]||0;if(!_tp3)return;var _mx3=icStageMetricsForMatch(i,_s3);var _sm3=_mx3.find(function(m){return String(m.id)===String(_rt.id);});if(!_sm3)return;_ap3+=(_sm3.hf||0)/_tp3*icStageMaxPts(_s3);});return Object.assign({},_rt,{totalStagePts:_ap3});});var _re3=_refa.map(function(_rf){var _ts3=0;_sd3.forEach(function(_s3){var _k3=String(_s3.number);var _pr3=projectReferenceShooterForStage(_rf,_s3);if(!_pr3)return;var _mp3=icStageMaxPts(_s3);var _tp3=_thp3[_k3]||0;if(!_mp3||!_tp3)return;_ts3+=(_pr3.benchmarkHF/_tp3)*_mp3;});return{id:'ref_'+_rf.id,name:_rf.name,isRef:true,totalStagePts:_ts3};});var _cb3=_ra3.concat(_re3);_cb3.sort(function(a,b){return b.totalStagePts-a.totalStagePts;});var _tpx3=_cb3.length>0?_cb3[0].totalStagePts:0;s+='<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;overflow:hidden">';s+='<table style="width:100%;font-size:12px;border-collapse:collapse"><thead><tr style="background:var(--bg3);color:var(--muted)"><th style="padding:8px 12px;text-align:left">#</th><th style="padding:8px 12px;text-align:left">Shooter</th><th style="padding:8px 12px;text-align:right">STG PTS</th><th style="padding:8px 12px;text-align:right">%</th></tr></thead><tbody>';_cb3.forEach(function(_e3,_ix3){var _pct3=_tpx3>0?(_e3.totalStagePts/_tpx3*100).toFixed(2):'0.00';var _imR=!_e3.isRef&&_e3.id&&($.find(function(m){var sh=icCurrentShooter(m);return sh&&sh.id===_e3.id;}));var _nc3=_e3.isRef?'var(--blue)':(_imR?'var(--accent)':'var(--text)');var _rb3=_e3.isRef?'rgba(74,158,255,0.06)':'';var _cls3=_e3.isRef?'rs-rf':'rs-sh';s+='<tr class="'+_cls3+'" style="border-top:1px solid var(--border);background:'+_rb3+'">';s+='<td style="padding:8px 12px;font-weight:700">'+(_ix3+1)+'</td>';s+='<td style="padding:8px 12px;font-weight:'+(_e3.isRef?'600':'700')+';color:'+_nc3+'">';s+=_e3.name+(_e3.isRef?' <span style="font-size:10px;background:rgba(74,158,255,0.2);color:var(--blue);padding:2px 5px;border-radius:3px">REF</span>':'')+'</td>';s+='<td style="padding:8px 12px;text-align:right;font-weight:700">'+_e3.totalStagePts.toFixed(2)+'</td>';s+='<td style="padding:8px 12px;text-align:right;font-weight:700;color:var(--accent)">'+_pct3+'%</td></tr>';});s+='</tbody></table></div>';}else{s+='<div style="padding:16px;text-align:center;color:var(--muted);font-size:13px;">Enable reference shooters in match settings.</div>';}s+='</div>';s+='</div>';s+='</div>';s+='</div>';s+='</div>';s+="</div>";s+='</div>';}e.innerHTML=s}async function uploadProfilePhoto(input){
...
html+="</div>";return html;}function icSeasonAnalysis(matches){
  ...
  var matchStats=matches.map(function(m){
    ...
    stages.forEach(function(s){
      ...
      const _pf1=icResultPF(g.powerFactor||"minor");const _pts1=icScoreFromHits(_pf1,(s.a||0),(s.c||0),(s.d||0),(s.miss||0),(s.ns||0),(s.proc||0));const _hf1=s.time>0?_pts1/s.time:0;tHF+=_hf1; tA+=s.a||0; tHits+=hits;
  ...
  if(tips.length){
    ...
    tips.forEach(function(t){
      html+='<div style="font-size:12px;padding:6px 8px;background:rgba(255,255,255,0.04);border-radius:6px;margin-bottom:4px;border-left:3px solid var(--accent);">&gt; '+t+'</div>';
    });
    html+='</div>';
  }
...
function icTrainingSeasonAnalysis(matches){
  ...
  var matchStats=tMatches.map(function(m){
    ...
    stages.forEach(function(s){var hits=(s.a||0)+(s.c||0)+(s.d||0)+(s.miss||0);const _pf2=icResultPF(g.powerFactor||"minor");const _pts2=icScoreFromHits(_pf2,(s.a||0),(s.c||0),(s.d||0),(s.miss||0),(s.ns||0),(s.proc||0));const _hf2=s.time>0?_pts2/s.time:0;tHF+=_hf2;tA+=s.a||0;tHits+=hits;tMiss+=s.miss||0;sc++;});
  ...
  var html='<button onclick="var x=document.getElementById(\'ta-body\');x&&(x.style.display=x.style.display===\'none\'?\'block\':\'none\');this.querySelector(\'.ta-arrow\').textContent=x&&x.style.display===\'block\'?\'\u25bc\':\'\u25b6\';" style="width:100%;padding:10px 14px;background:transparent;border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:var(--accent);font-size:13px;font-weight:700;cursor:pointer;text-align:left;margin-bottom:8px;">';
  html+='▶ Training Analysis</button>';
...
`
  ...
  overlay.innerHTML=`
    <div style="background:var(--card);border-radius:16px;padding:20px;width:100%;max-width:420px;max-height:85vh;overflow-y:auto;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;"><div style="font-size:15px;font-weight:700;">ESS/SSI Import</div><button onclick="var _ov=document.getElementById('ess-paste-modal');if(_ov)_ov.remove();" style="background:none;border:none;color:var(--muted);font-size:20px;cursor:pointer;padding:0 4px;line-height:1;">&times;</button></div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px;">
        ...
        <button id="ess-import-btn" onclick="essConfirmPaste('selected')" style="padding:10px;background:var(--accent);border:none;border-radius:6px;color:#0a0c0e;font-weight:700;cursor:pointer;font-size:12px;display:none;">Import selected</button>
      </div>
    </div>`;
  ...
  o("ess-paste-input").addEventListener("input",function(){
    ...
    if(allParsed&&allParsed.stages&&allParsed.stages.length&&prev){
      ...
      var hfStr=singleParsed?singleParsed.hf.toFixed(2):"—";
...
async function essConfirmPaste(mode){
  ...
  if(mode==="all"||mode==="selected"){
    ...
    var shooter=icFindShooter(match,selectedShooterId)||null;
    ...
    var cd=o("ocr-confirm-desc");if(cd)cd.textContent="HF: "+parsed.hf.toFixed(2)+" · Tid: "+parsed.time+"s";
    var modal=o("ess-paste-modal");if(modal)modal.remove();
...
function essParseSSIAll(text){
  ...
  var lines=text.trim().split("\n");
  var rows=[];
  function _pn(s){return parseFloat((s||"").replace(/,/g,".").trim())||0;}
  function _pi(s){return parseInt((s||"").replace(/,/g,".").trim())||0;}
  for(var li=0;li<lines.length;li++){
    var t=lines[li].trim();
    if(!t||t.toLowerCase().startsWith("total"))continue;
    var cols=t.split(/	+/);
    if(cols.length<8)continue;
    if(isNaN(_pn(cols[0])))continue;
    var hf=_pn(cols[0]);
    var time=_pn(cols[1]);
    var pts=_pi(cols[2]);
    var a=_pi(cols[3]);
    ...
    var miss=_pi(cols[6]);
    var proc=_pi(cols[7]);
    var ns=cols.length>8?_pi(cols[8]):0;
...
window.setAppLang=function setAppLang(lang){
  ...
  if(btnNo&&btnEn){
    if(no){
      btnNo.style.borderColor="#f5c800";btnNo.style.background="rgba(245,200,0,0.15)";btnNo.style.color="#f5c800";
      btnEn.style.borderColor="rgba(255,255,255,0.15)";btnEn.style.background="transparent";btnEn.style.color="var(--muted)";
    }else{
      btnEn.style.borderColor="#f5c800";btnEn.style.background="rgba(245,200,0,0.15)";btnEn.style.color="#f5c800";
...
async function Ks(){ke&&ke(),xe&&xe(),ze_unsub&&ze_unsub(),await $t(),window.location.reload()}const ht=document.getElementById("app");ht.innerHTML=`<div style="position:fixed;inset:0;background:#0a0c10;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:9999;" id="loading-screen"><img src="/Logo_IPSC-insight.png" alt="Insight Dynamics Shooting" style="width:88px;height:88px;border-radius:20px;box-shadow:0 8px 32px rgba(245,200,0,0.12);margin-bottom:28px;"><div style="font-size:26px;font-weight:900;color:#f5f7fb;letter-spacing:1px;margin-bottom:8px;">INSIGHT DYNAMICS</div><div style="font-size:14px;color:#7d8598;font-weight:500;margin-bottom:40px;font-style:italic;">Performance. Precision. Progress.</div><div style="width:180px;height:3px;background:rgba(255,255,255,0.08);border-radius:99px;overflow:hidden;"><div style="height:100%;width:40%;background:#f5c800;border-radius:99px;animation:ipsc-loading 1.2s ease-in-out infinite;"></div></div><style>@keyframes ipsc-loading{0%{transform:translateX(-100%);width:40%}50%{width:60%}100%{transform:translateX(350%);width:40%}}</style></div>`;function Vs(){Bt(ht,ft)}function ft(){ts(ht)}const _splashSlowMsg=setTimeout(()=>{const l=document.getElementById("loading-screen");if(l){const msg=document.createElement("div");msg.id="loading-slow-msg";msg.style.cssText="font-size:12px;color:#7d8598;margin-top:18px;text-align:center;";msg.textContent="Taking longer than usual — please wait…";l.appendChild(msg);}},5000);const _splashTimeout=setTimeout(()=>{clearTimeout(_splashSlowMsg);const l=document.getElementById("loading-screen");if(l){l.style.opacity="0";l.style.transition="opacity 0.3s";setTimeout(()=>{l.remove();Vs()},300)}},15000);Ct(e=>{clearTimeout(_splashTimeout);clearTimeout(_splashSlowMsg);const l=document.getElementById("loading-screen");if(l){l.style.opacity="0";l.style.transition="opacity 0.3s";setTimeout(()=>{l.remove();e?ft():Vs()},300)}else{e?ft():Vs()}});
// ── NOMINATIM GEOCODING ──────────────────────────────────────────────────
var _geoTimer=null;
function _geoDebounce(val,prefix){
  clearTimeout(_geoTimer);
  var res=document.getElementById(prefix+"-match-geo-results");
  if(res)res.innerHTML="";
  document.getElementById(prefix+"-match-lat").value="";
  document.getElementById(prefix+"-match-lng").value="";
  if(!val||val.length<2)return;
  _geoTimer=setTimeout(function(){_geoFetch(val,prefix);},400);
}
function _geoFetch(val,prefix){
  var url="https://nominatim.openstreetmap.org/search?format=json&q="+encodeURIComponent(val)+"&limit=5&accept-language=no,en";
  fetch(url,{headers:{"Accept-Language":"no,en"}})
    .then(function(r){return r.json();})
    .then(function(data){_geoRender(data,prefix);})
    .catch(function(){});
}
function _geoRender(data,prefix){
  var res=document.getElementById(prefix+"-match-geo-results");
  if(!res)return;
...
function _geoSelect(prefix,name,lat,lng){
  ...
  var latEl=document.getElementById(prefix+"-match-lat");
  var lngEl=document.getElementById(prefix+"-match-lng");
  if(latEl)latEl.value=lat;
  if(lngEl)lngEl.value=lng;
  var res=document.getElementById(prefix+"-match-geo-results");
  if(res)res.innerHTML="";
}
window._geoDebounce=_geoDebounce;
// ── END NOMINATIM GEOCODING ──────────────────────────────────────────────

function ptSilent(e){R=e!=null?e.toString():e;const i=$.find(t=>t.id!=null&&t.id.toString()===R);if(i){const t=i.id?"Match ID "+i.id+" "+i.name:i.name||"Match";["home-chip-name","prog-chip-name","results-chip-name","teams-chip-name"].forEach(a=>{const n=o(a);n&&(n.textContent=t)})}xe&&xe(),R&&(xe=Wt(R,t=>{const s=$.findIndex(a=>a.id!=null&&a.id.toString()===R);s!==-1&&t&&($[s]=t,te(),_e(),De())}))}window.selectMatchSilent=ptSilent;
