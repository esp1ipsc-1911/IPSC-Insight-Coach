...
return e.teams.map(team=>{
  ...
  stages.forEach(stage=>{
    ...
    // Filter to team members, zero out DQ shooters
    const teamMetrics=metrics
      .filter(m=>team.shooterIds&&team.shooterIds.includes(m.id))
      .map(m=>dqIds.has(String(m.id))?{...m,stagePts:0,hf:0,dq:true}:m);
    const sorted=teamMetrics.slice().sort((a,b)=>(b.stagePts||0)-(a.stagePts||0));
    const top3=sorted.slice(0,3);
    let stagePts=0;
    top3.forEach(m=>{
      stagePts+=(m.stagePts||0);
      shooterTotals[m.id]=(shooterTotals[m.id]||0)+(m.stagePts||0);
    });
    totalPts+=stagePts;
    stageBreakdown.push({stageNum:stage.number||stage.num,stagePts,top3:top3.map(m=>({id:m.id,name:m.name,stagePts:m.stagePts||0,hf:m.hf||0,dq:!!m.dq}))});
  });
  // Determine which 3 shooters contributed most overall
  const shooterIds=team.shooterIds||[];
  const shooters=(e.shooters||[]).filter(s=>shooterIds.includes(s.id));
  const stagesCount=stages.filter(st=>{
    const m=icStageMetricsForMatch(e,st);
    return m.some(mm=>shooterIds.includes(mm.id));
  }).length;
  return{id:team.id,name:team.name,shooterIds,totalPts,stageBreakdown,shooterTotals,shooters,stagesCount,dqIds};
}).sort((a,b)=>b.totalPts-a.totalPts)}function renderTeamsScreen(){
  ...
  // ── Standings table with expanded stats ──
  let sh='<div class="card"><div class="card-header"><div class="card-title">Team Standings</div></div>';
  sh+='<div class="standings-table-wrap"><table class="standings-table">';
  sh+='<thead><tr class="standings-header-row"><th class="standings-th">#</th><th class="standings-th">TEAM</th><th class="standings-th standings-th-pts">STG PTS</th><th class="standings-th standings-th-pct">%</th></tr></thead><tbody>';
  standings.forEach((t,i)=>{
    ...
    sh+="</tr>";
    // Expandable detail row: per-stage breakdown
    ...
    sh+='<tr style="color:var(--muted);border-bottom:1px solid var(--border);"><th style="padding:4px 8px;text-align:left;">Stage</th><th style="padding:4px 4px;text-align:right;">STG PTS</th><th style="padding:4px 8px;text-align:left;">Top 3 shooters</th></tr>';
    (t.stageBreakdown||[]).forEach(function(sb){
      const shooterList=sb.top3.map(function(m){
        return '<span style="color:'+(m.dq?'#ef4444':'var(--text)')+';margin-right:6px;">'
          +m.name.split(' ')[0]
          +' <span style="color:var(--accent);">'+m.hf.toFixed(2)+'</span>'
  ...
  // ── Team cards with DQ button and division validation ──
  let lh="";
  teams.forEach((team,tidx)=>{
    const allShooters=(match.shooters||[]);
    const members=allShooters.filter(s=>team.shooterIds&&team.shooterIds.includes(s.id));
    const memberNames=members.map(s=>((s.firstName||"")+" "+(s.lastName||"")).trim()||d('shooter_default'));
    // Division check warning
    const divs=[...new Set(members.map(s=>s.division||'').filter(Boolean))];
    const divWarning=divs.length>1?'<div style="font-size:11px;color:#ef4444;margin-top:6px;">⚠ Mixed divisions: '+divs.join(', ')+'</div>':'';
    lh+='<div class="card" style="margin-bottom:10px;">';
    lh+='<div class="card-header"><div class="card-title">'+(team.country?team.country+' ':'')+team.name+'</div>';
    lh+='<div style="display:flex;gap:8px;">';
    lh+='<button onclick="editTeam('+tidx+')" style="padding:6px 12px;background:var(--accent);color:#1a202c;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">'+d('edit_btn')+'</button>';
    lh+='<button onclick="deleteTeam('+tidx+')" style="padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">'+d('delete_btn')+'</button>';
    lh+='</div></div>';
    lh+='<div style="font-size:13px;color:var(--muted);margin-top:8px;">'+(memberNames.length?memberNames.join(", "):d('no_shooters'))+'</div>';
    lh+=divWarning;
    // Members with DQ toggle
    lh+='<div style="margin-top:10px;">';
    members.forEach(function(s){
      const isDQ=!!s.dq;
      const sName=((s.firstName||"")+" "+(s.lastName||"")).trim();
      lh+='<div style="display:flex;align-items:center;justify-content:space-between;padding:5px 0;border-bottom:1px solid rgba(255,255,255,0.05);">';
      lh+='<span style="font-size:13px;'+(isDQ?'color:#ef4444;text-decoration:line-through;':'')+'">'+sName+(isDQ?' <span style="font-size:11px;">[DQ]</span>':'')+'</span>';
      lh+='<button onclick="toggleDQ('+JSON.stringify(match.id)+','+JSON.stringify(s.id)+')" style="padding:3px 8px;font-size:11px;border:none;border-radius:6px;cursor:pointer;background:'+(isDQ?'rgba(239,68,68,0.2)':'rgba(255,255,255,0.08)')+';color:'+(isDQ?'#ef4444':'var(--muted)')+';">'+(isDQ?'Undo DQ':'Mark DQ')+'</button>';
      lh+='</div>';
    });
    lh+='</div>';
    lh+="</div>";
