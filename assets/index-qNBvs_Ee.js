...
const mentalTips=[];
if(totalMiss>stageCount*0.5)mentalTips.push({t:"Indre dialog etter miss",b:"Identifiser den indre sabotøren — stemmen som sier du er dårlig. Bruk SOAL: Stopp tanken, Observer den uten å reagere, Aksepter at den er der, La den gå. Hjernen husker det siste bildet — visualiser straks hvordan du ville skutt den stageen riktig, så du går videre med et positivt bilde."});
if(stageAPs.length>1){const _aps=stageAPs.map(function(x){return x.ap;});if(Math.max.apply(null,_aps)-Math.min.apply(null,_aps)>20)mentalTips.push({t:"Spenningsregulering — stor variasjon",b:"Stor variasjon mellom stages skyldes ofte ujevnt spenningsnivå. Finn ditt optimale nivå med en pustepause: pust inn 4 sek, hold 4, pust ut 6. Identifiser dine egne knagg-signaler — hva kjenner du i kroppen når du er i flytsonen? Bruk disse signalene bevisst før neste stage."});}
if(overallAP>=90&&avgHF<3.5)mentalTips.push({t:"Regulere opp — treffbildet er klart",b:"Kroppen er klar for mer tempo. Bruk et energigivende triggerord («Eksplosiv!», «Nå!») og visualiser stagene med høy intensitet og flyt. Tenk på hva denne matchen betyr for deg — indre motivasjon driver raskere og mer stabilt enn ytre press."});
if(totalProc>0)mentalTips.push({t:"Selektiv oppmerksomhet — procedurals",b:"Procedurals skjer når oppmerksomheten er rettet mot feil ting. Øv på å filtrere bort støy — RO-avgjørelser, baneforhold, andre skyttere. Før du går inn på neste stage: lukk øynene, visualiser stage-planen steg for steg og sjekk at alle regler er på plass i hodet."});
const _gt=[
{t:"Triggerord",b:"Velg 1-2 ord som skaper riktig tilstand umiddelbart. Unngå «nei»-ord — underbevisstheten filtrerer dem ut og du fokuserer på det du vil unngå. Gode eksempler: «Pust», «Flyt», «Jeg er klar», «Rå». Skriv dem ned og test dem i trening før du bruker dem i match."},
{t:"Visualisering med PETTLEP",b:"Visualiser i førsteperson, i riktig kroppsstilling, med riktig tempo og alle sanser aktivert. Inkluder følelsene — kontroll, intensitet, glede. Får du frem negative bilder? Si stopp og spol tilbake. Avslutt alltid med et positivt utfall. 10 minutter hver kveld i tre uker gir merkbar fremgang."},
{t:"Selvtillit gjennom mestring",b:"Selvtillit bygges av prestasjoner, ikke av resultatmål. Fokuser på mestringsmål: trekk, bevegelse, stage-plan. Etter matchen — skriv ned tre ting du gjorde bra før du ser på forbedringsområder. Dette styrker selvtilliten og gjør evalueringen mer konstruktiv."},
{t:"Indre motivasjon",b:"Hva driver deg egentlig? Glede av skyting, fellesskap, personlig utvikling? Nær den indre motivasjonen aktivt — ytre press som resultater og rangering gjør deg sårbar når ting går mot deg. Ha det gøy, nyt fellesskapet, og sørg for at du stadig opplever fremgang på noe."},
{t:"Målsetting",b:"Sett mestringsmål (HVA du gjør) fremfor bare resultatmål. Lag din egen skala 1–10 for f.eks. konsentrasjon, trekk eller stage-plan-etterlevelse. Skriv ned både målet og HVORDAN du skal jobbe for å nå det. Hvert delmål du når bringer deg ett steg nærmere det langsiktige målet."}
];
...
if(rActive&&rActive.length>0&&h&&u){
  ...
  if(matchRefs.length>0){
    ...
    matchRefs.forEach(function(ref){
      ...
      html+='<div style="color:'+(dHF>=0?"var(--green)":"var(--red)")+';font-size:11px;">'+(dHF>=0?"+":"")+dHF.toFixed(2)+'</div></div>';
      if(refAP!=null){
        html+='<div><div style="color:var(--muted);">A%</div>';
        html+='<div style="font-weight:700;">'+refAP+'%</div>';
        html+='<div style="color:'+(dAP>=0?"var(--green)":"var(--red)")+';font-size:11px;">'+(dAP>=0?"+":"")+dAP+'%</div></div>';
      }
      if(refCP!=null){
        html+='<div><div style="color:var(--muted);">C%</div>';
        html+='<div style="font-weight:700;">'+refCP+'%</div>';
        html+='<div style="color:'+(dCP<=0?"var(--green)":"var(--red)")+';font-size:11px;">'+(dCP>=0?"+":"")+dCP+'%</div></div>';
      }
      if(refDP!=null){
        html+='<div><div style="color:var(--muted);">D%</div>';
        html+='<div style="font-weight:700;">'+refDP+'%</div>';
        html+='<div style="color:'+(dDP<=0?"var(--green)":"var(--red)")+';font-size:11px;">'+(dDP>=0?"+":"")+dDP+'%</div></div>';
      }
      html+='</div></div>';
    });
    html+='</div>';
