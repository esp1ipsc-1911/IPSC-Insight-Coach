 ...
 * {
 ...
 <button id="langNo" class="lang-btn" type="button" aria-label="Norsk">🇳🇴</button>
 <button id="langEn" class="lang-btn active" type="button" aria-label="English">🇺🇸</button>
 </div>
 </div>
...
<div class="screen" id="screen-prognose">
 ...
 <div class="card">
 <div class="card-header"><div class="card-title">"+d("shooter_data")+"</div><span class="badge badge-blue">Auto</span></div>
 ...
 <div id="prog-data-status" style="margin-top:12px;padding:12px;background:var(--bg);border-radius:8px;font-size:13px;color:var(--muted);text-align:center;display:none;">
  d("no_stages_shot"). d("data_after_results").
 </div>
 </div>
...
<div class="modal-overlay" id="modal-new-match" onclick="closeModalOutside(event,'modal-new-match')">
 ...
 <option value=d("match_type_practice")>${d("match_types_trening")}</option>
...
<div class="modal-overlay" id="modal-upload-result" onclick="closeModalOutside(event,'modal-upload-result')">
 ...
 <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;"><button class="btn-secondary" id="upload-manual-btn" onclick="icOpenManualResult()">Registrer manuelt</button><button class="btn-primary" id="upload-ess-btn" onclick="importESSVerify(event)">ESS/SSI</button></div>
 </div>
 </div>
</div>
...
<div class="modal-overlay" id="modal-confirm-delete" onclick="closeModalOutside(event,'modal-confirm-delete')">
 ...
 <button onclick="closeModal('modal-confirm-delete')" style="flex:1;padding:12px;background:#374151;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">"+d("cancel")+"</button>
...
window.icOpenEditStageResult=icOpenEditStageResult;function icPopulateTeamCountry(selected){
  ...
  if(!el)return;
  el.innerHTML='<option value="">Velg land...</option>';
...
window.icPopulateTeamCountry=icPopulateTeamCountry,window.deleteCurrentStage=deleteCurrentStage,window.openInviteUser=ks,window.openInvitationsModal=Is,window.acceptInvitation=Cs,window.declineInvitation=_s,window.searchUsers=xs,window.toggleUserSelection=Ss,window.sendMultipleInvitations=Ls,window.searchUsersNewMatch=Ps,window.toggleUserNewMatch=Es,window.searchUsersEditMatch=Ts,window.toggleUserEditMatch=Ms,window.confirmDeleteMatch=gs,window.deleteMatchConfirmed=vs,window.uploadAndScanResult=Hs,window.saveOCRResult=Gs,window.icOpenManualResult=icOpenManualResult,window.importESSVerify=importESSVerify,window.essConfirmPaste=essConfirmPaste,window.openCreateStageFromUpload=Us,window.toggleMatchDropdown=ds,window.selectMatchFromDropdown=cs,window.toggleReferenceShootersEnabled=toggleReferenceShootersEnabled,window.toggleReferenceShooterChoice=toggleReferenceShooterChoice,window.updateReferenceShooterOverride=updateReferenceShooterOverride,window.toggleLiveFilter=toggleLiveFilter,window.toggleTips=toggleTips,window.deleteRun=deleteRun,window.openCreateTeam=openCreateTeam,window.editTeam=editTeam,window.deleteTeam=deleteTeam,window.saveTeam=saveTeam,window.renderTeamsScreen=renderTeamsScreen}function toggleLiveFilter(){liveShowAll=!liveShowAll,_e()}function toggleTips(id){const el=document.getElementById(id);if(el)el.style.display=el.style.display==="none"?"block":"none"}async function deleteRun(stageNum,runIdx){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match)return;const shooter=icCurrentShooter(match);if(!shooter)return;const stageIdx=shooter.stages.findIndex(s=>(s.num||s.number)==stageNum);if(stageIdx<0)return;const stage=shooter.stages[stageIdx];if(!stage.runs||stage.runs.length<=1){alert("Kan ikke slette siste run. Slett hele resultatet i stedet.");return;}if(!confirm((dt==="no"?"Slette Run ":"Delete Run ")+(runIdx+1)+"?"))return;stage.runs.splice(runIdx,1);stage.runs.forEach((r,i)=>{r.runNum=i+1});const best=stage.runs.reduce((a,b)=>(b.hf||0)>(a.hf||0)?b:a,stage.runs[0]);Object.assign(stage,best,{num:stage.num,name:stage.name,runs:stage.runs});const res=await Ee(match.id,{shooters:match.shooters});if(res.success){te(),_e(),De()}else alert("Kunne ikke slette run: "+res.error);}function icTeamStandings(e){if(!e||!e.teams||!e.teams.length)return[];const stages=icStageDefs(e);return e.teams.map(team=>{let totalPts=0;stages.forEach(stage=>{const metrics=icStageMetricsForMatch(e,stage);const teamMetrics=metrics.filter(m=>team.shooterIds&&team.shooterIds.includes(m.id));const sorted=teamMetrics.slice().sort((a,b)=>(b.stagePts||0)-(a.stagePts||0));const top3=sorted.slice(0,3);top3.forEach(m=>{totalPts+=(m.stagePts||0)})});return{id:team.id,name:team.name,shooterIds:team.shooterIds||[],totalPts}}).sort((a,b)=>b.totalPts-a.totalPts)}function renderTeamsScreen(){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));const sc=o("teams-standings-container");const lc=o("teams-list-container");if(!sc||!lc)return;if(!match){sc.innerHTML='<div class="empty-state"><div class="empty-sub">Velg en match først</div></div>';lc.innerHTML="";return;}const teams=match.teams||[];if(!teams.length){sc.innerHTML='<div class="empty-state"><div class="empty-title">Ingen lag ennå</div><div class="empty-sub">Opprett et lag for å se standings</div></div>';lc.innerHTML="";return;}const standings=icTeamStandings(match);const topPts=standings.length?standings[0].totalPts:0;let sh='<div class="card"><div class="card-header"><div class="card-title">Lag-standings</div></div>';sh+='<div class="standings-table-wrap"><table class="standings-table">';sh+='<thead><tr class="standings-header-row"><th class="standings-th">#</th><th class="standings-th">LAG</th><th class="standings-th standings-th-pts">STG PTS</th><th class="standings-th standings-th-pct">%</th></tr></thead><tbody>';standings.forEach((t,i)=>{const pct=topPts>0?(t.totalPts/topPts*100).toFixed(2):"0.00";sh+='<tr class="standings-row">';sh+='<td class="standings-td">'+(i+1)+"</td>";sh+='<td class="standings-td">'+t.name+"</td>";sh+='<td class="standings-td standings-td-pts">'+t.totalPts.toFixed(2)+"</td>";sh+='<td class="standings-td standings-td-pct">'+pct+"%</td>";sh+="</tr>"});sh+="</tbody></table></div></div>";sc.innerHTML=sh;let lh="";teams.forEach((team,idx)=>{const allShooters=(match.shooters||[]);const memberNames=allShooters.filter(s=>team.shooterIds&&team.shooterIds.includes(s.id)).map(s=>((s.firstName||"")+" "+(s.lastName||"")).trim()||"Skytter");lh+='<div class="card" style="margin-bottom:10px;">';lh+='<div class="card-header"><div class="card-title">'+(team.country?team.country+' ':'')+team.name+"</div>";lh+='<div style="display:flex;gap:8px;">';lh+='<button onclick="editTeam('+idx+')" style="padding:6px 12px;background:var(--accent);color:#1a202c;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">"+d("edit_btn")+"</button>';lh+='<button onclick="deleteTeam('+idx+')" style="padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">Slett</button>';lh+="</div></div>";lh+='<div style="font-size:13px;color:var(--muted);margin-top:8px;">'+( memberNames.length?memberNames.join(", "):"Ingen skyttere")+"</div>";lh+="</div>"});lc.innerHTML=lh}let currentEditTeamIdx=null;function openCreateTeam(){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match){alert("Velg en match først");return;}currentEditTeamIdx=null;o("team-modal-title").textContent="Opprett lag";o("team-name-input").value="";icPopulateTeamCountry("");renderTeamShooterList(match,null);ie("modal-create-team")}function editTeam(idx){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match||!match.teams||!match.teams[idx])return;currentEditTeamIdx=idx;const team=match.teams[idx];o("team-modal-title").textContent="Rediger lag";o("team-name-input").value=team.name||"";icPopulateTeamCountry(team.country||"");renderTeamShooterList(match,team.shooterIds||[]);ie("modal-create-team")}function renderTeamShooterList(match,selectedIds){const container=o("team-shooter-list");if(!container)return;const allShooters=(match.shooters||[]);const usedIds=new Set();(match.teams||[]).forEach((t,i)=>{if(i===currentEditTeamIdx)return;(t.shooterIds||[]).forEach(id=>usedIds.add(id))});let html="";allShooters.forEach(s=>{const name=((s.firstName||"")+" "+(s.lastName||"")).trim()||"Skytter";const checked=selectedIds&&selectedIds.includes(s.id);const disabled=!checked&&usedIds.has(s.id);html+='<label style="display:flex;align-items:center;gap:10px;padding:10px;background:var(--bg);border-radius:8px;margin-bottom:8px;cursor:'+(disabled?"not-allowed":"pointer")+';">';html+='<input type="checkbox" value="'+s.id+'" '+(checked?"checked":"")+(disabled?" disabled":"")+(disabled?"":' onchange="enforceTeamLimit(this)"')+ ' style="width:18px;height:18px;">';html+='<div><div style="font-weight:600;"'+(disabled?' style="color:var(--muted);"':'')+">"+ name+"</div>";if(disabled)html+='<div style="font-size:11px;color:var(--muted);">Allerede på et lag</div>';html+="</div></label>"});if(!allShooters.length)html='<div style="color:var(--muted);font-size:13px;">Ingen skyttere i matchen. Legg til skyttere først.</div>';container.innerHTML=html}function enforceTeamLimit(el){const checked=o("team-shooter-list").querySelectorAll("input:checked");if(checked.length>4){el.checked=false;alert("Maks 4 skyttere per lag")}}async function saveTeam(){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match)return;const name=(o("team-name-input").value||"").trim();if(!name){alert("Lagnavn kan ikke være tomt");return;}const country=(o("team-country-input")&&o("team-country-input").value)||"";const checkboxes=o("team-shooter-list").querySelectorAll("input[type=checkbox]:checked");const shooterIds=Array.from(checkboxes).map(c=>c.value);if(shooterIds.length>4){alert("Maks 4 skyttere per lag");return;}match.teams=match.teams||[];if(currentEditTeamIdx!==null){match.teams[currentEditTeamIdx]={...match.teams[currentEditTeamIdx],name,country,shooterIds}}else{match.teams.push({id:"team_"+Date.now(),name,country,shooterIds})}const res=await Ee(match.id,{teams:match.teams});if(res.success){G("modal-create-team");renderTeamsScreen()}else alert("Kunne ikke lagre lag: "+res.error)}async function deleteTeam(idx){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match||!match.teams)return;const team=match.teams[idx];if(!confirm("Slette laget "+team.name+"?"))return;match.teams.splice(idx,1);const res=await Ee(match.id,{teams:match.teams});if(res.success)renderTeamsScreen();else alert("Kunne ikke slette lag: "+res.error)}function ss(e){document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active")),document.querySelectorAll(".tab-item").forEach(s=>s.classList.remove("active")),o(e).classList.add("active");const i=["screen-home","screen-matches","screen-prognose","screen-results","screen-teams","screen-profile"].indexOf(e),t=document.querySelectorAll(".tab-item");t[i]&&t[i].classList.add("active");document.querySelectorAll(".tab-bar").forEach(function(bar){var items=bar.querySelectorAll(".tab-item");if(items[i])items[i].classList.add("active");}),e==="screen-home"&&te(),e==="screen-matches"&&fe(),e==="screen-results"&&_e(),e==="screen-teams"&&(renderTeamsScreen(),R&&pt(R))}function ie(e){o(e).classList.add("open"),e==="modal-upload-result"&&as(),e==="modal-add-shooter"&&icUpdateNewShooterPFOptions(),e==="modal-add"&&icInitManualResult()}async function as(){const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e)return;const i=o("upload-match-name");i&&(i.textContent=e.name||"Ukjent match");const t=o("upload-stage-select"),s=icStageDefs(e);if(t.innerHTML="",s.length>0)s.forEach(n=>{const r=document.createElement("option");r.value=n.number,r.textContent=n.name&&n.name!=="Stage "+n.number?`Stage ${n.number} - ${n.name}`:`Stage ${n.number}`,t.appendChild(r)});else{const n=document.createElement("option");n.value="",n.textContent="Ingen stages - opprett ny",t.appendChild(n)}const a=o("upload-shooter-select");a.innerHTML="";const n=new Set,r=icCurrentShooterId(),u=document.createElement("option");u.value=r,u.textContent="Meg ("+(g.firstName||"")+" "+(g.lastName||"")+")",a.appendChild(u),n.add(r);if(e.participants&&e.participants.length>0)for(const m of e.participants)if(!n.has(m))try{const b=await Ut(m);if(b){const f=document.createElement("option");f.value=m,f.textContent=(b.firstName||"")+" "+(b.lastName||""),a.appendChild(f),n.add(m)}}catch(b){console.error("Error fetching user:",m,b)}e.shooters&&e.shooters.length>0&&e.shooters.forEach(m=>{if(!n.has(m.id)){const b=document.createElement("option");b.value=m.id,b.textContent=(m.firstName||"")+" "+(m.lastName||""),a.appendChild(b),n.add(m.id)}})}function G(e){o(e).classList.remove("open")}function ns(e,i){e.target.id===i&&G(i)}function rs(e,i){ue=e,document.querySelectorAll(".filter-chip").forEach(t=>t.classList.remove("active")),i.classList.add("active"),fe()}async function os(){var t;const e={name:F("new-match-name")||"Ny match",type:F("new-match-type")||d("match_type_practice"),date:F("new-match-date")||new Date().toISOString().split("T")[0],location:F("new-match-location")||"",plannedStages:A("new-match-stages",6),searchable:((t=o("new-match-searchable"))==null?void 0:t.checked)!==!1,status:"active",stages:[],shooters:[],referenceShootersEnabled:!1,referenceShooterIds:[],referenceOverrides:{}};showSpinner("Oppretter match…");zt(e).then(function(i){hideSpinner();if(i.success){let s=0;Promise.all(ve.map(function(a){return Ge(a.email,{matchId:i.matchId,matchName:e.name})})).then(function(results){results.forEach(function(r){if(r.success)s++});G("modal-new-match");o("new-match-name").value="";o("new-match-location").value="";o("new-match-stages").value="6";o("new-match-searchable")&&(o("new-match-searchable").checked=!0);o("new-match-user-search").value="";o("new-match-search-results").innerHTML="";ve=[];if(s>0)alert("Match opprettet! Invitasjoner sendt til "+s+" bruker(e).")})}else alert("Kunne ikke opprette match: "+i.error)})}async function ls(){const e=F("match-id-search").trim();if(!e){alert("Skriv inn et match-ID");return}const i=await Kt(e);if(i.success){const t=$.findIndex(s=>s.id&&s.id.toString()===i.match.id.toString());t!==-1?R=$[t].id:($.unshift(i.match),R=i.match.id),pt(R),oe&&oe("screen-home"),o("match-id-search").value=""}else alert(`Fant ingen match med ID ${e}${i.error?": "+i.error:""}`)}function pt(e){R=e!=null?e.toString():e;const i=$.find(t=>t.id!=null&&t.id.toString()===R);if(i){const t=i.id?"Match ID "+i.id+" "+i.name:i.name||"Match";["home-chip-name","prog-chip-name","results-chip-name","teams-chip-name"].forEach(a=>{const n=o(a);n&&(n.textContent=t)})}xe&&xe(),R&&(xe=Wt(R,t=>{const s=$.findIndex(a=>a.id!=null&&a.id.toString()===R);s!==-1&&t&&($[s]=t,te(),_e())})),te(),_e(),De(),fe()}function ds(e){const i=e+"-match-dropdown",t=o(i);if(!t)return;if(t.classList.contains("open")){t.classList.remove("open");return}document.querySelectorAll(".match-dropdown").forEach(a=>a.classList.remove("open"));let s="";$.length===0?s+='<div class="match-dropdown-item" style="color:var(--muted);text-align:center;">Ingen matcher ennå</div>':$.forEach(a=>{const n=a.id!=null&&a.id.toString()===String(R),r=a.id?"Match ID "+a.id+" "+a.name:a.name||"Match";s+='<div class="match-dropdown-item '+(n?"active":"")+`" onclick="selectMatchFromDropdown('`+a.id+"', '"+e+`')">`,s+='<div class="match-dropdown-dot"></div>',s+='<div class="match-dropdown-info">',s+='<div class="match-dropdown-name">'+r+"</div>",s+='<div class="match-dropdown-meta">'+We(a.date)+" · "+a.location+"</div>",s+="</div>",s+="</div>"}),t.innerHTML=s,t.classList.add("open")}function cs(e,i){pt(e);const t=i+"-match-dropdown",s=o(t);s&&s.classList.remove("open")}function ps(){const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e){alert("Ingen match valgt");return}o("edit-match-name").value=e.name||"",o("edit-match-type").value=e.type||d("match_type_practice"),o("edit-match-date").value=e.date||"",o("edit-match-location").value=e.location||"",o("edit-match-stages").value=e.plannedStages||0,o("edit-match-searchable")&&(o("edit-match-searchable").checked=e.searchable!==!1),o("edit-match-finished")&&(o("edit-match-finished").checked=e.status==="finished");const i=o("edit-match-rival");i.innerHTML='<option value="">Ingen rival valgt</option>';const t=document.createElement("option");t.value="me",t.textContent="Meg ("+(g.firstName||"")+" "+(g.lastName||"")+")",i.appendChild(t),e.shooters&&e.shooters.length>0&&e.shooters.forEach(n=>{const r=document.createElement("option");r.value=n.id,r.textContent=n.firstName+" "+n.lastName,i.appendChild(r)}),e.rivalId&&(i.value=e.rivalId),o("edit-match-user-search").value="",o("edit-match-search-results").innerHTML="",me=[],initReferenceEditState(e),o("edit-match-reference-enabled")&&(o("edit-match-reference-enabled").checked=!!e.referenceShootersEnabled),renderReferenceShooterConfig();const s=Pe(),a=o("delete-match-btn");a&&s&&e.ownerId===s.uid?a.style.display="block":a&&(a.style.display="none"),icRenderEditMatchShootersList(e),ie("modal-edit-match")}async function us(){var s,a;const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e){alert("Ingen match valgt");return}refEditState.enabled=!!((o("edit-match-reference-enabled")||{}).checked);const i={name:F("edit-match-name")||e.name,type:F("edit-match-type")||e.type,date:F("edit-match-date")||e.date,location:F("edit-match-location")||e.location,plannedStages:A("edit-match-stages",e.plannedStages),searchable:((s=o("edit-match-searchable"))==null?void 0:s.checked)!==!1,status:(a=o("edit-match-finished"))!=null&&a.checked?"finished":"active",rivalId:F("edit-match-rival")||null,referenceShootersEnabled:!!refEditState.enabled,referenceShooterIds:(refEditState.ids||[]).slice(),referenceOverrides:JSON.parse(JSON.stringify(refEditState.overrides||{}))};showSpinner(d("saving_match"));Ee(e.id,i).then(function(t){hideSpinner();if(t.success){let n=0;Promise.all(me.map(function(r){return Ge(r.email,{matchId:e.id,matchName:i.name})})).then(function(results){results.forEach(function(r){if(r.success)n++});G("modal-edit-match");if(n>0)alert("Match oppdatert! Invitasjoner sendt til "+n+" bruker(e).")})}else alert("Kunne ikke oppdatere match: "+t.error)})}function gs(){const e=$.find(t=>t.id!=null&&t.id.toString()===String(R));if(!e){alert("Ingen match valgt");return}const i=e.id?"Match ID "+e.id+" "+e.name:e.name;o("delete-match-name").textContent=i,ie("modal-confirm-delete")}async function vs(){const e=$.find(t=>t.id!=null&&t.id.toString()===String(R));if(!e){alert("Ingen match valgt");return}showSpinner("Sletter match…");Gt(e.id).then(function(i){hideSpinner();if(i.success){G("modal-confirm-delete");G("modal-edit-match");R=null;te();fe();alert("Match slettet")}else alert("Kunne ikke slette match: "+i.error)})}let Ce=null,Me=null;function Re(){var i;const e=$.find(t=>t.id!=null&&t.id.toString()===String(R));if(!e){alert("Ingen match valgt");return}Ce=null,o("stage-modal-title").textContent=d("create_stage"),o("stage-number").value=(((i=e.stages)==null?void 0:i.length)||0)+1,o("stage-name").value="",o("stage-paper-targets").value=0,o("stage-poppers").value=0,o("stage-plates").value=0,o("stage-no-shoots").value=0,o("stage-bonus-paper-targets").value=0,ie("modal-create-stage")}function ms(){G("modal-edit-match"),Re()}function hs(){G("modal-new-match"),Re()}function fs(e){const i=$.find(s=>s.id!=null&&s.id.toString()===String(R));if(!i||!i.stages||!i.stages[e]){alert(d("stage_not_found"));return}Ce=e;const t=i.stages[e];o("stage-modal-title").textContent=d("edit_stage"),o("stage-number").value=t.number||e+1,o("stage-name").value=t.name||"",o("stage-paper-targets").value=t.paperTargets||0,o("stage-poppers").value=t.poppers||0,o("stage-plates").value=t.plates||0,o("stage-no-shoots").value=t.noShoots||0,o("stage-bonus-paper-targets").value=t.bonusPaperTargets||0,ie("modal-create-stage")}function bs(e){const i=o("stage-number"),t=Math.max(1,parseInt(i.value)+e);i.value=t}function ys(e,i){const t=o("stage-"+e),s=Math.max(0,parseInt(t.value)+i);t.value=s}async function ws(){var a;const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e){alert("Ingen match valgt");return}const i={number:A("stage-number",1),name:F("stage-name")||"",paperTargets:A("stage-paper-targets",0),poppers:A("stage-poppers",0),plates:A("stage-plates",0),noShoots:A("stage-no-shoots",0),bonusPaperTargets:A("stage-bonus-paper-targets",0),bonusIncluded:true},t=e.stages||[];Ce!==null?t[Ce]=i:t.push(i);showSpinner("Lagrer stage…");Ee(e.id,{stages:t}).then(function(s){hideSpinner();if(s.success)G("modal-create-stage");else alert("Kunne ikke lagre stage: "+s.error)})}function icOpenEditStageResult(stageNum,shooterId){
  ...
  const aEl=document.getElementById("new-result-a-val");if(aEl)aEl.value=res.a||0;
  const cEl=document.getElementById("new-result-c-val");if(cEl)cEl.value=res.c||0;
  const dEl=document.getElementById("new-result-d-val");if(dEl)dEl.value=res.d||0;
  const missEl=document.getElementById("new-result-miss-val");if(missEl)missEl.value=res.miss||0;
...
(function(){
  ...
  s+='<span>"+d("match_progression")+"</span><span class="mu-arrow">\u25b6</span></button>';
  ...
  s+='<button onclick="var w=this.closest(\'.mu-wrap\');w.querySelector(\'.mu-cum\').style.display=\'block\';w.querySelector(\'.mu-stg\').style.display=\'none\';w.querySelectorAll(\'.mu-tab\').forEach(function(b){b.style.background=\'var(--bg3)\';b.style.color=\'var(--muted)\';});this.style.background=\'var(--accent)\';this.style.color=\'#000\';" class="mu-tab" style="flex:1;padding:7px 0;font-size:12px;font-weight:600;border-radius:8px;border:none;cursor:pointer;background:var(--accent);color:#000;">"+d("match_progression")+"</button>';
  s+='<button onclick="var w=this.closest(\'.mu-wrap\');w.querySelector(\'.mu-cum\').style.display=\'none\';w.querySelector(\'.mu-stg\').style.display=\'block\';w.querySelectorAll(\'.mu-tab\').forEach(function(b){b.style.background=\'var(--bg3)\';b.style.color=\'var(--muted)\';});this.style.background=\'var(--accent)\';this.style.color=\'#000\';" class="mu-tab" style="flex:1;padding:7px 0;font-size:12px;font-weight:600;border-radius:8px;border:none;cursor:pointer;background:var(--bg3);color:var(--muted);">"+d("stage_wins")+"</button>';
  s+='</div>';
...
html+="</div>";return html;}function icSeasonAnalysis(matches){
  ...
  if(tips.length){
    html+='<div style="margin-bottom:12px;">';
    html+='<div style="font-size:11px;color:var(--muted);font-weight:700;margin-bottom:6px;">"+d("training_tips")+"</div>';
...
function icTrainingSeasonAnalysis(matches){
  var tMatches=matches.filter(function(m){return(m.type||"")===d("match_type_practice");});
  ...
  var matchStats=tMatches.map(function(m){
    ...
    return{name:m.name||d("match_type_practice"),date:m.date,avgHF:sc>0?tHF/sc:0,ap:tHits>0?Math.round(tA/tHits*100):0,missRate:sc>0?tMiss/sc:0,stageCount:sc};
  }).filter(Boolean);
...
function renderProfileMatchTips(){const container=o("profile-match-tips");if(!container||!$.length){if(container)container.innerHTML="";return;}const myMatches=$.filter(function(m){const s=icCurrentShooter(m);return s&&s.stages&&s.stages.length>0;});if(!myMatches.length){container.innerHTML="";return;}let html='<div class="card" style="margin-bottom:16px;"><div class="card-header"><div class="card-title">"+d("matches_count")+" &amp; tips</div></div>'+icSeasonAnalysis(myMatches)+'<div style="height:1px;background:rgba(255,255,255,0.06);margin:4px 0;"></div>'+icTrainingSeasonAnalysis(myMatches);if(myMatches.length){html+='<div style="height:1px;background:var(--border);margin:8px 0;"></div>';}myMatches.forEach(function(match,mi){const myShooter=icCurrentShooter(match);const sc=myShooter.stages.length;let tHF=0,tA=0,tHits=0;myShooter.stages.forEach(function(s){tHF+=s.hf||0;tA+=s.a||0;tHits+=(s.a||0)+(s.c||0)+(s.d||0)+(s.miss||0);});const avgHF=(tHF/sc).toFixed(2);const apStr=tHits>0?Math.round(tA/tHits*100)+"%":"--";const mid="mtips-"+mi;html+='<div style="margin-bottom:10px;padding:12px;background:var(--bg);border-radius:8px;">';html+='<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">';html+='<div><div style="font-weight:600;font-size:14px;">'+( match.name||"Match")+"</div>";html+='<div style="font-size:12px;color:var(--muted);">'+We(match.date)+' &middot; <span style="color:#e8b84b;font-weight:600;">'+( match.type||"")+'</span> &middot; '+sc+" stages &middot; HF "+avgHF+" &middot; "+apStr+"</div></div>";html+='<button onclick="toggleTips(&quot;mtips-'+mi+'&quot;)" style="flex-shrink:0;padding:6px 10px;background:transparent;border:1px solid rgba(255,255,255,.12);border-radius:8px;color:var(--muted);font-size:11px;cursor:pointer;">Vis tips</button>';html+="</div>";html+='<div id="'+mid+'" style="display:none;margin-top:10px;">'+icMatchTips(match)+"</div>";html+="</div>";});html+="</div>";container.innerHTML=html;}function Ns(){o("edit-firstname").value=g.firstName||"",o("edit-lastname").value=g.lastName||"",o("edit-club").value=g.club||"",o("edit-draw").value=g.draw||"",o("edit-reload").value=g.reloadTime||"";let e="";Yi.forEach(s=>{e+='<option value="'+s+'"'+(s===g.division?" selected":"")+">"+s+"</option>"}),o("edit-division").innerHTML=e;let i="";Qi.forEach(s=>{i+='<option value="'+s+'"'+(s===g.category?" selected":"")+">"+s+"</option>"}),o("edit-category").innerHTML=i;let t="";Xi.forEach(s=>{t+='<option value="'+s+'"'+(s===g.region?" selected":"")+">"+s+"</option>"}),o("edit-region").innerHTML=t,mt(),ie("modal-edit-profile")}function mt(){const e=F("edit-division"),i=Zi[e]||["minor","major"];let t="";i.forEach(s=>{const a=g.powerFactor===s;t+='<label class="pf-option'+(a?" active":"")+`" onclick="selectPF(this,'`+s+`')">`,t+='<input type="radio" name="pf" value="'+s+'">',t+='<div class="pf-label">'+s.toUpperCase()+"</div>",t+='<div class="pf-sub">'+(s==="major"?"170 PF":"<170 PF")+"</div>",t+="</label>"}),o("pf-options").innerHTML=t,i.indexOf(g.powerFactor)<0&&(g.powerFactor=i[0])}function $s(e,i){document.querySelectorAll(".pf-option").forEach(t=>t.classList.remove("active")),e.classList.add("active"),g.powerFactor=i}async function Fs(){g.firstName=F("edit-firstname").trim()||"",g.lastName=F("edit-lastname").trim()||"",g.division=F("edit-division")||"",g.category=F("edit-category")||"",g.region=F("edit-region")||"",g.club=F("edit-club").trim()||"",g.draw=he("edit-draw")||null,g.reloadTime=he("edit-reload")||null;const e=await Ot(g),i=o("save-profile-btn");e.success?(i.textContent=" "+d("saved"),i.style.background="var(--green)",setTimeout(()=>{i.textContent=d("save_profile"),i.style.background=""},1800)):(i.textContent=" "+(dt==="no"?"Feil!":"Error!"),i.style.background="var(--red)",setTimeout(()=>{i.textContent=d("save_profile"),i.style.background=""},1800)),gt(),De(),te(),G("modal-edit-profile")}function As(e,i,t,s){if(!e||!i||!s)return null;const a=icCurrentShooter(e);if(!a)return null;const n=icMatchTotals(e),r=n.find(v=>String(v.id)===String(a.id));if(!r)return null;let u=null;if(e.rivalId&&e.rivalId!=="me")u=(e.shooters||[]).find(v=>v&&String(v.id)===String(e.rivalId));if(!u){const v=n.filter(S=>String(S.id)!==String(a.id)).sort((S,x)=>x.totalStagePts-S.totalStagePts);v.length&&(u=(e.shooters||[]).find(S=>String(S.id)===String(v[0].id))||null)}if(!u)return null;const m=n.find(v=>String(v.id)===String(u.id));if(!m)return null;const b=((u.firstName||"")+" "+(u.lastName||"")).trim()||"Rival",f=(r.totalStagePts||0)-(m.totalStagePts||0),p=f>=0,h=Math.abs(f),E=icProjectNext(i,s);if(!E)return{statusText:(p?"LEDER ":"BAK ")+h.toFixed(1)+" stg pts",advice:p?`Du leder over ${b}. Hold prosessen stabil og unngå unødvendige tap.`:`Du ligger bak ${b}. Neste stage blir viktig.`,rivalName:b,gap:f,targetHF:null,estHF:null,deltaHF:null};const _curStageNum=Math.max(...((e.stages||[]).map(function(_s){return _s.number||_s.num||0;}).concat(0)));const k=((u.stages||[]).filter(function(v){return(v.num||v.number||0)<_curStageNum;}).slice().sort((v,S)=>(v.num||0)-(S.num||0))),P=icFormFromResults(k,null,u),C=P?icProjectNext(P,s):null,L=Math.max(E.estHF||0,(C&&C.estHF)||0,.01),M=L>0?(E.estHF/L*E.maxPts):0,D=L>0?(((C&&C.estHF)||0)/L*E.maxPts):0;let B=null,_=null,K=null,j="",w="";if(p){B=Math.max(0,D-h);_=L>0?(B/E.maxPts*L):null;K=_!=null?_-E.estHF:null;j=`LEDER ${h.toFixed(1)} stg pts`;{const _rnd=Math.floor(Math.random()*3);if(B<=0){const _msgs=dt==="no"?[`Du leder over ${b}. Hold flyten på ${E.estHF.toFixed(2)} HF og ledelsen er din.`,`Du leder over ${b}. ${E.estHF.toFixed(2)} HF holder — kjør din stage.`,`Ledelsen er trygg. Kjør din stage — ${E.estHF.toFixed(2)} HF er nok.`,`Du styrer løpet. Prosess før resultat — ${E.estHF.toFixed(2)} HF holder.`,`${b} er bak deg. Hold ${E.estHF.toFixed(2)} HF og avslutt sterkt.`,`Godt posisjonert. ${E.estHF.toFixed(2)} HF er alt du trenger neste stage.`,`Du har kontroll. Én stage til på ${E.estHF.toFixed(2)} HF og ledelsen holder.`]:[`You lead over ${b}. Maintain ${E.estHF.toFixed(2)} HF and the lead is yours.`,`You lead over ${b}. ${E.estHF.toFixed(2)} HF holds — shoot your stage.`,`The lead is secure. Shoot your stage — ${E.estHF.toFixed(2)} HF is enough.`,`You control the match. Process over result — ${E.estHF.toFixed(2)} HF holds.`,`${b} is behind you. Hold ${E.estHF.toFixed(2)} HF and finish strong.`,`Well positioned. ${E.estHF.toFixed(2)} HF is all you need next stage.`,`You have control. One more stage at ${E.estHF.toFixed(2)} HF and the lead holds.`];w=_msgs[_rnd];}else if(K!=null&&K<=0){const _msgs=dt==="no"?[`Du leder over ${b}. Sikter du mot ${_.toFixed(2)} HF holder du ledelsen.`,`Ledelsen er der. ${_.toFixed(2)} HF neste stage og du beholder plassen.`,`Du er foran ${b}. Hold nivået på ${_.toFixed(2)} HF — det er nok.`,`${_.toFixed(2)} HF neste stage og rivalen er bak deg hele veien.`,`God posisjon. ${_.toFixed(2)} HF fra deg og ledelsen er sikret.`,`Du har et forsprang. ${_.toFixed(2)} HF holder — ikke overtenk det.`,`Ledelsen er din å miste. ${_.toFixed(2)} HF neste stage holder rivalen unna.`]:[`You lead over ${b}. Hit ${_.toFixed(2)} HF and you keep the lead.`,`The lead is there. ${_.toFixed(2)} HF next stage and you keep your position.`,`You are ahead of ${b}. Hold ${_.toFixed(2)} HF — that is enough.`,`${_.toFixed(2)} HF next stage and the rival stays behind you.`,`Good position. ${_.toFixed(2)} HF from you and the lead is secured.`,`You have a lead. ${_.toFixed(2)} HF holds — don't overthink it.`,`The lead is yours to lose. ${_.toFixed(2)} HF next stage keeps the rival back.`];w=_msgs[_rnd];}else if(K!=null&&K<=.35){const _msgs=dt==="no"?[`Du leder, men ${b} kan ta det igjen. Sikter du mot ${_.toFixed(2)} HF holder du rivalen bak deg.`,`Tett i toppen. ${_.toFixed(2)} HF neste stage beskytter ledelsen din.`,`Ledelsen kan ryke. Hold fokus — ${_.toFixed(2)} HF er det du trenger.`,`${b} er på jakt. ${_.toFixed(2)} HF holder rivalen bak deg — kjør rent.`,`Marginene er tette. En ren stage på ${_.toFixed(2)} HF og ledelsen er trygg.`,`Du leder fortsatt — men det krever ${_.toFixed(2)} HF å holde rivalen unna.`,`Hold hodet kaldt. ${_.toFixed(2)} HF neste stage er nok til å beholde plassen.`]:[`You lead, but ${b} can catch up. Hit ${_.toFixed(2)} HF and you keep the rival behind you.`,`Tight at the top. ${_.toFixed(2)} HF next stage protects your lead.`,`The lead could slip. Stay focused — ${_.toFixed(2)} HF is what you need.`,`${b} is hunting. ${_.toFixed(2)} HF keeps the rival behind you — shoot clean.`,`Margins are tight. A clean stage at ${_.toFixed(2)} HF and the lead is safe.`,`You still lead — but it takes ${_.toFixed(2)} HF to keep the rival back.`,`Keep your head cool. ${_.toFixed(2)} HF next stage is enough to hold your position.`];w=_msgs[_rnd];}else{const _msgs=dt==="no"?[`Du leder over ${b}, men denne stagen kan snu alt. ${_.toFixed(2)} HF holder ledelsen — hold prosessen stabil.`,`Tett kamp. ${_.toFixed(2)} HF sikrer ledelsen — unngå miss og procedurals.`,`Rivalen er rett bak. En ren stage på ${_.toFixed(2)} HF er alt som trengs.`,`${b} kan passere deg denne stagen. ${_.toFixed(2)} HF er svaret — én stage av gangen.`,`Ledelsen henger i en tråd. Fokus på prosessen — ${_.toFixed(2)} HF holder deg foran.`,`Det er tett. Ikke tenk på resultatet — kjør ${_.toFixed(2)} HF og la det skje.`,`Du er under press. Bryt det ned: én ren stage på ${_.toFixed(2)} HF er nok.`]:[`You lead over ${b}, but this stage can change everything. ${_.toFixed(2)} HF holds the lead — keep your process stable.`,`Tight race. ${_.toFixed(2)} HF secures the lead — avoid misses and procedurals.`,`The rival is right behind. A clean stage at ${_.toFixed(2)} HF is all it takes.`,`${b} can pass you this stage. ${_.toFixed(2)} HF is the answer — one stage at a time.`,`The lead is hanging by a thread. Focus on process — ${_.toFixed(2)} HF keeps you ahead.`,`It's tight. Don't think about the result — shoot ${_.toFixed(2)} HF and let it happen.`,`You are under pressure. Break it down: one clean stage at ${_.toFixed(2)} HF is enough.`];w=_msgs[_rnd];}}}else{B=D+h+.01;_=L>0?(B/E.maxPts*L):null;K=_!=null?_-E.estHF:null;j=`BAK ${h.toFixed(1)} stg pts`;{const _rnd=Math.floor(Math.random()*3);if(_==null){const _msgs=dt==="no"?[`Du ligger bak ${b}. Kjør en ren og effektiv stage.`,`${b} er foran. Hold fokus og ta det du kan.`,`Fokus på prosessen nå — en god stage er det viktigste.`,`Du ligger bak. Ta én stage av gangen og kjør ditt beste.`,`${b} leder — men matchen er ikke over. Kjør din stage fullt ut.`,`Ingen grunn til å spekulere. Kjør din beste stage og se hva som skjer.`,`Bak i stillingen — men det avgjøres på banen, ikke i hodet. Kjør rent.`]:[`You are behind ${b}. Shoot a clean and efficient stage.`,`${b} is ahead. Stay focused and take what you can.`,`Focus on the process now — a good stage is what matters most.`,`You are behind. Take one stage at a time and shoot your best.`,`${b} leads — but the match isn't over. Shoot your stage fully.`,`No reason to speculate. Shoot your best stage and see what happens.`,`Behind in the standings — but it's decided on the range, not in your head. Shoot clean.`];w=_msgs[_rnd];}else if(K<=.2){const _msgs=dt==="no"?[`Du er rett bak ${b}. ${_.toFixed(2)} HF neste stage — litt over ditt snitt på ${E.estHF.toFixed(2)} HF — er nok til å snu løpet.`,`Lite skiller deg fra ${b}. ${_.toFixed(2)} HF holder — det er bare ${K.toFixed(2)} HF over det du forventer å levere.`,`${b} er innen rekkevidde. Én god stage på ${_.toFixed(2)} HF kan være nok.`,`Gapet er lite. ${_.toFixed(2)} HF neste stage og du er forbi rivalen.`,`Du er tett på. ${_.toFixed(2)} HF — marginalt over ditt snitt — snur løpet.`,`${b} er nesten innen rekkevidde. Press litt ekstra: ${_.toFixed(2)} HF gjør jobben.`,`Kort avstand til toppen. En skarp stage på ${_.toFixed(2)} HF og du leder.`]:[`You are right behind ${b}. ${_.toFixed(2)} HF next stage — just above your average of ${E.estHF.toFixed(2)} HF — is enough to turn it around.`,`Not much separates you from ${b}. ${_.toFixed(2)} HF holds — it's only ${K.toFixed(2)} HF above what you expect to deliver.`,`${b} is within reach. One good stage at ${_.toFixed(2)} HF may be enough.`,`The gap is small. ${_.toFixed(2)} HF next stage and you pass the rival.`,`You are close. ${_.toFixed(2)} HF — marginally above your average — turns the match around.`,`${b} is almost within reach. Push a little harder: ${_.toFixed(2)} HF gets the job done.`,`Short distance to the top. A sharp stage at ${_.toFixed(2)} HF and you lead.`];w=_msgs[_rnd];}else if(K<=.75){const y=E.expTime&&_>0?Math.max(0,E.expTime-E.expPts/_):0,H=E.expTime?Math.max(0,_*E.expTime-E.expPts):0;const _msgs=dt==="no"?[`Du må opp til ${_.toFixed(2)} HF for å ta ${b} — det er ${K.toFixed(2)} HF mer enn forventet. Spar tid i bevegelsesfasene og hold A-raten.`,`${b} er foran. ${_.toFixed(2)} HF neste stage tar deg forbi — tenk ${y.toFixed(2)}s raskere totalt eller ${H.toFixed(1)} ekstra råpoeng.`,`Gapet til ${b} krever ${_.toFixed(2)} HF. Stram inn tempoet og unngå straffetrekk.`,`${_.toFixed(2)} HF er hva som skal til — over snitt, men innen rekkevidde med en skarp stage.`,`Du trenger ${_.toFixed(2)} HF for å passere rivalen. Reduser bevegelsestid og hold treffbildet rent.`,`${K.toFixed(2)} HF over ditt snitt — det krever fokus, men er fullt mulig med en god stage.`,`${b} kan tas. ${_.toFixed(2)} HF neste stage gjør jobben — kjør aggressivt og rent.`]:[`You need ${_.toFixed(2)} HF to catch ${b} — that's ${K.toFixed(2)} HF above expected. Save time in transitions and hold your A-zone rate.`,`${b} is ahead. ${_.toFixed(2)} HF next stage takes you past — think ${y.toFixed(2)}s faster total or ${H.toFixed(1)} extra raw points.`,`The gap to ${b} requires ${_.toFixed(2)} HF. Tighten your tempo and avoid penalties.`,`${_.toFixed(2)} HF is what it takes — above average, but within reach with a sharp stage.`,`You need ${_.toFixed(2)} HF to pass the rival. Reduce transition time and keep your hit factor clean.`,`${K.toFixed(2)} HF above your average — it takes focus, but is fully achievable with a good stage.`,`${b} can be caught. ${_.toFixed(2)} HF next stage gets the job done — shoot aggressively and clean.`];w=_msgs[_rnd];}else{const y=E.expTime&&_>0?Math.max(0,E.expTime-E.expPts/_):0,H=E.expTime?Math.max(0,_*E.expTime-E.expPts):0;const _msgs=dt==="no"?[`Det er et stykke opp til ${b}. Du trenger ${_.toFixed(2)} HF — ${K.toFixed(2)} HF mer enn snitt. Fokuser på én ting: tempo.`,`${b} er godt foran. ${_.toFixed(2)} HF er målet — aggressivt, men mulig med en ren stage.`,`Stor jobb neste stage. ${_.toFixed(2)} HF krever fullt trykk — tenk ${y.toFixed(2)}s raskere totalt og ${H.toFixed(1)} ekstra råpoeng.`,`Gapet er stort, men ikke umulig. ${_.toFixed(2)} HF krever at alt klaffer — kjør for det.`,`Du trenger en toppstage. ${_.toFixed(2)} HF mot ${b} — fokus på tempo og treffbilde fra første skudd.`,`${b} leder komfortabelt. Det tar ${_.toFixed(2)} HF å hente rivalen — la alt annet gå og kjør fullt ut.`,`Ingen enkle svar her. ${_.toFixed(2)} HF er hva som kreves — del stagen opp mentalt og kjør den del for del.`]:[`There's a gap to ${b}. You need ${_.toFixed(2)} HF — ${K.toFixed(2)} HF above average. Focus on one thing: tempo.`,`${b} is well ahead. ${_.toFixed(2)} HF is the target — aggressive, but achievable with a clean stage.`,`Big task next stage. ${_.toFixed(2)} HF requires full pressure — think ${y.toFixed(2)}s faster total and ${H.toFixed(1)} extra raw points.`,`The gap is large, but not impossible. ${_.toFixed(2)} HF requires everything to click — go for it.`,`You need a top stage. ${_.toFixed(2)} HF against ${b} — focus on tempo and hit factor from the first shot.`,`${b} leads comfortably. It takes ${_.toFixed(2)} HF to catch the rival — let everything else go and shoot flat out.`,`No easy answers here. ${_.toFixed(2)} HF is what's required — break the stage down mentally and shoot it section by section.`];w=_msgs[_rnd];}}}return{statusText:j,advice:w,rivalName:b,gap:f,targetHF:_,estHF:E.estHF,deltaHF:K,rivalEstHF:(C&&C.estHF)||null,yourEstStagePts:M,rivalEstStagePts:D,requiredStagePts:B,maxStagePts:E.maxPts}}function De(){const e=ct(),i=o("prog-a-rate");i&&(i.textContent=e&&e.aPercent!==void 0?Math.round(e.aPercent*100)+"%":"—");const t=o("prog-data-status");t&&(t.style.display=!e||e.completedStages===0?"block":"none"),Rs()}function Rs(){const e=$.find(a=>a.id!=null&&a.id.toString()===String(R)),i=o("prog-stages-container");if(!i||!e){i&&(i.innerHTML="");return}const t=icStageDefs(e);if(!t.length){i.innerHTML="";return}const s=icCurrentShooter(e),a=icCurrentResults(e),n=icCurrentShooterId(),rActive=getActiveReferenceShooters(e),commonNums=icCommonStageNumbers(e),commonResults=a.filter(u=>commonNums.includes(String(u.num||u.number))),nextStage=t.find(u=>!a.some(m=>String(m.num||m.number)===String(u.number))),overallForm=icFormFromResults(commonResults.length?commonResults:a,null,s),overallProjection=nextStage&&overallForm?icProjectNext(overallForm,nextStage):null;let r="";const renderedStageNums=new Set();t.forEach(u=>{const m=icStageMetricsForMatch(e,u).map(E=>({...E,isMe:String(E.id)===String(n)||E.isMe})),b=m.length>0;let h=null;r+='<div class="card" style="margin-top:15px;border-left:4px solid '+(b?"var(--green)":"var(--muted)")+';">',r+='<div class="card-header">',r+='<div class="card-title">Stage '+u.number+". "+(u.name||("Stage "+u.number))+' <span style="color:#e8b84b;font-weight:600;font-size:13px;">'+(icStageMaxPts(u)<=60?"Short":icStageMaxPts(u)<=120?"Medium":"Long")+"</span></div>",r+='<span class="badge '+(b?"badge-green":"badge-gray")+'">'+(b?" "+d("shot"):" "+d("not_shot"))+"</span>",r+="</div>";if(b){r+='<div style="overflow-x:auto;margin-top:12px;">',r+='<table style="width:100%;font-size:12px;border-collapse:collapse;">',r+='<tr style="border-bottom:1px solid var(--border);color:var(--muted);">',r+='<th style="padding:4px 3px;text-align:left;">#</th>',r+='<th style="padding:4px 3px;text-align:left;">"+d("name_col")+"</th>',r+='<th style="padding:4px 3px;text-align:right;">PTS</th>',r+='<th style="padding:4px 3px;text-align:right;">STG</th>',r+='<th style="padding:4px 3px;text-align:right;">%</th>',r+='<th style="padding:4px 3px;text-align:right;">HF</th>',r+='<th style="padding:4px 3px;text-align:right;">A%</th>',r+="</tr>",m.forEach(E=>{const k=E.isMe?"background:var(--accent-fade);font-weight:600;":"",detailId="stage-detail-"+u.number+"-"+E.id,res=E.res||{};r+='<tr style="'+k+'border-bottom:1px solid var(--border);cursor:pointer;" onclick="const d=document.getElementById(\''+detailId+'\');d&&(d.style.display=d.style.display===\'none\'?\'table-row\':\'none\')">',r+='<td style="padding:4px 3px;">'+E.rank+"</td>",r+='<td style="padding:4px 3px;">'+E.name+"</td>",r+='<td style="padding:8px 6px;text-align:right;color:var(--accent);">'+E.pts.toFixed(1)+"</td>",r+='<td style="padding:4px 3px;text-align:right;">'+E.stagePts.toFixed(2)+"</td>",r+='<td style="padding:4px 3px;text-align:right;">'+E.stagePct.toFixed(2)+"%</td>",r+='<td style="padding:4px 3px;text-align:right;">'+(E.hf||0).toFixed(2)+"</td>",r+='<td style="padding:8px 6px;text-align:right;color:var(--green);">'+E.stageAPercent.toFixed(0)+"%</td>",r+="</tr>",r+='<tr id="'+detailId+'" style="display:none;background:rgba(255,255,255,0.03);">',r+='<td colspan="7" style="padding:10px 8px;">',r+='<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;font-size:11px;">',r+='<div><div style="color:var(--muted);">'+(dt==="no"?"Tid":"Time")+'</div><div style="font-weight:600;">'+(res.time||0).toFixed(2)+"s</div></div>",r+='<div><div style="color:var(--muted);">A</div><div style="font-weight:600;color:var(--green);">'+(res.a||0)+"</div></div>",r+='<div><div style="color:var(--muted);">C</div><div style="font-weight:600;">'+(res.c||0)+"</div></div>",r+='<div><div style="color:var(--muted);">D</div><div style="font-weight:600;">'+(res.d||0)+"</div></div>",r+='<div><div style="color:var(--muted);">Miss</div><div style="font-weight:600;color:var(--red);">'+(res.miss||0)+"</div></div>",r+='<div><div style="color:var(--muted);">NS</div><div style="font-weight:600;color:var(--red);">'+(res.ns||0)+"</div></div>",r+='<div><div style="color:var(--muted);">Proc</div><div style="font-weight:600;color:var(--red);">'+(res.proc||0)+"</div></div>",r+="</div>",r+='<div style="margin-top:8px;text-align:right;">'+'<button data-stage="'+u.number+'" data-shooter="'+E.id+'" onclick="icOpenEditStageResult(this.dataset.stage,this.dataset.shooter)" style="padding:5px 12px;font-size:11px;font-weight:600;background:var(--bg3);border:1px solid rgba(255,255,255,0.2);border-radius:6px;color:var(--text);cursor:pointer;">✎ Rediger</button>'+'</div>',r+="</td>",r+="</tr>"}),r+="</table>",r+="</div>";const f=a.filter(E=>(E.num||E.number)<=u.number).sort((E,k)=>(E.num||0)-(k.num||0)),p=m.find(E=>E.isMe);h=p?p.res:null;if((e.type||"")===d("match_type_practice")){const myShooterT=icCurrentShooter(e);const myStageRes=myShooterT&&myShooterT.stages?myShooterT.stages.find(function(sr){return(sr.num||sr.number)==u.number}):null;if(myStageRes&&myStageRes.runs&&myStageRes.runs.length>0){r+='<div style="margin-top:15px;padding:12px;background:var(--bg);border-radius:8px;">';r+='<div style="font-size:11px;color:var(--muted);margin-bottom:10px;">"+d("my_runs")+"</div>';r+='<table style="width:100%;font-size:12px;border-collapse:collapse;">';r+='<tr style="border-bottom:1px solid var(--border);color:var(--muted);"><th style="padding:6px 4px;text-align:left;">Run</th><th style="padding:6px 4px;text-align:right;">Tid</th><th style="padding:6px 4px;text-align:right;">HF</th><th style="padding:6px 4px;text-align:right;">A</th><th style="padding:6px 4px;text-align:right;">C</th><th style="padding:6px 4px;text-align:right;">D</th><th style="padding:6px 4px;text-align:right;">Miss</th><th style="padding:6px 4px;"></th></tr>';const bestHF=myStageRes.runs.reduce(function(bst,rn){return Math.max(bst,rn.hf||0)},0);myStageRes.runs.forEach(function(rn,ri){const isBest=(rn.hf||0)===bestHF;r+='<tr style="border-bottom:1px solid var(--border);background:'+(isBest?"rgba(232,184,75,0.08)":"transparent")+'">';r+='<td style="padding:6px 4px;font-weight:'+(isBest?"700":"400")+';">'+"Run "+(rn.runNum||ri+1)+(isBest?" ★":"")+"</td>";r+='<td style="padding:6px 4px;text-align:right;">'+(rn.time||0).toFixed(2)+"s</td>";r+='<td style="padding:6px 4px;text-align:right;color:var(--accent);">'+(rn.hf||0).toFixed(2)+"</td>";r+='<td style="padding:6px 4px;text-align:right;color:var(--green);">'+(rn.a||0)+"</td>";r+='<td style="padding:6px 4px;text-align:right;">'+(rn.c||0)+"</td>";r+='<td style="padding:6px 4px;text-align:right;">'+(rn.d||0)+"</td>";r+='<td style="padding:6px 4px;text-align:right;color:var(--red);">'+(rn.miss||0)+"</td>";r+='<td style="padding:6px 4px;text-align:right;"><button onclick="deleteRun('+u.number+','+ri+')" style="padding:2px 6px;background:#ef4444;color:white;border:none;border-radius:4px;font-size:10px;cursor:pointer;">✕</button></td>';r+="</tr>";});r+='</table></div>';const analId='train-anal-'+u.number;r+='<button onclick="var x=document.getElementById(\''+analId+'\');x&&(x.style.display=x.style.display===\'none\'?\'block\':\'none\')" style="margin-top:10px;width:100%;padding:8px;background:transparent;border:1px solid rgba(255,255,255,.12);border-radius:8px;color:var(--muted);font-size:12px;cursor:pointer;text-align:left;">"+d("show_train_analysis")+"</button>';r+='<div id="'+analId+'" style="display:none;margin-top:8px;">';r+=icTrainingAnalysis(myStageRes.runs,u);r+='</div>';}}renderedStageNums.add(String(u.number));const _eUpTo=Object.assign({},e,{shooters:(e.shooters||[]).map(function(sh){return Object.assign({},sh,{stages:(sh.stages||[]).filter(function(sr){return renderedStageNums.has(String(sr.num||sr.number));})});})});const allShootersTotals=icMatchTotals(_eUpTo,!1);if(allShootersTotals.length>0){r+='<div style="margin-top:12px;padding:10px;background:var(--bg);border-radius:8px;">',r+='<div style="font-size:11px;color:var(--muted);margin-bottom:8px;">KUMULATIV A% I MATCHEN SÅ LANGT</div>',r+='<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:6px;">';allShootersTotals.forEach(S=>{const totalAP=S.totalHits>0?(S.totalA/S.totalHits*100).toFixed(0)+"%":"—";r+='<div style="font-size:12px;"><div style="color:var(--muted);font-size:11px;">'+S.name+"</div><div style='font-weight:700;color:var(--green);'>"+totalAP+"</div></div>"});r+="</div>",r+="</div>"}const E=icFormFromResults(f,u.number,s);if(E&&(e.type||"")===d("match_type_practice")){const kT=t.find(P=>P.number>u.number);const refCompareT=h?{estHF:h.hf!=null?h.hf:null,expTime:h.time!=null?h.time:null}:null;rActive.length&&(r+=renderReferenceBenchmarkBlock(e,u,refCompareT));}else if(E){const k=(t.find(function(P){return P.number>u.number;})||t.slice().sort(function(a,b){return a.number-b.number;})[0]||null);const P=k?icProjectNext(E,k):null;var estForThisStage=null;{const _pR=a.filter(E=>(E.num||E.number)<u.number);const _pF=_pR.length?icFormFromResults(_pR,null,s):null;estForThisStage=_pF?icProjectNext(_pF,u):null;}P&&k&&(()=>{r+='<div style="margin-top:15px;padding:12px;background:var(--accent-fade);border-radius:8px;border-left:3px solid var(--accent);">',r+='<div style="font-size:12px;color:var(--muted);margin-bottom:8px;">NESTE: '+(k.name||("STAGE "+k.number)).toUpperCase()+" ("+P.shots+(dt==="no"?" SKUDD, ":" SHOTS, ")+P.reloads+(dt==="no"?" RELOAD":" RELOAD")+(P.reloads===1?"":"S")+(dt==="no"?", MAX ":", MAX ")+P.maxPts+" PTS)</div>",r+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;font-size:12px;margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid rgba(255,255,255,0.08);">',r+='<div><div style="color:var(--muted);margin-bottom:3px;">'+(dt==="no"?"t/skudd":"time/shot")+'</div><div style="font-weight:600;">'+E.avgSplit.toFixed(3)+"s</div></div>",r+='<div><div style="color:var(--muted);margin-bottom:3px;">'+(dt==="no"?"Treff":"Hits")+'</div><div style="font-weight:600;color:var(--green);">'+( E.aPercent*100).toFixed(0)+"%A</div></div>",r+='<div><div style="color:var(--muted);margin-bottom:3px;">Est. HF</div><div style="font-weight:600;">'+(estForThisStage?estForThisStage.estHF.toFixed(2):"—")+"</div></div>",r+="</div>",r+='<div style="font-size:14px;line-height:1.5;">'+Ds(h,k,E,e)+"</div>";var myDivN=(E&&E.division)||g.division||"Classic";var ctN=getReferenceCourseType(k);var shotsN=icStageShots(k);var refsN=rActive?rActive.filter(function(rf){return(rf.division||"")==myDivN;}):[];if(refsN.length>0){r+='<div style="margin-top:12px;padding:10px;background:rgba(255,255,255,0.04);border-radius:8px;">';r+='<div style="font-size:11px;color:var(--muted);margin-bottom:8px;font-weight:700;">d("ref_shooters")+" — '+ctN.toUpperCase()+' COURSE</div>';refsN.forEach(function(rf){var rHF=ctN==="short"?rf.shortHF:ctN==="medium"?rf.mediumHF:rf.longHF;var rAP=ctN==="short"?rf.shortAP:ctN==="medium"?rf.mediumAP:rf.longAP;var rCP=ctN==="short"?rf.shortCP:ctN==="medium"?rf.mediumCP:rf.longCP;var rDP=ctN==="short"?rf.shortDP:ctN==="medium"?rf.mediumDP:rf.longDP;if(!rHF)return;r+='<div style="margin-bottom:8px;padding:8px;background:var(--bg);border-radius:6px;">';r+='<div style="font-weight:700;font-size:12px;margin-bottom:6px;">'+rf.name+'</div>';r+='<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:4px;font-size:11px;">';r+='<div><div style="color:var(--muted);">HF</div><div style="font-weight:700;color:var(--accent);">'+rHF.toFixed(2)+'</div></div>';if(rAP!=null)r+='<div><div style="color:var(--muted);">A%</div><div style="font-weight:700;color:var(--green);">'+rAP+'%</div></div>';if(rCP!=null)r+='<div><div style="color:var(--muted);">C%</div><div style="font-weight:700;">'+rCP+'%</div></div>';if(rDP!=null)r+='<div><div style="color:var(--muted);">D%</div><div style="font-weight:700;">'+rDP+'%</div></div>';r+='</div>';if(shotsN>0&&P&&P.expTime>0){var pf4=h?h.pf||"minor":"minor";var aV4=5,cV4=pf4==="major"?4:3,dV4=pf4==="major"?2:1;var bt=P.expTime;r+='<div style="margin-top:6px;font-size:10px;color:var(--muted);">"+(dt==="no"?"Effekt: C→A: +":"Impact: C→A: +")+((aV4-cV4)/bt).toFixed(2)+(dt==="no"?" HF | D→A: +":" HF | D→A: +")+((aV4-dV4)/bt).toFixed(2)+(dt==="no"?" HF | Ingen miss: +":" HF | No misses: +")+(10/bt).toFixed(2)+" HF</div>";})();const prevE=f.length>1?icFormFromResults(f.slice(0,f.length-1),null,s):null;const prevP=prevE&&k?icProjectNext(prevE,k):null;const _filteredMatch=Object.assign({},e,{shooters:(e.shooters||[]).map(function(sh){return Object.assign({},sh,{stages:(sh.stages||[]).filter(function(sr){return (sr.num||sr.number)<=u.number;})});})});const C=Bs(f.length>1?f[f.length-2]:null,h,E),L=(k&&f.length>1)?As(_filteredMatch,prevE,prevP?prevP.estHF:null,k):null;C&&(r+='<div style="margin-top:15px;padding:12px;background:var(--card-bg);border-radius:8px;">',r+='<div style="font-size:12px;color:var(--muted);margin-bottom:8px;">💢 REFLEKSJON</div>',r+='<div style="font-size:13px;line-height:1.5;color:var(--text);">'+C+"</div>",r+="</div>"),L&&(r+='<div style="margin-top:15px;padding:12px;background:var(--bg);border-radius:8px;border-left:3px solid '+(L.gap>=0?"var(--green)":"var(--red)")+';">',r+='<div style="font-size:12px;color:var(--muted);margin-bottom:8px;">"+d("match_standings")+"</div>',r+='<div style="font-size:14px;font-weight:700;margin-bottom:8px;">'+L.statusText+"</div>",r+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;font-size:12px;margin-bottom:10px;">',r+='<div><div style="color:var(--muted);margin-bottom:4px;">Est HF</div><div style="font-weight:700;color:var(--accent);">'+(L.estHF!=null?L.estHF.toFixed(2):"—")+"</div></div>",r+='<div><div style="color:var(--muted);margin-bottom:4px;">Target HF</div><div style="font-weight:700;color:var(--text);">'+(L.targetHF!=null?L.targetHF.toFixed(2):"—")+"</div></div>",r+='<div><div style="color:var(--muted);margin-bottom:4px;">Delta HF</div><div style="font-weight:700;color:'+(L.deltaHF!=null&&L.deltaHF>0?"var(--red)":"var(--green)")+';">'+(L.deltaHF!=null?(L.deltaHF>0?"+":"")+L.deltaHF.toFixed(2):"—")+"</div></div>",r+="</div>",r+='<div style="font-size:14px;line-height:1.55;">'+L.advice+"</div>",r+="</div>",r+='<div style="margin-top:8px;">',r+='<button onclick="var ax=document.getElementById(\'anal-'+u.number+'\');ax&&(ax.style.display=ax.style.display===\'none\'?\'block\':\'none\');" style="width:100%;padding:8px;background:transparent;border:1px solid rgba(255,255,255,.12);border-radius:8px;color:var(--muted);font-size:12px;cursor:pointer;text-align:left;">"+d("show_analysis")+"</button>',r+='<div id="anal-'+u.number+'" style="display:none;margin-top:8px;padding:10px;background:var(--bg);border-radius:8px;">',r+=icStageDeepAnalysis(h,u,m,E,rActive,estForThisStage),r+="</div>",(function(){const stMentalId="stm"+Math.random().toString(36).substr(2,5);const stTips=[];const stRes=h||{};const stHits=(stRes.a||0)+(stRes.c||0)+(stRes.d||0)+(stRes.miss||0);const stAP=stHits>0?Math.round((stRes.a||0)/stHits*100):0;if((stRes.miss||0)>0)stTips.push(dt==="no"?{t:"Indre dialog ved miss",b:"Bruk SOAL: Stopp tanken, Observer uten å reagere, Aksepter at den er der, La den gå. Hjernen husker det siste bildet — visualiser straks hvordan du ville skutt stageen riktig og gå videre med et positivt bilde."}:{t:"Inner dialogue on miss",b:"Use the STOP method: Stop the thought, Observe without reacting, Accept that it's there, Let it go. The brain remembers its last image — immediately visualize how you would have shot the stage correctly and move on with a positive picture."});if(stAP<75&&stHits>0)stTips.push(dt==="no"?{t:"Visualisering før neste stage",b:"Lukk øynene og visualiser neste stage i førsteperson — kropp, miljø, timing og følelse. Aktivere alle sanser og inkluder følelsen av kontroll og flyt. Hjernen skiller ikke mellom tanke og handling."}:{t:"Visualization before next stage",b:"Close your eyes and visualize the next stage in first person — body, environment, timing, and feel. Engage all your senses and include the feeling of control and flow. The brain cannot distinguish between thought and action."});if((stRes.proc||0)>0)stTips.push(dt==="no"?{t:"Konsentrasjon",b:"Øv på å filtrere bort støy som RO-avgjørelser og baneforhold. Før neste stage: lukk øynene, gå gjennom stage-planen steg for steg og sjekk at alle regler sitter. 100% fokus på oppgaven."}:{t:"Concentration",b:"Practice filtering out noise like RO decisions and range conditions. Before the next stage: close your eyes, walk through the stage plan step by step, and verify all rules. 100% focus on the task."});const stGen=dt==="no"?[{"t":"Triggerord","b":"Velg ett ord som skaper riktig tilstand umiddelbart — Pust, Flyt eller Klar. Unngå ikke-ord, underbevisstheten filtrerer dem ut. Si triggerordet stille til deg selv mens du går mot neste stage."},{"t":"Selvtillitsboost","b":"Husk en konkret stage der du presterte bra — se den for deg i detalj. Gjenopplev følelsen av kontroll og mestring. Bruk det bildet aktivt nå for å styrke troen på egne evner."},{"t":"Pustepause","b":"Pust inn i 4 sekunder, hold i 4, pust ut i 6. Gjenta tre ganger. Stress gjør pusten grunn og rask, noe som svekker koordinasjon og konsentrasjon. Rolig pust signaliserer til kroppen at du er klar."},{"t":"Positiv indre dialog","b":"Si til deg selv: Jeg er forberedt. Jeg kjenner teknikken. Jeg vet hva jeg skal gjøre. Positive tanker øker motivasjonen og gjør deg bedre rustet til å prestere på ditt beste."},{"t":"Mestringssmål","b":"Sett ett konkret mestringssmål for neste stage — ikke et resultatsmål. For eksempel: rolig trekk, holde stage-planen hele veien, eller ett spesifikt teknisk fokus. Mestringssmål gir deg kontroll over det som skjer her og nå."},{"t":"Visualisering","b":"Lukk øynene og gå gjennom neste stage i førsteperson — kropp, miljø, timing og følelse. Inkluder følelsen av kontroll og flyt. Hjernen skiller ikke mellom en klar visualisering og virkeligheten."},{"t":"Nullstilling","b":"Slipp forrige stage helt. Den er ferdig — det eneste som teller er de neste sekundene. Trekk pusten, senk skuldrene og gå inn i neste stage med blanke ark."},{"t":"Fokuspunkt","b":"Velg ett teknisk fokuspunkt til neste stage — ikke to, ikke tre. Én ting du skal gjøre bra. Enkelt fokus gir bedre utførelse enn mange parallelle instruksjoner til deg selv."},{"t":"Kroppsspråk","b":"Hold ryggen rett, hodet opp og gangen rolig mot neste stage. Kroppsspråket påvirker mentalitet like mye som omvendt. Oppfør deg som en skytter i flytsonen — og du vil merke at hodet følger etter."},{"t":"Aksept","b":"Ikke alle stages blir perfekte — og det trenger de ikke å være. Aksepter det som skjedde, ta med deg det du lærte, og gå videre. Langsiktig utvikling bygges av mange gode nok stages, ikke av perfeksjonisme."}]:[{"t":"Trigger word","b":"Choose one word that creates the right state immediately — Breathe, Flow, or Ready. Avoid negatives — your subconscious filters them out. Say the trigger word silently to yourself as you walk to the next stage."},{"t":"Confidence boost","b":"Recall a specific stage where you performed well — picture it in detail. Relive the feeling of control and mastery. Use that image actively now to strengthen your belief in your own abilities."},{"t":"Breathing reset","b":"Breathe in for 4 seconds, hold for 4, breathe out for 6. Repeat three times. Stress makes your breathing shallow and fast, which weakens coordination and concentration. Calm breathing signals to your body that you are ready."},{"t":"Positive self-talk","b":"Tell yourself: I am prepared. I know the technique. I know what to do. Positive thoughts increase motivation and make you better equipped to perform at your best."},{"t":"Process goal","b":"Set one concrete process goal for the next stage — not a result goal. For example: smooth draw, executing the stage plan all the way through, or one specific technical focus. Process goals give you control over what happens here and now."},{"t":"Visualization","b":"Close your eyes and walk through the next stage in first person — body, environment, timing, and feel. Include the feeling of control and flow. Your brain does not distinguish between a clear visualization and reality."},{"t":"Reset","b":"Let the previous stage go completely. It is done — the only thing that matters is the next few seconds. Take a breath, drop your shoulders, and walk into the next stage with a clean slate."},{"t":"One focus","b":"Choose one technical focus point for the next stage — not two, not three. One thing to do well. Simple focus produces better execution than many parallel instructions to yourself."},{"t":"Body language","b":"Keep your back straight, head up, and walk calmly toward the next stage. Body language influences your mindset just as much as the other way around. Act like a shooter in the zone — and you will find that your head follows."},{"t":"Acceptance","b":"Not every stage will be perfect — and they don't have to be. Accept what happened, take what you learned, and move on. Long-term development is built from many good enough stages, not from perfectionism."}];stTips.push(stGen[u.number%stGen.length]);r+='<div style="margin-top:10px;border-top:1px solid var(--border);padding-top:8px;">';r+='<div onclick="toggleTips(\x27'+stMentalId+'\x27)" style="cursor:pointer;font-size:11px;color:var(--muted);font-weight:700;margin-bottom:4px;user-select:none;">&#9658; MENTAL TRENING</div>';r+='<div id="'+stMentalId+'" style="display:none;">';stTips.forEach(function(tip){r+='<div style="font-size:12px;padding:8px 10px;background:rgba(76,175,125,0.08);border-radius:6px;margin-bottom:6px;border-left:3px solid #4caf7d;"><div style="font-weight:700;color:#4caf7d;margin-bottom:3px;">'+tip.t+'</div><div style="color:var(--text);">'+tip.b+'</div></div>';});r+='</div></div>';})(),r+="</div>")}}else r+='<div style="padding:16px 20px;text-align:center;color:var(--muted);font-size:13px;">'+'Ingen resultater ennå. Last opp resultat eller legg til manuelt.'+'<div style="margin-top:8px;font-size:12px;color:var(--muted);">'+(u.paperTargets?'Paper: '+u.paperTargets+' · ':'')+(u.poppers?'Poppers: '+u.poppers+' · ':'')+(u.plates?'Plates: '+u.plates+' · ':'')+'<span style="color:#e8b84b;font-weight:600;">'+(icStageMaxPts(u)<=60?'Short':icStageMaxPts(u)<=120?'Medium':'Long')+'</span>'+(icStageShots(u)?' · '+icStageShots(u)+(dt==="no"?' skudd':' shots'):'')+(icStageMaxPts(u)?' · '+(dt==="no"?'maks ':'max ')+icStageMaxPts(u)+' pts':'')+'</div></div>';const refCompare=h?{estHF:h.hf!=null?h.hf:null,expTime:h.time!=null?h.time:null}:nextStage&&String(u.number)===String(nextStage.number)&&overallProjection?overallProjection:null;(e.type||"")===d("match_type_practice")||(rActive.length&&(r+=renderReferenceBenchmarkBlock(e,u,refCompare))),r+="</div>"}),i.innerHTML=r}function Ds(e,i,t,s){if(!i||!t)return dt==="no"?"Hold rytmen. Stol på baseline.":"Hold the rhythm. Trust the baseline.";const a=icStageShots(i),n=Se(a,t.division||"Classic",t.pf||"minor"),r=Math.round((t.aPercent||0)*100),u=t.avgSplit?t.avgSplit.toFixed(3):"0.000";const _rnd=Math.floor(Math.random()*12);if(t.aPercent<.7){const _m=dt==="no"?[`Treffbildet kan forbedres. Gi siktet litt mer tid — tempo er sekundært. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}. Baseline: ${r}%A og ${u}s/skudd.`,`${r}%A er under ideelt. Bremse litt og prioriter siktbildet neste stage. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Fokus på treffbilde nå. Du har tempo — bruk litt av det på presisjon. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Kvalitet over hastighet. ${r}%A forteller deg at siktet trenger mer tid. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Løft A-raten fra ${r}% — det er der poengene ligger. Behold rytmen, men la siktet sette seg. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Treffbildet er under baseline. En bevisst nedbremsing kan hente inn poengene. Neste stage: ${a} skudd, ${u}s/skudd baseline.`,`${r}%A koster deg mer enn du tror. Roliggjør inngangen til hvert mål og la siktet bekreftes. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Ikke jag tempoet på bekostning av treffbildet. ${r}%A er utgangspunktet — neste stage kan du forbedre det. ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Sikt kontroll neste stage. ${a} skudd med ${n} reload${n===1?"":"s"} — gi hvert mål den tiden det krever.`,`Treffbildet svikter litt. Prioriter A-treff fremfor split-tid. Baseline: ${r}%A og ${u}s/skudd. Neste stage: ${a} skudd.`,`En bevisst innsats på siktbildet neste stage kan flytte resultatet betydelig. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`,`${r}%A sier at det er rom for forbedring i treffbildet. Senk skuldrene og la siktet komme til deg. Neste stage: ${a} skudd.`]:[`Hit factor can improve. Give your sight picture a little more time — tempo is secondary. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}. Baseline: ${r}%A and ${u}s/shot.`,`${r}%A is below ideal. Slow down slightly and prioritize your sight picture next stage. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`Focus on hit factor now. You have tempo — use some of it on precision. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`Quality over speed. ${r}%A tells you your sight picture needs more time. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`Lift the A-zone rate from ${r}% — that is where the points are. Keep the rhythm, but let the sight picture settle. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`Hit pattern is below baseline. A deliberate slowdown can recover the points. Next stage: ${a} shots, ${u}s/shot baseline.`,`${r}%A costs you more than you think. Slow your entry to each target and let the sight confirm. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`Don't chase tempo at the expense of hit factor. ${r}%A is the starting point — next stage you can improve it. ${a} shots, ${n} reload${n===1?"":"s"}.`,`Sight control next stage. ${a} shots with ${n} reload${n===1?"":"s"} — give each target the time it needs.`,`Hit pattern is slipping. Prioritize A-zone hits over split time. Baseline: ${r}%A and ${u}s/shot. Next stage: ${a} shots.`,`A deliberate focus on sight picture next stage can shift the result significantly. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`${r}%A says there is room to improve the hit pattern. Drop your shoulders and let the sight come to you. Next stage: ${a} shots.`];return _m[_rnd];}if(t.avgSplit<.22){const _m=dt==="no"?[`Solid tempo. Ikke press mer enn nødvendig — ${u}s/skudd er bra. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}. Hold ${r}%A.`,`Flyten sitter. Kjør samme rytme inn i neste stage — ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Bra split-tid. Ikke jag tempoet videre — kvalitet i treffbildet er neste steg. Baseline: ${r}%A og ${u}s/skudd.`,`Du er rask nok. Fokuser på ${r}%A og la tempoet ta seg av seg selv. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`,`${u}s/skudd er effektivt — ikke press det. Neste stage er ${a} skudd med ${n} reload${n===1?"":"s"}. Kjør planen.`,`Godt tempo etablert. Prioriter treffbilde og la flyten bære deg videre. Baseline: ${r}%A og ${u}s/skudd.`,`Du skyter raskt. Bruk det som en fordel, men la siktbildet bekrefte seg. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Tempoet er der. Nå handler det om å bruke det riktig — presisjon i overgangene og rent siktbilde. Baseline: ${r}%A.`,`Rask rytme. Ikke overtenk det — kjør det samme inn i neste stage. ${a} skudd, ${n} reload${n===1?"":"s"}.`,`${u}s/skudd viser god flyt. Hold ${r}%A og la resten skje. Neste stage: ${a} skudd.`,`God fremdrift. Ikke endre noe — samme tempo, samme fokus. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Du er i flytsonen. Ikke analyser det for mye — bare gjenta det du gjør. Baseline: ${r}%A og ${u}s/skudd.`]:[`Solid tempo. Don't push harder than needed — ${u}s/shot is good. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}. Hold ${r}%A.`,`The flow is there. Carry the same rhythm into the next stage — ${a} shots, ${n} reload${n===1?"":"s"}.`,`Good split time. Don't chase more tempo — quality in the hit pattern is the next step. Baseline: ${r}%A and ${u}s/shot.`,`You are fast enough. Focus on ${r}%A and let tempo take care of itself. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`${u}s/shot is efficient — don't force it. Next stage is ${a} shots with ${n} reload${n===1?"":"s"}. Execute the plan.`,`Good tempo established. Prioritize hit pattern and let the flow carry you forward. Baseline: ${r}%A and ${u}s/shot.`,`You are shooting fast. Use it as an advantage, but let the sight picture confirm. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`The tempo is there. Now it's about using it well — precision in transitions and a clean sight picture. Baseline: ${r}%A.`,`Fast rhythm. Don't overthink it — carry the same into the next stage. ${a} shots, ${n} reload${n===1?"":"s"}.`,`${u}s/shot shows good flow. Hold ${r}%A and let the rest happen. Next stage: ${a} shots.`,`Good momentum. Don't change anything — same tempo, same focus. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`You are in the zone. Don't analyze it too much — just repeat what you are doing. Baseline: ${r}%A and ${u}s/shot.`];return _m[_rnd];}const _m=dt==="no"?[`Hold rytmen. Neste stage er ${a} skudd med ${n} reload${n===1?"":"s"}. Baseline: ${r}%A og ${u}s/skudd.`,`Stabil baseline. Kjør samme plan inn i neste stage — ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Du vet hva du kan. ${r}%A og ${u}s/skudd er baseline — ta den med deg videre.`,`Bra grunnlag. ${r}%A og ${u}s/skudd — neste stage er ${a} skudd med ${n} reload${n===1?"":"s"}.`,`Rytmen sitter. Baseline er ${r}%A og ${u}s/skudd — kjør det samme neste stage.`,`Solid grunnlag å bygge på. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}. Hold ${r}%A og ${u}s/skudd.`,`Baseline er etablert. ${r}%A og ${u}s/skudd — stol på prosessen. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Ingen store justeringer nødvendig. ${r}%A og ${u}s/skudd fungerer. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`,`Du leverer jevnt. Hold det oppe — ${a} skudd med ${n} reload${n===1?"":"s"} og samme fokus som forrige stage.`,`Konsistent skyting. Baseline ${r}%A og ${u}s/skudd — ta det med deg videre. Neste stage: ${a} skudd.`,`Stø kurs. Neste stage er ${a} skudd med ${n} reload${n===1?"":"s"} — kjør din plan og stol på baseline.`,`Solid og jevnt. ${r}%A og ${u}s/skudd er der det skal være. Neste stage: ${a} skudd, ${n} reload${n===1?"":"s"}.`]:[`Hold the rhythm. Next stage is ${a} shots with ${n} reload${n===1?"":"s"}. Baseline: ${r}%A and ${u}s/shot.`,`Stable baseline. Execute the same plan into the next stage — ${a} shots, ${n} reload${n===1?"":"s"}.`,`You know what you can do. ${r}%A and ${u}s/shot is baseline — carry it forward.`,`Good foundation. ${r}%A and ${u}s/shot — next stage is ${a} shots with ${n} reload${n===1?"":"s"}.`,`The rhythm is there. Baseline is ${r}%A and ${u}s/shot — shoot the same next stage.`,`Solid foundation to build on. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}. Hold ${r}%A and ${u}s/shot.`,`Baseline is established. Trust the process. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`No major adjustments needed. ${r}%A and ${u}s/shot is working. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`You are delivering consistently. Keep it up — ${a} shots with ${n} reload${n===1?"":"s"} and same focus.`,`Consistent shooting. Baseline ${r}%A and ${u}s/shot — carry it forward. Next stage: ${a} shots.`,`Steady course. Execute your plan and trust the baseline. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`,`Solid and consistent. ${r}%A and ${u}s/shot is right where it should be. Next stage: ${a} shots, ${n} reload${n===1?"":"s"}.`];return _m[_rnd];}function Bs(e,i,t){if(!e||!i||!t)return null;const s=icStageShots(e),a=icStageShots(i);if(!s||!a)return null;const n=((e.time||0)-t.draw-Se(s,t.division||"Classic",t.pf||"minor")*t.reloadTime)/s,r=((i.time||0)-t.draw-Se(a,t.division||"Classic",t.pf||"minor")*t.reloadTime)/a,u=s>0?(e.a||0)/s:0,m=a>0?(i.a||0)/a:0;if(r>n+.05&&m<u-.1){const _bMsgs=dt==="no"?[`Tempo og treff falt fra forrige stage (${n.toFixed(3)}s → ${r.toFixed(3)}s/skudd, ${(u*100).toFixed(0)}%A → ${(m*100).toFixed(0)}%A). Tenk gjennom hva som stjal tid og fokus.`,`Nedgang i både tempo og treffbilde siden forrige stage. Hva endret seg — banen, hodet eller begge deler?`,`Både splittet og A-raten gikk ned. Var det et bevisst valg, eller skjedde det uten at du merket det?`]:[`Both tempo and hit factor dropped from the previous stage (${n.toFixed(3)}s → ${r.toFixed(3)}s/shot, ${(u*100).toFixed(0)}%A → ${(m*100).toFixed(0)}%A). Think through what stole your time and focus.`,`Drop in both tempo and hit pattern since last stage. What changed — the course, your head, or both?`,`Both split time and A-zone rate went down. Was it a deliberate choice, or did it happen without you noticing?`];return _bMsgs[Math.floor(Math.random()*_bMsgs.length)];}if(r>n+.05){const _bMsgs=dt==="no"?[`Tempoet falt fra forrige stage (${n.toFixed(3)}s → ${r.toFixed(3)}s/skudd). Reflekter over hva som stjal tid.`,`Du var tregere enn forrige stage. Bevegelsesfasene eller selve skytingen — hva tok tid?`,`Splittet økte denne stagen. Var det banen, eller ble du mer forsiktig enn nødvendig?`]:[`Tempo dropped from the previous stage (${n.toFixed(3)}s → ${r.toFixed(3)}s/shot). Reflect on what cost you time.`,`You were slower than the previous stage. Transitions or the shooting itself — what took time?`,`Split time increased this stage. Was it the course, or did you become more cautious than necessary?`];return _bMsgs[Math.floor(Math.random()*_bMsgs.length)];}if(m<u-.1){const _bMsgs=dt==="no"?[`Treffbildet falt fra forrige stage (${(u*100).toFixed(0)}%A → ${(m*100).toFixed(0)}%A). Vurder om du presset for hardt på tempo.`,`Færre A-treff enn forrige stage. Jaget du for mye tempo, eller var det noe annet som påvirket siktet?`,`A-raten gikk ned. Var det et bevisst valg å prioritere tempo, eller skjedde det ubevisst?`]:[`Hit pattern dropped from the previous stage (${(u*100).toFixed(0)}%A → ${(m*100).toFixed(0)}%A). Consider whether you pushed too hard on tempo.`,`Fewer A-zone hits than the previous stage. Were you chasing too much tempo, or was there something else affecting your sight picture?`,`A-zone rate went down. Was it a deliberate choice to prioritize tempo, or did it happen unconsciously?`];return _bMsgs[Math.floor(Math.random()*_bMsgs.length)];}if(r<n-.03&&m>=u-.05){const _bMsgs=dt==="no"?[`Bra flyt. Du gikk raskere enn forrige stage uten stort tap i treffbildet.`,`Solid stage. Raskere tempo og treffbildet holdt — det er den kombinasjonen som gir resultater.`,`Godt håndverk. Du presset tempoet uten at det gikk ut over A-raten.`]:[`Good flow. You were faster than the previous stage without significant loss in hit pattern.`,`Solid stage. Faster tempo and the hit pattern held — that is the combination that produces results.`,`Good execution. You pushed the tempo without it affecting your A-zone rate.`];return _bMsgs[Math.floor(Math.random()*_bMsgs.length)];}return null}function icStageDeepAnalysis(h,u,m,E,rActive,estProg){if(!h||!u||!m||!m.length)return "";var progBlock="";if(estProg&&h&&h.hf!=null){var actHF=h.hf>0?h.hf:(h.pts||0)/(h.time||1);var deltaHF=actHF-estProg.estHF;var actTotal=(h.a||0)+(h.c||0)+(h.d||0)+(h.miss||0);var actAP=actTotal>0?Math.round((h.a||0)/actTotal*100):0;var hfColor=deltaHF>=0?"var(--green)":"var(--red)";var hfSign=deltaHF>=0?"+":"";progBlock+='<div style="margin-bottom:14px;padding:10px;background:rgba(232,184,75,0.08);border-radius:8px;border-left:3px solid var(--accent);">';progBlock+='<div style="font-size:11px;color:var(--muted);margin-bottom:8px;font-weight:700;">"+d("forecast_actual")+"</div>';progBlock+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;font-size:12px;">';progBlock+='<div><div style="color:var(--muted);">Est. HF</div><div style="font-weight:700;">'+estProg.estHF.toFixed(2)+'</div></div>';progBlock+='<div><div style="color:var(--muted);">'+(dt==="no"?"Faktisk HF":"Actual HF")+'</div><div style="font-weight:700;color:var(--accent);">'+actHF.toFixed(2)+'</div></div>';progBlock+='<div><div style="color:var(--muted);">Delta HF</div><div style="font-weight:700;color:'+hfColor+';">'+hfSign+deltaHF.toFixed(2)+'</div></div>';progBlock+='</div>';var verdict=dt==="no"?(deltaHF>=0.3?"Betydelig over prognosen — bra skyting!":deltaHF>=0?"Over prognosen — bra jobbet.":deltaHF>=-0.3?"Litt under prognosen — mulig å hente inn.":"Under prognosen — vurder tempo og treffbilde neste stage."):(deltaHF>=0.3?"Significantly above forecast — great shooting!":deltaHF>=0?"Above forecast — well done.":deltaHF>=-0.3?"Slightly below forecast — recoverable.":"Below forecast — consider tempo and hit pattern next stage.");progBlock+='<div style="margin-top:8px;font-size:12px;line-height:1.5;">'+verdict+'</div>';progBlock+='</div>';}var html=progBlock;const shots=icStageShots(u);if(!shots||!h.time||h.time<=0)return "";const myHF=h.hf&&h.hf>0?h.hf:(h.pts||0)/h.time;const myTime=h.time,myPts=h.pts||0;const myA=h.a||0,myC=h.c||0,myD=h.d||0,myMiss=h.miss||0,myProc=h.proc||0;const pf=h.pf||"minor";const aVal=5,cVal=pf==="major"?4:3,dVal=pf==="major"?2:1;const winner=m[0];const winHF=winner?winner.hf||0:0;const allAPts=Math.max(0,shots*aVal+(h.ns||0)*(-10)+myProc*(-10));const hfAllA=myTime>0?allAPts/myTime:0;const deltaHFallA=hfAllA-myHF;const timeAllA=myHF>0?allAPts/myHF:0;const extraTime=timeAllA-myTime;const timeToWin=winHF>0?myPts/winHF:0;const deltaTimeWin=myTime-timeToWin;const secPerMiss=myHF>0?10/myHF:0;const secPerProc=myHF>0?10/myHF:0;const ptsLostC=myC*(aVal-cVal);const ptsLostD=myD*(aVal-dVal);const ptsLostTotal=ptsLostC+ptsLostD;const draw=E&&E.draw?E.draw:g.draw||1.42;const reloadTime=E&&E.reloadTime?E.reloadTime:g.reloadTime||1.8;const reloads=Se(shots,E&&E.division?E.division:g.division||"Classic",pf);const baselineTime=E&&E.avgSplit?draw+shots*E.avgSplit+reloads*reloadTime:0;const baselineHF=baselineTime>0?myPts/baselineTime:0;const deltaBaselineHF=baselineHF-myHF;const myIdx=m.findIndex(function(x){return x.isMe});const above=myIdx>0?m[myIdx-1]:null;const aboveHF=above?above.hf||0:0;const marginHF=aboveHF>0?aboveHF-myHF:0;const marginSec=myHF>0&&aboveHF>0?myPts/myHF-myPts/aboveHF:0;const pts=[];if(deltaHFallA>0.01)pts.push((dt==="no"?"Hadde du skutt alle A på samme tid, ville HF vært ":"If you had shot all A at the same time, HF would have been ")+hfAllA.toFixed(2)+" (+"+deltaHFallA.toFixed(2)+")");if(extraTime>0.01)pts.push((dt==="no"?"Med alle A kunne du brukt ":"With all A you could have used ")+extraTime.toFixed(2)+(dt==="no"?" sek mer (":" sec more (")+timeAllA.toFixed(2)+(dt==="no"?" sek totalt) og beholdt samme HF":" sec total) and kept the same HF"));if(ptsLostTotal>0.5)pts.push((dt==="no"?"Du mistet ":"You lost ")+ptsLostTotal.toFixed(0)+(dt==="no"?" poeng på C/D-treff (C: ":" points on C/D hits (C: ")+ptsLostC.toFixed(0)+(dt==="no"?" poeng, D: ":" pts, D: ")+ptsLostD.toFixed(0)+(dt==="no"?" poeng) sammenlignet med alle A":" pts) compared to all A"));if(E&&E.avgSplit&&baselineHF>0&&Math.abs(deltaBaselineHF)>0.05)pts.push((dt==="no"?"Med din baseline-split (":"With your baseline split (")+E.avgSplit.toFixed(3)+(dt==="no"?"s/skudd) og samme treffbilde ville HF vært ":"s/shot) and the same hit pattern, HF would have been ")+(deltaBaselineHF>=0?"+":"")+deltaBaselineHF.toFixed(2)+" ("+baselineHF.toFixed(2)+")");if(myMiss>0)pts.push((dt==="no"?"Hver miss kostet deg ca. ":"Each miss cost you approx. ")+secPerMiss.toFixed(2)+(dt==="no"?" sek i HF-tap":" sec in HF"));if(myProc>0)pts.push((dt==="no"?"Hver procedural kostet deg ca. ":"Each procedural cost you approx. ")+secPerProc.toFixed(2)+(dt==="no"?" sek i HF-tap":" sec in HF"));if(above&&marginHF>0.01)pts.push((dt==="no"?"Margin til «":"Margin to “")+(above.name||"skytteren over deg")+(dt==="no"?"» er ":"” is ")+marginHF.toFixed(2)+(dt==="no"?" HF — tilsvarer ca. ":" HF — equals approx. ")+Math.abs(marginSec).toFixed(2)+(dt==="no"?" sek på denne stagen":" sec on this stage"));if(winHF>0&&myHF<winHF&&deltaTimeWin>0)pts.push((dt==="no"?"For å vinne stagen måtte du skutt ":"To win the stage you needed to shoot ")+deltaTimeWin.toFixed(2)+(dt==="no"?" sek raskere (":" sec faster (")+timeToWin.toFixed(2)+(dt==="no"?" sek totalt) med samme treffbilde":" sec total) with the same hit pattern"));else if(winHF>0&&myHF>=winHF)pts.push((dt==="no"?"Du vant stagen — bra skyting!":"You won the stage — great shooting!"));if(!pts.length)return html;html='<ul style="margin:0;padding:0 0 0 16px;font-size:12px;color:var(--text);line-height:1.7;">';pts.forEach(function(p){html+='<li style="margin-bottom:6px;">'+p+"</li>"});html+="</ul>";if(rActive&&rActive.length>0&&h&&u){const myDivision=(E&&E.division)||g.division||"Classic";const ct=getReferenceCourseType(u);const shots=icStageShots(u);const myTotal=(h.a||0)+(h.c||0)+(h.d||0)+(h.miss||0);const myAP=myTotal>0?Math.round((h.a||0)/myTotal*100):0;const myCP=myTotal>0?Math.round((h.c||0)/myTotal*100):0;const myDP=myTotal>0?Math.round((h.d||0)/myTotal*100):0;const myHF=h.hf&&h.hf>0?h.hf:(h.pts||0)/h.time;const matchRefs=rActive.filter(function(r){return(r.division||"")===myDivision;});if(matchRefs.length>0){html+='<div style="margin-top:14px;padding:10px;background:rgba(232,184,75,0.06);border-radius:8px;border-left:3px solid var(--accent);">';html+='<div style="font-size:11px;color:var(--muted);margin-bottom:8px;font-weight:700;">REFERANSESKYTTERE — '+ct.toUpperCase()+' COURSE</div>';matchRefs.forEach(function(ref){const refHF=ct==="short"?ref.shortHF:ct==="medium"?ref.mediumHF:ref.longHF;const refAP=ct==="short"?ref.shortAP:ct==="medium"?ref.mediumAP:ref.longAP;const refCP=ct==="short"?ref.shortCP:ct==="medium"?ref.mediumCP:ref.longCP;const refDP=ct==="short"?ref.shortDP:ct==="medium"?ref.mediumDP:ref.longDP;if(!refHF)return;const dHF=myHF-refHF;const dAP=refAP!=null?myAP-refAP:null;const dCP=refCP!=null?myCP-refCP:null;const dDP=refDP!=null?myDP-refDP:null;const pf=(ref.powerFactor||"minor").charAt(0).toUpperCase()+(ref.powerFactor||"minor").slice(1);html+='<div style="margin-bottom:10px;padding:8px;background:var(--bg);border-radius:6px;">';html+='<div style="font-size:12px;font-weight:700;margin-bottom:6px;">'+ref.name+' <span style="font-weight:400;color:var(--muted);">('+ref.division+' '+pf+')</span></div>';html+='<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;font-size:11px;">';html+='<div><div style="color:var(--muted);">HF</div>';html+='<div style="font-weight:700;">'+refHF.toFixed(2)+'</div>';html+='<div style="color:'+(dHF>=0?"var(--green)":"var(--red)")+';font-size:11px;">'+(dHF>=0?"+":"")+dHF.toFixed(2)+'</div></div>';if(refAP!=null){html+='<div><div style="color:var(--muted);">A%</div>';html+='<div style="font-weight:700;">'+refAP+'%</div>';html+='<div style="color:'+(dAP>=0?"var(--green)":"var(--red)")+';font-size:11px;">'+(dAP>=0?"+":"")+dAP+'%</div></div>';}if(refCP!=null){html+='<div><div style="color:var(--muted);">C%</div>';html+='<div style="font-weight:700;">'+refCP+'%</div>';html+='<div style="color:'+(dCP<=0?"var(--green)":"var(--red)")+';font-size:11px;">'+(dCP>=0?"+":"")+dCP+'%</div></div>';}if(refDP!=null){html+='<div><div style="color:var(--muted);">D%</div>';html+='<div style="font-weight:700;">'+refDP+'%</div>';html+='<div style="color:'+(dDP<=0?"var(--green)":"var(--red)")+';font-size:11px;">'+(dDP>=0?"+":"")+dDP+'%</div></div>';}html+='</div></div>';});html+='</div>';}}return html;}function icTrainingAnalysis(runs,stageDef){if(!runs||runs.length<1)return "";const shots=icStageShots(stageDef);const pf=runs[0].pf||"minor";const aVal=5,cVal=pf==="major"?4:3,dVal=pf==="major"?2:1;let html="";if(runs.length>1){html+='<div style="margin-bottom:12px;">';html+='<div style="font-size:11px;color:var(--muted);margin-bottom:6px;font-weight:700;">"+d("run_progression")+"</div>';for(let ri=1;ri<runs.length;ri++){const prev=runs[ri-1],curr=runs[ri];const hfDiff=(curr.hf||0)-(prev.hf||0);const timeDiff=(curr.time||0)-(prev.time||0);const pt=(prev.a||0)+(prev.c||0)+(prev.d||0)+(prev.miss||0);const ct=(curr.a||0)+(curr.c||0)+(curr.d||0)+(curr.miss||0);const pAP=pt>0?Math.round((prev.a||0)/pt*100):0;const cAP=ct>0?Math.round((curr.a||0)/ct*100):0;const aPDiff=cAP-pAP;html+='<div style="font-size:12px;padding:8px;background:var(--bg);border-radius:6px;margin-bottom:4px;">';html+="Run "+(prev.runNum||ri)+" → Run "+(curr.runNum||ri+1)+": ";html+='<span style="color:'+(hfDiff>=0?"var(--green)":"var(--red)")+';font-weight:600;">'+(hfDiff>=0?"+":"")+hfDiff.toFixed(2)+" HF</span>";html+=" · ";html+='<span style="color:'+(timeDiff<=0?"var(--green)":"var(--red)")+';">'+(timeDiff>=0?"+":"")+timeDiff.toFixed(2)+"s</span>";html+=" · ";html+='<span style="color:'+(aPDiff>=0?"var(--green)":"var(--red)")+';">'+(aPDiff>=0?"+":"")+aPDiff+"%A</span>";html+="</div>";}html+="</div>";}const bestHFRun=runs.reduce(function(a,b){return(b.hf||0)>(a.hf||0)?b:a},runs[0]);const bestTimeRun=runs.reduce(function(a,b){return(b.time||0)<(a.time||0)?b:a},runs[0]);const bestAPRun=runs.reduce(function(a,b){const at=(a.a||0)+(a.c||0)+(a.d||0)+(a.miss||0);const bt=(b.a||0)+(b.c||0)+(b.d||0)+(b.miss||0);return bt>0&&(b.a||0)/bt>(at>0?(a.a||0)/at:0)?b:a},runs[0]);html+='<div style="margin-bottom:12px;">';html+='<div style="font-size:11px;color:var(--muted);margin-bottom:6px;font-weight:700;">"+d("best_by_cat")+"</div>';html+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;font-size:11px;">';const bapT=(bestAPRun.a||0)+(bestAPRun.c||0)+(bestAPRun.d||0)+(bestAPRun.miss||0);const bapStr=bapT>0?Math.round((bestAPRun.a||0)/bapT*100)+"%":"--";html+='<div style="background:var(--bg);padding:8px;border-radius:6px;"><div style="color:var(--muted);">'+(dt==="no"?"Beste HF":"Best HF")+'</div><div style="font-weight:700;color:var(--accent);">'+( bestHFRun.hf||0).toFixed(2)+"</div><div>Run "+(bestHFRun.runNum||1)+"</div></div>";html+='<div style="background:var(--bg);padding:8px;border-radius:6px;"><div style="color:var(--muted);">'+(dt==="no"?"Beste tid":"Best time")+'</div><div style="font-weight:700;color:var(--green);">'+( bestTimeRun.time||0).toFixed(2)+"s</div><div>Run "+(bestTimeRun.runNum||1)+"</div></div>";html+='<div style="background:var(--bg);padding:8px;border-radius:6px;"><div style="color:var(--muted);">'+(dt==="no"?"Beste A%":"Best A%")+'</div><div style="font-weight:700;color:var(--green);">'+bapStr+"</div><div>Run "+(bestAPRun.runNum||1)+"</div></div>";html+="</div></div>";if(runs.length>1){const potTime=bestTimeRun.time||0;const potPts=bestAPRun.pts||0;const potHF=potTime>0?potPts/potTime:0;const bestActual=bestHFRun.hf||0;if(potHF>bestActual+0.01){html+='<div style="margin-bottom:12px;padding:10px;background:rgba(232,184,75,0.08);border-radius:6px;border-left:3px solid var(--accent);">';html+='<div style="font-size:11px;color:var(--muted);margin-bottom:4px;">"+d("potential")+"</div>';html+='<div style="font-size:12px;">Med Run '+( bestTimeRun.runNum||1)+" sin tid ("+potTime.toFixed(2)+"s) og Run "+(bestAPRun.runNum||1)+" sitt treffbilde ville HF vært ";html+='<span style="color:var(--accent);font-weight:700;">'+potHF.toFixed(2)+"</span> (+"+( potHF-bestActual).toFixed(2)+" over beste run)</div>";html+="</div>";}}const best=bestHFRun;const bestTime=best.time||0,bestPts=best.pts||0,bestHF=best.hf||0;if(bestTime>0&&shots>0){html+='<div style="margin-bottom:12px;">';html+='<div style="font-size:11px;color:var(--muted);margin-bottom:6px;font-weight:700;">'+(dt==="no"?"HVA HVIS (beste run ★)":"WHAT IF (best run ★)")+'</div>';const allAPts=Math.max(0,shots*aVal+(best.ns||0)*(-10)+(best.proc||0)*(-10));const hfAllA=bestTime>0?allAPts/bestTime:0;if(hfAllA>bestHF+0.01)html+='<div style="font-size:12px;padding:5px 0;">&#8226; Alle A samme tid: HF '+hfAllA.toFixed(2)+" (+"+( hfAllA-bestHF).toFixed(2)+")</div>";if((best.c||0)>0){const noC=Math.max(0,bestPts+(best.c||0)*(aVal-cVal));const hfNoC=bestTime>0?noC/bestTime:0;html+='<div style="font-size:12px;padding:5px 0;">&#8226; Ingen C-treff: HF '+hfNoC.toFixed(2)+" (+"+( hfNoC-bestHF).toFixed(2)+")</div>";}if((best.d||0)>0){const noD=Math.max(0,bestPts+(best.d||0)*(aVal-dVal));const hfNoD=bestTime>0?noD/bestTime:0;html+='<div style="font-size:12px;padding:5px 0;">&#8226; Ingen D-treff: HF '+hfNoD.toFixed(2)+" (+"+( hfNoD-bestHF).toFixed(2)+")</div>";}if((best.miss||0)>0){const noMiss=Math.max(0,bestPts+(best.miss||0)*10);const hfNoMiss=bestTime>0?noMiss/bestTime:0;html+='<div style="font-size:12px;padding:5px 0;">&#8226; Ingen miss: HF '+hfNoMiss.toFixed(2)+" (+"+( hfNoMiss-bestHF).toFixed(2)+")</div>";}if((best.proc||0)>0){const noProc=Math.max(0,bestPts+(best.proc||0)*10);const hfNoProc=bestTime>0?noProc/bestTime:0;html+='<div style="font-size:12px;padding:5px 0;">&#8226; Ingen procedurals: HF '+hfNoProc.toFixed(2)+" (+"+( hfNoProc-bestHF).toFixed(2)+")</div>";}html+="</div>";}const refleks=[];if(runs.length>1){const first=runs[0],last=runs[runs.length-1];const hfUp=(last.hf||0)>(first.hf||0);const timeDown=(last.time||0)<(first.time||0);const ft=(first.a||0)+(first.c||0)+(first.d||0)+(first.miss||0);const lt=(last.a||0)+(last.c||0)+(last.d||0)+(last.miss||0);const fAP=ft>0?(first.a||0)/ft:0;const lAP=lt>0?(last.a||0)/lt:0;if(hfUp&&lAP>fAP+0.05)refleks.push((dt==="no"?"Du forbedret både HF og treffbilde — hva endret du mellom runs?":"You improved both HF and hit pattern — what did you change between runs?"));else if(hfUp&&timeDown)refleks.push((dt==="no"?"Du gikk raskere og forbedret HF — hva skapte den bedre flyten?":"You went faster and improved HF — what created the better flow?"));else if(hfUp)refleks.push((dt==="no"?"HF gikk opp — hva var den viktigste justeringen mellom runs?":"HF went up — what was the key adjustment between runs?"));else refleks.push((dt==="no"?"HF gikk ned — hva tror du stjal tid eller treffkvalitet?":"HF went down — what do you think cost you time or hit quality?"));if(lAP>fAP+0.05)refleks.push((dt==="no"?"A% økte — visste du om C/D-treffene underveis på forrige run, eller oppdaget du dem etterpå?":"A% improved — were you aware of the C/D hits during the previous run, or did you notice afterwards?"));if(lAP<fAP-0.05)refleks.push((dt==="no"?"Treffbildet falt — presset du for hardt på tempo?":"Hit pattern dropped — did you push too hard on tempo?"));}if((best.miss||0)>0)refleks.push((dt==="no"?"Du hadde miss — var det tempo, siktebilde eller noe annet som forårsaket det?":"You had misses — was it tempo, sight picture, or something else?"));if((best.c||0)>2)refleks.push((dt==="no"?"Flere C-treff — er det spesifikke mål som er vanskeligere, eller et generelt mønster?":"Multiple C hits — are there specific targets that are harder, or is it a general pattern?"));refleks.push("Hva ville du gjort annerledes på neste run?");if(refleks.length){html+='<div style="margin-bottom:8px;">';html+='<div style="font-size:11px;color:var(--muted);margin-bottom:6px;font-weight:700;">REFLEKSJON</div>';refleks.forEach(function(q){html+='<div style="font-size:12px;padding:6px 8px;background:var(--bg);border-radius:6px;margin-bottom:4px;border-left:3px solid var(--accent);">❓ '+q+"</div>";});html+="</div>";}return html;}async function Os(){const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e)return;const i=F("new-shooter-firstname").trim(),t=F("new-shooter-lastname").trim(),s=F("new-shooter-division")||"Classic",a=(F("new-shooter-pf")||(Zi[s]||["minor"])[0]||"minor").toLowerCase();if(!i||!t){alert(d("enter_name"));return}const n={id:"s_"+Date.now(),isMe:!1,firstName:i,lastName:t,division:s,pf:a,club:"",stages:[]};e.shooters||(e.shooters=[]),e.shooters.push(n);showSpinner(d("saving_shooter"));Ee(e.id,{shooters:e.shooters}).then(function(r){hideSpinner();if(r.success){const _fn=document.getElementById("new-shooter-firstname");const _ln=document.getElementById("new-shooter-lastname");if(_fn)_fn.value="";if(_ln)_ln.value="";G("modal-add-shooter");icRenderEditMatchShootersList(e);te();_e();De()}else alert((dt==="no"?"Kunne ikke lagre skytter: ":"Could not save shooter: ")+r.error)})}async function js(){if(window._icEditMode){const _em=window._icEditMode;if(!confirm((dt==="no"?"Er du sikker på at du vil overskrive resultatet for ":"Are you sure you want to overwrite the result for ")+_em.stageName+" for "+_em.shooterName+"?")){return;}window._icEditMode=null;}const e=$.find(f=>f.id!=null&&f.id.toString()===String(R));if(!e)return;const i=A("new-result-stage",1),t=he("new-result-time",0),s=icNormalizeManualHits(),a=icRecalcPoints("new-result"),n=icStageDefs(e).find(r=>Number(r.number)===Number(i));if(!n){alert(d("stage_not_found"));return}if(t<=0||a<=0){alert(d("time_pts_required"));return}if(s.a+s.c+s.d+s.miss!==s.totalHits){alert(d("hit_count_mismatch"));return}const r=await icEnsureShooter(e,icUploadShooterSel||icCurrentShooterId());if(!r){alert(d("shooter_not_found"));return}const u=t>0?a/t:0,m={num:n.number,name:n.name||("Stage "+n.number),hf:u,time:t,pts:a,pf:icResultPF(r.pf||g.powerFactor||"minor"),a:s.a,c:s.c,d:s.d,miss:s.miss,ns:s.ns,proc:s.proc,paperTargets:n.paperTargets||0,poppers:n.poppers||0,plates:n.plates||0};const isTraining=(e.type||"")===d("match_type_practice");icUpsertStageResult(r,m,isTraining);showSpinner(d("saving_result"));Ee(e.id,{shooters:e.shooters}).then(function(b){hideSpinner();if(b.success){window._icEditMode=null;G("modal-add");te();_e();De()}else alert((dt==="no"?"Kunne ikke lagre resultat: ":"Could not save result: ")+b.error)});}function Us(){G("modal-upload-result"),Re()}async function Hs(e){if(!$.find(u=>u.id!=null&&u.id.toString()===String(R))){alert(d("no_active_match"));return}const i=o("upload-stage-select"),t=o("upload-shooter-select"),s=o("upload-result-file");if(!i.value||!t.value){alert(d("select_stage_shooter"));return}if(!s.files||s.files.length===0){alert(d("select_file"));return}const a=s.files[0];Me=i.value,icUploadShooterSel=t.value,icSetResultDialogMode("ocr");const n=e&&e.currentTarget?e.currentTarget:o("upload-scan-btn"),r=n?n.textContent:d("upload_and_scan");n&&(n.textContent=d("scanning"),n.disabled=!0);try{const u=await new Promise((m,b)=>{const f=new FileReader;f.onerror=()=>b(new Error("Kunne ikke lese filen")),f.onload=p=>m(p.target.result),f.readAsDataURL(a)}),m=await qi.recognize(u,"eng+nor",{logger:b=>console.log(b)}),f=(m&&m.data&&m.data.text?m.data.text:"").trim();if(console.log("OCR Text:",f),!f)throw new Error(d("no_text_in_image"));const p=zs(f);if(p.time==null&&p.a===0&&p.c===0&&p.d===0&&p.miss===0&&p.ns===0&&p.proc===0)throw new Error(d("no_results_in_image"));o("ocr-time").value=p.time||"",o("ocr-a").value=p.a||0,o("ocr-c").value=p.c||0,o("ocr-d").value=p.d||0,o("ocr-miss").value=p.miss||0,o("ocr-ns").value=p.ns||0,o("ocr-proc").value=p.proc||0,icRecalcPoints("ocr"),G("modal-upload-result"),ie("modal-ocr-confirm")}catch(u){console.error("OCR Error:",u),alert(d("scan_error")+": "+u.message)}finally{n&&(n.textContent=r,n.disabled=!1)}}function zs(e){const i={time:null,points:null,a:0,c:0,d:0,miss:0,ns:0,proc:0},t=e.replace(/[\n\r]+/g," ").toLowerCase(),s=[/time[:\s]*(\d+\.?\d*)/i,/(\d+\.\d+)\s*s(?:ec)?/i,/^(\d+\.\d+)$/m];for(const p of s){const h=t.match(p);if(h){i.time=parseFloat(h[1]);break}}const a=[/(?:points?|pts?)[:\s]*(\d+)/i,/score[:\s]*(\d+)/i];for(const p of a){const h=t.match(p);if(h){i.points=parseInt(h[1]);break}}const n=[/(\d+)\s*a(?:lpha)?(?:\s|$)/i,/a(?:lpha)?[:\s]*(\d+)/i];for(const p of n){const h=t.match(p);if(h){i.a=parseInt(h[1]);break}}const r=[/(\d+)\s*c(?:harlie)?(?:\s|$)/i,/c(?:harlie)?[:\s]*(\d+)/i];for(const p of r){const h=t.match(p);if(h){i.c=parseInt(h[1]);break}}const u=[/(\d+)\s*d(?:elta)?(?:\s|$)/i,/d(?:elta)?[:\s]*(\d+)/i];for(const p of u){const h=t.match(p);if(h){i.d=parseInt(h[1]);break}}const m=[/(\d+)\s*m(?:iss)?(?:\s|$)/i,/m(?:iss)?[:\s]*(\d+)/i];for(const p of m){const h=t.match(p);if(h){i.miss=parseInt(h[1]);break}}const b=[/(\d+)\s*ns(?:\s|$)/i,/ns[:\s]*(\d+)/i,/no-?shoot[:\s]*(\d+)/i];for(const p of b){const h=t.match(p);if(h){i.ns=parseInt(h[1]);break}}const f=[/(\d+)\s*p(?:roc)?(?:\s|$)/i,/p(?:roc)?[:\s]*(\d+)/i,/procedural[:\s]*(\d+)/i];for(const p of f){const h=t.match(p);if(h){i.proc=parseInt(h[1]);break}}return i}async function Gs(){const e=$.find(P=>P.id!=null&&P.id.toString()===String(R));if(!e)return;const i=he("ocr-time",0),t=icRecalcPoints("ocr"),s=A("ocr-a",0),a=A("ocr-c",0),n=A("ocr-d",0),r=A("ocr-miss",0),u=A("ocr-ns",0),m=A("ocr-proc",0);if(i<=0||t<=0){alert(d("time_pts_required"));return}const b=icStageDefs(e).find(P=>P.number==Me);if(!b){alert(d("stage_not_found"));return}const f=await icEnsureShooter(e,icUploadShooterSel||icCurrentShooterId());if(!f){alert(d("shooter_not_found"));return}const p=i>0?t/i:0,h={num:b.number,name:b.name||("Stage "+b.number),hf:p,time:i,pts:t,pf:icResultPF(f.pf||g.powerFactor||"minor"),a:s,c:a,d:n,miss:r,ns:u,proc:m,paperTargets:b.paperTargets||0,poppers:b.poppers||0,plates:b.plates||0};icUpsertStageResult(f,h),await Ee(e.id,{shooters:e.shooters}),G("modal-ocr-confirm"),te(),_e(),De(),Me=null,icUploadShooterSel=null,icSetResultDialogMode("ocr")}
async function importESSVerify(e){
  ...
  overlay.innerHTML=`
    <div style="background:var(--card);border-radius:16px;padding:20px;width:100%;max-width:420px;max-height:85vh;overflow-y:auto;">
      ...
      <textarea id="ess-paste-input" style="width:100%;height:140px;background:var(--bg);border:1px solid rgba(255,255,255,.15);border-radius:8px;color:var(--text);padding:10px;font-size:12px;resize:vertical;" placeholder=d("ess_paste_placeholder")></textarea>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:12px;">
        <button onclick="document.getElementById(\'ess-paste-modal\').remove();" style="padding:10px;background:var(--bg3);border:none;border-radius:8px;color:var(--text);font-weight:600;cursor:pointer;font-size:12px;">"+d("cancel")+"</button>
        <button onclick="essConfirmPaste(\'single\')" style="padding:10px;background:var(--bg3);border:1px solid var(--accent);border-radius:8px;color:var(--accent);font-weight:700;cursor:pointer;font-size:12px;">Stage ${stageNum}</button>
        <button onclick="essConfirmPaste(\'all\')" style="padding:10px;background:var(--accent);border:none;border-radius:8px;color:#000;font-weight:700;cursor:pointer;font-size:12px;">Alle stages</button>
      </div>
    </div>`;
...
async function essConfirmPaste(mode){
  ...
  if(mode==="all"){
    ...
    if(!parsed){alert("Kunne ikke lese data for stage "+stageNum+".");return;}
