...
`;function At(e,i){const t=document.getElementById("gdpr-modal");t&&t.remove();const s=document.createElement("div");s.id="gdpr-modal",s.className="gdpr-modal",s.innerHTML=`
 ...
 ${dt==="no"?"Jeg har lest og godtar":"I have read and accept"} 
 <a href="#" class="gdpr-link" id="gdpr-open-modal">${dt==="no"?"personvernerklæringen":"the privacy policy"}</a>
 </span>
 </label>
 `,setTimeout(()=>{const i=e.querySelector("#gdpr-open-modal");i&&i.addEventListener("click",t=>{t.preventDefault(),At(()=>{const s=document.getElementById("gdpr-consent-checkbox");s&&(s.checked=!0)},()=>{const s=document.getElementById("gdpr-consent-checkbox");s&&(s.checked=!1)})})},0),e}function Dt(){const e=document.getElementById("gdpr-consent-checkbox");return!e||!e.checked?{valid:!1,error:dt==="no"?"You must accept the privacy policy to register":"You must accept the privacy policy to register"}:{valid:!0}}function Bt(e,i){e.innerHTML=`
 <style>
 * {
 ...
 <div id="brandSubtitle" class="brand-subtitle">Analysis. Precision. Results.</div>
...
<div class="screen active" id="screen-home">
 ...
 <div class="tab-item" onclick="switchTab('screen-teams')"><div class="tab-icon">👕</div><span>Teams</span></div>
...
<div class="screen" id="screen-profile">
 ...
 <div class="card" style="margin-bottom:12px;">
  <div class="card-header"><div class="card-title" id="prof-lang-title">${dt==="no"?"Language / Language":"Language / Language"}</div></div>
...
<div class="screen" id="screen-teams">
 ...
 <div class="tab-item" onclick="switchTab('screen-prognose')"><div class="tab-icon">&#128200;</div><span class="lang-prognosis">${d("prognosis")}</span></div>
 <div class="tab-item" onclick="switchTab('screen-results')"><div class="tab-icon">&#127970;</div><span class="lang-results">${d("results")}</span></div>
 <div class="tab-item active" onclick="switchTab('screen-teams')"><div class="tab-icon">👕</div><span>Teams</span></div>
 <div class="tab-item" onclick="switchTab('screen-profile')"><div class="tab-icon">&#128100;</div><span class="lang-profile">${d("profile")}</span></div>
 </div>
</div>
...
window.icPopulateTeamCountry=icPopulateTeamCountry,window.deleteCurrentStage=deleteCurrentStage,window.openInviteUser=ks,window.openInvitationsModal=Is,window.acceptInvitation=Cs,window.declineInvitation=_s,window.searchUsers=xs,window.toggleUserSelection=Ss,window.sendMultipleInvitations=Ls,window.searchUsersNewMatch=Ps,window.toggleUserNewMatch=Es,window.searchUsersEditMatch=Ts,window.toggleUserEditMatch=Ms,window.confirmDeleteMatch=gs,window.deleteMatchConfirmed=vs,window.uploadAndScanResult=Hs,window.saveOCRResult=Gs,window.icOpenManualResult=icOpenManualResult,window.importESSVerify=importESSVerify,window.essConfirmPaste=essConfirmPaste,window.openCreateStageFromUpload=Us,window.toggleMatchDropdown=ds,window.selectMatchFromDropdown=cs,window.toggleReferenceShootersEnabled=toggleReferenceShootersEnabled,window.toggleReferenceShooterChoice=toggleReferenceShooterChoice,window.updateReferenceShooterOverride=updateReferenceShooterOverride,window.toggleLiveFilter=toggleLiveFilter,window.toggleTips=toggleTips,window.deleteRun=deleteRun,window.openCreateTeam=openCreateTeam,window.editTeam=editTeam,window.deleteTeam=deleteTeam,window.saveTeam=saveTeam,window.renderTeamsScreen=renderTeamsScreen}function toggleLiveFilter(){liveShowAll=!liveShowAll,_e()}function toggleTips(id){const el=document.getElementById(id);if(el)el.style.display=el.style.display==="none"?"block":"none"}async function deleteRun(stageNum,runIdx){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match)return;const shooter=icCurrentShooter(match);if(!shooter)return;const stageIdx=shooter.stages.findIndex(s=>(s.num||s.number)==stageNum);if(stageIdx<0)return;const stage=shooter.stages[stageIdx];if(!stage.runs||stage.runs.length<=1){alert("Cannot delete the last run. Delete the entire result instead.");return;}if(!confirm("Delete Run "+(runIdx+1)+"?"))return;stage.runs.splice(runIdx,1);stage.runs.forEach((r,i)=>{r.runNum=i+1});const best=stage.runs.reduce((a,b)=>(b.hf||0)>(a.hf||0)?b:a,stage.runs[0]);Object.assign(stage,best,{num:stage.num,name:stage.name,runs:stage.runs});const res=await Ee(match.id,{shooters:match.shooters});if(res.success){te(),_e(),De()}else alert("Could not delete run: "+res.error);}function icTeamStandings(e){if(!e||!e.teams||!e.teams.length)return[];const stages=icStageDefs(e);return e.teams.map(team=>{let totalPts=0;stages.forEach(stage=>{const metrics=icStageMetricsForMatch(e,stage);const teamMetrics=metrics.filter(m=>team.shooterIds&&team.shooterIds.includes(m.id));const sorted=teamMetrics.slice().sort((a,b)=>(b.stagePts||0)-(a.stagePts||0));const top3=sorted.slice(0,3);top3.forEach(m=>{totalPts+=(m.stagePts||0)})});return{id:team.id,name:team.name,shooterIds:team.shooterIds||[],totalPts}}).sort((a,b)=>b.totalPts-a.totalPts)}function renderTeamsScreen(){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));const sc=o("teams-standings-container");const lc=o("teams-list-container");if(!sc||!lc)return;if(!match){sc.innerHTML='<div class="empty-state"><div class="empty-sub">'+d('select_match_first')+'</div></div>';lc.innerHTML="";return;}const teams=match.teams||[];if(!teams.length){sc.innerHTML='<div class="empty-state"><div class="empty-title">'+d('no_teams')+'</div><div class="empty-sub">'+d('create_team_hint')+'</div></div>';lc.innerHTML="";return;}const standings=icTeamStandings(match);const topPts=standings.length?standings[0].totalPts:0;let sh='<div class="card"><div class="card-header"><div class="card-title">Lag-standings</div></div>';sh+='<div class="standings-table-wrap"><table class="standings-table">';sh+='<thead><tr class="standings-header-row"><th class="standings-th">#</th><th class="standings-th">LAG</th><th class="standings-th standings-th-pts">STG PTS</th><th class="standings-th standings-th-pct">%</th></tr></thead><tbody>';standings.forEach((t,i)=>{const pct=topPts>0?(t.totalPts/topPts*100).toFixed(2):"0.00";sh+='<tr class="standings-row">';sh+='<td class="standings-td">'+(i+1)+"</td>";sh+='<td class="standings-td">'+t.name+"</td>";sh+='<td class="standings-td standings-td-pts">'+t.totalPts.toFixed(2)+"</td>";sh+='<td class="standings-td standings-td-pct">'+pct+"%</td>";sh+="</tr>"});sh+="</tbody></table></div></div>";sc.innerHTML=sh;let lh="";teams.forEach((team,idx)=>{const allShooters=(match.shooters||[]);const memberNames=allShooters.filter(s=>team.shooterIds&&team.shooterIds.includes(s.id)).map(s=>((s.firstName||"")+" "+(s.lastName||"")).trim()||d('shooter_default'));lh+='<div class="card" style="margin-bottom:10px;">';lh+='<div class="card-header"><div class="card-title">'+(team.country?team.country+' ':'')+team.name+"</div>";lh+='<div style="display:flex;gap:8px;">';lh+='<button onclick="editTeam('+idx+')" style="padding:6px 12px;background:var(--accent);color:#1a202c;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">Rediger</button>';lh+='<button onclick="deleteTeam('+idx+')" style="padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">Slett</button>';lh+="</div></div>";lh+='<div style="font-size:13px;color:var(--muted);margin-top:8px;">'+( memberNames.length?memberNames.join(", "):d('no_shooters'))+"</div>";lh+="</div>"});lc.innerHTML=lh}let currentEditTeamIdx=null;function openCreateTeam(){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match){alert("Select a match first");return;}currentEditTeamIdx=null;o("team-modal-title").textContent="Opprett lag";o("team-name-input").value="";icPopulateTeamCountry("");renderTeamShooterList(match,null);ie("modal-create-team")}function editTeam(idx){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match||!match.teams||!match.teams[idx])return;currentEditTeamIdx=idx;const team=match.teams[idx];o("team-modal-title").textContent="Rediger lag";o("team-name-input").value=team.name||"";icPopulateTeamCountry(team.country||"");renderTeamShooterList(match,team.shooterIds||[]);ie("modal-create-team")}function renderTeamShooterList(match,selectedIds){const container=o("team-shooter-list");if(!container)return;const allShooters=(match.shooters||[]);const usedIds=new Set();(match.teams||[]).forEach((t,i)=>{if(i===currentEditTeamIdx)return;(t.shooterIds||[]).forEach(id=>usedIds.add(id))});let html="";allShooters.forEach(s=>{const name=((s.firstName||"")+" "+(s.lastName||"")).trim()||"Skytter";const checked=selectedIds&&selectedIds.includes(s.id);const disabled=!checked&&usedIds.has(s.id);html+='<label style="display:flex;align-items:center;gap:10px;padding:10px;background:var(--bg);border-radius:8px;margin-bottom:8px;cursor:'+(disabled?"not-allowed":"pointer")+';">';html+='<input type="checkbox" value="'+s.id+'" '+(checked?"checked":"")+(disabled?" disabled":"")+(disabled?"":' onchange="enforceTeamLimit(this)"')+ ' style="width:18px;height:18px;">';html+='<div><div style="font-weight:600;"'+(disabled?' style="color:var(--muted);"':'')+">"+ name+"</div>";if(disabled)html+='<div style="font-size:11px;color:var(--muted);">Allerede på et lag</div>';html+="</div></label>"});if(!allShooters.length)html='<div style="color:var(--muted);font-size:13px;">'+d('no_shooters_in_match')+'</div>';container.innerHTML=html}function enforceTeamLimit(el){const checked=o("team-shooter-list").querySelectorAll("input:checked");if(checked.length>4){el.checked=false;alert("Maximum 4 shooters per team")}}async function saveTeam(){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match)return;const name=(o("team-name-input").value||"").trim();if(!name){alert("Team name cannot be empty");return;}const country=(o("team-country-input")&&o("team-country-input").value)||"";const checkboxes=o("team-shooter-list").querySelectorAll("input[type=checkbox]:checked");const shooterIds=Array.from(checkboxes).map(c=>c.value);if(shooterIds.length>4){alert("Maximum 4 shooters per team");return;}match.teams=match.teams||[];if(currentEditTeamIdx!==null){match.teams[currentEditTeamIdx]={...match.teams[currentEditTeamIdx],name,country,shooterIds}}else{match.teams.push({id:"team_"+Date.now(),name,country,shooterIds})}const res=await Ee(match.id,{teams:match.teams});if(res.success){G("modal-create-team");renderTeamsScreen()}else alert("Could not save team: "+res.error)}async function deleteTeam(idx){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match||!match.teams)return;const team=match.teams[idx];if(!confirm("Delete team "+team.name+"?"))return;match.teams.splice(idx,1);const res=await Ee(match.id,{teams:match.teams});if(res.success)renderTeamsScreen();else alert("Could not delete team: "+res.error)}function ss(e){document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active")),document.querySelectorAll(".tab-item").forEach(s=>s.classList.remove("active")),o(e).classList.add("active");const i=["screen-home","screen-matches","screen-prognose","screen-results","screen-teams","screen-profile"].indexOf(e),t=document.querySelectorAll(".tab-item");t[i]&&t[i].classList.add("active");document.querySelectorAll(".tab-bar").forEach(function(bar){var items=bar.querySelectorAll(".tab-item");if(items[i])items[i].classList.add("active");}),e==="screen-home"&&te(),e==="screen-matches"&&fe(),e==="screen-results"&&_e(),e==="screen-teams"&&(renderTeamsScreen(),R&&pt(R))}function ie(e){o(e).classList.add("open"),e==="modal-upload-result"&&as(),e==="modal-add-shooter"&&icUpdateNewShooterPFOptions(),e==="modal-add"&&icInitManualResult()}async function as(){const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e)return;const i=o("upload-match-name");i&&(i.textContent=e.name||"Ukjent match");const t=o("upload-stage-select"),s=icStageDefs(e);if(t.innerHTML="",s.length>0)s.forEach(n=>{const r=document.createElement("option");r.value=n.number,r.textContent=n.name&&n.name!=="Stage "+n.number?`Stage ${n.number} - ${n.name}`:`Stage ${n.number}`,t.appendChild(r)});else{const n=document.createElement("option");n.value="",n.textContent="Ingen stages - opprett ny",t.appendChild(n)}const a=o("upload-shooter-select");a.innerHTML="";const n=new Set,r=icCurrentShooterId(),u=document.createElement("option");u.value=r,u.textContent="Meg ("+(g.firstName||"")+" "+(g.lastName||"")+")",a.appendChild(u),n.add(r);if(e.participants&&e.participants.length>0)for(const m of e.participants)if(!n.has(m))try{const b=await Ut(m);if(b){const f=document.createElement("option");f.value=m,f.textContent=(b.firstName||"")+" "+(b.lastName||""),a.appendChild(f),n.add(m)}}catch(b){console.error("Error fetching user:",m,b)}e.shooters&&e.shooters.length>0&&e.shooters.forEach(m=>{if(!n.has(m.id)){const b=document.createElement("option");b.value=m.id,b.textContent=(m.firstName||"")+" "+(m.lastName||""),a.appendChild(b),n.add(m.id)}})}function G(e){o(e).classList.remove("open")}function ns(e,i){e.target.id===i&&G(i)}function rs(e,i){ue=e,document.querySelectorAll(".filter-chip").forEach(t=>t.classList.remove("active")),i.classList.add("active"),fe()}async function os(){var t;const e={name:F("new-match-name")||"Ny match",type:F("new-match-type")||"Trening",date:F("new-match-date")||new Date().toISOString().split("T")[0],location:F("new-match-location")||"",plannedStages:A("new-match-stages",6),searchable:((t=o("new-match-searchable"))==null?void 0:t.checked)!==!1,status:"active",stages:[],shooters:[],referenceShootersEnabled:!1,referenceShooterIds:[],referenceOverrides:{}};showSpinner("Creating match…");zt(e).then(function(i){hideSpinner();if(i.success){let s=0;Promise.all(ve.map(function(a){return Ge(a.email,{matchId:i.matchId,matchName:e.name})})).then(function(results){results.forEach(function(r){if(r.success)s++});G("modal-new-match");o("new-match-name").value="";o("new-match-location").value="";o("new-match-stages").value="6";o("new-match-searchable")&&(o("new-match-searchable").checked=!0);o("new-match-user-search").value="";o("new-match-search-results").innerHTML="";ve=[];if(s>0)alert("Match opprettet! Invitasjoner sendt til "+s+" bruker(e).")})}else alert("Could not create match: "+i.error)})}async function ls(){const e=F("match-id-search").trim();if(!e){alert("Enter a match ID");return}const i=await Kt(e);if(i.success){const t=$.findIndex(s=>s.id&&s.id.toString()===i.match.id.toString());t!==-1?R=$[t].id:($.unshift(i.match),R=i.match.id),pt(R),oe&&oe("screen-home"),o("match-id-search").value=""}else alert(`Fant ingen match med ID ${e}${i.error?": "+i.error:""}`)}function pt(e){R=e!=null?e.toString():e;const i=$.find(t=>t.id!=null&&t.id.toString()===R);if(i){const t=i.id?"Match ID "+i.id+" "+i.name:i.name||"Match";["home-chip-name","prog-chip-name","results-chip-name","teams-chip-name"].forEach(a=>{const n=o(a);n&&(n.textContent=t)})}xe&&xe(),R&&(xe=Wt(R,t=>{const s=$.findIndex(a=>a.id!=null&&a.id.toString()===R);s!==-1&&t&&($[s]=t,te(),_e())})),te(),_e(),De(),fe()}function ds(e){const i=e+"-match-dropdown",t=o(i);if(!t)return;if(t.classList.contains("open")){t.classList.remove("open");return}document.querySelectorAll(".match-dropdown").forEach(a=>a.classList.remove("open"));let s="";$.length===0?s+='<div class="match-dropdown-item" style="color:var(--muted);text-align:center;">Ingen matcher ennå</div>':$.forEach(a=>{const n=a.id!=null&&a.id.toString()===String(R),r=a.id?"Match ID "+a.id+" "+a.name:a.name||"Match";s+='<div class="match-dropdown-item '+(n?"active":"")+`" onclick="selectMatchFromDropdown('`+a.id+"', '"+e+`')">`,s+='<div class="match-dropdown-dot"></div>',s+='<div class="match-dropdown-info">',s+='<div class="match-dropdown-name">'+r+"</div>",s+='<div class="match-dropdown-meta">'+We(a.date)+" · "+a.location+"</div>",s+="</div>",s+="</div>"}),t.innerHTML=s,t.classList.add("open")}function cs(e,i){pt(e);const t=i+"-match-dropdown",s=o(t);s&&s.classList.remove("open")}function ps(){const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e){alert("No match selected");return}o("edit-match-name").value=e.name||"",o("edit-match-type").value=e.type||"Trening",o("edit-match-date").value=e.date||"",o("edit-match-location").value=e.location||"",o("edit-match-stages").value=e.plannedStages||0,o("edit-match-searchable")&&(o("edit-match-searchable").checked=e.searchable!==!1),o("edit-match-finished")&&(o("edit-match-finished").checked=e.status==="finished");const i=o("edit-match-rival");i.innerHTML='<option value="">Ingen rival valgt</option>';const t=document.createElement("option");t.value="me",t.textContent="Meg ("+(g.firstName||"")+" "+(g.lastName||"")+")",i.appendChild(t),e.shooters&&e.shooters.length>0&&e.shooters.forEach(n=>{const r=document.createElement("option");r.value=n.id,r.textContent=n.firstName+" "+n.lastName,i.appendChild(r)}),e.rivalId&&(i.value=e.rivalId),o("edit-match-user-search").value="",o("edit-match-search-results").innerHTML="",me=[],initReferenceEditState(e),o("edit-match-reference-enabled")&&(o("edit-match-reference-enabled").checked=!!e.referenceShootersEnabled),renderReferenceShooterConfig();const s=Pe(),a=o("delete-match-btn");a&&s&&e.ownerId===s.uid?a.style.display="block":a&&(a.style.display="none"),icRenderEditMatchShootersList(e),ie("modal-edit-match")}async function us(){var s,a;const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e){alert("No match selected");return}refEditState.enabled=!!((o("edit-match-reference-enabled")||{}).checked);const i={name:F("edit-match-name")||e.name,type:F("edit-match-type")||e.type,date:F("edit-match-date")||e.date,location:F("edit-match-location")||e.location,plannedStages:A("edit-match-stages",e.plannedStages),searchable:((s=o("edit-match-searchable"))==null?void 0:s.checked)!==!1,status:(a=o("edit-match-finished"))!=null&&a.checked?"finished":"active",rivalId:F("edit-match-rival")||null,referenceShootersEnabled:!!refEditState.enabled,referenceShooterIds:(refEditState.ids||[]).slice(),referenceOverrides:JSON.parse(JSON.stringify(refEditState.overrides||{}))};showSpinner("Saving match…");Ee(e.id,i).then(function(t){hideSpinner();if(t.success){let n=0;Promise.all(me.map(function(r){return Ge(r.email,{matchId:e.id,matchName:i.name})})).then(function(results){results.forEach(function(r){if(r.success)n++});G("modal-edit-match");if(n>0)alert("Match oppdatert! Invitasjoner sendt til "+n+" bruker(e).")})}else alert("Could not update match: "+t.error)})}function gs(){const e=$.find(t=>t.id!=null&&t.id.toString()===String(R));if(!e){alert("No match selected");return}const i=e.id?"Match ID "+e.id+" "+e.name:e.name;o("delete-match-name").textContent=i,ie("modal-confirm-delete")}async function vs(){const e=$.find(t=>t.id!=null&&t.id.toString()===String(R));if(!e){alert("No match selected");return}showSpinner("Deleting match…");Gt(e.id).then(function(i){hideSpinner();if(i.success){G("modal-confirm-delete");G("modal-edit-match");R=null;te();fe();alert("Match deleted")}else alert("Could not delete match: "+i.error)})}let Ce=null,Me=null;function Re(){var i;const e=$.find(t=>t.id!=null&&t.id.toString()===String(R));if(!e){alert("No match selected");return}Ce=null,o("stage-modal-title").textContent=d("create_stage"),o("stage-number").value=(((i=e.stages)==null?void 0:i.length)||0)+1,o("stage-name").value="",o("stage-paper-targets").value=0,o("stage-poppers").value=0,o("stage-plates").value=0,o("stage-no-shoots").value=0,o("stage-bonus-paper-targets").value=0,ie("modal-create-stage")}function ms(){G("modal-edit-match"),Re()}function hs(){G("modal-new-match"),Re()}function fs(e){const i=$.find(s=>s.id!=null&&s.id.toString()===String(R));if(!i||!i.stages||!i.stages[e]){alert("Stage not found");return}Ce=e;const t=i.stages[e];o("stage-modal-title").textContent=d("edit_stage"),o("stage-number").value=t.number||e+1,o("stage-name").value=t.name||"",o("stage-paper-targets").value=t.paperTargets||0,o("stage-poppers").value=t.poppers||0,o("stage-plates").value=t.plates||0,o("stage-no-shoots").value=t.noShoots||0,o("stage-bonus-paper-targets").value=t.bonusPaperTargets||0,ie("modal-create-stage")}function bs(e){const i=o("stage-number"),t=Math.max(1,parseInt(i.value)+e);i.value=t}function ys(e,i){const t=o("stage-"+e),s=Math.max(0,parseInt(t.value)+i);t.value=s}async function ws(){var a;const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e){alert("No match selected");return}const i={number:A("stage-number",1),name:F("stage-name")||"",paperTargets:A("stage-paper-targets",0),poppers:A("stage-poppers",0),plates:A("stage-plates",0),noShoots:A("stage-no-shoots",0),bonusPaperTargets:A("stage-bonus-paper-targets",0),bonusIncluded:true},t=e.stages||[];Ce!==null?t[Ce]=i:t.push(i);showSpinner("Saving stage…");Ee(e.id,{stages:t}).then(function(s){hideSpinner();if(s.success)G("modal-create-stage");else alert("Could not save stage: "+s.error)})}function icOpenEditStageResult(stageNum,shooterId){
  ...
  if(!stageDef){alert("Stage not found");return;}
...
function deleteCurrentStage(){const e=$.find(n=>n.id!=null&&n.id.toString()===String(R));if(!e||Ce===null||Ce===undefined){alert("No stage selected");return;}const t=e.stages||[];const st=t[Ce];const stName=st?(st.name||"Stage "+(st.number||"")):"denne stagen";if(!confirm("Are you sure you want to delete "+stName+"? This cannot be undone.")){return;}showSpinner("Deleting stage…");t.splice(Ce,1);Ee(e.id,{stages:t}).then(function(s){hideSpinner();if(s.success){G("modal-create-stage");}else alert("Could not delete stage: "+s.error);});}
let ee=[],Q=[],ve=[],me=[];function ks(){if(!R){alert("No match selected");return}Q=[],o("user-search-input").value="",o("user-search-results").innerHTML="",o("send-invitations-btn").style.display="none",ie("modal-invite-user")}async function xs(){const e=F("user-search-input").trim();if(e.length===0){o("user-search-results").innerHTML='<p style="color:#9ca3af;text-align:center;">Skriv inn et søk</p>';return}const i=await Ke(e),t=o("user-search-results");if(!t)return;if(i.length===0){t.innerHTML='<p style="color:#9ca3af;text-align:center;">No users found</p>';return}let s='<div style="margin-bottom:10px;font-weight:600;">RESULTATER ('+i.length+"):</div>";i.forEach(a=>{const n=a.displayName||a.email,r=Q.some(m=>m.uid===a.uid),u=n.replace(/'/g,"\\'");s+=`
 ...
 `}),t.innerHTML=s,ut()}function Ss(e,i,t){const s=Q.findIndex(a=>a.uid===e);s>-1?Q.splice(s,1):Q.push({uid:e,email:i,displayName:t}),ut()}function ut(){const e=o("send-invitations-btn");e&&(Q.length>0?(e.style.display="block",e.textContent=`Send invitasjoner (${Q.length} valgt)`):e.style.display="none")}async function Ls(){if(Q.length===0){alert("No users selected");return}const e=$.find(s=>s.id!=null&&s.id.toString()===String(R));if(!e)return;let i=0,t=0;showSpinner("Inviting user…");Promise.all(Q.map(function(s){return Ge(s.email,{matchId:e.id,matchName:e.name})})).then(function(results){var i=0,t=0;results.forEach(function(r){if(r.success)i++;else t++});hideSpinner();G("modal-invite-user"),i>0&&alert(`Invitasjoner sendt til ${i} bruker(e)!`),t>0&&alert(`${t} invitasjon(er) feilet.`),Q=[]})}async function Ps(){const e=F("new-match-user-search").trim();if(e.length===0){o("new-match-search-results").innerHTML="";return}const i=await Ke(e),t=o("new-match-search-results");if(!t)return;if(i.length===0){t.innerHTML='<p style="color:#9ca3af;text-align:center;font-size:14px;">No users found</p>';return}let s='<div style="margin-bottom:8px;font-size:14px;font-weight:600;">RESULTATER ('+i.length+"):</div>";i.forEach(a=>{const n=a.displayName||a.email,r=ve.some(m=>m.uid===a.uid),u=n.replace(/'/g,"\\'");s+=`
...
html+="</div>";return html;}function icSeasonAnalysis(matches){
  ...
  else if(avgAP<85)problemer.push("A% at "+avgAP+"% — rom for forbedring i treffbildet");
  if(avgMiss>1)problemer.push("Gjentagende miss ("+avgMiss.toFixed(1)+" per stage) — practice controlled trigger press");
  ...
  if(apSpread>20)problemer.push("Stor variasjon i A% mellom matcher ("+apSpread+" pp) — consistency is the key");
  ...
  // Mental trening
  var mental=[];
  if(apSpread>15)mental.push({t:"Spenningsregulering",b:"Large variation between matches is often caused by uneven activation levels. Find your optimal zone: what do you feel in your body when you are in the flow state? Use this som referansepunkt og juster bevisst før hver match."});
  if(avgMiss>1)mental.push({t:"Indre dialog ved miss",b:"Use SOAL: Stop the thought, Observe without reacting, Accept that it is there, Let it go. The brain remembers the last image — immediately visualize how you wolle skutt riktig og gå videre."});
  mental.push({t:"Season goal",b:"Set a concrete technical goal for the next match — not a placement goal. E.g.: d('tip_hold_a') or d('tip_no_miss'). Mastery goals give you control regarhengig av motstanderne."});
...
function icTrainingSeasonAnalysis(matches){
  ...
  var tips=[];
  if(avgAP<85)tips.push("Slow-fire drills: 20 shots per session, 100% focus on A-zone.");
  else tips.push("Bra A% i trening ("+avgAP+"%) — next step is to maintain this under match pressure.");
...
async function importESSVerify(e){
  ...
  if(!stageNum){alert("Select a stage first");return;}
  const stageDef=icStageDefs(match).find(s=>Number(s.number)===stageNum);
  if(!stageDef){alert("Stage not found in match");return;}
...
<div class="gdpr-content">
 ...
 <p class="gdpr-intro">
 By registering with Insight Dynamics Shooting, you agree that we process your personal data
 in accordance with this privacy policy and the GDPR (General Data Protection Regulation).
 </p>

 <h3>1. Data Controller</h3>
 <p>
 Insight Dynamics Shooting is responsible for the processing of your personal data.
 </p>
 ...
 <li><strong>Personal data:</strong> First name and last name</li>
 <li><strong>IPSC-related information:</strong> Division, category, power factor, region and club</li>
 <li><strong>Performance data:</strong> Match results, stages, shots, times and points from IPSC competitions</li>
 <li><strong>Technical information:</strong> User ID, registration date, last update</li>
 <li><strong>Invitation code:</strong> To verify your access to the service</li>
 </ul>

 <h3>3. Purpose and legal basis</h3>
 <p>We process your personal data for the following purposes:</p>
 <ul>
 <li><strong>User account and authentication:</strong> To create and manage your account (legal basis: consent)</li>
 <li><strong>Training analysis:</strong> To provide insights into your performance and development (legal basis: consent)</li>
 <li><strong>Match data sharing:</strong> To share results with other users you invite (legal basis: consent)</li>
 <li><strong>Technical operation:</strong> To ensure the service functions correctly (legal basis: legitimate interest)</li>
 </ul>

 <h3>4. Storage and technical solution</h3>
 <p>Your data is stored and processed using:</p>
 <ul>
 <li><strong>Google Firebase Authentication:</strong> Secure handling of login and user accounts</li>
 <li><strong>Google Firebase Firestore:</strong> Database storage of user profile and match data</li>
 <li><strong>Google Firebase Cloud Functions:</strong> Validation of invitation codes and server logic</li>
 <li><strong>GitHub Pages:</strong> Hosting of the web application</li>
 </ul>
 <p>
 ...
 <h3>5. Sharing of data</h3>
 <p>Your personal data is only shared in the following cases:</p>
 <ul>
 <li><strong>Match participants:</strong> When you invite other users to view or collaborate on a match, they will see your match results and name</li>
 <li><strong>Technical providers:</strong> Google Firebase and GitHub (data processors) have access to the data as necessary to deliver the service</li>
 </ul>
 <p>We never sell or disclose your data to third parties for marketing purposes.</p>

 <h3>6. Retention period</h3>
 <p>
 Your personal data is stored for as long as you have an active account with us.
 When you delete your account, all your personal data will be deleted within 30 days.
 Match data you have shared with other users will remain visible to them, but the link to your account is removed.
 </p>

 <h3>7. Your rights</h3>
 <p>You have the following rights regarding your personal data:</p>
 <ul>
 <li><strong>Access:</strong> You can at any time request access to the data we hold about you</li>
 <li><strong>Correction:</strong> You can change your profile data directly in the app</li>
 ...
 <li><strong>Complaint:</strong> You can file a complaint with the relevant supervisory authority if you believe we are processing your data unlawfully</li>
 </ul>

 <h3>8. Information security</h3>
 <p>
 We use Firebase Authentication with secure encryption for login.
 All communication between you and the service is encrypted with HTTPS.
 Access control ensures that only you and those you invite can view your match data.
 </p>

 <h3>9. Contact information</h3>
 <p>
 If you have questions about privacy or wish to exercise your rights, please contact us at:<br>
 <a href="mailto:post@insight-dynamics-shooting.org"><strong>post@insight-dynamics-shooting.org</strong></a>
 </p>
 ...
 In the event of significant changes, you will be notified upon login.
 </p>

 <div class="gdpr-consent-box">
 <p><strong>By accepting this declaration you consent to:</strong></p>
 <ul>
 <li>That we store and process your personal data as described above</li>
 <li>That we use Firebase (Google) as data processor</li>
 <li>That match data you share with other users becomes visible to them</li>
 </ul>
 </div>
  ...
  overlay.innerHTML=`
    <div style="background:var(--card);border-radius:16px;padding:20px;width:100%;max-width:420px;max-height:85vh;overflow-y:auto;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:12px;">
        <button onclick="document.getElementById(\'ess-paste-modal\').remove();" style="padding:10px;background:var(--bg3);border:none;border-radius:8px;color:var(--text);font-weight:600;cursor:pointer;font-size:12px;">Cancel</button>
...
window.setAppLang=function setAppLang(lang){
  ...
  var _map={
    ...
    "Lagre lag":"Save team","Team name":"Team name","Land":"Country",
