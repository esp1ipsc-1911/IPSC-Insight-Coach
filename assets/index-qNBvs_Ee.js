...
async function importESSVerify(e){
  ...
  overlay.innerHTML=`
    <div style="background:var(--card);border-radius:16px;padding:20px;width:100%;max-width:420px;max-height:85vh;overflow-y:auto;">
      <div style="font-size:16px;font-weight:700;margin-bottom:8px;">ESS/SSI</div>
...
async function essConfirmPasteResult(mode){
  ...
  if(mode==="all"){
    ...
    const stageDefs=icStageDefs(match);
    for(const sr of allParsed.stages){
      if(essDetectSSI(text)&&stageDefs[sr.num-1])sr.name=stageDefs[sr.num-1].name||("Stage "+sr.num);
...
function openPortalImportModal(){
  ...
  if(existing)existing.remove();
  const overlay=document.createElement("div");
  overlay.id="portal-import-modal";
  overlay.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;";
  overlay.innerHTML=`
    <div style="background:var(--card);border-radius:16px;padding:20px;width:100%;max-width:420px;max-height:85vh;overflow-y:auto;">
      <div style="font-size:16px;font-weight:700;margin-bottom:8px;">Importer rival fra portal</div>
      <div style="font-size:13px;color:var(--muted);margin-bottom:12px;">
        1. Gå til ESS verify-siden for rivalen<br>
        2. Søk opp skytter-ID<br>
        3. Trykk Cmd+A, Cmd+C<br>
        4. Lim inn under
      </div>
      <textarea id="portal-paste-input" style="width:100%;height:160px;background:var(--bg);border:1px solid rgba(255,255,255,.15);border-radius:8px;color:var(--text);padding:10px;font-size:12px;resize:vertical;" placeholder="Lim inn tekst fra verify-siden her..."></textarea>
      <div id="portal-paste-preview" style="display:none;margin-top:10px;padding:10px;background:var(--bg);border-radius:8px;font-size:13px;"></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px;">
        <button onclick="document.getElementById(\'portal-import-modal\').remove();" style="padding:12px;background:var(--bg3);border:none;border-radius:8px;color:var(--text);font-weight:600;cursor:pointer;">Avbryt</button>
        <button onclick="confirmPortalImport()" style="padding:12px;background:var(--accent);border:none;border-radius:8px;color:#000;font-weight:700;cursor:pointer;">Importer</button>
      </div>
    </div>`;
  document.body.appendChild(overlay);
  o("portal-paste-input").addEventListener("input",function(){
    const parsed=portalParseAllStages(this.value);
    const prev=o("portal-paste-preview");
    if(parsed&&prev){
      prev.style.display="block";
      prev.innerHTML="<strong>"+parsed.shooterName+"</strong><br>"+parsed.division+" · "+parsed.pf.toUpperCase()+"<br><span style='color:var(--accent)'>"+parsed.stages.length+" stage"+(parsed.stages.length!==1?"r":"")+" funnet</span>";
    }else if(prev){
      prev.style.display="none";
    }
  });
...
function portalParseAllStages(text){
  ...
  // Auto-detect SSI format
  if(essDetectSSI(text)){
    const ssiRows=essParseSSIAll(text);
    if(!ssiRows||!ssiRows.length)return null;
    const stages=ssiRows.map((r,i)=>({num:i+1,...r}));
    return{shooterName:"SSI import",firstName:"",lastName:"SSI",division:"",pf:"minor",stages};
  }
  try{
    ...
    for(const line of lines){
      ...
      if(m){
        ...
        const hf=parseFloat(m[2]);
        const pts=parseInt(m[3]);
        const a=parseInt(m[4]);
        const c=parseInt(m[5]);
        const d=parseInt(m[6]);
        const miss=parseInt(m[7]);
        const ns=parseInt(m[8]);
        const proc=parseInt(m[9]);
        const time=parseFloat(m[10]);
        stages.push({num,hf,pts,a,c,d,miss,ns,proc,time});
      }
    }
    if(!stages.length)return null;
    return{shooterName,firstName,lastName,division,pf,stages};
  }catch(err){
    console.error("Portal parse error:",err);
    return null;
  }
}

async function confirmPortalImport(){
  const text=o("portal-paste-input")?o("portal-paste-input").value:"";
  const parsed=portalParseAllStages(text);
  if(!parsed||!parsed.stages.length){
    alert("Kunne ikke lese data. Sjekk at teksten er limt inn riktig.");
    return;
  }
  const match=$.find(u=>u.id!=null&&u.id.toString()===String(R));
  if(!match){alert("Ingen aktiv match valgt");return;}
  match.shooters||(match.shooters=[]);
  // Finn eksisterende skytter på navn
  let shooter=match.shooters.find(s=>!s.isMe&&(
    s.lastName.toLowerCase()===parsed.lastName.toLowerCase()||
    (s.firstName.toLowerCase()===parsed.firstName.toLowerCase()&&parsed.firstName)
  ));
  if(!shooter){
    // Ny skytter
    shooter={
      ...
      division:parsed.division||"Standard",
      pf:parsed.pf||"minor",
      club:"",
      stages:[]
    };
    ...
    // Oppdater divisjon/pf om ny info
    if(parsed.division)shooter.division=parsed.division;
    if(parsed.pf)shooter.pf=parsed.pf;
  }
  shooter.stages||(shooter.stages=[]);
  // Upsert alle stages
  for(const sr of parsed.stages){
    const stageResult={
      num:sr.num,
      name:"Stage "+sr.num,
      hf:sr.hf,
      ...
      pts:sr.pts,
      pf:parsed.pf||"minor",
      a:sr.a,c:sr.c,d:sr.d,
      miss:sr.miss,ns:sr.ns,proc:sr.proc
    };
    icUpsertStageResult(shooter,stageResult);
  }
  await Ee(match.id,{shooters:match.shooters});
  const modal=o("portal-import-modal");
  if(modal)modal.remove();
  // Oppdater UI
  te();_e();De();
  alert(parsed.shooterName+" • "+parsed.stages.length+" stage"+(parsed.stages.length!==1?"r":"")+" importert");
}
async function Ks(){ke&&ke(),xe&&xe(),await $t(),window.location.reload()}const ht=document.getElementById("app");ht.innerHTML=`<div style="position:fixed;inset:0;background:#0a0c10;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:9999;" id="loading-screen"><img src="/Logo_IPSC-insight.png" alt="Insight Dynamics Shooting" style="width:88px;height:88px;border-radius:20px;box-shadow:0 8px 32px rgba(232,184,75,0.18);margin-bottom:28px;"><div style="font-size:26px;font-weight:900;color:#f5f7fb;letter-spacing:1px;margin-bottom:8px;">INSIGHT DYNAMICS</div><div style="font-size:14px;color:#7d8598;font-weight:500;margin-bottom:40px;font-style:italic;">Analyse. Presisjon. Resultat.</div><div style="width:180px;height:3px;background:rgba(255,255,255,0.08);border-radius:99px;overflow:hidden;"><div style="height:100%;width:40%;background:#e8b84b;border-radius:99px;animation:ipsc-loading 1.2s ease-in-out infinite;"></div></div><style>@keyframes ipsc-loading{0%{transform:translateX(-100%);width:40%}50%{width:60%}100%{transform:translateX(350%);width:40%}}</style></div>`;function Vs(){Bt(ht,ft)}function ft(){ts(ht)}Ct(e=>{const l=document.getElementById("loading-screen");if(l){l.style.opacity="0";l.style.transition="opacity 0.3s";setTimeout(()=>{l.remove();e?ft():Vs()},300)}else{e?ft():Vs()}});function essParseVerifyText(text,stageNum){
  if(!text||!text.trim())return null;
  // Detect SSI format: lines of tab-separated numbers without stage names
  if(essDetectSSI(text)){
    return essParseSSISingle(text,stageNum);
  }
  try{
    ...
    let stageLine=null;
    for(const line of lines){
      if(line.includes(stageStr)||line.includes(stageStr2)){stageLine=line;break;}
    }
  ...
  }catch(err){console.error("ESS parse error:",err);return null;}
}

function essDetectSSI(text){
  if(!text)return false;
  const lines=text.trim().split("\n").filter(l=>l.trim());
  // SSI: first data line starts with a number (HF), no "Stage" text in data rows
  const dataLines=lines.filter(l=>{
    const t=l.trim();
    return t&&!t.toLowerCase().startsWith("total")&&/^[\d\.\t\s]+/.test(t);
  });
  if(!dataLines.length)return false;
  const firstCols=dataLines[0].trim().split(/[\t]+/);
  // SSI has HF as first col (decimal like 6.6508), no stage label
  return firstCols.length>=5&&!isNaN(parseFloat(firstCols[0]))&&parseFloat(firstCols[0])>0;
}

function essParseSSISingle(text,stageNum){
  const rows=essParseSSIAll(text);
  if(!rows||!rows.length)return null;
  const row=rows[stageNum-1];
  if(!row)return null;
  return{shooterName:"SSI",division:"",pf:"minor",
    hf:row.hf,pts:row.pts,a:row.a,c:row.c,d:row.d,
    miss:row.miss,ns:row.ns,proc:row.proc,time:row.time};
}
...
function essParseSSIAll(text){
  ...
  for(const line of lines){
    ...
    const hf=parseFloat(cols[0])||0;
    const time=parseFloat(cols[1])||0;
    const pts=parseInt(cols[2])||0;
    const a=parseInt(cols[3])||0;
    const c=parseInt(cols[4])||0;
    const d=parseInt(cols[5])||0;
    const miss=parseInt(cols[6])||0;
    const proc=parseInt(cols[7])||0;
    const ns=parseInt(cols[8])||0;
    if(hf>0&&time>0)rows.push({hf,time,pts,a,c,d,miss,ns,proc});
  }
  return rows;
}
}
