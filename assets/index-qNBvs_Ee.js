...
async function Jt(e){const i=W();if(!i)return{success:!1,error:"Authentication required"};try{const matchDoc=await be(U(O,"matches",e.toString()));if(!matchDoc.exists())return{success:!1,error:"Match could not be found"};const matchData=matchDoc.data();const existingParticipants=matchData.participants||[];const alreadyParticipant=existingParticipants.includes(i.uid);const newParticipants=alreadyParticipant?existingParticipants:[...existingParticipants,i.uid];const existingShooters=matchData.shooters||[];const alreadyShooter=existingShooters.some(function(sh){return sh.uid===i.uid||sh.id===i.uid;});const _prof=Pe()||{};const newShooters=alreadyShooter?existingShooters:[...existingShooters,{id:i.uid,isMe:!1,uid:i.uid,firstName:_prof.firstName||"",lastName:_prof.lastName||"",division:_prof.division||"Standard",pf:(_prof.powerFactor||"minor").toLowerCase(),club:_prof.club||"",stages:[]}];const newInvitedUids=(matchData.invitedUids||[]).filter(function(uid){return uid!==i.uid;});const newInvitedUsers=(matchData.invitedUsers||[]).filter(function(u){return u&&u.uid!==i.uid;});// Step 1: add to participants only (non-participant rule: hasOnly(['participants']))
if(!alreadyParticipant){await de(U(O,"matches",e.toString()),{participants:newParticipants});}// Phase 1 done - user is now a participant. Run phase 2 in background.
const _phase2=Promise.all([de(U(O,"matches",e.toString()),{shooters:newShooters,updatedAt:Ie()}),de(U(O,"matches",e.toString()),{invitedUids:newInvitedUids,invitedUsers:newInvitedUsers,updatedAt:Ie()}),de(U(O,"users",i.uid,"invitations",e.toString()),{status:"accepted"})]).catch(function(err){console.warn("[Jt] Phase 2 background error:",err);});return{success:!0,phase2:_phase2}}catch(t){return console.error("[Jt] Uventet feil:",t),{success:!1,error:t.message}}}
