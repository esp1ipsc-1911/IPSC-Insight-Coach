...
const mentalTips=[];
if(totalMiss>stageCount*0.5)mentalTips.push({t:"Indre dialog ved miss",b:"Bruk SOAL: Stopp tanken, Observer den uten følelser, Aksepter den, La den gå. Ikke bli hengende i feilen - neste stage er det som teller."});
if(stageAPs.length>1){const _aps=stageAPs.map(function(x){return x.ap;});if(Math.max.apply(null,_aps)-Math.min.apply(null,_aps)>20)mentalTips.push({t:"Visualisering mellom stages",b:"Visualiser neste stage i førsteperson før du går inn - kropp, miljø, timing og følelse. Hjernen skiller ikke mellom tanke og handling."});}
if(overallAP>=90&&avgHF<3.5)mentalTips.push({t:"Spenningsregulering opp",b:"Bra treffbilde, men tempoet kan økes. Prøv energigivende musikk i oppvarmingen, eller visualiser stagene med høy intensitet og god flyt."});
if(totalProc>0)mentalTips.push({t:"Konsentrasjon og stage-plan",b:"Procedurals oppstår ofte når konsentrasjonen er rettet feil sted. Gå gjennom stage-briefen med 100% fokus - se for deg bevegelsene steg for steg."});
const _gt=[{t:"Triggerord",b:"Velg 1-2 ord som trigger riktig tilstand: Pust, Fokus eller Jeg er klar. Unngå ikke-ord - hjernen filtrerer dem ut."},{t:"Selvtillitsboost",b:"Husk en tidligere stage der du presterte godt. Gjenopplev følelsen - bruk den som drivstoff før neste stage."},{t:"Pustepause",b:"Pust rolig og dypt mellom stages. Stress gjør pusten grunn og rask, noe som svekker koordinasjon og konsentrasjon."},{t:"Målsetting",b:"Sett ett prosessmål for neste trening - eks: Holde fokus gjennom hele stage-briefen."},{t:"Positiv indre dialog",b:"Du er din egen viktigste støttespiller. Bytt ut selvkritikk med: Hva kan jeg gjøre bedre?"}];
