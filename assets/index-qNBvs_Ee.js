...
(function(){
  ...
  // Prognose: dagsform = snitt HF fra skutte stages
  shooterData.forEach(function(sd){
  ...
  // Convert avgHF to % of best avgHF
  var bestAvgHF=Math.max.apply(null,shooterData.map(function(sd){return sd.avgHF;}));
  shooterData.forEach(function(sd){
    sd.prognPct=bestAvgHF>0?(sd.avgHF/bestAvgHF*100):0;
  });
  ...
  // Prognose SVG - stiplet horisontallinje per skytter (dagsform)
  ...
  sortedByPrg.forEach(function(sd){
    if(!sd.avgHF||sd.avgHF<=0)return;
    var yPrg=PT+gH*(1-sd.prognPct/100);
    var x1=PL,x2=W-PR;
    // Dashed line across all stages
    svgPrg+='<line x1="'+x1+'" y1="'+yPrg.toFixed(1)+'" x2="'+x2+'" y2="'+yPrg.toFixed(1)+'" stroke="'+sd.color+'" stroke-width="'+(sd.isMe?'2.5':'1.5')+'" stroke-dasharray="5,3" opacity="'+(sd.isMe?'1':'0.75')+'"  />';
    // Dot at right end
    svgPrg+='<circle cx="'+x2+'" cy="'+yPrg.toFixed(1)+'" r="'+(sd.isMe?'3.5':'2.5')+'" fill="'+sd.color+'" opacity="'+(sd.isMe?'1':'0.75')+'"/>';
    // HF label at right
    svgPrg+='<text x="'+(x2-4)+'" y="'+(yPrg-4)+'" font-size="7" fill="'+sd.color+'" text-anchor="end" opacity="0.9">'+sd.avgHF.toFixed(2)+'</text>';
  });
  svgPrg+='</svg>';
  var legPrg=buildLegend(sortedByPrg);
  var tblPrg='<div style="margin-top:12px;">';
  tblPrg+='<div style="font-size:11px;color:var(--muted);margin-bottom:8px;">'+d('avg_hf')+' (dagsform)</div>';
...
if(overallAP>=90&&avgHF<3.5)mentalTips.push({t:"Activation regulation",b:"The body is ready for more tempo. Use an energizing trigger word like Explosive or Now, and visualize stages with high intensity and flow. Inner motivatsjon driver raskere og mer stabilt enn ytre press."});
