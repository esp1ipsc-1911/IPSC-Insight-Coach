...
<div class="modal-overlay" id="modal-invite-user" onclick="closeModalOutside(event,'modal-invite-user')">
 ...
 <div id="invite-result-msg" style="display:none;padding:10px;border-radius:8px;background:rgba(34,197,94,0.12);text-align:center;font-weight:600;margin-bottom:10px;"></div><button id="send-invitations-btn" class="btn-primary" onclick="sendMultipleInvitations()" style="margin-top:15px;display:none;">Send Invitations (0 selected)</button>
 </div>
 </div>
</div>
...
var _searchDebounceTimer=null;function _debounceSearch(fn,delay){clearTimeout(_searchDebounceTimer);_searchDebounceTimer=setTimeout(fn,delay||400);}
function searchUsersEditMatchLive(){_debounceSearch(function(){window.searchUsersEditMatch&&window.searchUsersEditMatch();},400);}
function searchUsersNewMatchLive(){_debounceSearch(function(){window.searchUsersNewMatch&&window.searchUsersNewMatch();},400);}
function searchUsersLive(){_debounceSearch(function(){window.searchUsers&&window.searchUsers();},400);}
window.searchUsersEditMatchLive=searchUsersEditMatchLive;window.searchUsersNewMatchLive=searchUsersNewMatchLive;window.searchUsersLive=searchUsersLive;
...
let ee=[],Q=[],ve=[],me=[];function ks(){if(!R){alert("No match selected");return}Q=[],o("user-search-input").value="",o("user-search-results").innerHTML="",o("send-invitations-btn").style.display="none",ie("modal-invite-user")}async function xs(){const e=F("user-search-input").trim();if(e.length===0){o("user-search-results").innerHTML='<p style="color:#9ca3af;text-align:center;">Skriv inn et søk</p>';return}const i=await Ke(e),t=o("user-search-results");if(!t)return;if(i.length===0){t.innerHTML='<p style="color:#9ca3af;text-align:center;">No users found.</p>';return}let s='<div style="margin-bottom:10px;font-weight:600;">RESULTS ('+i.length+"):</div>";i.forEach(a=>{const n=a.displayName||a.email,r=Q.some(m=>m.uid===a.uid),u=n.replace(/'/g,"\\'");s+=`
 ...
 `}),t.innerHTML=s,ut()}function Ss(e,i,t){const s=Q.findIndex(a=>a.uid===e);s>-1?Q.splice(s,1):Q.push({uid:e,email:i,displayName:t}),ut()}function ut(){const e=o("send-invitations-btn");e&&(Q.length>0?(e.style.display="block",e.textContent=`Send Invitations (${Q.length} selected)`):e.style.display="none")}async function Ls(){if(Q.length===0){alert("No users selected");return}const e=$.find(s=>s.id!=null&&s.id.toString()===String(R));if(!e)return;showSpinner("Sending invitation…");Promise.all(Q.map(function(s){return Ge(s.email,{matchId:e.id,matchName:e.name})})).then(function(results){var sent=0,failed=0;results.forEach(function(r){if(r.success)sent++;else failed++;});hideSpinner();const resultEl=document.getElementById("invite-result-msg");if(resultEl){resultEl.textContent=sent>0?"Invitation"+(sent>1?"s":"")+" sent to "+sent+" user"+(sent>1?"s":"")+"! ✓":"Could not send invitation.";resultEl.style.color=sent>0?"#22c55e":"#ef4444";resultEl.style.display="block";setTimeout(function(){resultEl.style.display="none";G("modal-invite-user");},2500);}else{if(sent>0)alert("Invitation"+(sent>1?"s":"")+" sent to "+sent+" user"+(sent>1?"s":"")+"!");G("modal-invite-user");}if(failed>0)console.warn(failed+" invitation(s) could not be sent.");Q=[];})}async function Ps(){const e=F("new-match-user-search").trim();if(e.length===0){o("new-match-search-results").innerHTML="";return}const i=await Ke(e),t=o("new-match-search-results");if(!t)return;if(i.length===0){t.innerHTML='<p style="color:#9ca3af;text-align:center;font-size:14px;">No users found.</p>';return}let s='<div style="margin-bottom:8px;font-size:14px;font-weight:600;">RESULTS ('+i.length+"):</div>";i.forEach(a=>{const n=a.displayName||a.email,r=ve.some(m=>m.uid===a.uid),u=n.replace(/'/g,"\\'");s+=`
