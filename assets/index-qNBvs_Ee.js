...
<div id="global-spinner"><div class="spinner-ring"></div><div class="spinner-text" id="spinner-text"></div></div>
<div class="phone">
...
<div class="modal-overlay" id="modal-create-team" onclick="closeModalOutside(event,'modal-create-team')">
 ...
 `}),t.innerHTML=s,ut()}function Ss(e,i,t){const s=Q.findIndex(a=>a.uid===e);s>-1?Q.splice(s,1):Q.push({uid:e,email:i,displayName:t}),ut()}function ut(){const e=o("send-invitations-btn");e&&(Q.length>0?(e.style.display="block",e.textContent=`Send invitasjoner (${Q.length} valgt)`):e.style.display="none")}async function Ls(){if(Q.length===0){alert("Ingen brukere valgt");return}const e=$.find(s=>s.id!=null&&s.id.toString()===String(R));if(!e)return;let i=0,t=0;showSpinner("Inviterer bruker…");for(const s of Q)(await Ge(s.email,{matchId:e.id,matchName:e.name})).success?i++:t++;hideSpinner();G("modal-invite-user"),i>0&&alert(`Invitasjoner sendt til ${i} bruker(e)!`),t>0&&alert(`${t} invitasjon(er) feilet.`),Q=[]}async function Ps(){const e=F("new-match-user-search").trim();if(e.length===0){o("new-match-search-results").innerHTML="";return}const i=await Ke(e),t=o("new-match-search-results");if(!t)return;if(i.length===0){t.innerHTML='<p style="color:#9ca3af;text-align:center;font-size:14px;">Ingen brukere funnet</p>';return}let s='<div style="margin-bottom:8px;font-size:14px;font-weight:600;">RESULTATER ('+i.length+"):</div>";i.forEach(a=>{const n=a.displayName||a.email,r=ve.some(m=>m.uid===a.uid),u=n.replace(/'/g,"\\'");s+=`
...
s+='<tr style="border-top:2px solid var(--accent);font-weight:700;background:rgba(232,184,75,0.06);">';
s+='<td style="padding:8px 6px;">SUM</td>';
s+='<td style="padding:8px 6px;text-align:right;">'+sumTime.toFixed(2)+'s</td>';
...
s+='<td style="padding:8px 6px;text-align:right;">'+sumC+'<br><span style="font-size:10px;color:var(--muted);">'+cPct+'%</span></td>';
s+='<td style="padding:8px 6px;text-align:right;">'+sumD+'<br><span style="font-size:10px;color:var(--muted);">'+dPct+'%</span></td>';
