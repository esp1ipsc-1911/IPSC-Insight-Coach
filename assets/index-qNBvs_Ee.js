...
let matchDoc;
try{matchDoc=await be(U(O,"matches",e.toString()));}catch(e1){return{success:!1,error:"Steg1: "+e1.message};}
if(!matchDoc.exists())return{success:!1,error:"Match could not be found"};
const matchData=matchDoc.data();
const existingParticipants=matchData.participants||[];
const newParticipants=existingParticipants.includes(i.uid)?existingParticipants:[...existingParticipants,i.uid];
// Steg 2: Hent brukerens profildata
let userProfile=null;
try{userProfile=await Ut(i.uid);}catch(eu){console.error("[Jt] Kunne ikke hente brukerprofil:",eu.message);}
// Steg 3: Legg bruker til i shooters-arrayen hvis ikke allerede der
const existingShooters=matchData.shooters||[];
const alreadyShooter=existingShooters.find(function(s){return String(s.id)===String(i.uid);});
let newShooters=existingShooters;
if(!alreadyShooter){
  const newShooter={
    id:i.uid,
    isMe:false,
    firstName:(userProfile&&userProfile.firstName)||"Shooter",
    lastName:(userProfile&&userProfile.lastName)||"",
    division:(userProfile&&userProfile.division)||"Classic",
    pf:(userProfile&&userProfile.powerFactor)||"minor",
    club:(userProfile&&userProfile.club)||"",
    stages:[]
  };
  newShooters=[...existingShooters,newShooter];
}
// Steg 4: Oppdater match med participants og shooters
try{await de(U(O,"matches",e.toString()),{participants:newParticipants,shooters:newShooters});}catch(e2){return{success:!1,error:"Steg2: "+e2.message};}
// Steg 5: Marker invitasjonen som akseptert
try{await de(U(O,"users",i.uid,"invitations",e.toString()),{status:"accepted"});}catch(e3){return{success:!1,error:"Steg3: "+e3.message};}
...
<div class="modal-overlay" id="modal-edit-match" onclick="closeModalOutside(event,'modal-edit-match')">
 ...
 <button class="btn-primary" onclick="saveEditMatch()" style="padding:12px 8px;font-size:13px;">${d("save")}</button>
 <button id="edit-match-finished-btn" onclick="toggleMatchFinished()" style="padding:12px 8px;background:var(--bg3);color:var(--text);border:1px solid var(--border);border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;letter-spacing:0.04em;">MARK FINISHED</button>
 <button id="delete-match-btn" onclick="confirmDeleteMatch()" style="padding:12px 8px;background:#ef4444;color:white;border:none;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;display:none;letter-spacing:0.04em;">DELETE</button>
 </div>
 </div>
 </div>
</div>
...
function essSelectAllStages(checked){
  var boxes=document.querySelectorAll("#ess-stage-checkboxes .ess-match-chk");
...
function renderProfileMatchTips(){
  var container=o("profile-match-tips");
  if(!container||!$.length){if(container)container.innerHTML="";return;}
  var myMatches=$.filter(function(m){var s=icCurrentShooter(m);return s&&s.stages&&s.stages.length>0;});
...
`
  ...
  overlay.innerHTML=`
    <div style="background:var(--card);border-radius:16px;padding:20px;width:100%;max-width:420px;max-height:85vh;overflow-y:auto;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;"><div style="font-size:15px;font-weight:700;">ESS/SSI Import</div><button onclick="this.closest('.modal-overlay').classList.remove('open')" style="background:none;border:none;color:var(--muted);font-size:20px;cursor:pointer;line-height:1;padding:0 4px;">&times;</button></div>
  ...
  o("ess-paste-input").addEventListener("input",function(){
    ...
    if(allParsed&&allParsed.stages&&allParsed.stages.length&&prev){
      ...
      // Populate stage checkboxes - show all match stages, user maps to SSI rows
      ...
      if(stageSelDiv&&stageChkDiv){
        ...
        var ssiRows=allParsed.stages;
        var ssiOpts='<option value="">—</option>'+ssiRows.map(function(sr){
          return '<option value="'+sr.num+'">S'+sr.num+(sr.hf>0?' HF '+sr.hf.toFixed(2):'')+'</option>';
        }).join('');
        var matchItems=stageDefs2.map(function(sd){
          var defaultSsi=ssiRows.find(function(sr){return sr.num===sd.number;})?sd.number:'';
          var opts=ssiOpts.replace('value="'+defaultSsi+'"','value="'+defaultSsi+'" selected');
          return '<div style="display:flex;align-items:center;gap:5px;padding:3px 0;">'
            +'<input type="checkbox" class="ess-match-chk" data-stagenum="'+sd.number+'" '+(defaultSsi?'checked':'')+' style="width:15px;height:15px;flex-shrink:0;accent-color:var(--accent);">'
            +'<span style="font-size:11px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+(sd.name||'Stage '+sd.number)+'</span>'
            +'<select class="ess-ssi-sel" data-stagenum="'+sd.number+'" style="font-size:10px;padding:2px 3px;background:var(--bg3);border:1px solid rgba(255,255,255,.15);border-radius:4px;color:var(--text);max-width:80px;">'+opts+'</select>'
            +'</div>';
        });
        var half=Math.ceil(matchItems.length/2);
        stageChkDiv.innerHTML='<div style="display:grid;grid-template-columns:1fr 1fr;gap:0 10px;">'
          +'<div>'+matchItems.slice(0,half).join('')+'</div>'
          +'<div>'+matchItems.slice(half).join('')+'</div>'
          +'</div>';
...
async function essConfirmPaste(mode){
  ...
  if(mode==="all"||mode==="selected"){
    ...
    // Filter to selected stages if mode==="selected" — read match-stage checkboxes + SSI mapping
    if(mode==="selected"){
      var chkBoxes=document.querySelectorAll("#ess-stage-checkboxes .ess-match-chk:checked");
      if(!chkBoxes.length){alert("No stages selected.");return;}
      var mappedStages=[];
      Array.from(chkBoxes).forEach(function(cb){
        var matchStageNum=parseInt(cb.dataset.stagenum);
        var selEl=document.querySelector(".ess-ssi-sel[data-stagenum='"+matchStageNum+"']");
        var ssiRowNum=selEl?parseInt(selEl.value):NaN;
        if(isNaN(ssiRowNum)||!ssiRowNum)return;
        var ssiRow=allParsed.stages.find(function(sr){return sr.num===ssiRowNum;});
        if(!ssiRow)return;
        // Import this SSI row but assign it to the chosen match stage number
        mappedStages.push(Object.assign({},ssiRow,{num:matchStageNum}));
      });
      if(!mappedStages.length){alert("No valid stage mappings found. Please select an SSI row for each checked stage.");return;}
      allParsed={shooterName:allParsed.shooterName,stages:mappedStages};
    }
...
function essParseVerifyText(text,stageNum){
  ...
  try{
    ...
    var hf=parseFloat((cols[2]||"").replace(/,/g,"."))||0;
...
function essParseSSIAll(text){
  ...
  for(var li=0;li<lines.length;li++){
    ...
    var hf=parseFloat((cols[0]||"").replace(/,/g,"."))||0;
    var time=parseFloat((cols[1]||"").replace(/,/g,"."))||0;
