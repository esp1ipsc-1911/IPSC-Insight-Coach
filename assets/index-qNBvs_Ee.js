...
})();const t=Pe();const[i,_matches]=await Promise.all([Promise.resolve(Pe()),Vt()]);i?g=i:g={firstName:t.name||((s=t.email)==null?void 0:s.split("@")[0])||"",lastName:"",division:"",category:"",powerFactor:"",region:"",club:"",draw:null,reloadTime:null};$=_matches;loadReferenceShooters().then(refs=>{refShooters=refs;}).catch(()=>{});if($&&$.length>0){const a=new Date,n=$.filter(r=>r.status!=="finished"&&r.date);if(n.length>0){let r=n[0],u=Math.abs(new Date(n[0].date)-a);for(const m of n){const b=new Date(m.date),f=Math.abs(b-a);f<u&&(u=f,r=m)}R=r.id}}ke&&ke(),ke=qt(a=>{$=a,fe(),te(),De()}),ze_unsub&&ze_unsub(),ze_unsub=Zt(a=>{ee=a,Ye()}),e.innerHTML=`
