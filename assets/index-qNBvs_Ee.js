...
function toggleLiveFilter(){liveShowAll=!liveShowAll,_e()}function toggleTips(id){const el=document.getElementById(id);if(el){const open=el.style.display!=="none";el.style.display=open?"none":"block";const ch=document.getElementById("chev-"+id);if(ch)ch.style.transform=open?"rotate(0deg)":"rotate(180deg)";}}async function deleteRun(stageNum,runIdx){const match=$.find(e=>e.id!=null&&e.id.toString()===String(R));if(!match)return;const shooter=icCurrentShooter(match);if(!shooter)return;const stageIdx=shooter.stages.findIndex(s=>(s.num||s.number)==stageNum);if(stageIdx<0)return;const stage=shooter.stages[stageIdx];if(!stage.runs||stage.runs.length<=1){alert("Cannot delete the last run. Delete the entire result instead.");return;}if(!confirm("Delete Run "+(runIdx+1)+"?"))return;stage.runs.splice(runIdx,1);stage.runs.forEach((r,i)=>{r.runNum=i+1});const best=stage.runs.reduce((a,b)=>(b.hf||0)>(a.hf||0)?b:a,stage.runs[0]);Object.assign(stage,best,{num:stage.num,name:stage.name,runs:stage.runs});const res=await Ee(match.id,{shooters:match.shooters});if(res.success){te(),_e(),De()}else alert("Unable to delete run: "+res.error);}function icTeamStandings(e){
  ...
  standings.forEach(function(t,i){
    var pct=topPts>0?(t.totalPts/topPts*100).toFixed(4):"0.0000";
    ...
    sh+="<td class='standings-td standings-td-pts'>"+t.totalPts.toFixed(4)+"</td>";
    ...
    (t.stageBreakdown||[]).forEach(function(sb){
      ...
      sh+="<td style='padding:4px 4px;text-align:right;font-weight:600;'>"+sb.stagePts.toFixed(4)+"</td>";
...
(function(){
  ...
  sortedByPrg.forEach(function(sd){
    ...
    if(aPts.length>0){
      ...
      svgPrg+='<text x="'+(lp.x+4)+'" y="'+(lp.y-3)+'" font-size="7" fill="'+sd.color+'" text-anchor="start" opacity="0.9">'+lp.hf.toFixed(4)+'</text>';
    }
  });
  svgPrg+='</svg>';
  ...
  sortedByPrg.forEach(function(sd,rowIdx){
    ...
    tblPrg+='<td style="padding:6px 4px;text-align:right;color:var(--muted);">'+(sd.formHF>0?sd.formHF.toFixed(4):'—')+'</td>';
    tblPrg+='<td style="padding:6px 4px;text-align:right;font-weight:'+(sd.isMe?'700':'400')+';color:var(--text);">'+(sd.avgHF>0?sd.avgHF.toFixed(4):'—')+'</td>';
    ...
    sd.stagePoints.forEach(function(sp,si){
      ...
      var dStr=delta==null?'—':(delta>=0?'+':'')+delta.toFixed(4);
      ...
      tblPrg+='<td style="padding:3px 4px;text-align:right;color:var(--muted);">'+(formHFStage>0?formHFStage.toFixed(4):'—')+'</td>';
      tblPrg+='<td style="padding:3px 4px;text-align:right;">'+(actualHF?actualHF.toFixed(4):'—')+'</td>';
...
s+='<td style="padding:4px 3px;text-align:right;">'+sumTime.toFixed(4)+'s</td>';
...
html+="</div>";return html;}function icSeasonAnalysis(matches){
  ...
  matchStats.slice(-4).forEach(function(m){
    ...
    html+='<div style="font-weight:700;font-size:16px;color:'+col+';">'+m.avgHF.toFixed(4)+'</div>';
  ...
  if(Math.abs(hfTrend)>0.05){
    ...
    var trendTxt=hfTrend>0?'HF has increased by '+hfTrend.toFixed(4)+' since the first match — positive development.':'HF har falt med '+Math.abs(hfTrend).toFixed(4)+' since the first match — check what changed.';
  ...
  if(bestMatch.ap>=90)styrker.push("Best match: "+bestMatch.name+" ("+bestMatch.avgHF.toFixed(4)+" HF, "+bestMatch.ap+"%A)");
...
function essParseVerifyText(text,stageNum){
  ...
  try{
    ...
    return{shooterName:shooterName,division:division,pf:pf,hf:hf,pts:pts,a:a,c:cc,d:d,miss:miss,ns:ns,proc:proc,time:parseFloat(time.toFixed(4))};
...
function portalParseAllStages(text){
  ...
  try{
    ...
    for(var li=0;li<lines.length;li++){
      ...
      if(m){
        stages.push({num:parseInt(m[1]),hf:parseFloat(m[2]),pts:parseFloat(m[3]),a:parseInt(m[4]),c:parseInt(m[5]),d:parseInt(m[6]),miss:parseInt(m[7]),ns:parseInt(m[8]),proc:parseInt(m[9]),time:parseFloat(m[10])});
      }
    }
