...
if(rActive&&rActive.length>0&&h&&u){
  ...
  if(matchRefs.length>0){
    ...
    matchRefs.forEach(function(ref){
      ...
      html+='<div style="color:'+(dHF>=0?"var(--green)":"var(--red)");
      html+=';font-size:10px;">'+(dHF>=0?"+":"")+dHF.toFixed(2)+'</div></div>';
      if(refAP!=null){
        html+='<div><div style="color:var(--muted);">A%</div>';
        html+='<div style="font-weight:700;">'+refAP+'%</div>';
        html+='<div style="color:'+(dAP>=0?"var(--green)":"var(--red)");
        html+=';font-size:10px;">'+(dAP>=0?"+":"")+dAP+'%</div></div>';
      }
      if(refCP!=null){
        html+='<div><div style="color:var(--muted);">C%</div>';
        html+='<div style="font-weight:700;">'+refCP+'%</div>';
        html+='<div style="color:'+(dCP<=0?"var(--green)":"var(--red)");
        html+=';font-size:10px;">'+(dCP>=0?"+":"")+dCP+'%</div></div>';
      }
      if(refDP!=null){
        html+='<div><div style="color:var(--muted);">D%</div>';
        html+='<div style="font-weight:700;">'+refDP+'%</div>';
        html+='<div style="color:'+(dDP<=0?"var(--green)":"var(--red)");
        html+=';font-size:10px;">'+(dDP>=0?"+":"")+dDP+'%</div></div>';
      }
      html+='</div></div>';
    });
    html+='</div>';
