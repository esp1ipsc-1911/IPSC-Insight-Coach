import{initializeApp as Re}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";import{getAuth as Ae,onAuthStateChanged as je,signInWithEmailAndPassword as Oe,createUserWithEmailAndPassword as Ue,signOut as He}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-auth.js";import{getFirestore as ze,getDoc as q,doc as k,setDoc as ie,query as se,collection as ae,where as ne,getDocs as Pe,onSnapshot as ve,serverTimestamp as X,updateDoc as D,arrayUnion as Se,deleteDoc as Ve}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";import{getFunctions as Ge,httpsCallable as Ke}from"https://www.gstatic.com/firebasejs/10.12.2/firebase-functions.js";(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&s(l)}).observe(document,{childList:!0,subtree:!0});function e(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function s(a){if(a.ep)return;a.ep=!0;const o=e(a);fetch(a.href,o)}})();const qe={apiKey:"AIzaSyA11OPapSOfwLQzs-oEoF7papT9S3T5p7Q",authDomain:"ipsc-insight-coach.firebaseapp.com",projectId:"ipsc-insight-coach",storageBucket:"ipsc-insight-coach.firebasestorage.app",messagingSenderId:"864793320312",appId:"1:864793320312:web:c586e384bbe365444bc68d",measurementId:"G-3Y4NZTBBS2"},Le=Re(qe),re=Ae(Le),b=ze(Le),Je=Ge();let ge=null,F=null;function We(t){je(re,async i=>{if(i){ge=i;try{const e=await q(k(b,"users",i.uid));e.exists()?F={uid:i.uid,...e.data()}:F={uid:i.uid,email:i.email,name:i.email?i.email.split("@")[0]:"Bruker",role:"user"},t(F)}catch(e){console.error("Feil ved lasting av brukerprofil:",e),F={uid:i.uid,email:i.email,name:i.email?i.email.split("@")[0]:"Bruker",role:"user"},t(F)}}else ge=null,F=null,t(null)})}async function Ye(t,i){try{const e=(t||"").trim();return{success:!0,user:(await Oe(re,e,i||"")).user}}catch(e){console.error("Innlogging feilet:",e);let s="Innlogging feilet";return e.code==="auth/user-not-found"||e.code==="auth/wrong-password"||e.code==="auth/invalid-credential"?s="Feil e-post eller passord":e.code==="auth/invalid-email"?s="Ugyldig e-postadresse":e.code==="auth/user-disabled"&&(s="Denne kontoen er deaktivert"),{success:!1,error:s}}}async function Qe(t,i,e,s,a,o,l,p,v,y){try{const u=(t||"").trim(),h=i||"",I=(e||"").trim(),P=(s||"").trim(),J=(a||"").trim(),W=(o||"").trim(),de=(l||"").trim(),H=(p||"minor").trim(),z=(v||"").trim(),Y=(y||"").trim(),_=(await Ue(re,u,h)).user,ce=Ke(Je,"validateInviteCode");try{await ce({code:I,userId:_.uid,userEmail:u})}catch(C){await _.delete();let g="Ugyldig invitasjonskode";return C.code==="functions/not-found"?g="Ugyldig invitasjonskode":C.code==="functions/permission-denied"?g="Denne koden er deaktivert":C.code==="functions/resource-exhausted"?g="Denne koden har nådd maksimalt antall bruk":C.code==="functions/already-exists"?g="Du har allerede brukt denne koden":C.message&&(g=C.message),{success:!1,error:g}}return await ie(k(b,"users",_.uid),{email:u,firstName:P,lastName:J,division:W,category:de,powerFactor:H,region:z,club:Y,role:"user",inviteCode:I,createdAt:new Date,draw:null,reloadTime:null,gdprConsent:!0,gdprConsentDate:new Date,gdprVersion:"1.0"}),{success:!0,user:_}}catch(u){console.error("Registrering feilet:",u);let h="Registrering feilet";return u.code==="auth/email-already-in-use"?h="E-postadressen er allerede i bruk":u.code==="auth/weak-password"?h="Passordet må være minst 6 tegn":u.code==="auth/invalid-email"?h="Ugyldig e-postadresse":u.message&&(h=u.message),{success:!1,error:h}}}async function Ze(){try{return await He(re),{success:!0}}catch(t){return console.error("Utlogging feilet:",t),{success:!1,error:"Kunne ikke logge ut"}}}function S(){return ge}function ue(){return F}const Xe=`
...
`;function et(t,i){const e=document.getElementById("gdpr-modal");e&&e.remove();const s=document.createElement("div");s.id="gdpr-modal",s.className="gdpr-modal",s.innerHTML=`
  ...
  `,document.body.appendChild(s);const a=s.querySelector(".gdpr-close-btn"),o=s.querySelector(".gdpr-btn-accept"),l=s.querySelector(".gdpr-btn-decline"),p=s.querySelector(".gdpr-modal-overlay"),v=()=>{s.remove()};a.addEventListener("click",()=>{v(),i&&i()}),p.addEventListener("click",()=>{v(),i&&i()}),l.addEventListener("click",()=>{v(),i&&i()}),o.addEventListener("click",()=>{v(),t&&t()}),document.body.style.overflow="hidden";const y=v,u=()=>{document.body.style.overflow="",y()};a.onclick=()=>{u(),i&&i()},p.onclick=()=>{u(),i&&i()},l.onclick=()=>{u(),i&&i()},o.onclick=()=>{u(),t&&t()}}function tt(){const t=document.createElement("div");return t.className="gdpr-checkbox-container",t.innerHTML=`
  ...
  `,setTimeout(()=>{const i=t.querySelector("#gdpr-open-modal");i&&i.addEventListener("click",e=>{e.preventDefault(),et(()=>{const s=document.getElementById("gdpr-consent-checkbox");s&&(s.checked=!0)},()=>{const s=document.getElementById("gdpr-consent-checkbox");s&&(s.checked=!1)})})},0),t}function it(){const t=document.getElementById("gdpr-consent-checkbox");return!t||!t.checked?{valid:!1,error:"Du må godta personvernerklæringen for å registrere deg"}:{valid:!0}}function st(t,i){t.innerHTML=`
    <style>
      * {
...
<div class="modal-overlay" id="modal-confirm-delete" onclick="closeModalOutside(event,'modal-confirm-delete')">
  ...
  `,Lt(),Me(),$(),U(),_e(),le()}function Lt(){window.switchTab=Et,window.setFilter=Tt,window.openModal=N,window.closeModal=L,window.closeModalOutside=It,window.createMatch=Mt,window.searchMatchByIdHandler=_t,window.openEditProfile=Zt,window.saveProfileData=ei,window.selectPF=Xt,window.updatePFOptions=Ce,window.calcPrognose=le,window.renderMatchList=U,window.selectMatch=Ct,window.addShooter=ti,window.addStageResult=ii,window.handleLogout=si,window.openEditMatch=$t,window.saveEditMatch=Nt,window.openCreateStage=Dt,window.openEditStage=Rt,window.changeStageNumber=At,window.changeStageField=jt,window.saveStage=Ot,window.openInviteUser=Ut,window.sendInvitation=sendInvitation,window.openInvitationsModal=Wt,window.acceptInvitation=Yt,window.declineInvitation=Qt,window.searchUsers=Ht,window.toggleUserSelection=zt,window.sendMultipleInvitations=Vt,window.searchUsersNewMatch=Gt,window.toggleUserNewMatch=Kt,window.searchUsersEditMatch=qt,window.toggleUserEditMatch=Jt,window.confirmDeleteMatch=Ft,window.deleteMatchConfirmed=Bt}function Et(t){document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active")),document.querySelectorAll(".tab-item").forEach(s=>s.classList.remove("active")),r(t).classList.add("active");const i=["screen-home","screen-matches","screen-prognose","screen-results","screen-profile"].indexOf(t),e=document.querySelectorAll(".tab-item");e[i]&&e[i].classList.add("active"),t==="screen-home"&&$(),t==="screen-matches"&&U(),t==="screen-results"&&te()}function N(t){r(t).classList.add("open")}function L(t){r(t).classList.remove("open")}function It(t,i){t.target.id===i&&L(i)}function Tt(t,i){R=t,document.querySelectorAll(".filter-chip").forEach(e=>e.classList.remove("active")),i.classList.add("active"),U()}async function Mt(){var e;const t={name:m("new-match-name")||"Ny match",type:m("new-match-type")||"Trening",date:m("new-match-date")||new Date().toISOString().split("T")[0],location:m("new-match-location")||"",plannedStages:x("new-match-stages",6),searchable:((e=r("new-match-searchable"))==null?void 0:e.checked)!==!1,stages:[],shooters:[],stageDefs:[]},i=await ot(t);if(i.success){let s=0;for(const a of j)(await me(a.email,{matchId:i.matchId,matchName:t.name})).success&&s++;L("modal-new-match"),r("new-match-name").value="",r("new-match-location").value="",r("new-match-stages").value="6",r("new-match-searchable")&&(r("new-match-searchable").checked=!0),r("new-match-user-search").value="",r("new-match-search-results").innerHTML="",j=[],s>0&&alert(`Match opprettet! Invitasjoner sendt til ${s} bruker(e).`)}else alert("Kunne ikke opprette match: "+i.error)}async function _t(){const t=m("match-id-search").trim();if(!t){alert("Skriv inn et match-ID");return}const i=await dt(t);i.success?(alert(`Match funnet: ${i.match.name} (ID: ${i.match.id})`),r("match-id-search").value=""):alert(`Fant ingen match med ID ${t}${i.error?": "+i.error:""}`)}function Ct(t){w=t;const i=f.find(e=>e.id==t);if(i){const e=i.id?"Match ID "+i.id+" "+i.name:i.name||"Match";["home-chip-name","prog-chip-name","results-chip-name"].forEach(a=>{const o=r(a);o&&(o.textContent=e)})}G&&G(),t&&(G=gt(t,e=>{const s=f.findIndex(a=>a.id==t);s!==-1&&e&&(f[s]=e,$(),te())})),$(),te(),le()}function $t(){const t=f.find(s=>s.id==w);if(!t){alert("Ingen match valgt");return}r("edit-match-name").value=t.name||"",r("edit-match-type").value=t.type||"Trening",r("edit-match-date").value=t.date||"",r("edit-match-location").value=t.location||"",r("edit-match-stages").value=t.plannedStages||0,r("edit-match-searchable")&&(r("edit-match-searchable").checked=t.searchable!==!1),r("edit-match-user-search").value="",r("edit-match-search-results").innerHTML="",O=[];const i=ue(),e=r("delete-match-btn");e&&i&&t.ownerId===i.uid?e.style.display="block":e&&(e.style.display="none"),N("modal-edit-match")}async function Nt(){var s;const t=f.find(a=>a.id==w);if(!t){alert("Ingen match valgt");return}const i={name:m("edit-match-name")||t.name,type:m("edit-match-type")||t.type,date:m("edit-match-date")||t.date,location:m("edit-match-location")||t.location,plannedStages:x("edit-match-stages",t.plannedStages),searchable:((s=r("edit-match-searchable"))==null?void 0:s.checked)!==!1},e=await oe(t.id,i);if(e.success){let a=0;for(const o of O)(await me(o.email,{matchId:t.id,matchName:i.name})).success&&a++;L("modal-edit-match"),a>0&&alert(`Match oppdatert! Invitasjoner sendt til ${a} bruker(e).`)}else alert("Kunne ikke oppdatere match: "+e.error)}function Ft(){const t=f.find(e=>e.id==w);if(!t){alert("Ingen match valgt");return}const i=t.id?"Match ID "+t.id+" "+t.name:t.name;r("delete-match-name").textContent=i,N("modal-confirm-delete")}async function Bt(){const t=f.find(e=>e.id==w);if(!t){alert("Ingen match valgt");return}const i=await lt(t.id);i.success?(L("modal-confirm-delete"),L("modal-edit-match"),w=null,$(),U(),alert("Match slettet")):alert("Kunne ikke slette match: "+i.error)}let ee=null;function Dt(){var i;const t=f.find(e=>e.id==w);if(!t){alert("Ingen match valgt");return}ee=null,r("stage-modal-title").textContent=n("create_stage"),r("stage-number").value=(((i=t.stages)==null?void 0:i.length)||0)+1,r("stage-name").value="",r("stage-paper-targets").value=0,r("stage-poppers").value=0,r("stage-plates").value=0,r("stage-no-shoots").value=0,r("stage-bonus-paper-targets").value=0,r("stage-bonus-included").checked=!1,N("modal-create-stage")}function Rt(t){const i=f.find(s=>s.id==w);if(!i||!i.stages||!i.stages[t]){alert("Stage ikke funnet");return}ee=t;const e=i.stages[t];r("stage-modal-title").textContent=n("edit_stage"),r("stage-number").value=e.number||t+1,r("stage-name").value=e.name||"",r("stage-paper-targets").value=e.paperTargets||0,r("stage-poppers").value=e.poppers||0,r("stage-plates").value=e.plates||0,r("stage-no-shoots").value=e.noShoots||0,r("stage-bonus-paper-targets").value=e.bonusPaperTargets||0,r("stage-bonus-included").checked=e.bonusIncluded||!1,N("modal-create-stage")}function At(t){const i=r("stage-number"),e=Math.max(1,parseInt(i.value)+t);i.value=e}function jt(t,i){const e=r("stage-"+t),s=Math.max(0,parseInt(e.value)+i);e.value=s}async function Ot(){var a;const t=f.find(o=>o.id==w);if(!t){alert("Ingen match valgt");return}const i={number:x("stage-number",1),name:m("stage-name")||"",paperTargets:x("stage-paper-targets",0),poppers:x("stage-poppers",0),plates:x("stage-plates",0),noShoots:x("stage-no-shoots",0),bonusPaperTargets:x("stage-bonus-paper-targets",0),bonusIncluded:((a=r("stage-bonus-included"))==null?void 0:a.checked)||!1},e=t.stages||[];ee!==null?e[ee]=i:e.push(i);const s=await oe(t.id,{stages:e});s.success?L("modal-create-stage"):alert("Kunne ikke lagre stage: "+s.error)}let M=[],E=[],j=[],O=[];function Ut(){if(!w){alert("Ingen match valgt");return}E=[],r("user-search-input").value="",r("user-search-results").innerHTML="",r("send-invitations-btn").style.display="none",N("modal-invite-user")}async function Ht(){const t=m("user-search-input").trim();if(t.length===0){r("user-search-results").innerHTML='<p style="color:#9ca3af;text-align:center;">Skriv inn et søk</p>';return}const i=await searchUsersByNameOrEmail(t),e=r("user-search-results");if(!e)return;if(i.length===0){e.innerHTML='<p style="color:#9ca3af;text-align:center;">Ingen brukere funnet</p>';return}let s='<div style="margin-bottom:10px;font-weight:600;">RESULTATER ('+i.length+"):</div>";i.forEach(a=>{const o=a.displayName||a.email,l=E.some(p=>p.uid===a.uid);s+=`
      <label style="display:flex;align-items:center;gap:10px;padding:10px;background:#2d3748;border-radius:8px;margin-bottom:8px;cursor:pointer;">
        <input type="checkbox" ${l?"checked":""} onchange="toggleUserSelection('${a.uid}', '${a.email}', '${o}')" style="width:18px;height:18px;">
        <div>
          <div style="font-weight:600;">${o}</div>
          <div style="font-size:14px;color:#9ca3af;">${a.email}</div>
        </div>
      </label>
    `}),e.innerHTML=s,Te()}function zt(t,i,e){const s=E.findIndex(a=>a.uid===t);s>-1?E.splice(s,1):E.push({uid:t,email:i,displayName:e}),Te()}function Te(){const t=r("send-invitations-btn");t&&(E.length>0?(t.style.display="block",t.textContent=`Send invitasjoner (${E.length} valgt)`):t.style.display="none")}async function Vt(){if(E.length===0){alert("Ingen brukere valgt");return}const t=f.find(s=>s.id==w);if(!t)return;let i=0,e=0;for(const s of E)(await me(s.email,{matchId:t.id,matchName:t.name})).success?i++:e++;L("modal-invite-user"),i>0&&alert(`Invitasjoner sendt til ${i} bruker(e)!`),e>0&&alert(`${e} invitasjon(er) feilet.`),E=[]}async function Gt(){const t=m("new-match-user-search").trim();if(t.length===0){r("new-match-search-results").innerHTML="";return}const i=await searchUsersByNameOrEmail(t),e=r("new-match-search-results");if(!e)return;if(i.length===0){e.innerHTML='<p style="color:#9ca3af;text-align:center;font-size:14px;">Ingen brukere funnet</p>';return}let s='<div style="margin-bottom:8px;font-size:14px;font-weight:600;">RESULTATER ('+i.length+"):</div>";i.forEach(a=>{const o=a.displayName||a.email,l=j.some(p=>p.uid===a.uid);s+=`
      <label style="display:flex;align-items:center;gap:8px;padding:8px;background:#2d3748;border-radius:8px;margin-bottom:6px;cursor:pointer;font-size:14px;">
        <input type="checkbox" ${l?"checked":""} onchange="toggleUserNewMatch('${a.uid}', '${a.email}', '${o}')" style="width:16px;height:16px;">
        <div>
          <div style="font-weight:600;">${o}</div>
          <div style="font-size:12px;color:#9ca3af;">${a.email}</div>
        </div>
      </label>
    `}),e.innerHTML=s}function Kt(t,i,e){const s=j.findIndex(a=>a.uid===t);s>-1?j.splice(s,1):j.push({uid:t,email:i,displayName:e})}async function qt(){const t=m("edit-match-user-search").trim();if(t.length===0){r("edit-match-search-results").innerHTML="";return}const i=await searchUsersByNameOrEmail(t),e=r("edit-match-search-results");if(!e)return;if(i.length===0){e.innerHTML='<p style="color:#9ca3af;text-align:center;font-size:14px;">Ingen brukere funnet</p>';return}let s='<div style="margin-bottom:8px;font-size:14px;font-weight:600;">RESULTATER ('+i.length+"):</div>";i.forEach(a=>{const o=a.displayName||a.email,l=O.some(p=>p.uid===a.uid);s+=`
      <label style="display:flex;align-items:center;gap:8px;padding:8px;background:#2d3748;border-radius:8px;margin-bottom:6px;cursor:pointer;font-size:14px;">
        <input type="checkbox" ${l?"checked":""} onchange="toggleUserEditMatch('${a.uid}', '${a.email}', '${o}')" style="width:16px;height:16px;">
        <div>
          <div style="font-weight:600;">${o}</div>
          <div style="font-size:12px;color:#9ca3af;">${a.email}</div>
        </div>
      </label>
    `}),e.innerHTML=s}function Jt(t,i,e){const s=O.findIndex(a=>a.uid===t);s>-1?O.splice(s,1):O.push({uid:t,email:i,displayName:e})}function Wt(){he(),N("modal-invitations")}function he(){const t=r("invitations-list");if(!t)return;if(!M||M.length===0){t.innerHTML='<div class="empty-state"><div class="empty-sub">'+n("no_invitations")+"</div></div>";return}let i="";M.forEach((e,s)=>{i+='<div class="card" style="margin-bottom:10px;">',i+='<div style="margin-bottom:10px;"><strong>'+n("invited_to_match")+"</strong></div>",i+='<div style="margin-bottom:10px;">Match ID '+e.matchId+" "+e.matchName+"</div>",i+='<div style="margin-bottom:10px;color:#94a3b8;">Fra: '+e.invitedBy+"</div>",i+='<div style="display:flex;gap:10px;">',i+='<button class="btn-primary" onclick="acceptInvitation('+s+')" style="flex:1;">'+n("accept")+"</button>",i+='<button onclick="declineInvitation('+s+')" style="flex:1;padding:12px;background:#374151;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">'+n("decline")+"</button>",i+="</div>",i+="</div>"}),t.innerHTML=i}async function Yt(t){const i=M[t];if(!i)return;const e=await vt(i.matchId);e.success?(M.splice(t,1),fe(),he()):alert("Kunne ikke akseptere invitasjon: "+e.error)}async function Qt(t){const i=M[t];if(!i)return;const e=await ut(i.matchId);e.success?(M.splice(t,1),fe(),he()):alert("Kunne ikke avvise invitasjon: "+e.error)}function fe(){const t=r("invitation-badge");if(!t)return;const i=M.length;i>0?(t.textContent=i,t.style.display="flex"):t.style.display="none"}function $(){var a,o;const t=r("home-content");if(!t)return;const i=f.find(l=>l.id===w);if(!i){t.innerHTML=`
    ...
    `;return}let e="";e+='<div class="card">',e+='<div class="mhc-name">'+i.name+"</div>",e+='<div class="mhc-meta">'+Ie(i.date)+" · "+i.type+"</div>",e+='<div class="mhc-stats">',e+='<div><div class="mhc-val">'+(((a=i.stages)==null?void 0:a.length)||0)+'</div><div class="mhc-lbl">Stages</div></div>',e+='<div><div class="mhc-val">'+(((o=i.shooters)==null?void 0:o.length)||0)+'</div><div class="mhc-lbl">Skyttere</div></div>',e+="</div>",e+='<div style="display:flex;gap:10px;margin-top:15px;">',e+='<button class="btn-primary" onclick="openEditMatch()" style="flex:1;">Rediger match</button>',e+='<button class="btn-primary" onclick="openCreateStage()" style="flex:1;">Ny stage</button>',e+="</div>",e+='<div style="margin-top:10px;">',e+='<button class="btn-primary" onclick="openInviteUser()" style="width:100%;">Inviter bruker</button>',e+="</div>";const s=ue();s&&i.ownerId===s.uid&&(e+='<div style="margin-top:10px;">',e+='<button onclick="confirmDeleteMatch()" style="width:100%;padding:12px;background:#ef4444;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">Slett match</button>',e+="</div>"),e+="</div>",i.stages&&i.stages.length>0&&(e+='<div class="section-title">Stages</div>',e+='<div class="card">',i.stages.forEach((l,p)=>{e+='<div class="stage-row" style="cursor:pointer;border-bottom:1px solid #2d3748;padding:12px 0;" onclick="openEditStage('+p+')">',e+='<div class="stage-num">S'+(l.number||p+1)+"</div>",e+='<div class="stage-info">',e+='<div class="stage-name">'+(l.name||"Stage "+(l.number||p+1))+"</div>",e+='<div class="stage-meta">',l.paperTargets&&(e+="Paper: "+l.paperTargets+" "),l.poppers&&(e+="Poppers: "+l.poppers+" "),l.plates&&(e+="Plates: "+l.plates+" "),l.noShoots&&(e+="NS: "+l.noShoots+" "),l.bonusPaperTargets&&(e+="Bonus: "+l.bonusPaperTargets+(l.bonusIncluded?" (included)":"")),e+="</div>",e+="</div>",e+="</div>"}),e+="</div>"),t.innerHTML=e}function U(){const t=r("match-list-container");if(!t)return;let i=f.filter(s=>{if(R==="all")return!0;if(R==="active")return s.id===w;if(R==="trening")return s.type==="Trening";if(R==="stevne")return s.type==="Stevne";const a=s.date?new Date(s.date).getFullYear().toString():"";return R===a});if(i.length===0){t.innerHTML='<div class="empty-state"><div class="empty-sub">Ingen matcher funnet</div></div>';return}let e="";i.forEach(s=>{var l;const a=s.id===w,o=s.id?"Match ID "+s.id+" "+s.name:s.name;e+='<div class="match-row">',e+='<div class="match-row-icon'+(a?" is-active":"")+`" onclick="selectMatch('`+s.id+`')">🏆</div>`,e+=`<div class="match-row-info" onclick="selectMatch('`+s.id+`')">`,e+='<div class="match-row-name">'+o+"</div>",e+='<div class="match-row-meta">'+Ie(s.date)+" · "+(s.location||s.type)+"</div>",e+="</div>",e+='<div class="match-row-right">',e+=`<button onclick="event.stopPropagation(); selectMatch('`+s.id+`'); openEditMatch();" style="padding:8px 16px;background:#e8b84b;color:#1a202c;border:none;border-radius:8px;font-weight:600;cursor:pointer;margin-bottom:8px;">Rediger</button>`,e+='<div class="match-stg-count">'+(((l=s.stages)==null?void 0:l.length)||0)+"</div>",e+='<div class="match-stg-lbl">stages</div>',e+="</div>",e+="</div>"}),t.innerHTML=e}function te(){var o;const t=r("results-content");if(!t)return;const i=f.find(l=>l.id===w);if(!i){t.innerHTML='<div class="empty-state"><div class="empty-sub">Velg en match først</div></div>';return}if(!i.shooters||i.shooters.length===0){t.innerHTML='<div class="empty-state"><div class="empty-icon">👥</div><div class="empty-title">Ingen skyttere</div><div class="empty-sub">Trykk + for å legge til skyttere</div></div>';return}let e='<div class="card">';e+='<div class="card-header"><div class="card-title">Standings</div></div>',e+='<div class="standings-table-wrap"><table class="standings-table">',e+='<thead><tr class="standings-header-row">',e+='<th class="standings-th standings-th-rank">#</th>',e+='<th class="standings-th standings-th-shooter">Skytter</th>',e+='<th class="standings-th standings-th-pts">Pts</th>',e+='<th class="standings-th standings-th-pct">%</th>',e+="</tr></thead>",e+="<tbody>";const s=i.shooters.map(l=>{var v;const p=((v=l.stages)==null?void 0:v.reduce((y,u)=>y+(u.pts||0),0))||0;return{...l,totalPts:p}}).sort((l,p)=>p.totalPts-l.totalPts),a=((o=s[0])==null?void 0:o.totalPts)||0;s.forEach((l,p)=>{const v=a>0?(l.totalPts/a*100).toFixed(2):"0.00";e+='<tr class="standings-row">',e+='<td class="standings-td standings-td-rank">'+(p+1)+"</td>",e+='<td class="standings-td standings-td-shooter">',e+='<div class="standings-shooter-name">'+l.firstName+" "+l.lastName+"</div>",e+='<div class="standings-shooter-meta">'+l.division+" · "+A(l.pf||"minor")+"</div>",e+="</td>",e+='<td class="standings-td standings-td-pts">'+l.totalPts.toFixed(2)+"</td>",e+='<td class="standings-td standings-td-pct">'+v+"%</td>",e+="</tr>"}),e+="</tbody></table></div>",e+="</div>",t.innerHTML=e}function Me(){const t=B();["prof-avatar","nav-av-home","nav-av-matches","nav-av-prog","nav-av-results"].forEach(p=>{const v=r(p);v&&(v.textContent=t)});const e=r("prof-name");e&&(e.textContent=(d.firstName||"")+" "+(d.lastName||""));const s=r("prof-div");s&&(s.textContent=(d.division||"—")+" · "+(d.club||"—"));const a=r("prof-badge-pf");a&&(a.textContent=d.powerFactor?A(d.powerFactor):"—");const o=r("prof-badge-region");o&&(o.textContent=d.region||"—");const l={"info-firstname":d.firstName||"—","info-lastname":d.lastName||"—","info-division":d.division||"—","info-category":d.category||"—","info-pf":d.powerFactor?A(d.powerFactor):"—","info-region":d.region||"—","info-club":d.club||"—"};Object.keys(l).forEach(p=>{const v=r(p);v&&(v.textContent=l[p])}),_e()}function _e(){const t=[];f.forEach(h=>{h.stages&&h.stages.forEach(I=>t.push(I))});let i=0,e=0,s=0;t.forEach(h=>{i+=h.a||0,e+=(h.a||0)+(h.c||0)+(h.d||0),s+=h.hf||0});const a=t.length?(s/t.length).toFixed(2):"—",o=e?Math.round(i/e*100)+"%":"—",l=r("stat-matches");l&&(l.textContent=f.length);const p=r("stat-stages");p&&(p.textContent=t.length);const v=r("stat-avg-hf");v&&(v.textContent=a);const y=r("stat-a-rate");y&&(y.textContent=o);const u=r("prog-a-rate");u&&(u.textContent=o)}function Zt(){r("edit-firstname").value=d.firstName||"",r("edit-lastname").value=d.lastName||"",r("edit-club").value=d.club||"",r("edit-draw").value=d.draw||"",r("edit-reload").value=d.reloadTime||"";let t="";bt.forEach(s=>{t+='<option value="'+s+'"'+(s===d.division?" selected":"")+">"+s+"</option>"}),r("edit-division").innerHTML=t;let i="";wt.forEach(s=>{i+='<option value="'+s+'"'+(s===d.category?" selected":"")+">"+s+"</option>"}),r("edit-category").innerHTML=i;let e="";kt.forEach(s=>{e+='<option value="'+s+'"'+(s===d.region?" selected":"")+">"+s+"</option>"}),r("edit-region").innerHTML=e,Ce(),N("modal-edit-profile")}function Ce(){const t=m("edit-division"),i=yt[t]||["minor","major"];let e="";i.forEach(s=>{const a=d.powerFactor===s;e+='<label class="pf-option'+(a?" active":"")+`" onclick="selectPF(this,'`+s+`')">`,e+='<input type="radio" name="pf" value="'+s+'">',e+='<div class="pf-label">'+s.toUpperCase()+"</div>",e+='<div class="pf-sub">'+(s==="major"?"≥170 PF":"<170 PF")+"</div>",e+="</label>"}),r("pf-options").innerHTML=e,i.indexOf(d.powerFactor)<0&&(d.powerFactor=i[0])}function Xt(t,i){document.querySelectorAll(".pf-option").forEach(e=>e.classList.remove("active")),t.classList.add("active"),d.powerFactor=i}async function ei(){d.firstName=m("edit-firstname").trim()||"",d.lastName=m("edit-lastname").trim()||"",d.division=m("edit-division")||"",d.category=m("edit-category")||"",d.region=m("edit-region")||"",d.club=m("edit-club").trim()||"",d.draw=K("edit-draw")||null,d.reloadTime=K("edit-reload")||null;const t=await at(d),i=r("save-profile-btn");t.success?(i.textContent="✓ Lagret!",i.style.background="var(--green)",setTimeout(()=>{i.textContent=n("save_profile"),i.style.background=""},1800)):(i.textContent="❌ Feil!",i.style.background="var(--red)",setTimeout(()=>{i.textContent=n("save_profile"),i.style.background=""},1800)),Me(),le(),$(),L("modal-edit-profile")}function le(){const t=x("prog-shots",12),i=x("prog-targets",6),e=x("prog-steel",2),s=K("prog-move",3),a=K("prog-draw",d.draw||1.42),o=d.division||"Classic",l=d.powerFactor||"minor",p=Pt(t,o,l);r("prog-reloads").value=p;const v=d.reloadTime||1.8,y=.18,u=a+t*y+p*v+s,h=i*10+e*10,I=u>0?h/u:0;r("prog-hf-out").textContent=I.toFixed(2);let P="";P+='<div class="prog-breakdown-detail">',P+="Trekk: "+a.toFixed(2)+"s · ",P+="Skudd: "+(t*y).toFixed(2)+"s · ",P+="Reload: "+(p*v).toFixed(2)+"s · ",P+="Beveg: "+s.toFixed(2)+"s",P+="</div>",r("prog-breakdown").innerHTML=P}async function ti(){const t=f.find(o=>o.id===w);if(!t)return;const i=m("new-shooter-firstname").trim(),e=m("new-shooter-lastname").trim(),s=m("new-shooter-division")||"Classic";if(!i||!e){alert("Fyll inn navn");return}const a={id:"s_"+Date.now(),isMe:!1,firstName:i,lastName:e,division:s,pf:"minor",club:"",stages:[]};t.shooters||(t.shooters=[]),t.shooters.push(a),await oe(t.id,t),L("modal-add-shooter"),te()}async function ii(){const t=f.find(l=>l.id===w);if(!t)return;const i=x("new-result-stage",1),e=K("new-result-time",0),s=x("new-result-points",0),a=e>0?s/e:0,o={num:i,name:"Stage "+i,hf:a,time:e,pts:s,pf:d.powerFactor||"minor"};t.stages||(t.stages=[]),t.stages.push(o),await oe(t.id,t),L("modal-add"),$()}async function si(){V&&V(),G&&G(),await Ze(),window.location.reload()}const $e=document.getElementById("app");function ai(){st($e,Ne)}function Ne(){St($e)}We(t=>{t?Ne():ai()});
